...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "hc:w:")) != -1) {
		...
		case 'w':
			...
			if (thickness <= 0) {
				osddemo_log_err("invalid thickness: %d", thickness);
				return -EINVAL;
			}
			printf("set pt:%d", thickness);
			break;
		case 'c':
			channel = atoi(argv[optind - 1]);
			if ((channel < 0) || (channel > 2)) {
				osddemo_log_err("invalid channel: %d", channel);
				return -EINVAL;
			}
			osddemo_log_info("set to channel :%d", channel);
			break;
		default:
			help();
			exit(1);
	...
	if (ret != MPI_SUCCESS) {
		osddemo_log_err("failed to get encoder channel %d  rect, %d", MPI_GET_ENC_CHN(echn_idx), ret);
		return -EACCES;
	}

	int chn_width = chn_attr.res.width;
	int chn_height = chn_attr.res.height;
	osddemo_log_info("get chn %d res (%d, %d)", MPI_GET_ENC_CHN(echn_idx), chn_width, chn_height);
	...
	if (ret != 0) {
		osddemo_log_err("failed to add new line: %d", ret);
		return -EINVAL;
	}
	char color[4] = { 0x00, 0x00, 0x00, 0xff };
	color[0] = rand() % 255;
	color[1] = rand() % 255;
	color[2] = rand() % 255;

	memcpy(&newLIne.color[0], &color[0], sizeof(color));
	osddemo_log_info("new line (%d, %d) (%d, %d) %d", newLIne.start.x, newLIne.start.y, newLIne.end.x,
	                 newLIne.end.y, newLIne.thickness);

	for (int i = 0; i < 2; i++) {
		echn_idx.chn = i;
		ret = MPI_ENC_stopChn(echn_idx);
		if (ret != MPI_SUCCESS) {
			osddemo_log_err("Stop encoder channel %d failed. %d", MPI_GET_ENC_CHN(echn_idx), ret);
			return -EACCES;
		}
	}
	...
	if (ret != MPI_SUCCESS) {
		osddemo_log_err("failed to update canvas: %d", ret);
	...
	osddemo_log_info("resume ENC0 & 1");
	...
	for (int i = 0; i < 2; i++) {
		...
		if (ret != MPI_SUCCESS) {
			osddemo_log_err("Bind encoder channel %d failed. %d", MPI_GET_ENC_CHN(echn_idx), ret);
			return MPI_FAILURE;
		}

		ret = MPI_ENC_startChn(echn_idx);
		if (ret != MPI_SUCCESS) {
			osddemo_log_err("Start encoder channel %d failed. %d", MPI_GET_ENC_CHN(echn_idx), ret);
			return MPI_FAILURE;
		}
	}
	...
	osddemo_log_info("start unbind & destroy canvas");

	osddemo_log_info("stop all ENC");
