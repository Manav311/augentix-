...
static int clearMdCtx(MPI_WIN idx, AVFTR_MD_CTX_S *ctx)
{
	int ctx_idx = findMdCtx(idx, ctx, NULL);
	if (ctx_idx < 0) {
		log_err("The MD ctx doesn't exist.");
		return -EINVAL;
	}
	memset(&ctx[ctx_idx], 0, sizeof(AVFTR_MD_CTX_S));
	return 0;
}

static int findShdCtx(const MPI_WIN idx, const AVFTR_SHD_CTX_S *ctx, int *empty)
{
	int find_idx = -1;
	int emp_idx = -1;
	int i = 0;
                   ...
                   VFTR_SHD_LONGTERM_LIST_S *shd_long_list)
{
	...
	/* init MD ctx */
	AVFTR_MD_CTX_S *md_ctx;
	set_idx = findMdCtx(win_idx, vftr_res_shm->md_ctx, &empty_idx);
	if (set_idx >= 0) {
		log_warn("MD is already registered on win(%u, %u, %u).", win_idx.dev, win_idx.chn, win_idx.win);
		md_ctx = &vftr_res_shm->md_ctx[set_idx];
	} else {
		if (empty_idx < 0) {
			log_err("Failed to create MD instance on win(%u, %u, %u).", win_idx.dev, win_idx.chn,
			        win_idx.win);
			return -ENOMEM;
		}

		md_ctx = &vftr_res_shm->md_ctx[empty_idx];
		md_ctx->en = 1;
		md_ctx->reg = 1;
		md_ctx->idx = win_idx;
		md_ctx->cb = NULL;
	...
	if (set_idx >= 0) {
		...
		shd_ctx = &vftr_res_shm->shd_ctx[set_idx];
	} else {
		if (empty_idx < 0) {
			log_err("Failed to create SHD instance on win(%u, %u, %u).", win_idx.dev, win_idx.chn,
			        win_idx.win);
			return -ENOMEM;
		}

		shd_ctx = &vftr_res_shm->shd_ctx[empty_idx];
		shd_ctx->en = 1;
		shd_ctx->reg = 1;
		shd_ctx->idx = win_idx;
	}
	...
	/* create MD instance */
	md = VFTR_MD_newInstance();
	if (!md) {
		log_err("Failed to create MD instance.");
		return -EINVAL;
	}

	/* set parameter to md */
	ret = VFTR_MD_setParam(md, md_attr);
	if (ret) {
		log_err("Failed to set MD parameters. err: %d", ret);
		return -EINVAL;
	}
	...
	while (g_md_running) {
		...
		//DBG("still alive %u\n", timestamp);
#endif
		VFTR_MD_STATUS_S *md_res;
#ifdef CONFIG_APP_MD_SUPPORT_SEI
		md_res = &md_ctx->md_res[buf_idx];
#else
		 ...
		 */
		ret = VFTR_MD_detectMotion(md, obj_list, md_res);
		if (ret != MPI_SUCCESS) {
			log_err("Failed to detect motion.");
			continue;
		}
	...
	/* delete MD instance */
	VFTR_MD_deleteInstance(&md);

	VFTR_exit();

	clearOdCtx(win_idx, vftr_res_shm->od_ctx);
	clearMdCtx(win_idx, vftr_res_shm->md_ctx);
