...
class concurrent_vector_base {
	...
	void __TBB_EXPORTED_METHOD internal_copy(const concurrent_vector_base &src, size_type element_size,
...
#endif /* defined(_WIN64) && defined(_MSC_VER) */
{
	...
	template <typename T> friend class tbb::concurrent_vector;
#else
...
//! Concurrent vector
/** @ingroup containers */
template <typename T> class concurrent_vector : private internal::concurrent_vector_base {
    public:
	...
	template <typename I> class generic_range_type : public blocked_range<I> {
		...
		typedef I iterator;
		typedef ptrdiff_t difference_type;
		generic_range_type(I begin_, I end_, size_t grainsize_)
...
template <typename T> T &concurrent_vector<T>::internal_subscript(size_type index) const
{
...
template <typename T> void concurrent_vector<T>::initialize_array(void *begin, size_type n)
{
...
template <typename T> void concurrent_vector<T>::copy_array(void *dst, const void *src, size_type n)
{
...
template <typename T> void concurrent_vector<T>::assign_array(void *dst, const void *src, size_type n)
{
	T *d = static_cast<T *>(dst);
	const T *s = static_cast<const T *>(src);
	for (size_type j = 0; j < n; ++j)
...
template <typename T> void concurrent_vector<T>::destroy_array(void *begin, size_type n)
{
	T *array = static_cast<T *>(begin);
