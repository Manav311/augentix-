...
class concurrent_queue_iterator_base : no_assign {
	...
	//! Copy constructor
	concurrent_queue_iterator_base(const concurrent_queue_iterator_base &i)
	        : my_rep(NULL)
	        , my_item(NULL)
	{
		assign(i);
...
template <typename Container, typename Value> class concurrent_queue_iterator : public concurrent_queue_iterator_base {
	...
	template <typename T> friend class ::tbb::concurrent_queue;
#else
...
/** Multiple threads may each push and pop concurrently.
    ...
    @ingroup containers */
template <typename T> class concurrent_queue : public internal::concurrent_queue_base {
...
template <typename T> concurrent_queue<T>::~concurrent_queue()
{
	while (!empty()) {
		T value;
		internal_pop(&value);
