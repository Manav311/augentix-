...
template <typename M> void TestUniqueLockMoveConstructorAndAssignOp()
{
	...
	struct MutexAndLockFixture {
		...
		MutexAndLockFixture(source lckd_src)
		        : lock(mutex)
		        , was_locked(lckd_src.value)
		{
			if (!lckd_src.value)
				lock.unlock();
	...
	struct TestCases {
		...
		TestCases(const char *a_filename, int a_line)
		        : filename(a_filename)
		        , line(a_line)
		{
		}
		...
		void TestMoveAssignment(source locked_src, destination locked_dest)
		{
			...
			ASSERT_CUSTOM(unique_lock_t(dst.mutex, try_to_lock).owns_lock(),
			              "unique_lock should release owned mutex on assignment", filename, line);
			AssertOwnershipWasTransfered(dst.lock, src.lock, src.was_locked, &src.mutex);
		}

		void AssertOwnershipWasTransfered(unique_lock_t const &dest_lock, unique_lock_t const &src_lck,
		                                  const bool was_locked, const M *mutex)
		{
			ASSERT_CUSTOM(dest_lock.owns_lock() == was_locked,
			              "moved to lock object should have the same state as source before move", filename,
			              line);
			ASSERT_CUSTOM(dest_lock.mutex() == mutex,
			              "moved to lock object should have the same state as source before move", filename,
			              line);
			ASSERT_CUSTOM(src_lck.owns_lock() == false, "moved from lock object must not left locked",
			              filename, line);
			ASSERT_CUSTOM(src_lck.mutex() == NULL, "moved from lock object must not has mutex", filename,
			              line);
		}
	};
...
template <typename M> struct Counter {
...
template <typename M> void Counter<M>::flog_once_lock_guard(size_t mode)
...
template <typename M> void Counter<M>::flog_once_unique_lock(size_t mode)
/** Increments counter once for each iteration in the iteration space. */
{
	switch (mode & 7) {
	case 0: { // implicitly acquire and release mutex with unique_lock
		unique_lock<M> ul(mutex);
		value = value + 1;
		ASSERT(ul == true, NULL);
	} break;
	case 1: { // unique_lock with defer_lock
	...
	case 2: { // unique_lock::try_lock() with try_to_lock
	...
	case 3: { // unique_lock::try_lock_for() with try_to_lock
		unique_lock<M> ul(mutex, defer_lock);
	...
	default: { // unique_lock with adopt_lock, and release()
		mutex.lock();
		unique_lock<M> ul(mutex, adopt_lock);
		ASSERT(ul == true, NULL);
		value = value + 1;
...
template <typename State, long TestSize> struct WorkForLocks : NoAssign {
	static const size_t chunk = 100;
	State &state;
	WorkForLocks(State &state_)
	        : state(state_)
	{
	}
...
template <typename M> void TestLocks(const char *name, int nthread)
{
...
template <typename M> struct WorkForCondVarCtor : NoAssign {
	condition_variable &my_cv;
	M &my_mtx;
	WorkForCondVarCtor(condition_variable &cv_, M &mtx_)
	        : my_cv(cv_)
	        , my_mtx(mtx_)
	{
	}
...
template <typename M> struct WorkForCondVarDtor : NoAssign {
	int nthread;
	M &my_mtx;
	WorkForCondVarDtor(int n, M &mtx_)
	        : nthread(n)
	        , my_mtx(mtx_)
	{
	}
...
template <typename M> struct WorkForCondVarTimedWait : NoAssign {
	int nthread;
	condition_variable &test_cv;
	M &my_mtx;
	WorkForCondVarTimedWait(int n_, condition_variable &cv_, M &mtx_)
	        : nthread(n_)
	        , test_cv(cv_)
	        , my_mtx(mtx_)
	{
	...
	void operator()(int tid) const
	{
		...
		for (int i = 1; i < 10; ++i) {
			...
			{
				/** Some version of glibc return EINVAL instead 0 when spurious wakeup occurs on pthread_cond_timedwait() **/
				ASSERT(false == test_cv.wait_for(ul, intv, TestPredicateFalse()),
		...
		if (tid == 0) {
			...
			while (n_waiters < (nthread - 1))
				__TBB_Yield(); // wait until all other threads block on cv. flag_1
			...
			while (n_done_2 < 2 * nthread)
				__TBB_Yield();
			ASSERT(n_signaled > 0, "too small an interval?");
			n_signaled = 0;

		} else {
			while (n_done_2 < nthread + 1)
				__TBB_Yield(); // gate 1
		...
		if (tid == 0) {
			...
			while (n_waiters < (nthread - 1))
				__TBB_Yield(); // wait until all other threads block on cv.
			for (int i = 0; i < 2 * short_delay; ++i)
				__TBB_Yield(); // give some time to waiters so that all of them in the waitq
			ul.lock();
			false_to_true = true;
			test_cv.notify_all(); // open gate 4
			ul.unlock();

			while (n_done < nthread)
				__TBB_Yield(); // wait until all other threads wake up.
			ASSERT(n_signaled > 0, "too small an interval?");
		} else {
			while (n_done < 1)
				__TBB_Yield(); // gate 3
...
template <typename M> struct WorkForCondVarWaitAndNotifyOne : NoAssign {
	int nthread;
	condition_variable &test_cv;
	M &my_mtx;
	WorkForCondVarWaitAndNotifyOne(int n_, condition_variable &cv_, M &mtx_)
	        : nthread(n_)
	        , test_cv(cv_)
	        , my_mtx(mtx_)
	{
		...
		if (tid & 1) {
			...
			while (ticket_for_wakeup < max_ticket) {
				...
				for (;;) {
					unique_lock<M> ul(my_mtx, defer_lock);
					ul.lock();
					if (n_waiters > 0 && my_ticket <= ticket_for_sleep &&
					    my_ticket == (wokeup_ticket + 1)) {
...
struct TestPredicate1 {
	int target;
	TestPredicate1(int i_)
	        : target(i_)
	{
	}
	bool operator()()
	{
		return signaled_ticket == target;
	}
};

template <typename M> struct WorkForCondVarWaitPredAndNotifyAll : NoAssign {
	int nthread;
	condition_variable &test_cv;
	M &my_mtx;
	int multiple;
...
template <typename M> void TestConditionVariable(const char *name, int nthread)
{
...
#define TRY_AND_CATCH_RUNTIME_ERROR(op, msg)         \
	try {                                        \
		...
		ASSERT(strstr(e.what(), msg), NULL); \
	} catch (...) {                              \
		++err_count;                         \
	}

template <typename M> void TestUniqueLockException(const char *name)
{
...
template <typename M> void TestConditionVariableException(const char *name)
{
	REMARK("testing %s in TestConditionVariableException; yet to be implemented\n", name);
}
#endif /* TBB_USE_EXCEPTIONS */

template <typename Mutex, typename RecursiveMutex> void DoCondVarTest()
{
#if __TBB_CPP11_RVALUE_REF_PRESENT
