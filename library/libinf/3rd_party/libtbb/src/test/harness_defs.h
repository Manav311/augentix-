...
#define __TBB_TEST_SKIP_GCC_BUILTINS_MODE \
	(__TBB_TEST_BUILTINS && (!__TBB_GCC_BUILTIN_ATOMICS_PRESENT || __INTEL_COMPILER))
...
#define __TBB_ICC_BUILTIN_ATOMICS_POINTER_ALIASING_BROKEN \
	(TBB_USE_ICC_BUILTINS && __INTEL_COMPILER < 1400 && __INTEL_COMPILER > 1200)
...
#if __INTEL_COMPILER
#define __TBB_CPP11_REFERENCE_WRAPPER_PRESENT      \
	(_TBB_CPP0X && __INTEL_COMPILER >= 1200 && \
...
#elif __clang__
#define __TBB_CPP11_REFERENCE_WRAPPER_PRESENT \
	(_TBB_CPP0X && __cplusplus >= 201103L && (__TBB_GCC_VERSION >= 40400 || _LIBCPP_VERSION))
...
#define __TBB_CPP11_SMART_POINTERS_PRESENT \
	(_MSC_VER >= 1600 || _TBB_CPP0X && __TBB_GCC_VERSION >= 40400 || _LIBCPP_VERSION)
...
#define __TBB_CPP11_STD_PLACEHOLDERS_LINKAGE_BROKEN                                              \
	((__INTEL_COMPILER == 1300 || __INTEL_COMPILER == 1310) && __GXX_EXPERIMENTAL_CXX0X__ && \
	 __TBB_GCC_VERSION < 40500)
// Intel(R) Compiler have an issue when a scoped enum with a specified underlying type has negative values.
#define __TBB_ICC_SCOPED_ENUM_WITH_UNDERLYING_TYPE_NEGATIVE_VALUE_BROKEN \
	(_MSC_VER && !__TBB_DEBUG && __INTEL_COMPILER && __INTEL_COMPILER <= 1500)
// Intel(R) Compiler have an issue with __atomic_load_explicit from a scoped enum with a specified underlying type.
#define __TBB_ICC_SCOPED_ENUM_WITH_UNDERLYING_TYPE_ATOMIC_LOAD_BROKEN \
	(TBB_USE_ICC_BUILTINS && !__TBB_DEBUG && __INTEL_COMPILER && __INTEL_COMPILER <= 1500)
...
#define __TBB_FUNC_PTR_AS_TEMPL_PARAM_BROKEN \
	(((__linux__ || __APPLE__) && __INTEL_COMPILER && __INTEL_COMPILER < 1100) || __SUNPRO_CC)
...
#define __TBB_THROW_FROM_DTOR_BROKEN                                                   \
	(__clang__ && (__apple_build_version__ && __apple_build_version__ < 5000279 || \
	               __TBB_CLANG_VERSION && __TBB_CLANG_VERSION < 50000))

//std::uncaught_exception is broken on some version of stdlibc++ (it returns true with no active exception)
#define __TBB_STD_UNCAUGHT_EXCEPTION_BROKEN \
	(__linux__ && (__TBB_GCC_VERSION == 40407 || __TBB_GCC_VERSION >= 40800 && __TBB_GCC_VERSION < 50300))
