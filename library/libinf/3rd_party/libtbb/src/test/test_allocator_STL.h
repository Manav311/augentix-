...
template <typename Container> void TestSequence(const typename Container::allocator_type &a)
{
	Container c(a);
...
template <typename Set> void TestSet(const typename Set::allocator_type &a)
{
...
template <typename Map> void TestMap(const typename Map::allocator_type &a)
{
...
struct MoveOperationTracker {
	...
	MoveOperationTracker(int value = 0)
	        : my_value(value)
	{
	}
	...
	MoveOperationTracker(MoveOperationTracker &&m) __TBB_NOEXCEPT(true)
	        : my_value(m.my_value)
	{
	}
...
template <typename Allocator> void TestAllocatorWithSTL(const Allocator &a = Allocator())
{
