...
template <typename T> struct degenerate_hash {
                           ...
                           int line = 0)
{
	...
	REMARK("#%d checking allocators: items %u/%u, allocs %u/%u\n", line, unsigned(a.items_allocated),
	       unsigned(a.items_freed), unsigned(a.allocations), unsigned(a.frees));
	ASSERT(a.items_allocated == a.allocations, NULL);
	ASSERT(a.items_freed == a.frees, NULL);
	if (exact) {
		ASSERT(a.allocations == expected_allocs, NULL);
		ASSERT(a.frees == expected_frees, NULL);
	} else {
		ASSERT(a.allocations >= expected_allocs, NULL);
		ASSERT(a.frees >= expected_frees, NULL);
...
template <typename T> struct strip_const {
	typedef T type;
};

template <typename T> struct strip_const<const T> {
	typedef T type;
};

// value generator for cumap
template <typename K, typename V = std::pair<const K, K> > struct ValueFactory {
	typedef typename strip_const<K>::type Kstrip;
	static V make(const K &value)
	{
		return V(value, value);
	}
	static Kstrip key(const V &value)
	{
		return value.first;
	}
	static Kstrip get(const V &value)
	{
		return (Kstrip)value.second;
	}
};

// generator for cuset
template <typename T> struct ValueFactory<T, T> {
	static T make(const T &value)
	{
		return value;
	}
	static T key(const T &value)
	{
		return value;
	}
...
template <typename T> struct Value : ValueFactory<typename T::key_type, typename T::value_type> {
};
...
std::pair<intptr_t, intptr_t> CheckRecursiveRange(RangeType range)
{
	...
	for (Iterator i = range.begin(), e = range.end(); i != e; ++i) {
		++sum.first;
		sum.second += Value<ContainerType>::get(*i);
	}
	if (range.is_divisible()) {
		...
		sum1.first += sum2.first;
		sum1.second += sum2.second;
		ASSERT(sum == sum1, "Mismatched ranges after division");
	}
	return sum;
}

template <typename T> struct SpecialTests {
	static void Test(const char *str)
	{
		REMARK("skipped -- specialized %s tests\n", str);
	}
};

#if __TBB_INITIALIZER_LISTS_PRESENT
template <typename container_type> bool equal_containers(container_type const &lhs, container_type const &rhs)
{
...
template <typename Table, typename MultiTable> void TestInitList(std::initializer_list<typename Table::value_type> il)
{
...
void check_value_state(/* typename do_check_element_state =*/tbb::internal::true_type, T const &t, const char *filename,
                       int line)
{
...
template <Harness::StateTrackableBase::State desired_state, typename T>
void check_value_state(/* typename do_check_element_state =*/tbb::internal::false_type, T const &, const char *, int)
{ /*do nothing*/
}
#define ASSERT_VALUE_STATE(do_check_element_state, state, value) \
	check_value_state<state>(do_check_element_state, value, __FILE__, __LINE__)

#if __TBB_CPP11_RVALUE_REF_PRESENT
template <typename T, typename do_check_element_state> void test_rvalue_insert()
{
	...
	ASSERT(ins.second == true && Value<container_t>::get(*(ins.first)) == 1,
...
std::pair<typename container_t::iterator, bool> call_emplace_impl(container_t &c, arg_t &&k,
...
template <typename container_t, typename arg_t, typename value_t>
typename container_t::iterator call_emplace_hint_impl(container_t &c, typename container_t::const_iterator hint,
                                                      arg_t &&k, value_t *)
{
	//this a set
...
template <typename container_t, typename arg_t, typename first_t, typename second_t>
typename container_t::iterator call_emplace_hint_impl(container_t &c, typename container_t::const_iterator hint,
                                                      arg_t &&k, std::pair<first_t, second_t> *)
{
	//this is a map
...
template <typename T, typename do_check_element_state> void test_emplace_insert()
{
	typedef T container_t;
...
template <typename T, typename do_check_element_state> void test_basic(const char *str, do_check_element_state)
{
	T cont;
	const T &ccont(cont);
	...
	if (T::allow_multimapping) {
		// std::pair<iterator, bool> insert(const value_type& obj);
		ASSERT(ins2.second == true && Value<T>::get(*(ins2.first)) == 1,
		       "Element 1 has not been inserted properly");
	...
	// T(const T& _Umap)
	T newcont = ccont;
	ASSERT(T::allow_multimapping ? (newcont.size() == 3) : (newcont.size() == 2),
	...
	ASSERT(T::allow_multimapping ? (cont.size() == 3) : (cont.size() == 2),
	...
	newcont.rehash(16);
	ASSERT(T::allow_multimapping ? (newcont.size() == 3) : (newcont.size() == 2),
	       "Rehash should not affect the container elements");
	...
	for (int i = 0; i < 256; i++) {
		...
		ASSERT(ins3.second == true && Value<T>::get(*(ins3.first)) == i,
		       "Element 1 has not been inserted properly");
	}
...
template <typename T> void test_basic(const char *str)
{
...
template <typename T> class FillTable : NoAssign {
	T &table;
	const int items;
	bool my_asymptotic;
    ...
    public:
	FillTable(T &t, int i, bool asymptotic)
	        : table(t)
	        , items(i)
	        , my_asymptotic(asymptotic)
	{
	...
	void operator()(int threadn) const
	{
		if (threadn == 0) { // Fill even keys forward (single thread)
			...
			for (int i = 0; i < items; i += 2) {
				...
				ASSERT(Value<T>::get(*(pib.first)) == (my_asymptotic ? 1 : i),
				       "Element not properly inserted");
				ASSERT(last_inserted || !pib.second,
				       "Previous key was not inserted but this is inserted");
			...
			for (int i = 1; i < items; i += 2)
				...
				if (i % 32 == 1 && i + 6 < items) {
					if (my_asymptotic) {
						table.insert(
						        { Value<T>::make(1), Value<T>::make(1), Value<T>::make(1) });
						ASSERT(Value<T>::get(*table.find(1)) == 1,
						       "Element not properly inserted");
					} else {
						table.insert({ Value<T>::make(i), Value<T>::make(i + 2),
						               Value<T>::make(i + 4) });
						ASSERT(Value<T>::get(*table.find(i)) == i,
						       "Element not properly inserted");
						ASSERT(Value<T>::get(*table.find(i + 2)) == i + 2,
						       "Element not properly inserted");
						ASSERT(Value<T>::get(*table.find(i + 4)) == i + 4,
						       "Element not properly inserted");
					}
					i += 4;
				} else
...
template <typename ContainerType, typename RangeType> struct ParallelTraverseBody : NoAssign {
	const int n;
...
template <typename T> class CheckTable : NoAssign {
	T &table;
...
template <typename T> void test_concurrent(const char *tablename, bool asymptotic = false)
{
#if TBB_USE_ASSERT
	int items = 2000;
#else
...
template <bool doCall> struct CallIf {
	template <typename FuncType> void operator()(FuncType func) const
	{
		func();
	}
};
...
template <typename ValueType> class TestRange : NoAssign {
	const std::list<ValueType> &my_lst;
	std::vector<tbb::atomic<bool> > &my_marks;

    public:
	TestRange(const std::list<ValueType> &lst, std::vector<tbb::atomic<bool> > &marks)
	        : my_lst(lst)
	        , my_marks(marks)
	{
	...
	{
		doTestRange(r.begin(), r.end());
	}
	template <typename Iterator> void doTestRange(Iterator i, Iterator j) const
	{
		for (Iterator it = i; it != j;) {
			Iterator prev_it = it++;
			typename std::list<ValueType>::const_iterator it2 =
			        std::search(my_lst.begin(), my_lst.end(), prev_it, it, Harness::IsEqual());
...
template <> class tbb_hash<std::shared_ptr<int> > {
    public:
	size_t operator()(const std::shared_ptr<int> &key) const
	{
...
template <> class tbb_hash<std::weak_ptr<int> > {
    public:
	size_t operator()(const std::weak_ptr<int> &key) const
	{
		return tbb_hasher(*key.lock());
	}
};
...
template <bool, typename Table> void TestMapSpecificMethods(Table &, const typename Table::value_type &)
{ /* do nothing for a common case */
}

template <bool defCtorPresent, typename Table> class CheckValue : NoAssign {
	Table &my_c;
...
struct unordered_move_traits_base {
	...
	static unordered_type &construct_container(tbb::aligned_space<unordered_type> &storage, iterator_type begin,
	                                           iterator_type end)
	{
	...
	{
		size_t deault_n_of_buckets =
		        8; //can not use concurrent_unordered_base::n_of_buckets as it is inaccessible
		new (storage.begin()) unordered_type(begin, end, deault_n_of_buckets, typename unordered_type::hasher(),
		                                     typename unordered_type::key_equal(), a);
		return *storage.begin();
	}
...
template <typename container_traits> void test_rvalue_ref_support(const char *container_name)
{
...
template <bool defCtorPresent, typename Table> void Examine(Table c, const std::list<typename Table::value_type> &lst)
{
	...
	for (typename std::list<value_type>::const_iterator it = lst.begin(); it != lst.end();) {
		...
		ASSERT(std::search(c.unsafe_begin(index), c.unsafe_end(index), prev_it, it, Harness::IsEqual()) !=
		               c.unsafe_end(index),
		       NULL);
	}
...
void TypeTester(const std::list<typename Table::value_type> &lst)
{
	...
	ASSERT(lst.size() <= 100,
	       "The test has O(n^2) complexity so a big number of elements can lead long execution time");
...
{
__TBB_STATIC_ASSERT((tbb::internal::select_size_t_constant<1234, 1234>::value == 1234),
                    "select_size_t_constant::value is not compile time constant");
...
__TBB_STATIC_ASSERT((tbb::internal::select_size_t_constant<0x12345678U, 0x091A2B3C091A2B3CULL>::value % ~0U ==
                     0x12345678U),
