...
template <class Counter> class check_type : Harness::NoAfterlife {
	Counter id;
	bool am_ready;
	...
	check_type(const check_type &other)
	        : Harness::NoAfterlife(other)
	{
		other.AssertLive();
		AssertLive();
		id = other.id;
...
template <class Counter> tbb::atomic<int> check_type<Counter>::check_type_counter;
...
template <class MyClass> struct Check {
	Check()
	{
	} // creation does nothing
	~Check()
	{
	} // destruction checks nothing
};

template <class Counttype> struct Check<check_type<Counttype> > {
	Check()
	{
		check_type<Counttype>::check_type_counter = 0;
	}
	~Check()
	{
		ASSERT(check_type<Counttype>::check_type_counter == 0,
		       "check_type constructions and destructions don't match");
	}
};
