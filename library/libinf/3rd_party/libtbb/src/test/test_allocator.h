...
template <typename A> struct is_zero_filling {
	static const bool value = false;
};

int NumberOfFoo;

template <typename T, size_t N> struct Foo {
	T foo_array[N];
	Foo()
	{
...
template <typename T, typename A> void TestBasic(A &a)
{
	T x;
	const T cx = T();
...
#if __TBB_ALLOCATOR_CONSTRUCT_VARIADIC
	{
		typedef typename A::template rebind<std::pair<typename A::value_type, typename A::value_type> >::other
		        pair_allocator_type;
		...
		ASSERT(NumberOfFoo == NumberOfFooBeforeConstruct + 2,
		       "constructor for Foo not called appropriate number of times?");

		pair_allocator.destroy(pair_pointer);
		ASSERT(NumberOfFoo == NumberOfFooBeforeConstruct,
		       "destructor for Foo not called appropriate number of times?");
...
template <typename A> struct Body : NoAssign {
	...
	void operator()(size_t thread_id) const
	{
		...
		for (size_t k = 0; k < max_k; ++k) {
			...
			if (!array[i])
				check_allocate(array, i, thread_id);
			else
				check_deallocate(array, i, thread_id);
		}
		for (size_t k = 0; k < 256; ++k)
			if (array[k])
...
template <typename U, typename A> void Test(A &a)
{
...
template <typename Allocator> int TestMain(const Allocator &a = Allocator())
{
	NumberOfFoo = 0;
