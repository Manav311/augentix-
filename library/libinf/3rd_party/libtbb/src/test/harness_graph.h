...
// only in these tests.
template <typename InputType, typename OutputType> struct convertor {
...
template <typename InputType> struct convertor<InputType, tbb::flow::continue_msg> {
...
template <typename OutputType> struct convertor<tbb::flow::continue_msg, OutputType> {
...
template <size_t N> struct mof_helper {
	template <typename InputType, typename ports_type>
	static inline void output_converted_value(const InputType &i, ports_type &p)
	{
		(void)tbb::flow::get<N - 1>(p).try_put(
		        convertor<InputType, typename tbb::flow::tuple_element<N - 1, ports_type>::type::output_type>::
		                convert_value(i));
		output_converted_value<N - 1>(i, p);
	}
};

template <> struct mof_helper<1> {
	template <typename InputType, typename ports_type>
	static inline void output_converted_value(const InputType &i, ports_type &p)
	{
		// just emit a default-constructed object
		(void)tbb::flow::get<0>(p).try_put(
		        convertor<InputType, typename tbb::flow::tuple_element<0, ports_type>::type::output_type>::
		                convert_value(i));
	}
};

template <typename InputType, typename OutputType> struct harness_graph_default_functor {
...
template <typename OutputType> struct harness_graph_default_functor<tbb::flow::continue_msg, OutputType> {
...
template <typename InputType> struct harness_graph_default_functor<InputType, tbb::flow::continue_msg> {
...
template <> struct harness_graph_default_functor<tbb::flow::continue_msg, tbb::flow::continue_msg> {
...
template <typename InputType, typename OutputSet> struct harness_graph_default_multifunction_functor {
...
template <typename InputType, typename OutputType> struct harness_graph_executor {
	typedef OutputType (*function_ptr_type)(InputType v);

	template <typename RW> struct mutex_holder {
		static RW mutex;
	};
	...
	template <typename RW> static inline OutputType tfunc(InputType v)
	{
	...
	template <typename RW> struct tfunctor {
		tbb::atomic<size_t> my_execute_count;
		tfunctor()
		{
			my_execute_count = 0;
		}
		tfunctor(const tfunctor &f)
		{
			my_execute_count = f.my_execute_count;
		}
...
template <typename InputType, typename OutputTuple> struct harness_graph_multifunction_executor {
	...
	template <typename RW> static inline void tfunc(const InputType &v, ports_type &p)
	{
	...
	template <typename RW> struct tfunctor {
		...
		{
			typename RW::scoped_lock l(harness_graph_multifunction_executor::mutex_holder<RW>::mutex,
			                           /*write=*/false);
			my_execute_count.fetch_and_increment();
...
template <typename InputType, typename OutputType>
typename harness_graph_executor<InputType, OutputType>::function_ptr_type
        harness_graph_executor<InputType, OutputType>::fptr =
...
template <typename InputType, typename OutputTuple>
typename harness_graph_multifunction_executor<InputType, OutputTuple>::mfunction_ptr_type
        harness_graph_multifunction_executor<InputType, OutputTuple>::fptr =
...
template <typename T> struct harness_counting_receiver : public tbb::flow::receiver<T>, NoCopy {
...
//! Counts the number of puts received
template <typename T> struct harness_mapped_receiver : public tbb::flow::receiver<T>, NoCopy {
	tbb::atomic<size_t> my_count;
	T max_value;
	size_t num_copies;
	...
	/* override */ tbb::task *try_put_task(const T &t)
	{
		if (my_map) {
			...
			std::pair<typename map_type::iterator, bool> r =
			        (*my_map).insert(typename map_type::value_type(t, a));
			if (r.second == false) {
...
//! Counts the number of puts received
template <typename T> struct harness_counting_sender : public tbb::flow::sender<T>, NoCopy {
	...
	harness_counting_sender(size_t limit)
	        : my_limit(limit)
	{
		my_receiver = NULL;
		my_count = 0;
...
template <typename T> struct serial_fn_body {
	tbb::atomic<int> *_flag;
	serial_fn_body(tbb::atomic<int> &myatomic)
	        : _flag(&myatomic)
	{
	}
...
template <typename T> struct serial_continue_body {
	tbb::atomic<int> *_flag;
	serial_continue_body(tbb::atomic<int> &myatomic)
	        : _flag(&myatomic)
	{
	}
...
template <typename SV, typename TV> bool lists_match(SV &sv, TV &tv)
{
	if (sv.size() != tv.size()) {
		return false;
	}
...
template <typename T, typename BufferType> void test_resets()
{
	const int NN = 3;
	...
	tbb::flow::function_node<T, T, tbb::flow::rejecting> sfn(g, tbb::flow::serial,
	                                                         serial_fn_body<T>(serial_fn_state0));
	...
	for (int icnt = 0; icnt < 2; ++icnt) {
		...
		ASSERT(2 == (int)outt || 1 == (int)outt,
		       " buffer had incorrect message"); // the one not consumed by the node.
...
template <typename NODE_TYPE> class test_buffer_base_extract {
    protected:
	tbb::flow::graph &g;
	NODE_TYPE &in0;
	NODE_TYPE &in1;
	...
	void make_and_validate_full_graph()
	{
		...
		ASSERT(in0.successor_count() == 1 && in0_s_list.size() == 1 && *(in0_s_list.begin()) == ms_ptr,
		       "expected 1 successor");
		ASSERT(in1.predecessor_count() == 0 && in1_p_list.size() == 0, "expected 0 predecessors");
		ASSERT(in1.successor_count() == 1 && in1_s_list.size() == 1 && *(in1_s_list.begin()) == ms_ptr,
		       "expected 1 successor");
		...
		ASSERT(out0.predecessor_count() == 1 && out0_p_list.size() == 1 && *(out0_p_list.begin()) == mp_ptr,
		       "expected 1 predecessor");
		ASSERT(out0.successor_count() == 0 && out0_s_list.size() == 0, "expected 0 successors");
		ASSERT(out1.predecessor_count() == 1 && out1_p_list.size() == 1 && *(out1_p_list.begin()) == mp_ptr,
		       "expected 1 predecessor");
		...
		mp_list_iter = mp_list.begin();
		++mp_list_iter;
		int second_pred = *mp_list_iter == ins[0] ? 0 : (*mp_list_iter == ins[1] ? 1 : -1);
		ASSERT(first_pred != -1 && second_pred != -1 && first_pred != second_pred,
		       "bad predecessor(s) for middle");
		...
		ASSERT(first_succ != -1 && second_succ != -1 && first_succ != second_succ,
		       "bad successor(s) for middle");
    ...
    public:
	test_buffer_base_extract(tbb::flow::graph &_g, NODE_TYPE &i0, NODE_TYPE &i1, NODE_TYPE &m, NODE_TYPE &o0,
	                         NODE_TYPE &o1)
	        : g(_g)
	        , in0(i0)
	        , in1(i1)
...
template <typename NODE_TYPE> class test_buffer_extract : public test_buffer_base_extract<NODE_TYPE> {
    protected:
	tbb::flow::graph my_g;
	NODE_TYPE my_in0;
	NODE_TYPE my_in1;
    ...
    public:
	test_buffer_extract()
	        : test_buffer_base_extract<NODE_TYPE>(my_g, my_in0, my_in1, my_middle, my_out0, my_out1)
	        , my_in0(my_g)
	        , my_in1(my_g)
	        , my_middle(my_g)
	        , my_out0(my_g)
...
template <>
class test_buffer_extract<tbb::flow::sequencer_node<int> >
        : public test_buffer_base_extract<tbb::flow::sequencer_node<int> > {
    protected:
    ...
    public:
	test_buffer_extract()
	        : test_buffer_base_extract<my_node_t>(my_g, my_in0, my_in1, my_middle, my_out0, my_out1)
	        , my_in0(my_g, always_zero())
	        , my_in1(my_g, always_zero())
	        , my_middle(my_g, always_inc(middle_count))
	        , my_out0(my_g, always_inc(out0_count))
	        , my_out1(my_g, always_inc(out1_count))
	{
	}
};
...
template <template <class> class NType, typename ItemType> void test_extract_on_node()
{
	tbb::flow::graph g;
	ItemType dont_care;
	...
	for (int i = 0; i < 2; ++i) {
		...
		ASSERT(node0.predecessor_count() == 2 && q0.successor_count() == 1 && q1.successor_count() == 1,
		       "bad predecessor count");
		...
		node0.copy_predecessors(pv);
		node0.copy_successors(sv);
		ASSERT(node0.predecessor_count() == 0 && q0.successor_count() == 0 && q1.successor_count() == 0,
		       "error in pred count after extract");
		ASSERT(pv.size() == 0, "error in pred array count after extract");
		ASSERT(node0.successor_count() == 0 && q2.predecessor_count() == 0,
		       "error in succ count after extract");
