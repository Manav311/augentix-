...
class coarse_grained_raii_lru_cache : tbb::internal::no_assign {
	...
	coarse_grained_raii_lru_cache(value_function_type f, std::size_t number_of_lru_history_items)
	        : my_value_function(f)
	...
	struct handle_move_t : tbb::internal::no_assign {
		...
		handle_move_t(coarse_grained_raii_lru_cache &cache_ref, typename map_storage_type::reference value_ref)
		        : my_cache_ref(cache_ref)
		        , my_value_ref(value_ref){};
	};
	class handle_object {
	    ...
	    public:
		handle_object(coarse_grained_raii_lru_cache &cache_ref, typename map_storage_type::reference value_ref)
		...
		value_type &value()
		{
			return my_value_ref.second.my_value;
		}
		~handle_object()
		{
			if (my_cache_pointer) {
