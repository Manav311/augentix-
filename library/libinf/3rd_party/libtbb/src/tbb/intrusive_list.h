...
struct intrusive_list_node {
	intrusive_list_node *my_prev_node, *my_next_node;
#if TBB_USE_ASSERT
	intrusive_list_node()
	{
		my_prev_node = my_next_node = this;
	}
...
template <class List, class T> class intrusive_list_base {
	...
	template <class Iterator> class iterator_impl {
		Iterator &self()
		{
	...
	void assert_ok() const
	{
		__TBB_ASSERT((my_head.my_prev_node == &my_head && !my_size) ||
		                     (my_head.my_next_node != &my_head && my_size > 0),
	...
	bool empty() const
	{
		return my_head.my_next_node == &my_head;
	}
	...
	iterator begin()
	{
		return iterator(my_head.my_next_node);
	}
	...
	{
		return const_iterator(my_head.my_next_node);
	}
	...
	void remove(T &val)
	{
		...
		__TBB_ASSERT(node(val).my_prev_node->my_next_node == &node(val) &&
		                     node(val).my_next_node->my_prev_node == &node(val),
		             "Element to remove is not in the list");
		--my_size;
...
class memptr_intrusive_list : public intrusive_list_base<memptr_intrusive_list<T, U, NodePtr>, T> {
	...
	static T &item(intrusive_list_node *node)
	{
		...
		return *reinterpret_cast<T *>((char *)node -
		                              ((ptrdiff_t) & (reinterpret_cast<T *>(0x1000)->*NodePtr) - 0x1000));
	}
...
/** The list does not have ownership of its items. Its purpose is to avoid dynamic
    ...
    The class is not thread safe. **/
template <class T> class intrusive_list : public intrusive_list_base<intrusive_list<T>, T> {
