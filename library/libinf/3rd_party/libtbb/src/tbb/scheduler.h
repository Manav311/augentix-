...
class generic_scheduler : public scheduler, public ::rml::job, public intrusive_list_node, public scheduler_state {
	...
	bool can_steal()
	{
		...
		return my_stealing_threshold < (uintptr_t)&anchor &&
		       (uintptr_t)__TBB_get_bsp() < my_rsb_stealing_threshold;
#else
		return my_stealing_threshold < (uintptr_t)&anchor;
#endif
	}
	...
	template <free_task_hint h> void free_task(task &t);
	...
	static void assert_context_valid(const task_group_context *tgc)
	{
		...
		__TBB_ASSERT(!(ctx & ~(3 | (7 << task_group_context::traits_offset))),
		             msg); // the value fits known values of versions and traits
	...
	enum cilk_state_t{ cs_none = 0xF000, // Start at nonzero value so that we can detect use of zeroed memory.
		           cs_running, cs_limbo, cs_freed };
...
template <free_task_hint hint> void generic_scheduler::free_task(task &t)
{
#if __TBB_HOARD_NONLOCAL_TASKS
	...
	__TBB_ASSERT(!(h & local_task) || (!p.origin || uintptr_t(p.origin) > uintptr_t(4096)),
	             "local_task means allocated");
...
inline intptr_t generic_scheduler::effective_reference_priority() const
{
	...
	return !worker_outermost_level() || (my_arena->my_num_workers_allotted < my_arena->num_workers_active()
