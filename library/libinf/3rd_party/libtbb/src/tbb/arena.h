...
class arena : public padded<arena_base> {
	...
	template <unsigned ref_param> inline void on_thread_leaving();
	...
	template <bool as_worker> size_t occupy_free_slot(generic_scheduler &s);
...
template <unsigned ref_param> inline void arena::on_thread_leaving()
{
	//
	...
	if (ref_param == ref_external && my_num_slots != my_num_reserved_slots && 0 == m->my_num_workers_soft_limit &&
