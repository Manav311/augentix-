...
class auto_empty_task {
	...
	~auto_empty_task()
	{
		my_scheduler->free_task<small_local_task>(*my_task);
	}
...
template <typename T, size_t max_segments = 16> class fast_reverse_vector {
    public:
	...
	size_t size() const
	{
		return m_size + m_cur_segment_size - m_pos;
	}
	...
	void push_back(const T &val)
	{
		if (!m_pos) {
			...
			m_segments[m_num_segments++] = m_cur_segment =
			        (T *)NFS_Allocate(m_cur_segment_size, sizeof(T), NULL);
