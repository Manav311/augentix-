...
#define ANDROID_ITTNOTIFY_DEFAULT_PATH_MASK(x) "/data/data/com.intel.vtune/perfrun/lib" #x "/runtime/libittnotify.so"
...
static __itt_group_alias group_alias[] = {
	{ "KMP_FOR_TPROFILE",
	  (__itt_group_id)(__itt_group_control | __itt_group_thread | __itt_group_sync | __itt_group_mark) },
	{ "KMP_FOR_TCHECK", (__itt_group_id)(__itt_group_control | __itt_group_thread | __itt_group_sync |
	                                     __itt_group_fsync | __itt_group_mark | __itt_group_suppress) },
...
#define ITT_STUB(api, type, name, args, params, nameindll, group, format)                  \
	{ ITT_TO_STR(ITT_JOIN(__itt_, nameindll)), (void **)(void *)&ITTNOTIFY_NAME(name), \
	  (void *)(size_t)&ITT_VERSIONIZE(ITT_JOIN(_N_(name), _init)),                     \
	  (void *)(size_t)&ITT_VERSIONIZE(ITT_JOIN(_N_(name), _init)), (__itt_group_id)(group) },
...
#define ITT_STUB(api, type, name, args, params, nameindll, group, format)                  \
	{ ITT_TO_STR(ITT_JOIN(__itt_, nameindll)), (void **)(void *)&ITTNOTIFY_NAME(name), \
	  (void *)(size_t)&ITT_VERSIONIZE(ITT_JOIN(_N_(name), _init)), NULL, (__itt_group_id)(group) },
#define ITT_STUBV ITT_STUB
#include "ittnotify_static.h"
...
static __itt_domain *ITTAPI ITT_VERSIONIZE(ITT_JOIN(_N_(domain_createW), _init))(const wchar_t *name)
{
	...
	if (_N_(_ittapi_global).api_initialized) {
		if (ITTNOTIFY_NAME(domain_createW) &&
		    ITTNOTIFY_NAME(domain_createW) != ITT_VERSIONIZE(ITT_JOIN(_N_(domain_createW), _init))) {
...
#endif /* ITT_PLATFORM==ITT_PLATFORM_WIN */
{
	...
	if (_N_(_ittapi_global).api_initialized) {
		...
		if (ITTNOTIFY_NAME(domain_createA) &&
		    ITTNOTIFY_NAME(domain_createA) != ITT_VERSIONIZE(ITT_JOIN(_N_(domain_createA), _init))) {
			__itt_mutex_unlock(&_N_(_ittapi_global).mutex);
			return ITTNOTIFY_NAME(domain_createA)(name);
		}
#else
		if (ITTNOTIFY_NAME(domain_create) &&
		    ITTNOTIFY_NAME(domain_create) != ITT_VERSIONIZE(ITT_JOIN(_N_(domain_create), _init))) {
			__itt_mutex_unlock(&_N_(_ittapi_global).mutex);
...
static void ITTAPI ITT_VERSIONIZE(ITT_JOIN(_N_(thread_set_nameW), _init))(const wchar_t *name)
{
	...
	if (ITTNOTIFY_NAME(thread_set_nameW) &&
	    ITTNOTIFY_NAME(thread_set_nameW) != ITT_VERSIONIZE(ITT_JOIN(_N_(thread_set_nameW), _init))) {
...
#endif /* ITT_PLATFORM==ITT_PLATFORM_WIN */
{
	...
	if (ITTNOTIFY_NAME(thread_set_nameA) &&
	    ITTNOTIFY_NAME(thread_set_nameA) != ITT_VERSIONIZE(ITT_JOIN(_N_(thread_set_nameA), _init))) {
		ITTNOTIFY_NAME(thread_set_nameA)(name);
	}
#else /* ITT_PLATFORM==ITT_PLATFORM_WIN */
	if (ITTNOTIFY_NAME(thread_set_name) &&
	    ITTNOTIFY_NAME(thread_set_name) != ITT_VERSIONIZE(ITT_JOIN(_N_(thread_set_name), _init))) {
...
static void ITTAPI ITT_VERSIONIZE(ITT_JOIN(_N_(thread_ignore), _init))(void)
{
	...
	if (ITTNOTIFY_NAME(thread_ignore) &&
	    ITTNOTIFY_NAME(thread_ignore) != ITT_VERSIONIZE(ITT_JOIN(_N_(thread_ignore), _init))) {
...
static const char *__itt_get_lib_name(void)
{
	...
	if (lib_name == NULL) {
		...
		if (itt_marker_file_fd == -1) {
			...
			res = snprintf(app_sandbox_file, PATH_MAX - 1, "/data/data/%s/%s", package_name,
			               marker_filename);
			if (res < 0) {
				ITT_ANDROID_LOGE("Unable to concatenate marker file string.");
				return lib_name;
...
static __itt_group_id __itt_get_groups(void)
{
	...
	if (group_str != NULL) {
		...
		for (i = 0; group_list[i].id != __itt_group_none; i++)
			if (group_list[i].id != __itt_group_all && group_list[i].id > __itt_group_splitter_min &&
			...
			if (current_thread == 0) {
				...
				if (groups != __itt_group_none || lib_name != NULL) {
					_N_(_ittapi_global).lib =
					        __itt_load_lib((lib_name == NULL) ? ittnotify_lib_name : lib_name);
					...
					if (_N_(_ittapi_global).lib != NULL) {
						...
						case 1:
...
#if ITT_PLATFORM == ITT_PLATFORM_WIN
								ITTNOTIFY_NAME(sync_createA) =
								        ITTNOTIFY_NAME(sync_set_nameA);
								ITTNOTIFY_NAME(sync_createW) =
								        ITTNOTIFY_NAME(sync_set_nameW);
#else /* ITT_PLATFORM!=ITT_PLATFORM_WIN */
								ITTNOTIFY_NAME(sync_create) =
							...
							for (i = 0; _N_(_ittapi_global).api_list_ptr[i].name != NULL;
							     i++)
								if (_N_(_ittapi_global).api_list_ptr[i].group &
								    zero_group)
									*_N_(_ittapi_global).api_list_ptr[i].func_ptr =
									        _N_(_ittapi_global)
									                .api_list_ptr[i]
