...
enum DecreaseOrIncrease { decrease, increase };
...
template <typename Arg, typename Compare> inline void AtomicUpdate(Arg &location, Arg newVal, const Compare &cmp)
{
...
template <unsigned NUM> class BitMaskBasic {
...
template <unsigned NUM> class BitMaskMin : public BitMaskBasic<NUM> {
    public:
	void set(size_t idx, bool val)
	{
...
template <unsigned NUM> class BitMaskMax : public BitMaskBasic<NUM> {
    public:
	void set(size_t idx, bool val)
	{
...
*/
template <size_t MIN_SIZE, size_t MAX_SIZE, uint32_t CACHE_STEP, int TOO_LARGE, int ON_MISS, int LONG_WAIT>
...
template <typename Props> class LargeObjectCacheImpl {
    private:
	...
	class BinsSummary {
		...
		// "too large" criteria
		bool isLOCTooLarge() const
		{
			return cachedSz > Props::TooLargeFactor * usedSz;
		}
	...
	class CacheBin {
     ...
     Set on cache miss. */
		...
		void ExecuteOperation(CacheBinOperation *op, ExtMemoryPool *extMemPool, BinBitMask *bitMask, int idx,
		                      bool longLifeTime = true);
		...
		LargeMemoryBlock *putList(LargeMemoryBlock *head, LargeMemoryBlock *tail, BinBitMask *bitMask, int idx,
		                          int num);
...
class LargeObjectCache {
	static const size_t minLargeSize = 8 * 1024, maxLargeSize = 8 * 1024 * 1024,
	...
	static const uint32_t largeBlockCacheStep = 8 * 1024, hugeBlockCacheStep = 512 * 1024;
    ...
    */
	...
	{
		return size < maxLargeSize ? alignUp(size, largeBlockCacheStep) : alignUp(size, hugeBlockCacheStep);
	}
...
template <bool> struct MasterIndexSelect {
	typedef uint32_t master_type;
};

template <> struct MasterIndexSelect<false> {
...
class BackendSync {
	...
	intptr_t getNumOfMods() const
	{
		return FencedLoad(binsModifications);
	}
...
class CoalRequestQ { // queue of free blocks that coalescing was delayed
	...
	intptr_t blocksInFly() const
	{
		return FencedLoad(inFlyBlocks);
	}
};
...
*/
	...
	enum { VALID_BLOCK_IN_BIN = 1 // valid block added to bin, not returned as result
	};

    public:
	static const int freeBinsNum = (maxBinned_HugePage - minBinnedSize) / LargeObjectCache::largeBlockCacheStep + 1;
	...
	enum { NO_BIN = -1, HUGE_BIN = freeBinsNum - 1 };
	...
	class IndexedBins {
		...
		FreeBlock *getFromBin(int binIdx, BackendSync *sync, size_t size, bool resSlabAligned, bool alignedBin,
		                      bool wait, int *resLocked);

	    public:
		FreeBlock *findBlock(int nativeBin, BackendSync *sync, size_t size, bool resSlabAligned,
		                     bool alignedBin, int *numOfLockedBins);
	...
	class AdvRegionsBins {
	    ...
	    public:
		void registerBin(int regBin)
		{
			bins.set(regBin, 1);
		}
		int getMinUsedBin(int start) const
		{
			return bins.getMinTrue(start);
		}
		void reset()
		{
			bins.reset();
	...
	class MaxRequestComparator {
	    ...
	    public:
		MaxRequestComparator(const Backend *be)
		        : backend(be)
		{
		}
	...
	class UsedAddressRange {
		...
		uintptr_t leftBound, rightBound;
		MallocMutex mutex;
	...
	FreeBlock *askMemFromOS(size_t totalReqSize, intptr_t startModifiedCnt, int *lockedBinsThreshold,
	                        int numOfLockedBins, bool *splittable);
	FreeBlock *genericGetBlock(int num, size_t size, bool resSlabAligned);
	void genericPutBlock(FreeBlock *fBlock, size_t blockSz);
	FreeBlock *splitUnalignedBlock(FreeBlock *fBlock, int num, size_t size, bool needAlignedRes);
	FreeBlock *splitAlignedBlock(FreeBlock *fBlock, int num, size_t size, bool needAlignedRes);
	...
	intptr_t blocksInCoalescing() const
	{
		return coalescQ.blocksInFly();
	}
	void verify();
	...
	{
		BlockI *b = (BlockI *)genericGetBlock(num, slabSize, /*resSlabAligned=*/true);
	...
	bool ptrCanBeValid(void *ptr) const
	{
		return usedAddrRange.inRange(ptr);
	}
	...
	IndexedBins freeLargeBins, freeAlignedBins;
};
...
struct ExtMemoryPool {
	...
	rawFreeType rawFree;
	size_t granularity;
	bool keepAllMemory, delayRegsReleasing,
	        // TODO: implements fixedPool with calling rawFree on destruction
	        fixedPool;
	TLSKey tlsPointerKey; // per-pool TLS key

	bool init(intptr_t poolId, rawAllocType rawAlloc, rawFreeType rawFree, size_t granularity, bool keepAllMemory,
	          bool fixedPool);
	bool initTLS();
	...
	bool destroy()
	{
		MALLOC_ASSERT(isPoolValid(), "Possible double pool_destroy or heap corruption");
		if (!userPool()) {
			loc.reset();
			allLocalCaches.reset();
		}
	...
	void delayRegionsReleasing(bool mode)
	{
		delayRegsReleasing = mode;
	}
...
inline bool Backend::inUserPool() const
{
	return extMemPool->userPool();
}
