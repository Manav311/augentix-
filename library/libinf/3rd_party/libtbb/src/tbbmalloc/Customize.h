...
template <typename T> static inline bool isAligned(T *arg, uintptr_t alignment)
{
...
template <typename T> inline T itt_load_word_with_acquire(T &src)
{
...
template <typename OperationType> struct MallocAggregator {
	typedef tbb::internal::aggregator_generic<OperationType> type;
};

//! aggregated_operation base class
template <typename Derived> struct MallocAggregatedOperation {
