...
template <int N> struct join_helper {
	...
	template <typename TupleType> static inline void release_my_reservation(TupleType &my_input)
	{
	...
	{
		if (!tbb::flow::get<N - 1>(my_input).reserve(tbb::flow::get<N - 1>(out)))
			return false;
	...
	template <typename InputTuple> static inline void reset_ports(InputTuple &my_input)
	{
	...
	static inline void copy_key_functors(KeyFuncTuple &my_inputs, KeyFuncTuple &other_inputs)
	{
		if (tbb::flow::get<N - 1>(other_inputs).get_my_key_func()) {
			tbb::flow::get<N - 1>(my_inputs).set_my_key_func(
			        tbb::flow::get<N - 1>(other_inputs).get_my_key_func()->clone());
		}
...
template <> struct join_helper<1> {
	...
	static inline void copy_key_functors(KeyFuncTuple &my_inputs, KeyFuncTuple &other_inputs)
	{
		if (tbb::flow::get<0>(other_inputs).get_my_key_func()) {
			tbb::flow::get<0>(my_inputs).set_my_key_func(
			        tbb::flow::get<0>(other_inputs).get_my_key_func()->clone());
		}
	}
	template <typename InputTuple> static inline void reset_inputs(InputTuple &my_input, reset_flags f)
	{
...
class join_node_FE<reserving, InputTuple, OutputTuple> : public forwarding_base {
	...
	join_node_FE(const join_node_FE &other)
	        : forwarding_base(*(other.forwarding_base::graph_pointer))
	        , my_node(NULL)
	{
		ports_with_no_inputs = N;
		join_helper<N>::set_join_node_pointer(my_inputs, this);
...
class join_node_FE<key_matching<K, KHash>, InputTuple, OutputTuple>
        : public matching_forwarding_base<K>,
          ...
          public hash_buffer< // typedefed below to key_to_count_buffer_type
                  ...
                  internal::type_to_key_function_body<count_element<typename tbb::internal::strip<K>::type>,
	...
	typedef internal::type_to_key_function_body_leaf<count_element_type, unref_key_type &, key_to_count_func>
	...
	join_node_FE(const join_node_FE &other)
	        : forwarding_base_type(*(other.forwarding_base_type::graph_pointer))
	        , key_to_count_buffer_type()
	        , output_buffer_type()
	{
		my_node = NULL;
	...
	/*override*/ task *increment_key_count(unref_key_type const &t, bool handle_task)
	...
	/*override*/ task *decrement_port_count(bool /*handle_task*/)
	{
		__TBB_ASSERT(false, NULL);
		return NULL;
	}
...
class join_node_base : public graph_node, public join_node_FE<JP, InputTuple, OutputTuple>, public sender<OutputTuple> {
    protected:
		...
		while (op_list) {
			...
			case reg_succ: {
				...
				if (tuple_build_may_succeed() && !forwarder_busy &&
				    this->graph_node::my_graph.is_active()) {
					task *rtask = new (task::allocate_additional_child_of(
					        *(this->graph_node::my_graph.root_task())))
...
template <int N, template <class> class PT, typename OutputTuple, typename JP> struct join_base {
	typedef typename internal::join_node_base<JP, typename wrap_tuple_elements<N, PT, OutputTuple>::type,
	                                          OutputTuple>
	        type;
};
...
struct join_base<N, key_matching_port, OutputTuple, key_matching<K, KHash> > {
	...
	typedef typename internal::join_node_base<
	        key_traits_type,
	        // ports type
	        typename wrap_key_tuple_elements<N, key_matching_port, key_traits_type, OutputTuple>::type, OutputTuple>
	        type;
};
...
class unfolded_join_node<2, key_matching_port, OutputTuple, key_matching<K, KHash> >
	...
	unfolded_join_node(graph &g)
	        : base_type(g,
	                    func_initializer_type(
	                            new internal::type_to_key_function_body_leaf<T0, K, key_from_message_body<K, T0> >(
	                                    key_from_message_body<K, T0>()),
	                            new internal::type_to_key_function_body_leaf<T1, K, key_from_message_body<K, T1> >(
	...
	unfolded_join_node(graph &g, Body0 body0, Body1 body1)
	        : base_type(g, func_initializer_type(new internal::type_to_key_function_body_leaf<T0, K, Body0>(body0),
	...
	unfolded_join_node(graph &g)
	        : base_type(g,
	                    func_initializer_type(
	                            ...
	                            new internal::type_to_key_function_body_leaf<T1, K, key_from_message_body<K, T1> >(
	                                    key_from_message_body<K, T1>()),
	                            new internal::type_to_key_function_body_leaf<T2, K, key_from_message_body<K, T2> >(
	...
	unfolded_join_node(graph &g)
	        : base_type(g,
	                    func_initializer_type(
	                            ...
	                            new internal::type_to_key_function_body_leaf<T2, K, key_from_message_body<K, T2> >(
	                                    key_from_message_body<K, T2>()),
	                            new internal::type_to_key_function_body_leaf<T3, K, key_from_message_body<K, T3> >(
	...
	unfolded_join_node(graph &g)
	        : base_type(g,
	                    func_initializer_type(
	                            ...
	                            new internal::type_to_key_function_body_leaf<T3, K, key_from_message_body<K, T3> >(
	                                    key_from_message_body<K, T3>()),
	                            new internal::type_to_key_function_body_leaf<T4, K, key_from_message_body<K, T4> >(
	...
	unfolded_join_node(graph &g)
	        : base_type(g,
	                    func_initializer_type(
	                            ...
	                            new internal::type_to_key_function_body_leaf<T4, K, key_from_message_body<K, T4> >(
	                                    key_from_message_body<K, T4>()),
	                            new internal::type_to_key_function_body_leaf<T5, K, key_from_message_body<K, T5> >(
	...
	unfolded_join_node(graph &g)
	        : base_type(g,
	                    func_initializer_type(
	                            ...
	                            new internal::type_to_key_function_body_leaf<T5, K, key_from_message_body<K, T5> >(
	                                    key_from_message_body<K, T5>()),
	                            new internal::type_to_key_function_body_leaf<T6, K, key_from_message_body<K, T6> >(
	...
	unfolded_join_node(graph &g)
	        : base_type(g,
	                    func_initializer_type(
	                            ...
	                            new internal::type_to_key_function_body_leaf<T6, K, key_from_message_body<K, T6> >(
	                                    key_from_message_body<K, T6>()),
	                            new internal::type_to_key_function_body_leaf<T7, K, key_from_message_body<K, T7> >(
	...
	unfolded_join_node(graph &g)
	        : base_type(g,
	                    func_initializer_type(
	                            ...
	                            new internal::type_to_key_function_body_leaf<T7, K, key_from_message_body<K, T7> >(
	                                    key_from_message_body<K, T7>()),
	                            new internal::type_to_key_function_body_leaf<T8, K, key_from_message_body<K, T8> >(
...
class unfolded_join_node<10, key_matching_port, OutputTuple, key_matching<K, KHash> >
	...
	typedef typename tbb::flow::tuple<f0_p, f1_p, f2_p, f3_p, f4_p, f5_p, f6_p, f7_p, f8_p, f9_p>
	        func_initializer_type;
	...
	unfolded_join_node(graph &g)
	        : base_type(g,
	                    func_initializer_type(
	                            ...
	                            new internal::type_to_key_function_body_leaf<T8, K, key_from_message_body<K, T8> >(
	                                    key_from_message_body<K, T8>()),
	                            new internal::type_to_key_function_body_leaf<T9, K, key_from_message_body<K, T9> >(
