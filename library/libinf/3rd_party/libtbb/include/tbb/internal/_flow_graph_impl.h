...
template <typename Output, typename Body> class source_body_leaf : public source_body<Output> {
	...
	/*override*/ bool operator()(Output &output)
	{
		return body(output);
	}
...
template <typename T, typename = typename T::async_gateway_type> void set_async_gateway(T *body, void *g)
{
...
class type_to_key_function_body_leaf : public type_to_key_function_body<Input, Output> {
	...
	/*override*/ Output operator()(const Input &i)
	{
		return body(i);
	}
	B get_body()
