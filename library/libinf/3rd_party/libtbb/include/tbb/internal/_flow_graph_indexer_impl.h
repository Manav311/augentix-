		...
		while (op_list) {
			...
			case try__put_task: {
				...
				__TBB_store_with_release(current->status,
				                         SUCCEEDED); // return of try_put_task actual return value
			} break;
#if TBB_PREVIEW_FLOW_GRAPH_FEATURES
			case add_blt_succ:
...
template <typename InputTuple> struct input_types<6, InputTuple> {
	...
	typedef typename internal::tagged_msg<size_t, first_type, second_type, third_type, fourth_type, fifth_type,
	                                      sixth_type>
	        type;
};
