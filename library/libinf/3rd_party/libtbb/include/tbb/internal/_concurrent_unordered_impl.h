...
template <typename T, typename Allocator> class split_ordered_list {
	...
	struct node : tbb::internal::no_assign {
		...
		nodeptr_t atomic_set_next(nodeptr_t new_node, nodeptr_t current_node)
		{
			...
			nodeptr_t exchange_node =
			        tbb::internal::as_atomic(my_next).compare_and_swap(new_node, current_node);
	...
	nodeptr_t create_node_v(__TBB_FORWARDING_REF(Args) __TBB_PARAMETER_PACK args)
	{
		...
		__TBB_TRY
		{
			new (static_cast<void *>(&pnode->my_element))
			        T(__TBB_PACK_EXPANSION(tbb::internal::forward<Args>(args)));
		}
		__TBB_CATCH(...)
		{
			my_node_allocator.deallocate(pnode, 1);
		...
		for (;;) {
			...
			if (where == last || get_order_key(where) > order_key) {
				...
				nodeptr_t inserted_node =
				        try_insert_atomic(it.get_node_ptr(), dummy_node, where.get_node_ptr());
	...
	void move_all(self_type &source)
	{
		...
		for (raw_const_iterator it = first; it != last;) {
			...
			nodeptr_t dummy_node = pnode->is_dummy() ?
	...
	void check_range(raw_iterator first, raw_iterator last)
	{
		...
		for (raw_iterator it = first; it != last; ++it) {
			...
			__TBB_ASSERT(next == raw_end() || get_order_key(next) >= get_order_key(it),
...
template <typename Traits> class concurrent_unordered_base : public Traits {
	        ...
	        , my_maximum_bucket_size((float)initial_bucket_load)
	{
		...
		my_number_of_buckets = size_type(1)
		                       << __TBB_Log2((uintptr_t)n_of_buckets * 2 - 1); // round up to power of 2
		internal_init();
	}
			...
			if (!right.my_solist.empty()) {
				...
				for (raw_const_iterator it = ++(right.my_solist.raw_begin()),
					...
					if (pnode->is_dummy()) {
						...
						size_type bucket = __TBB_ReverseBits(pnode->get_order_key()) %
	...
	class const_range_type : tbb::internal::no_assign {
		...
		void set_midpoint() const
		{
			if (my_begin_node == my_end_node) // not divisible
				...
				size_t mid_bucket = __TBB_ReverseBits(begin_key + (end_key - begin_key) / 2) %
				...
				if (__TBB_ReverseBits(mid_bucket) > begin_key) {
					...
					my_midpoint_node =
					        my_table.my_solist.first_real_iterator(my_table.get_bucket(mid_bucket));
				} else {
	...
	std::pair<iterator, bool> internal_insert(__TBB_FORWARDING_REF(ValueType) value, nodeptr_t pnode = NULL)
	{
		...
		for (;;) {
			if (where == last || solist_t::get_order_key(where) > order_key ||
				...
				if (!pnode)
					pnode = my_solist.create_node(order_key,
					                              tbb::internal::forward<ValueType>(value));
				...
				if (result.second) {
					...
					continue;
			           ...
			           my_hash_compare(get_key(*where), get_key(value)) ==
			                   0) { // Element already in the list, return it
				if (pnode)
