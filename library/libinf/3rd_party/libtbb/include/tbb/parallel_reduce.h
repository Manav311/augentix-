...
template <typename Range, typename Body, typename Partitioner> task *start_reduce<Range, Body, Partitioner>::execute()
{
	...
	if (my_context == right_child) {
		...
		if (!itt_load_word_with_acquire(
		            parent_ptr->my_body)) { // TODO: replace by is_stolen_task() or by parent_ptr->ref_count() == 2???
...
template <typename Body> class finish_deterministic_reduce : public task {
	...
	template <typename Range, typename Body_> friend class start_deterministic_reduce;
};

//! Task type used to split the work of parallel_deterministic_reduce.
/** @ingroup algorithms */
template <typename Range, typename Body> class start_deterministic_reduce : public task {
...
template <typename Range, typename Body> task *start_deterministic_reduce<Range, Body>::execute()
{
...
{
	internal::start_reduce<Range, Body, const __TBB_DEFAULT_PARTITIONER>::run(range, body,
...
Value parallel_reduce(const Range &range, const Value &identity, const RealBody &real_body, const Reduction &reduction)
{
	...
	internal::start_reduce<Range, internal::lambda_reduce_body<Range, Value, RealBody, Reduction>,
...
/** @ingroup algorithms **/
template <typename Range, typename Body> void parallel_deterministic_reduce(const Range &range, Body &body)
{
                                    ...
                                    const Reduction &reduction)
{
	...
	internal::start_deterministic_reduce<
	        Range, internal::lambda_reduce_body<Range, Value, RealBody, Reduction> >::run(range, body);
	return body.result();
}
                                    ...
                                    const Reduction &reduction, task_group_context &context)
{
	internal::lambda_reduce_body<Range, Value, RealBody, Reduction> body(identity, real_body, reduction);
	internal::start_deterministic_reduce<
	        Range, internal::lambda_reduce_body<Range, Value, RealBody, Reduction> >::run(range, body, context);
	return body.result();
}
#endif /* __TBB_TASK_GROUP_CONTEXT */
//@}
