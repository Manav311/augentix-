...
static inline __TBB_IA32_CAS8_NOINLINE int64_t __TBB_machine_cmpswp8(volatile void *ptr, int64_t value,
                                                                     int64_t comparand)
{
//TODO: remove the extra part of condition once __TBB_GCC_BUILTIN_ATOMICS_PRESENT is lowered to gcc version 4.1.2
	...
	__asm__ __volatile__("lock\n\t cmpxchg8b %1\n\t"
	                     ...
	                     : "m"(*(__TBB_VOLATILE int64_t *)ptr), "0"(comparand), "b"(i32[0]), "c"(i32[1])
	                     : "memory");
#endif /* __PIC__ */
	return result;
...
static inline void __TBB_machine_or(volatile void *ptr, uint32_t addend)
{
	__asm__ __volatile__("lock\norl %1,%0"
	                     ...
	                     : "r"(addend), "m"(*(__TBB_VOLATILE uint32_t *)ptr)
	                     : "memory");
}
