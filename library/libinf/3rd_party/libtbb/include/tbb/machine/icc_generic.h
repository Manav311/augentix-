...
{
template <typename T> T convert_argument(T value)
{
	return value;
}
...
template <typename T> void *convert_argument(T *value)
{
	return (void *)value;
}
}
...
#define __TBB_MACHINE_DEFINE_ATOMICS(S, T, M)                                                               \
	...
	{                                                                                                   \
		__atomic_compare_exchange_strong_explicit((T *)ptr, &comparand, value,                      \
...
template <typename T> struct machine_load_store<T, 8> {
	...
	{
		if (tbb::internal::is_aligned(&location, 8)) {
			__atomic_store_explicit(&location, icc_intrinsics_port::convert_argument(value),
			                        memory_order_release);
		} else {
...
template <typename T> inline void __TBB_machine_OR(T *operand, T addend)
{
	__atomic_fetch_or_explicit(operand, addend, tbb::internal::memory_order_seq_cst);
}

template <typename T> inline void __TBB_machine_AND(T *operand, T addend)
{
