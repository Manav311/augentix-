...
class WaitingStateBase : public IWaitingState {
	...
	template <typename F> auto Run(F &&func) -> decltype(func(false))
	{ // mutex_
		std::lock_guard<std::mutex> scopedLock(mutex_);
		return func(isWaiting_);
	} // ~mutex_

	template <typename F> bool RunIfWaiting(F &&func)
	{ // mutex_
		std::lock_guard<std::mutex> scopedLock(mutex_);
	...
	template <typename F> bool RunIfNotWaiting(F &&func)
	{ // mutex_
		std::lock_guard<std::mutex> scopedLock(mutex_);
...
class BlockingCondition {
	...
	template <typename F> bool RunIfBlocked(F &&func)
	{ // mutex_
		std::lock_guard<std::mutex> scopedLock(mutex_);
	...
	template <typename F> bool RunIfUnblocked(F &&func)
	{ // mutex_
		std::lock_guard<std::mutex> scopedLock(mutex_);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename TMutex, bool is_enabled> class ConditionalCriticalSection;

template <typename TMutex> class ConditionalCriticalSection<TMutex, false> {
    public:
	template <typename F> void Access(const F &f)
	{
		f();
	}
};

template <typename TMutex> class ConditionalCriticalSection<TMutex, true> {
    public:
	template <typename F> void Access(const F &f)
	{ // mutex_
