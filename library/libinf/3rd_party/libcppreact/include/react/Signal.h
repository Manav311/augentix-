...
template <typename D, typename S> class VarSignal;

template <typename D, typename S, typename TOp> class TempSignal;

///////////////////////////////////////////////////////////////////////////////////////////////////
/// SignalPack - Wraps several nodes in a tuple. Create with comma operator.
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D, typename... TValues> class SignalPack {
    public:
...
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D, typename... TValues> auto With(const Signal<D, TValues> &... deps) -> SignalPack<D, TValues...>
{
...
{
	return VarSignal<D, S>(std::make_shared<REACT_IMPL::VarNode<D, S> >(std::forward<V>(value)));
}

template <typename D, typename S> auto MakeVar(std::reference_wrapper<S> value) -> VarSignal<D, S &>
{
	return VarSignal<D, S &>(std::make_shared<REACT_IMPL::VarNode<D, std::reference_wrapper<S> > >(value));
}

///////////////////////////////////////////////////////////////////////////////////////////////////
/// MakeVar (higher order reactives)
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D, typename V, typename S = typename std::decay<V>::type, typename TInner = typename S::ValueT,
          class = typename std::enable_if<IsSignal<S>::value>::type>
auto MakeVar(V &&value) -> VarSignal<D, Signal<D, TInner> >
{
	return VarSignal<D, Signal<D, TInner> >(
	        std::make_shared<REACT_IMPL::VarNode<D, Signal<D, TInner> > >(std::forward<V>(value)));
}
...
auto MakeVar(V &&value) -> VarSignal<D, Events<D, TInner> >
{
	return VarSignal<D, Events<D, TInner> >(
	        std::make_shared<REACT_IMPL::VarNode<D, Events<D, TInner> > >(std::forward<V>(value)));
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// Single arg
template <typename D, typename TValue, typename FIn, typename F = typename std::decay<FIn>::type,
...
auto MakeSignal(const Signal<D, TValue> &arg, FIn &&func) -> TempSignal<D, S, TOp>
{
	return TempSignal<D, S, TOp>(
	        std::make_shared<REACT_IMPL::SignalOpNode<D, S, TOp> >(std::forward<FIn>(func), GetNodePtr(arg)));
}

// Multiple args
template <typename D, typename... TValues, typename FIn, typename F = typename std::decay<FIn>::type,
...
auto MakeSignal(const SignalPack<D, TValues...> &argPack, FIn &&func) -> TempSignal<D, S, TOp>
{
	...
	return REACT_IMPL::apply(NodeBuilder_(std::forward<FIn>(func)), argPack.Data);
}
...
#define REACT_DECLARE_OP(op, name)                                                                                     \
	template <typename T> struct name##OpFunctor {                                                                 \
		...
		{                                                                                                      \
			return op v;                                                                                   \
                                                                                                                       ...
                                                                                                                       \
	template <typename TSignal, typename D = typename TSignal::DomainT, typename TVal = typename TSignal::ValueT,  \
	          class = typename std::enable_if<IsSignal<TSignal>::value>::type, typename F = name##OpFunctor<TVal>, \
	...
	{                                                                                                              \
		return TempSignal<D, S, TOp>(                                                                          \
		        std::make_shared<REACT_IMPL::SignalOpNode<D, S, TOp> >(F(), GetNodePtr(arg)));                 \
	}                                                                                                              \
                                                                                                                       \
	template <typename D, typename TVal, typename TOpIn, typename F = name##OpFunctor<TVal>,                       \
	          typename S = typename std::result_of<F(TVal)>::type,                                                 \
	          typename TOp = REACT_IMPL::FunctionOp<S, F, TOpIn> >                                                 \
	auto operator op(TempSignal<D, TVal, TOpIn> &&arg)->TempSignal<D, S, TOp>                                      \
	{                                                                                                              \
		return TempSignal<D, S, TOp>(                                                                          \
		        std::make_shared<REACT_IMPL::SignalOpNode<D, S, TOp> >(F(), arg.StealOp()));                   \
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
#define REACT_DECLARE_OP(op, name)                                                                                      \
	template <typename L, typename R> struct name##OpFunctor {                                                      \
		auto operator()(const L &lhs, const R &rhs) const -> decltype(std::declval<L>() op std::declval<R>())   \
		{                                                                                                       \
			return lhs op rhs;                                                                              \
		}                                                                                                       \
	};                                                                                                              \
                                                                                                                        \
	template <typename L, typename R> struct name##OpRFunctor {                                                     \
		...
		name##OpRFunctor(const name##OpRFunctor &other) = delete;                                               \
                                                                                                                        \
		auto operator()(const R &rhs) const -> decltype(std::declval<L>() op std::declval<R>())                 \
		{                                                                                                       \
		...
		L LeftVal;                                                                                              \
                                                                                                                        ...
                                                                                                                        \
	template <typename L, typename R> struct name##OpLFunctor {                                                     \
		...
		name##OpLFunctor(const name##OpLFunctor &other) = delete;                                               \
                                                                                                                        \
		auto operator()(const L &lhs) const -> decltype(std::declval<L>() op std::declval<R>())                 \
		{                                                                                                       \
		...
		R RightVal;                                                                                             \
                                                                                                                        ...
                                                                                                                        \
	template <typename TLeftSignal, typename TRightSignal, typename D = typename TLeftSignal::DomainT,              \
	          typename TLeftVal = typename TLeftSignal::ValueT, typename TRightVal = typename TRightSignal::ValueT, \
	          ...
	          typename TOp = REACT_IMPL::FunctionOp<S, F, REACT_IMPL::SignalNodePtrT<D, TLeftVal>,                  \
	                                                REACT_IMPL::SignalNodePtrT<D, TRightVal> > >                    \
	auto operator op(const TLeftSignal &lhs, const TRightSignal &rhs)->TempSignal<D, S, TOp>                        \
	{                                                                                                               \
		return TempSignal<D, S, TOp>(std::make_shared<REACT_IMPL::SignalOpNode<D, S, TOp> >(                    \
		        F(), GetNodePtr(lhs), GetNodePtr(rhs)));                                                        \
	}                                                                                                               \
                                                                                                                        \
	template <typename TLeftSignal, typename TRightValIn, typename D = typename TLeftSignal::DomainT,               \
	...
	{                                                                                                               \
		return TempSignal<D, S, TOp>(std::make_shared<REACT_IMPL::SignalOpNode<D, S, TOp> >(                    \
		        F(std::forward<TRightValIn>(rhs)), GetNodePtr(lhs)));                                           \
                                                                                                                        ...
                                                                                                                        \
	template <typename TLeftValIn, typename TRightSignal, typename D = typename TRightSignal::DomainT,              \
	          ...
	          typename TOp = REACT_IMPL::FunctionOp<S, F, REACT_IMPL::SignalNodePtrT<D, TRightVal> > >              \
	auto operator op(TLeftValIn &&lhs, const TRightSignal &rhs)->TempSignal<D, S, TOp>                              \
	{                                                                                                               \
		return TempSignal<D, S, TOp>(std::make_shared<REACT_IMPL::SignalOpNode<D, S, TOp> >(                    \
		        F(std::forward<TLeftValIn>(lhs)), GetNodePtr(rhs)));                                            \
	}                                                                                                               \
	template <typename D, typename TLeftVal, typename TLeftOp, typename TRightVal, typename TRightOp,               \
	          typename F = name##OpFunctor<TLeftVal, TRightVal>,                                                    \
	          typename S = typename std::result_of<F(TLeftVal, TRightVal)>::type,                                   \
	          typename TOp = REACT_IMPL::FunctionOp<S, F, TLeftOp, TRightOp> >                                      \
	auto operator op(TempSignal<D, TLeftVal, TLeftOp> &&lhs, TempSignal<D, TRightVal, TRightOp> &&rhs)              \
	        ->TempSignal<D, S, TOp>                                                                                 \
	{                                                                                                               \
		return TempSignal<D, S, TOp>(                                                                           \
		        std::make_shared<REACT_IMPL::SignalOpNode<D, S, TOp> >(F(), lhs.StealOp(), rhs.StealOp()));     \
	}                                                                                                               \
                                                                                                                        \
	template <typename D, typename TLeftVal, typename TLeftOp, typename TRightSignal,                               \
	          typename TRightVal = typename TRightSignal::ValueT,                                                   \
	          class = typename std::enable_if<IsSignal<TRightSignal>::value>::type,                                 \
	          typename F = name##OpFunctor<TLeftVal, TRightVal>,                                                    \
	          typename S = typename std::result_of<F(TLeftVal, TRightVal)>::type,                                   \
	          typename TOp = REACT_IMPL::FunctionOp<S, F, TLeftOp, REACT_IMPL::SignalNodePtrT<D, TRightVal> > >     \
	auto operator op(TempSignal<D, TLeftVal, TLeftOp> &&lhs, const TRightSignal &rhs)->TempSignal<D, S, TOp>        \
	{                                                                                                               \
		return TempSignal<D, S, TOp>(                                                                           \
		        std::make_shared<REACT_IMPL::SignalOpNode<D, S, TOp> >(F(), lhs.StealOp(), GetNodePtr(rhs)));   \
	}                                                                                                               \
                                                                                                                        \
	template <typename TLeftSignal, typename D, typename TRightVal, typename TRightOp,                              \
	          typename TLeftVal = typename TLeftSignal::ValueT,                                                     \
	          class = typename std::enable_if<IsSignal<TLeftSignal>::value>::type,                                  \
	          typename F = name##OpFunctor<TLeftVal, TRightVal>,                                                    \
	          typename S = typename std::result_of<F(TLeftVal, TRightVal)>::type,                                   \
	          typename TOp = REACT_IMPL::FunctionOp<S, F, REACT_IMPL::SignalNodePtrT<D, TLeftVal>, TRightOp> >      \
	auto operator op(const TLeftSignal &lhs, TempSignal<D, TRightVal, TRightOp> &&rhs)->TempSignal<D, S, TOp>       \
	{                                                                                                               \
		return TempSignal<D, S, TOp>(                                                                           \
		        std::make_shared<REACT_IMPL::SignalOpNode<D, S, TOp> >(F(), GetNodePtr(lhs), rhs.StealOp()));   \
	}                                                                                                               \
                                                                                                                        \
	template <typename D, typename TLeftVal, typename TLeftOp, typename TRightValIn,                                \
	          typename TRightVal = typename std::decay<TRightValIn>::type,                                          \
	          class = typename std::enable_if<!IsSignal<TRightVal>::value>::type,                                   \
	          typename F = name##OpLFunctor<TLeftVal, TRightVal>,                                                   \
	          typename S = typename std::result_of<F(TLeftVal)>::type,                                              \
	          typename TOp = REACT_IMPL::FunctionOp<S, F, TLeftOp> >                                                \
	auto operator op(TempSignal<D, TLeftVal, TLeftOp> &&lhs, TRightValIn &&rhs)->TempSignal<D, S, TOp>              \
	{                                                                                                               \
		return TempSignal<D, S, TOp>(std::make_shared<REACT_IMPL::SignalOpNode<D, S, TOp> >(                    \
		        F(std::forward<TRightValIn>(rhs)), lhs.StealOp()));                                             \
	}                                                                                                               \
                                                                                                                        \
	template <typename TLeftValIn, typename D, typename TRightVal, typename TRightOp,                               \
	          typename TLeftVal = typename std::decay<TLeftValIn>::type,                                            \
	          class = typename std::enable_if<!IsSignal<TLeftVal>::value>::type,                                    \
	          typename F = name##OpRFunctor<TLeftVal, TRightVal>,                                                   \
	          typename S = typename std::result_of<F(TRightVal)>::type,                                             \
	          typename TOp = REACT_IMPL::FunctionOp<S, F, TRightOp> >                                               \
	auto operator op(TLeftValIn &&lhs, TempSignal<D, TRightVal, TRightOp> &&rhs)->TempSignal<D, S, TOp>             \
	{                                                                                                               \
		return TempSignal<D, S, TOp>(std::make_shared<REACT_IMPL::SignalOpNode<D, S, TOp> >(                    \
...
auto operator,(const Signal<D, TLeftVal> &a, const Signal<D, TRightVal> &b) -> SignalPack<D, TLeftVal, TRightVal>
{
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// Single arg
template <typename D, typename F, template <typename D_, typename V_> class TSignal, typename TValue,
          class = typename std::enable_if<IsSignal<TSignal<D, TValue> >::value>::type>
auto operator->*(const TSignal<D, TValue> &arg, F &&func) -> Signal<D, typename std::result_of<F(TValue)>::type>
{
...
/// Signal
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D, typename S> class Signal : public REACT_IMPL::SignalBase<D, S> {
    private:
...
template <typename D, typename S> class Signal<D, S &> : public REACT_IMPL::SignalBase<D, std::reference_wrapper<S> > {
    private:
...
/// VarSignal
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D, typename S> class VarSignal : public Signal<D, S> {
    private:
...
// Specialize for references
template <typename D, typename S> class VarSignal<D, S &> : public Signal<D, std::reference_wrapper<S> > {
    private:
...
/// TempSignal
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D, typename S, typename TOp> class TempSignal : public Signal<D, S> {
    private:
...
#define REACTIVE_REF(obj, name)                                                                                    \
	Flatten(MakeSignal(obj,                                                                                    \
	                   [](const REACT_MSVC_NO_TYPENAME REACT_IMPL::Identity<decltype(obj)>::Type::ValueT &r) { \
...
#define REACTIVE_PTR(obj, name)                                                                                  \
	Flatten(MakeSignal(obj, [](REACT_MSVC_NO_TYPENAME REACT_IMPL::Identity<decltype(obj)>::Type::ValueT r) { \
		assert(r != nullptr);                                                                            \
		using T = decltype(r->name);                                                                     \
		using S = REACT_MSVC_NO_TYPENAME REACT::DecayInput<T>::Type;                                     \
		return static_cast<S>(r->name);                                                                  \
