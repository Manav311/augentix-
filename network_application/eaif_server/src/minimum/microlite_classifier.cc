...
class LiteClassifier
{
	...
	const tflite::Model *m_model_fb;

	tflite::MicroInterpreter *m_model;
...
int LiteClassifier::LoadModels(const char *model_path)
{
	...
	if (m_model_fb == nullptr) {
		//lite_err("Cannot parse %s model data", g_model_name);
		lite_err("Cannot parse model data from %s!\n", model_path);
		return -1;
	}

	if (m_model_fb->version() != TFLITE_SCHEMA_VERSION) {
		lite_err("Model version does not match Schema: %d vs model: %d.\n", TFLITE_SCHEMA_VERSION, m_model_fb->version());
		return -1;
	}
	...
	m_model = new tflite::MicroInterpreter(m_model_fb, micro_op_resolver, m_tensor_arena,
                                    m_tensor_size,
                                   &micro_error_reporter);

	lite_check(m_model->AllocateTensors() == kTfLiteOk);
	lite_info_h("model arena used total %u Bytes\n", (uint32_t)m_model->arena_used_bytes());
	// 2.1 Fetch Quant info
	TfLiteTensor* input = m_model->input(0);
	TfLiteTensor* output = m_model->output(0);
	const TfLiteQuantizationParams *p = &output->params;
	m_config.quant_zero = p->zero_point;
	m_config.quant_scale = p->scale;

	const TfLiteIntArray *outdims = output->dims;
	m_output_dim[0] = outdims->data[0];
	m_output_dim[1] = outdims->data[1];

	const TfLiteIntArray *indims = input->dims;
