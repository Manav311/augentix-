...
class WindowsAudioInputDevice_common : public AudioInputDevice {
    public:
	static Boolean openWavInPort(int index, unsigned numChannels, unsigned samplingFrequency,
	                             unsigned granularityInMS);
	static void waveIn_close();
	static void waveInProc(WAVEHDR *hdr); // Windows audio callback function

    protected:
	WindowsAudioInputDevice_common(UsageEnvironment &env, int inputPortNumber, unsigned char bitsPerSample,
	...
	static DWORD bytesUsedAtReadHead; // number of bytes that have already been
	        // read at head
	static double uSecsPerByte; // used to adjust the time for # bytes consumed
	        // since arrival
