...
AudioInputDevice *AudioInputDevice::createNew(UsageEnvironment &env, int inputPortNumber, unsigned char bitsPerSample,
                                              unsigned char numChannels, unsigned samplingFrequency,
                                              unsigned granularityInMS)
{
	Boolean success;
	WindowsAudioInputDevice *newSource = new WindowsAudioInputDevice(
	        env, inputPortNumber, bitsPerSample, numChannels, samplingFrequency, granularityInMS, success);
	if (!success) {
		delete newSource;
		newSource = NULL;
	}
...
WindowsAudioInputDevice::WindowsAudioInputDevice(UsageEnvironment &env, int inputPortNumber,
        ...
        : WindowsAudioInputDevice_common(env, inputPortNumber, bitsPerSample, numChannels, samplingFrequency,
                                         granularityInMS)
        , fCurMixerId(-1)
{
...
Boolean WindowsAudioInputDevice::setInputPort(int portIndex)
{
	...
	if (allowedDeviceNames != NULL) {
		...
		for (i = 0; allowedDeviceNames[i] != NULL; ++i) {
			if (strncmp(ourMixers[newMixerId].name, allowedDeviceNames[i], strlen(allowedDeviceNames[i])) ==
			    0) {
...
void Mixer::open(unsigned numChannels, unsigned samplingFrequency, unsigned granularityInMS)
{
	...
	do {
		...
		for (i = 0; i < nWavIn; ++i) {
			...
			if (mixerGetLineInfo((HMIXEROBJ)index, &ml,
			                     MIXER_GETLINEINFOF_TARGETTYPE /*|MIXER_OBJECTF_MIXER*/) ==
			    MMSYSERR_NOERROR) {
		...
		if (mixerOpen(&newHMixer, index, (unsigned long)NULL, (unsigned long)NULL, MIXER_OBJECTF_MIXER) !=
		    MMSYSERR_NOERROR)
			break;
		if (newHMixer == NULL)
			break;
		...
		if (!WindowsAudioInputDevice_common::openWavInPort(uWavIn, numChannels, samplingFrequency,
		                                                   granularityInMS))
			break;
...
Boolean Mixer::enableInputPort(unsigned portIndex, char const *&errReason, MMRESULT &errCode)
{
  ...
  if ((errCode = mixerGetLineControls((HMIXEROBJ)hMixer, &mlc, MIXER_GETLINECONTROLSF_ONEBYTYPE/*|MIXER_OBJECTF_HMIXER*/)) != MMSYSERR_NOERROR) {
	...
	if ((errCode = mixerGetLineInfo((HMIXEROBJ)hMixer, &ml, MIXER_GETLINEINFOF_LINEID /*|MIXER_OBJECTF_HMIXER*/)) !=
	    MMSYSERR_NOERROR) {
		errReason = "mixerGetLineInfo()2";
		return False;
	}

	// Get Mixer/MUX control information (need control id to set and get control
	// details)
	mlc.cbStruct = sizeof mlc;
	...
	if (mc.cMultipleItems > 1) {
		...
		if (mc.dwControlID != 0xDEADBEEF) { // we know the control id for real
			...
			if ((errCode = mixerGetControlDetails(
			             (HMIXEROBJ)hMixer, &mcd,
			...
			// Hack: We couldn't find a MUX or MIXER control, so try to guess the
			// control id:
	...
	if ((errCode = mixerGetControlDetails((HMIXEROBJ)hMixer, &mcd,
