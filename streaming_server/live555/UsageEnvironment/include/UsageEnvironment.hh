...
class UsageEnvironment {
	...
	// like setResultMsg(), except that an 'errno' message is appended.  (If "err
	// == 0", the "getErrno()" code is used instead.)
	...
	virtual void internalError(); // used to 'handle' a 'should not occur'-type
	        // error condition within the library.
...
#define SOCKET_EXCEPTION (1 << 3)
	...
	// Delayed tasks, background I/O handling, and other events are handled,
	// sequentially (as a single thread of control).
	// (If "watchVariable" is not NULL, then we return from this routine when
	// *watchVariable != 0)

	virtual EventTriggerId createEventTrigger(TaskFunc *eventHandlerProc) = 0;
	// Creates a 'trigger' for an event, which - if it occurs - will be handled
	// (from the event loop) using "eventHandlerProc".
	// (Returns 0 iff no such trigger can be created (e.g., because of
	// implementation limits on the number of triggers).)
	virtual void deleteEventTrigger(EventTriggerId eventTriggerId) = 0;

	virtual void triggerEvent(EventTriggerId eventTriggerId, void *clientData = NULL) = 0;
	// Causes the (previously-registered) handler function for the specified event
	// to be handled (from the event loop).
	// The handler function is called with "clientData" as parameter.
	// Note: This function (unlike other library functions) may be called from an
	// external thread - to signal an external event.

	// The following two functions are deprecated, and are provided for
