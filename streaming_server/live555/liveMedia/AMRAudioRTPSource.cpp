...
class RawAMRRTPSource : public MultiFramedRTPSource {
    public:
	static RawAMRRTPSource *createNew(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
	                                  Boolean isWideband, Boolean isOctetAligned, Boolean isInterleaved,
    ...
    private:
	RawAMRRTPSource(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat, Boolean isWideband,
...
class AMRDeinterleaver : public AMRAudioSource {
    public:
	static AMRDeinterleaver *createNew(UsageEnvironment &env, Boolean isWideband, unsigned numChannels,
	                                   unsigned maxInterleaveGroupSize, RawAMRRTPSource *inputSource);

    private:
	AMRDeinterleaver(UsageEnvironment &env, Boolean isWideband, unsigned numChannels,
	                 unsigned maxInterleaveGroupSize, RawAMRRTPSource *inputSource);
	// called only by "createNew()"
...
AMRAudioSource *AMRAudioRTPSource::createNew(UsageEnvironment &env, Groupsock *RTPgs, RTPSource *&resultRTPSource,
                                             unsigned char rtpPayloadFormat, Boolean isWideband, unsigned numChannels,
                                             Boolean isOctetAligned, unsigned interleaving, Boolean robustSortingOrder,
                                             Boolean CRCsArePresent)
{
	...
	if (robustSortingOrder) {
		...
		env << "AMRAudioRTPSource::createNew(): The \"interleaving\" parameter (" << interleaving
		    << ") is much too large!\n";
		return NULL;
	}
	...
	if (!isOctetAligned) {
		if (interleaving > 0 || robustSortingOrder || CRCsArePresent) {
			env << "AMRAudioRTPSource::createNew(): 'Bandwidth-efficient mode' was "
			       "specified, along with interleaving, 'robust sorting order', "
	...
	resultRTPSource = rawRTPSource = RawAMRRTPSource::createNew(env, RTPgs, rtpPayloadFormat, isWideband,
	                                                            isOctetAligned, isInterleaved, CRCsArePresent);
	if (resultRTPSource == NULL)
		return NULL;

	AMRDeinterleaver *deinterleaver =
	        AMRDeinterleaver::createNew(env, isWideband, numChannels, maxInterleaveGroupSize, rawRTPSource);
...
RawAMRRTPSource *RawAMRRTPSource::createNew(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
                                            Boolean isWideband, Boolean isOctetAligned, Boolean isInterleaved,
                                            Boolean CRCsArePresent)
{
	return new RawAMRRTPSource(env, RTPgs, rtpPayloadFormat, isWideband, isOctetAligned, isInterleaved,
	                           CRCsArePresent);
}

RawAMRRTPSource::RawAMRRTPSource(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
                                 Boolean isWideband, Boolean isOctetAligned, Boolean isInterleaved,
                                 Boolean CRCsArePresent)
...
static unsigned short const frameBytesFromFT[16] = {
	12, 13, 15, 17, 19, 20, 26, 31, 5, FT_INVALID, FT_INVALID, FT_INVALID, FT_INVALID, FT_INVALID, FT_INVALID, 0
};
static unsigned short const frameBytesFromFTWideband[16] = { 17,         23,         32, 36, 40,         46,
	                                                     50,         58,         60, 5,  FT_INVALID, FT_INVALID,
...
#ifdef DEBUG
	fprintf(stderr,
	        "AMRBufferedPacket::nextEnclosedFrameSize(): frame #: %d, "
	        "FT: %d, isWideband: %d => frameSize: %d (dataSize: %d)\n",
...
AMRDeinterleaver *AMRDeinterleaver::createNew(UsageEnvironment &env, Boolean isWideband, unsigned numChannels,
                                              unsigned maxInterleaveGroupSize, RawAMRRTPSource *inputSource)
{
	return new AMRDeinterleaver(env, isWideband, numChannels, maxInterleaveGroupSize, inputSource);
}

AMRDeinterleaver::AMRDeinterleaver(UsageEnvironment &env, Boolean isWideband, unsigned numChannels,
                                   unsigned maxInterleaveGroupSize, RawAMRRTPSource *inputSource)
...
void AMRDeinterleaver::doGetNextFrame()
{
	...
	if (fDeinterleavingBuffer->retrieveFrame(fTo, fMaxSize, fFrameSize, fNumTruncatedBytes, fLastFrameHeader,
	...
	if (!fInputSource->isCurrentlyAwaitingData()) {
		fInputSource->getNextFrame(fDeinterleavingBuffer->inputBuffer(),
		                           fDeinterleavingBuffer->inputBufferSize(), afterGettingFrame, this,
...
void AMRDeinterleavingBuffer::deliverIncomingFrame(unsigned frameSize, RawAMRRTPSource *source,
                                                   struct timeval presentationTime)
{
	fILL = source->ILL();
...
#ifdef DEBUG
		fprintf(stderr,
		        "AMRDeinterleavingBuffer::deliverIncomingFrame() param "
	...
	--frameIndex; // because it was incremented by the source when this frame was
	        // read
	u_int8_t frameHeader;
...
// We've moved to a new interleave group
#ifdef DEBUG
		fprintf(stderr, "AMRDeinterleavingBuffer::deliverIncomingFrame(): new "
		                "interleave group\n");
#endif
	...
	unsigned const binNumber = ((ILP + frameBlockIndex * (fILL + 1)) * fNumChannels + frameWithinFrameBlock) %
	                           fMaxInterleaveGroupSize; // the % is for sanity
#ifdef DEBUG
	fprintf(stderr,
	        "AMRDeinterleavingBuffer::deliverIncomingFrame(): frameIndex "
	        "%d (%d,%d) put in bank %d, bin %d (%d): size %d, header "
	        "0x%02x, presentationTime %lu.%06ld\n",
	        frameIndex, frameBlockIndex, frameWithinFrameBlock, fIncomingBankId, binNumber, fMaxInterleaveGroupSize,
	        frameSize, frameHeader, presentationTime.tv_sec, presentationTime.tv_usec);
#endif
                                               ...
                                               struct timeval &resultPresentationTime, Boolean &resultIsSynchronized)
{
	...
	if (outBin.fIsSynchronized) {
		// Don't consider the outgoing frame to be synchronized until we've received
		// at least a complete interleave cycle of
		// synchronized frames.  This ensures that the receiver will be getting all
		// synchronized frames from now on.
...
#ifdef DEBUG
	fprintf(stderr,
	        "AMRDeinterleavingBuffer::retrieveFrame(): from bank %d, bin "
	        "%d: size %d, header 0x%02x, presentationTime %lu.%06ld\n",
	        fIncomingBankId ^ 1, fNextOutgoingBin, resultFrameSize, resultFrameHeader,
	        resultPresentationTime.tv_sec, resultPresentationTime.tv_usec);
#endif
...
static unsigned short const frameBitsFromFTWideband[16] = { 132, 177, 253, 285, 317, 365, 397, 461,
	                                                    477, 40,  0,   0,   0,   0,   0,   0 };
