...
RTSPServer *RTSPServer::createNew(UsageEnvironment &env, Port ourPort, UserAuthenticationDatabase *authDatabase,
                                  unsigned reclamationTestSeconds, unsigned maxClientNumber)
{
	...
	if (maxClientNumber < 1) {
		fprintf(stderr, "Invalid Client number limit\r\n");
		return NULL;
	}
	return new RTSPServer(env, ourSocket, ourPort, authDatabase, reclamationTestSeconds, maxClientNumber);
}
...
void RTSPServer::addServerMediaSession(ServerMediaSession *serverMediaSession)
{
	...
	if (sessionName == NULL)
		sessionName = "";
	removeServerMediaSession(sessionName); // in case an existing
	        // "ServerMediaSession" with this name
	        // already exists
...
class RegisterRequestRecord : public RTSPRegisterSender {
	...
	RegisterRequestRecord(RTSPServer &ourServer, unsigned requestId, char const *remoteClientNameOrAddress,
	                      portNumBits remoteClientPortNum, char const *rtspURLToRegister,
	        ...
	        : RTSPRegisterSender(ourServer.envir(), remoteClientNameOrAddress, remoteClientPortNum,
	                             rtspURLToRegister, rtspRegisterResponseHandler, authenticator,
	...
	{
		// Remove ourself from the server's 'pending REGISTER requests' hash table
		// before we go:
	...
	{
		if (resultCode == 0) {
			// The "REGISTER" request succeeded, so use the still-open RTSP socket to
			// await incoming commands from the remote endpoint:
			int sock;
...
unsigned RTSPServer::registerStream(ServerMediaSession *serverMediaSession, char const *remoteClientNameOrAddress,
                                    portNumBits remoteClientPortNum, responseHandlerForREGISTER *responseHandler,
                                    char const *username, char const *password, Boolean receiveOurStreamViaTCP,
                                    char const *proxyURLSuffix)
{
	// Create a new "RegisterRequestRecord" that will send the "REGISTER" command.
	// (This object will automatically get deleted after we get a response to the
	// "REGISTER" command, or if we're deleted.)
	...
	new RegisterRequestRecord(*this, requestId, remoteClientNameOrAddress, remoteClientPortNum,
	                          rtspURL(serverMediaSession), responseHandler, authenticator, receiveOurStreamViaTCP,
	                          proxyURLSuffix);

	delete authenticator; // we can do this here because it was copied to the
...
char *RTSPServer::rtspURLPrefix(int clientSocket) const
{
	...
	if (clientSocket < 0) {
		...
		ourAddress.sin_addr.s_addr = ReceivingInterfaceAddr != 0 ? ReceivingInterfaceAddr :
	...
	if (portNumHostOrder == 554 /* the default port number */) {
		...
		sprintf(urlBuffer, "rtsp://%s:%hu/", AddressString(ourAddress).val(), portNumHostOrder);
	}
...
Boolean RTSPServer::setUpTunnelingOverHTTP(Port httpPort)
{
	...
	if (fHTTPServerSocket >= 0) {
		...
		envir().taskScheduler().turnOnBackgroundReadHandling(
		        fHTTPServerSocket, (TaskScheduler::BackgroundHandlerProc *)&incomingConnectionHandlerHTTP,
		        this);
		return True;
	}
...
// Don't disable it (by defining ALLOW_RTSP_SERVER_PORT_REUSE) unless you know
// what you're doing.
		...
		if (!increaseSendBufferTo(env, ourSocket, 512 * 1024))
			break;
...
Boolean RTSPServer::specialClientAccessCheck(int /*clientSocket*/, struct sockaddr_in & /*clientAddr*/,
...
RTSPServer::RTSPServer(UsageEnvironment &env, int ourSocket, Port ourPort, UserAuthenticationDatabase *authDatabase,
                       unsigned reclamationTestSeconds, unsigned maxClientNumber)
        : Medium(env)
        ...
        , fMaxAliveClientNumber(maxClientNumber)
{
	ignoreSigPipeOnSocket(ourSocket); // so that clients on the same host that are
	        // killed don't also kill us

	// Arrange to handle connections from others:
	env.taskScheduler().turnOnBackgroundReadHandling(
	        fRTSPServerSocket, (TaskScheduler::BackgroundHandlerProc *)&incomingConnectionHandlerRTSP, this);
}

RTSPServer::~RTSPServer()
{
	printf("RTSPServer::%s\n", __func__);
	printf("Close server sockets...\n");
	...
	printf("Close client connections...\n");
	...
	delete fClientConnectionsForHTTPTunneling; // all content was already removed
	        // as a result of the loop above

	printf("Close server media sessions...\n");
	// Delete all server media sessions
	ServerMediaSession *serverMediaSession;
	while ((serverMediaSession = (ServerMediaSession *)fServerMediaSessions->getFirst()) != NULL) {
		removeServerMediaSession(serverMediaSession); // will delete it, because it
		        // no longer has any 'client
		        // session' objects using it
	}
	delete fServerMediaSessions;

	printf("Delete pending REGISTER requests...\n");
	...
	printf("RTSPServer deconstructor done.\n");
}
...
void RTSPServer::incomingConnectionHandler(int serverSocket)
{
	...
	if (fClientSessions->numEntries() >= fMaxAliveClientNumber) {
		fprintf(stderr, "Failed to add new client, limit[%d]\n", fMaxAliveClientNumber);
		return ;
	}
...
RTSPServer::RTSPClientConnection::~RTSPClientConnection()
{
	...
	if (fOurSessionCookie != NULL) {
		// We were being used for RTSP-over-HTTP tunneling. Also remove ourselves
...
RTSPServer::RTSPClientConnection::ParamsForREGISTER::ParamsForREGISTER(RTSPServer::RTSPClientConnection *ourConnection,
...
void RTSPServer::RTSPClientConnection::handleCmd_GET_PARAMETER(char const * /*fullRequestStr*/)
{
	...
	// 'no op', and send back a dummy response.
	// (If you want to handle this type of "GET_PARAMETER" differently, you can do
	// so by defining a subclass of "RTSPServer"
	// and "RTSPServer::RTSPClientConnection", and then reimplement this virtual
...
void RTSPServer::RTSPClientConnection::handleCmd_SET_PARAMETER(char const * /*fullRequestStr*/)
{
	...
	// 'no op', and send back an empty response.
	// (If you want to handle this type of "SET_PARAMETER" differently, you can do
	// so by defining a subclass of "RTSPServer"
	// and "RTSPServer::RTSPClientConnection", and then reimplement this virtual
	// function in your subclass.)
...
void RTSPServer::RTSPClientConnection::handleCmd_DESCRIBE(char const *urlPreSuffix, char const *urlSuffix,
                                                          char const *fullRequestStr)
{
	...
	do {
		...
		if (session == NULL) {
			printf("%s: Failed to lookup ServerMediaSession\n", __func__);
			handleCmd_notFound();
			break;
		}

		printf("%s: Assembling SDP description... \n", __func__);
		...
		printf("%s: Generate RTSP URL \n", __func__);
		rtspURL = fOurServer.rtspURL(session, fClientInputSocket);
		printf("%s: URL found; printing... \n", __func__);
                          ...
                          unsigned resultMaxSize)
{
	...
	for (int i = 0; i < (int)(sourceLen - headerNameLen); ++i) {
		if (strncmp(&source[i], headerName, headerNameLen) == 0 && source[i + headerNameLen] == ':') {
			// We found the header.  Skip over any whitespace, then copy the rest of
			// the line to "resultStr":
			for (i += headerNameLen + 1; i < (int)sourceLen && (source[i] == ' ' || source[i] == '\t');
			     ++i) {
			}
			for (unsigned j = i; j < sourceLen; ++j) {
				if (source[j] == '\r' || source[j] == '\n') {
					// We've found the end of the line.  Copy it to the result (if it will
					// fit):
...
void RTSPServer::RTSPClientConnection::handleCmd_REGISTER(char const *url, char const *urlSuffix,
                                                          char const *fullRequestStr, Boolean reuseConnection,
                                                          Boolean deliverViaTCP, char const *proxyURLSuffix)
{
	char *responseStr;
	if (fOurServer.weImplementREGISTER(proxyURLSuffix, responseStr)) {
		...
		// We implement the "REGISTER" command by first replying to it, then
		// actually handling it
		// (in a separate event-loop task, that will get called after the reply has
		// been done):
		setRTSPResponse(responseStr == NULL ? "200 OK" : responseStr);
		delete[] responseStr;

		ParamsForREGISTER *registerParams =
		        new ParamsForREGISTER(this, url, urlSuffix, reuseConnection, deliverViaTCP, proxyURLSuffix);
...
{
	snprintf((char *)fResponseBuffer, sizeof fResponseBuffer, "HTTP/1.1 404 Not Found\r\n%s\r\n\r\n", dateHeader());
}
...
void RTSPServer::RTSPClientConnection::handleHTTPCmd_TunnelingGET(char const *sessionCookie)
{
	...
	// "POST" command (with the same 'session cookie')
	// can find us:
                                                                      ...
                                                                      unsigned extraDataSize)
{
	...
	// "RTSPClientConnection" object that should have been used to handle
	...
	if (prevClientConnection == NULL) {
		// There was no previous HTTP "GET" request; treat this "POST" request as
		// bad:
...
void RTSPServer::RTSPClientConnection::resetRequestBuffer()
{
	...
	fLastCRLF = &fRequestBuffer[-3]; // hack: Ensures that we don't think we have
	        // end-of-msg if the data starts with
	        // <CR><LF>
...
void RTSPServer::RTSPClientConnection::incomingRequestHandler1()
{
	...
	int bytesRead = readSocket(envir(), fClientInputSocket, &fRequestBuffer[fRequestBytesAlreadySeen],
...
void RTSPServer::RTSPClientConnection::handleAlternativeRequestByte1(u_int8_t requestByte)
{
	if (requestByte == 0xFF) {
		...
		// Another hack: The new handler of the input TCP socket no longer needs it,
		// so take back control of it:
...
static void parseTransportHeaderForREGISTER(char const *buf, Boolean &reuseConnection, Boolean &deliverViaTCP,
                                            char *&proxyURLSuffix)
{
...
// Terminate this connection:
		...
		if (fClientOutputSocket != fClientInputSocket && numBytesRemaining == 0) {
			// We're doing RTSP-over-HTTP tunneling, and input commands are assumed to
			// have been Base64-encoded.
			// We therefore Base64-decode as much of this new data as we can (i.e., up
			...
			if (numBytesToDecode > 0) {
...
#ifdef DEBUG
				fprintf(stderr, "Base64-decoded %d input bytes into %d new bytes:", numBytesToDecode,
				        decodedSize);
				...
				// Copy the new decoded bytes in place of the old ones (we can do this
				// because there are fewer decoded bytes than original):
				...
				newBytesRead = decodedSize + newBase64RemainderCount; // adjust to allow for the
				        // size of the new decoded
				        // data (+ remainder)
				delete[] decodedBytes;
			}
		...
		Boolean parseSucceeded = parseRTSPRequestString((char *)fRequestBuffer, fLastCRLF + 2 - fRequestBuffer,
		                                                ...
		                                                sizeof urlPreSuffix, urlSuffix, sizeof urlSuffix, cseq,
		                                                sizeof cseq, sessionIdStr, sizeof sessionIdStr,
		                                                contentLength);
		...
		if (parseSucceeded) {
			...
			fprintf(stderr,
			        ...
			        "\"%s\", Content-Length %u, with %ld bytes following the "
			        "message.\n",
			        cmdName, urlPreSuffix, urlSuffix, cseq, contentLength,
			        ptr + newBytesRead - (tmpPtr + 2));
#endif
			// If there was a "Content-Length:" header, then make sure we've received
			// all of the data that it specified:
			if (ptr + newBytesRead < tmpPtr + 2 + contentLength)
				break; // we still need more data; subsequent reads will give it to us

			// If the request included a "Session:" id, and it refers to a client
			// session that's
			// current ongoing, then use this command to indicate 'liveness' on that
			// client session:
			Boolean const requestIncludedSessionId = sessionIdStr[0] != '\0';
			if (requestIncludedSessionId) {
				clientSession = (RTSPServer::RTSPClientSession *)(fOurServer.fClientSessions->Lookup(
				        sessionIdStr));
			...
			if (strcmp(cmdName, "OPTIONS") == 0) {
				...
				// The special "*" URL means: an operation on the entire server.  This
				// works only for GET_PARAMETER and SET_PARAMETER:
				...
				if (!requestIncludedSessionId) {
					// No session id was present in the request.  So create a new
					// "RTSPClientSession" object
					// for this request.  Choose a random (unused) 32-bit integer for the
					// session id
					// (it will be encoded as a 8-digit hex number).  (We avoid choosing
					// session id 0,
					// because that has a special use (by
					...
					printf("Add client[%d]: %08X\r\n",
							fOurServer.fClientSessions->numEntries(), sessionId);
				...
				if (clientSession != NULL) {
					...
					if (requestIncludedSessionId) {
						/*client has ID and send, but not found in server*/
						handleCmd_sessionNotFound();
					} else {
						/*client has no ID . server can't create new for it*/
						handleCmd_notSupported();
					}
				}
				...
				// Because - unlike other commands - an implementation of this command
				// needs
				// the entire URL, we re-parse the command to get it:
				char *url = strDupSize((char *)fRequestBuffer);
				if (sscanf((char *)fRequestBuffer, "%*s %s", url) == 1) {
					// Check for special command-specific parameters in a "Transport:"
					// header:
					...
					handleCmd_REGISTER(url, urlSuffix, (char const *)fRequestBuffer,
...
#ifdef DEBUG
			fprintf(stderr, "parseRTSPRequestString() failed; checking now for HTTP "
			                "commands (for RTSP-over-HTTP tunneling)...\n");
#endif
			// The request was not (valid) RTSP, but check for a special case: HTTP
			// commands (for setting up RTSP-over-HTTP tunneling):
			char sessionCookie[RTSP_PARAM_STRING_MAX];
			char acceptStr[RTSP_PARAM_STRING_MAX];
			*fLastCRLF = '\0'; // temporarily, for parsing
			parseSucceeded = parseHTTPRequestString(cmdName, sizeof cmdName, urlSuffix, sizeof urlPreSuffix,
			...
			*fLastCRLF = '\r';
			if (parseSucceeded) {
				...
				// There must be a 'session cookie'.
				Boolean isValidHTTPCmd = True;
				if (strcmp(cmdName, "OPTIONS") == 0) {
					handleHTTPCmd_OPTIONS();
				} else if (sessionCookie[0] == '\0') {
					// There was no "x-sessioncookie:" header.  If there was an "Accept:
					// application/x-rtsp-tunnelled" header,
					// then this is a bad tunneling request.  Otherwise, assume that it's
					...
					// We might have received additional data following the HTTP "POST"
					// command - i.e., the first Base64-encoded RTSP command.
		...
		// Check whether there are extra bytes remaining in the buffer, after the
		// end of the request (a rare case).
		...
		if (numBytesRemaining > 0) {
			memcpy(fRequestBuffer, &fRequestBuffer[requestSize], numBytesRemaining);
	...
	if (!fIsActive) {
		if (fRecursionCount > 0)
			...
			delete this;
		// Note: The "fRecursionCount" test is for a pathological situation where we
		// reenter the event loop and get called recursively
		// while handling a command (e.g., while handling a "DESCRIBE", to get a SDP
		// description).
		// In such a case we don't want to actually delete ourself until we leave
		// the outermost call.
	}
}

static Boolean parseAuthorizationHeader(char const *buf, char const *&username, char const *&realm, char const *&nonce,
                                        char const *&uri, char const *&response)
{
	// Initialize the result parameters to default values:
...
Boolean RTSPServer::RTSPClientConnection::authenticationOK(char const *cmdName, char const *urlSuffix,
                                                           char const *fullRequestStr)
{
	...
	do {
		...
		if (!parseAuthorizationHeader(fullRequestStr, username, realm, nonce, uri, response) ||
		    username == NULL || realm == NULL || strcmp(realm, fCurrentAuthenticator.realm()) != 0 ||
		    nonce == NULL || strcmp(nonce, fCurrentAuthenticator.nonce()) != 0 || uri == NULL ||
		    response == NULL) {
			break;
		}
	...
	if (success) {
		...
		// Now allow subclasses a chance to validate the user against the IP address
		// and/or URL suffix.
		if (!fOurServer.specialClientUserAccessCheck(fClientInputSocket, fClientAddr, urlSuffix, username)) {
			// Note: We don't return a "WWW-Authenticate" header here, because the
			// user is valid,
...
void RTSPServer::RTSPClientConnection::changeClientInputSocket(int newSocketNum, unsigned char const *extraData,
                                                               unsigned extraDataSize)
{
	envir().taskScheduler().disableBackgroundHandling(fClientInputSocket);
...
void RTSPServer::RTSPClientConnection::continueHandlingREGISTER1(ParamsForREGISTER *params)
{
	...
	if (socketNumToBackEndServer >= 0) {
		...
		// incoming requests, we can now delete this
		// "RTSPClientConnection" object.  We do this now, in case the
		// "implementCmd_REGISTER()" call below would also end up
		// deleting this.
		fClientInputSocket = fClientOutputSocket = -1; // so the socket doesn't get closed when we get deleted
		delete this;
	}
...
static void parseTransportHeader(char const *buf, StreamingMode &streamingMode, char *&streamingModeString,
                                 char *&destinationAddressStr, u_int8_t &destinationTTL,
...
)
{
	...
	while (sscanf(fields, "%[^;\r\n]", field) == 1) {
		if (strcmp(field, "RTP/AVP/TCP") == 0) {
			...
			clientRTPPortNum = p1;
			clientRTCPPortNum = streamingMode == RAW_UDP ? 0 : p2; // ignore the second port number if
			        // the client asked for raw UDP
	...
	char transport_info[64];
	snprintf(&transport_info[0], 64, "Client transport from[%s]\r\n", streamingMode == RTP_TCP ? "TCP" : "UDP");
	fprintf(stderr, "%s", transport_info);
	delete[] field;
}
                                                    ...
                                                    char const *fullRequestStr)
{
	...
	// SETUP requests (i.e., without a track name),
	// in the special case where we have only a single track.  I.e., in this case,
	// we also handle:
	//    "urlPreSuffix" is empty and "urlSuffix" is the session (stream) name, or
	//    "urlPreSuffix" concatenated with "urlSuffix" (with "/" inbetween) is the
	//    session (stream) name.
	...
	do {
		...
		if (sms == NULL) {
			if (fOurServerMediaSession == NULL) {
				...
				// The client asked for a stream that doesn't exist, but using a stream
				// id for a stream that does exist. Bad request:
		...
		if (fStreamStates == NULL) {
			// This is the first "SETUP" for this session.  Set up our array of states
			// for all of this session's subsessions (tracks):
			...
			for (unsigned i = 0; i < fNumStreamStates; ++i) {
				...
				fStreamStates[i].streamToken = NULL; // for now; it may be changed by
				        // the "getStreamParameters()" call
				        // that comes later
			}
		}
		...
		// Look for a "Transport:" header in the request string, to extract client
		// parameters:
		...
		parseTransportHeader(fullRequestStr, streamingMode, streamingModeString, clientsDestinationAddressStr,
		                     clientsDestinationTTL, clientRTPPortNum, clientRTCPPortNum, rtpChannelId,
		                     rtcpChannelId);
		if ((streamingMode == RTP_TCP && rtpChannelId == 0xFF) ||
		    (streamingMode != RTP_TCP &&
		     ourClientConnection->fClientOutputSocket != ourClientConnection->fClientInputSocket)) {
			// An anomolous situation, caused by a buggy client.  Either:
			//     1/ TCP streaming was requested, but with no "interleaving=" fields.
			//     (QuickTime Player sometimes does this.), or
			//     2/ TCP streaming was not requested, but we're doing RTSP-over-HTTP
		...
		// Next, check whether a "Range:" or "x-playNow:" header is present in the
		// request.
		...
		if (fIsMulticast) {
			...
			case RTP_UDP: {
				snprintf((char *)ourClientConnection->fResponseBuffer,
				         ...
				         "Transport: "
				         "RTP/"
				         "AVP;multicast;destination=%s;source=%s;port=%d-%d;ttl=%d\r\n"
				         "Session: %08X%s\r\n\r\n",
				         ourClientConnection->fCurrentCSeq, dateHeader(), destAddrStr.val(),
				         sourceAddrStr.val(), ntohs(serverRTPPort.num()), ntohs(serverRTCPPort.num()),
			...
			case RTP_UDP: {
				snprintf((char *)ourClientConnection->fResponseBuffer,
				         ...
				         "Transport: "
				         "RTP/"
				         "AVP;unicast;destination=%s;source=%s;client_port=%d-%d;"
				         "server_port=%d-%d\r\n"
				         "Session: %08X%s\r\n\r\n",
				         ourClientConnection->fCurrentCSeq, dateHeader(), destAddrStr.val(),
				         sourceAddrStr.val(), ntohs(clientRTPPort.num()), ntohs(clientRTCPPort.num()),
				         ntohs(serverRTPPort.num()), ntohs(serverRTCPPort.num()), fOurSessionId,
			...
			case RTP_TCP: {
				snprintf((char *)ourClientConnection->fResponseBuffer,
				         ...
				         "Transport: "
				         "RTP/AVP/"
				         "TCP;unicast;destination=%s;source=%s;interleaved=%d-%d\r\n"
				         "Session: %08X%s\r\n\r\n",
				         ourClientConnection->fCurrentCSeq, dateHeader(), destAddrStr.val(),
			...
			case RAW_UDP: {
				snprintf((char *)ourClientConnection->fResponseBuffer,
				         ...
				         "%s"
				         "Transport: "
				         "%s;unicast;destination=%s;source=%s;client_port=%d;server_"
				         "port=%d\r\n"
				         "Session: %08X%s\r\n\r\n",
				         ourClientConnection->fCurrentCSeq, dateHeader(), streamingModeString,
                                                            ...
                                                            char const *urlSuffix, char const *fullRequestStr)
{
	...
	//   or "urlPreSuffix" and "urlSuffix" are both nonempty, but when
                                                       ...
                                                       ServerMediaSubsession *subsession)
{
	...
	// Optimization: If all subsessions have now been torn down, then we know that
	// we can reclaim our object now.
	// (Without this optimization, however, this object would still get reclaimed
	// later, as a result of a 'liveness' timeout.)
                                                   ...
                                                   ServerMediaSubsession *subsession, char const *fullRequestStr)
{
	...
	if (sawRangeHeader && absStart == NULL /*not seeking by 'absolute' time*/) {
		...
		// Make sure that "rangeStart" and "rangeEnd" (from the client's "Range:"
		// header)
		// have sane values, before we send back our own "Range:" header in our
		// response:
		if (rangeStart < 0.0)
			rangeStart = 0.0;
			...
			if (fStreamStates[i].subsession != NULL) {
				...
				if (absStart != NULL) {
					...
					if (!sawRangeHeader || startTimeIsNow) {
						// We're resuming streaming without seeking, so we just do a 'null'
						// seek
						// (to get our NPT, and to specify when to end streaming):
						fStreamStates[i].subsession->nullSeekStream(
						        fOurSessionId, fStreamStates[i].streamToken, rangeEnd,
						        numBytes);
					} else {
						// We do a real 'seek':
	...
	// (Note that we do this after seeking, in case the seeking operation changed
	...
	if (absStart != NULL) {
		...
		if (!sawRangeHeader || startTimeIsNow) {
			// We didn't seek, so in our response, begin the range with the current
			...
			for (i = 0; i < fNumStreamStates; ++i) {
				if (subsession == NULL /* means: aggregated operation */
					...
					// In this (unusual) case, we just return the largest NPT; I hope that
					// turns out OK...
				}
			}
			rangeStart = curNPT;
	...
	for (i = 0; i < fNumStreamStates; ++i) {
		if (subsession == NULL /* means: aggregated operation */
			...
			fStreamStates[i].subsession->startStream(
			        ...
			        rtpSeqNum, rtpTimestamp, RTSPServer::RTSPClientConnection::handleAlternativeRequestByte,
			        ourClientConnection);
			...
			sprintf(rtpInfo, rtpInfoFmt, prevRTPInfo, numRTPInfoItems++ == 0 ? "" : ",", rtspURL, urlSuffix,
                                                            ...
                                                            char const * /*fullRequestStr*/)
{
	...
	// back a dummy response.
	// (If you want to handle "GET_PARAMETER" properly, you can do so by defining
	// a subclass of "RTSPServer"
	// and "RTSPServer::RTSPClientSession", and then reimplement this virtual
	// function in your subclass.)
                                                            ...
                                                            char const * /*fullRequestStr*/)
{
	...
	// (If you want to handle "SET_PARAMETER" properly, you can do so by defining
	// a subclass of "RTSPServer"
	// and "RTSPServer::RTSPClientSession", and then reimplement this virtual
	// function in your subclass.)
	setRTSPResponse(ourClientConnection, "200 OK", fOurSessionId);
...
{
	if (fLivenessCheckTask == NULL && fOurServer.fReclamationTestSeconds > 0) {
		/*first time create note liveness task*/
		fLivenessCheckTask = envir().taskScheduler().scheduleDelayedTask(
		fOurServer.fReclamationTestSeconds * 1000000,  (TaskFunc *)livenessTimeoutTask, this);
	} else if (fOurServer.fReclamationTestSeconds > 0) {
...
void RTSPServer::RTSPClientSession::noteClientLiveness(RTSPClientSession *clientSession)
{
	...
	fprintf(stderr,
	        "RTSP client session (id \"%08X\", stream name \"%s\"): "
	        "Liveness indication\n",
...
void RTSPServer::RTSPClientSession::livenessTimeoutTask(RTSPClientSession *clientSession)
{
	...
	char const *streamName = (clientSession->fOurServerMediaSession == NULL) ?
	                                 ...
	                                 clientSession->fOurServerMediaSession->streamName();
	fprintf(stderr,
	        "RTSP client session (id \"%08X\", stream name \"%s\") has "
...
RTSPServerWithREGISTERProxying::createNew(UsageEnvironment &env, Port ourPort, UserAuthenticationDatabase *authDatabase,
                                          UserAuthenticationDatabase *authDatabaseForREGISTER,
                                          unsigned reclamationTestSeconds, Boolean streamRTPOverTCP,
                                          int verbosityLevelForProxying)
{
	...
	return new RTSPServerWithREGISTERProxying(env, ourSocket, ourPort, authDatabase, authDatabaseForREGISTER,
	                                          reclamationTestSeconds, streamRTPOverTCP, verbosityLevelForProxying);
}
...
RTSPServerWithREGISTERProxying::RTSPServerWithREGISTERProxying(UsageEnvironment &env, int ourSocket, Port ourPort,
        ...
        : RTSPServer(env, ourSocket, ourPort, authDatabase, reclamationTestSeconds, RTSP_DEFAULT_MAX_CLIENT)
...
char const *RTSPServerWithREGISTERProxying::allowedCommandNames()
{
	if (fAllowedCommandNames == NULL) {
		...
		fAllowedCommandNames =
		        new char[strlen(baseAllowedCommandNames) + strlen(newAllowedCommandName) + 1 /* for '\0' */];
...
void RTSPServerWithREGISTERProxying::implementCmd_REGISTER(char const *url, char const * /*urlSuffix*/,
                                                           int socketToRemoteServer, Boolean deliverViaTCP,
                                                           char const *proxyURLSuffix)
{
	...
	//    - There is no 'username' and 'password' for the back-end stream.  (Thus,
	//    access-controlled back-end streams will fail.)
	...
	// We don't support streaming from the back-end via RTSP/RTP/RTCP-over-HTTP;
	// only via RTP/RTCP-over-TCP or RTP/RTCP-over-UDP

	ServerMediaSession *sms = ProxyServerMediaSession::createNew(envir(), this, url, proxyStreamName, NULL, NULL,
	                                                             tunnelOverHTTPPortNum, fVerbosityLevelForProxying,
	                                                             socketToRemoteServer);
	addServerMediaSession(sms);

	// (Regardless of the verbosity level) announce the fact that we're proxying
	// this new stream, and the URL to use to access it:
