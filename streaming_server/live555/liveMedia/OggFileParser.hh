...
// A structure that counts the sizes of 'packets' given by each page's
// "segment_table":
...
class OggFileParser : public StreamParser {
    public:
	OggFileParser(OggFile &ourFile, FramedSource *inputSource, FramedSource::onCloseFunc *onEndFunc,
	              void *onEndClientData, OggDemux *ourDemux = NULL);
	...
	// Parsing functions:
	Boolean parse(); // returns True iff we have finished parsing all BOS pages
	        // (on initialization)
