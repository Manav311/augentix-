...
class SegmentQueue {
    ...
    private:
	static void sqAfterGettingSegment(void *clientData, unsigned numBytesRead, unsigned numTruncatedBytes,
...
ADUFromMP3Source *ADUFromMP3Source::createNew(UsageEnvironment &env, FramedSource *inputSource,
...
#ifdef DEBUG
	fprintf(stderr,
	        "m->a:outputting ADU %d<-%d, nbr:%d, sis:%d, dh:%d, "
	        "(descriptor size: %d)\n",
	        tailSeg->aduSize, tailSeg->backpointer, fFrameSize, tailSeg->sideInfoSize, tailSeg->dataHere(),
	        descriptorSize);
#endif
	if (descriptorSize + fFrameSize > fMaxSize) {
		envir() << "ADUFromMP3Source::doGetNextFrame1(): not enough room (" << descriptorSize + fFrameSize
		        << ">" << fMaxSize << ")\n";
		fFrameSize = 0;
		return False;
	}
...
MP3FromADUSource::MP3FromADUSource(UsageEnvironment &env, FramedSource *inputSource, Boolean includeADUdescriptors)
        ...
        , fSegments(new SegmentQueue(False /* because we're ADU->MP3 */, includeADUdescriptors))
{
}
...
MP3FromADUSource *MP3FromADUSource::createNew(UsageEnvironment &env, FramedSource *inputSource,
                                              Boolean includeADUdescriptors)
{
...
// Before returning a frame, we must enqueue at least one ADU:
	...
	NOTE:
		This code no longer works, because it uses synchronous reads,
		        which are no longer supported.static unsigned const framesPerPacket = 10;
		...
		while (1) {
			...
			if (packetIsLost) {
				...
				fInputSource->syncGetNextFrame(dummySegment.buf, sizeof dummySegment.buf, numBytesRead,
				                               presentationTime);
			} else {
				break; // from while (1)
			}
...
// We need to insert a dummy ADU in front of the tail
			...
			fprintf(stderr, "a->m:need to insert a dummy ADU (%d, %d, %d) [%d, %d]\n", tailSeg->backpointer,
			        prevADUend, tailSeg->dataHere(), fSegments->headIndex(), fSegments->nextFreeIndex());
#endif
		...
		if (startOfData <= (int)toOffset) {
			...
			if (bytesToZero > 0)
				fprintf(stderr, "a->m:outputting %d zero bytes (%d, %d, %d, %d)\n", bytesToZero,
				        startOfData, toOffset, frameOffset, seg->backpointer);
#endif
...
void SegmentQueue::enqueueNewSegment(FramedSource *inputSource, FramedSource *usingSource)
{
	...
	inputSource->getNextFrame(seg.buf, sizeof seg.buf, sqAfterGettingSegment, this, FramedSource::handleClosure,
	                          usingSource);
}
                                         ...
                                         struct timeval presentationTime, unsigned durationInMicroseconds)
{
	...
	if (segQueue->sqAfterGettingCommon(seg, numBytesRead)) {
		...
		fprintf(stderr, "%s:read frame %d<-%d, fs:%d, sis:%d, dh:%d, (descriptor size: %d)\n", direction,
		        seg.aduSize, seg.backpointer, seg.frameSize, seg.sideInfoSize, seg.dataHere(),
		        seg.descriptorSize);
#endif
	}
...
Boolean SegmentQueue::sqAfterGettingCommon(Segment &seg, unsigned numBytesRead)
{
	...
	if (!GetADUInfoFromMP3Frame(fromPtr, numBytesRead, hdr, seg.frameSize, sideInfo, seg.sideInfoSize,
