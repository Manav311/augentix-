...
void DVVideoStreamFramer::getProfile()
{
	...
	// that we can parse:
	fInputSource->getNextFrame(fSavedInitialBlocks, DV_SAVED_INITIAL_BLOCKS_SIZE, afterGettingFrame, this,
...
void DVVideoStreamFramer::doGetNextFrame()
{
	...
	// If we have saved initial blocks (and won't be seeking back to re-read this
	// data), so use this data first.
	if (fInitialBlocksPresent && !fSourceIsSeekable) {
		// For simplicity, we require the downstream object's buffer to be >= this
		// data's size:
...
{
	unsigned const totFrameSize = fOurProfile != NULL ? ((DVVideoProfile const *)fOurProfile)->dvFrameSize :
                                            ...
                                            struct timeval presentationTime)
{
	if (fOurProfile == NULL && frameSize >= DV_SAVED_INITIAL_BLOCKS_SIZE) {
		...
		// necessarily on a (150-block) sequence boundary.
		// We therefore scan each 80-byte block, until we find the 6-block header
		// that begins a sequence:
		u_int8_t const *data = (fTo == NULL) ? fSavedInitialBlocks : fTo;
		for (u_int8_t const *ptr = data; ptr + 6 * DV_DIF_BLOCK_SIZE <= &data[DV_SAVED_INITIAL_BLOCKS_SIZE];
			...
			if (sectionHeader == DV_SECTION_HEADER &&
				...
				// Use these three parameters (apt, sType, sequenceCount) to look up the
				// DV profile:
				...
				break; // because we found a correct sequence header (even if we don't
				        // happen to define a profile for it)
			}
		}
	}
		...
		if (fFrameSize < totFrameSize && fFrameSize < fMaxSize && numTruncatedBytes == 0) {
			...
			if (fOurProfile != NULL) {
				...
				double durationInMicroseconds =
				        (fFrameSize * ourProfile->frameDuration) / ourProfile->dvFrameSize;
