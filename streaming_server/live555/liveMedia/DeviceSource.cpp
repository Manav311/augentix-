        ...
        , fParams(params)
{
	...
	//
	// If the device can be accessed as a readable socket, then one easy way to do
	// this is using a call to
...
void DeviceSource::doGetNextFrame()
{
	...
	// Note: If, for some reason, the source device stops being readable (e.g., it
	// gets closed), then you do the following:
	...
	// No new data is immediately available to be delivered.  We don't do anything
	// more here.
	// Instead, our event trigger must be called (e.g., from a separate thread)
...
void DeviceSource::deliverFrame()
{
	...
	gettimeofdayMonotonic(&fPresentationTime, NULL); // If you have a more accurate time -
	        // e.g., from an encoder - then use
	        // that instead.
...
// become available.
// This (unlike other "LIVE555 Streaming Media" library code) may be called from
// a separate thread.
// (Note, however, that "triggerEvent()" cannot be called with the same 'event
// trigger id' from different threads.
// Also, if you want to have multiple device threads, each one using a different
// 'event trigger id', then you will need
