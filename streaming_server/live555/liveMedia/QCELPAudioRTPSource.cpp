...
class RawQCELPRTPSource : public MultiFramedRTPSource {
    public:
	static RawQCELPRTPSource *createNew(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
    ...
    private:
	RawQCELPRTPSource(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
	                  unsigned rtpTimestampFrequency);
...
FramedSource *QCELPAudioRTPSource::createNew(UsageEnvironment &env, Groupsock *RTPgs, RTPSource *&resultRTPSource,
                                             unsigned char rtpPayloadFormat, unsigned rtpTimestampFrequency)
{
	RawQCELPRTPSource *rawRTPSource;
	resultRTPSource = rawRTPSource =
	        RawQCELPRTPSource::createNew(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency);
...
RawQCELPRTPSource *RawQCELPRTPSource::createNew(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
                                                unsigned rtpTimestampFrequency)
{
	return new RawQCELPRTPSource(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency);
}

RawQCELPRTPSource::RawQCELPRTPSource(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
                                     unsigned rtpTimestampFrequency)
...
#ifdef DEBUG
	fprintf(stderr,
	        "QCELPBufferedPacket::nextEnclosedFrameSize(): frameSize: "
	        "%d, dataSize: %d\n",
	        frameSize, dataSize);
#endif
...
class QCELPDeinterleavingBuffer {
	...
	void deliverIncomingFrame(unsigned frameSize, unsigned char interleaveL, unsigned char interleaveN,
...
QCELPDeinterleaver::QCELPDeinterleaver(UsageEnvironment &env, RawQCELPRTPSource *inputSource)
...
void QCELPDeinterleaver::afterGettingFrame1(unsigned frameSize, struct timeval presentationTime)
{
	...
	fDeinterleavingBuffer->deliverIncomingFrame(frameSize, source->interleaveL(), source->interleaveN(),
...
void QCELPDeinterleavingBuffer::deliverIncomingFrame(unsigned frameSize, unsigned char interleaveL,
                                                     unsigned char interleaveN, unsigned char frameIndex,
                                                     unsigned short packetSeqNum, struct timeval presentationTime)
{
	...
	if (frameSize > QCELP_MAX_FRAME_SIZE || interleaveL > QCELP_MAX_INTERLEAVE_L || interleaveN > interleaveL ||
...
Boolean QCELPDeinterleavingBuffer::retrieveFrame(unsigned char *to, unsigned maxSize, unsigned &resultFrameSize,
