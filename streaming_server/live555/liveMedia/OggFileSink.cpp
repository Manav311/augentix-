...
OggFileSink *OggFileSink::createNew(UsageEnvironment &env, char const *fileName, unsigned samplingFrequency,
                                    char const *configStr, unsigned bufferSize, Boolean oneFilePerFrame)
{
	do {
		FILE *fid;
...
OggFileSink::OggFileSink(UsageEnvironment &env, FILE *fid, unsigned samplingFrequency, char const *configStr,
...
{
	// We still have the previously-arrived frame, so write it to the file before
	// we end:
	fHaveSeenEOF = True;
...
void OggFileSink::addData(unsigned char const *data, unsigned dataSize, struct timeval presentationTime)
{
	...
	if (fIsTheora) {
		// Special case for Theora: "fGranulePosition" is supposed to be made up of
		// a pair:
		...
		double ptDiff = (presentationTime.tv_sec - fFirstPresentationTime.tv_sec) +
		...
		if (newGranulePosition < fGranulePosition) {
			// Update "fGranulePositionAdjustment" so that "fGranulePosition" remains
			// monotonic
	...
	// Write the frame to the file as a single Ogg 'page' (or perhaps as multiple
	// pages
	// if it's too big for a single page).  We don't aggregate more than one frame
	// within
	// an Ogg page because that's not legal for some headers, and because that
	// would make
	// it difficult for us to properly set the 'eos' (end of stream) flag on the
	// last page.
	...
	// Note that if "dataSize" is a integral multiple of PAGE_DATA_MAX_SIZE, there
	// will
	...
	for (unsigned i = 0; i < numPagesToWrite; ++i) {
		...
		if (i < numPagesToWrite - 1) {
			// For pages where the frame does not end, set 'granule_position' in the
			// header to -1:
			fPageHeaderBytes[6] = fPageHeaderBytes[7] = fPageHeaderBytes[8] = fPageHeaderBytes[9] =
			        fPageHeaderBytes[10] = fPageHeaderBytes[11] = fPageHeaderBytes[12] =
		...
		// Compute the CRC from the 'page header' array, the 'segment_table', and
		// the frame data:
		u_int32_t crc = 0;
...
void OggFileSink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime)
{
	if (!fHaveWrittenFirstFrame) {
		...
		// ("identification", "comment", "setup"), unpack them and prepend them to
		// the file:
		if (fConfigStr != NULL && fConfigStr[0] != '\0') {
			...
			parseVorbisOrTheoraConfigStr(fConfigStr, identificationHdr, identificationHdrSize, commentHdr,
			                             commentHdrSize, setupHdr, setupHdrSize, identField);
			if (identificationHdrSize >= 42 &&
			    strncmp((const char *)&identificationHdr[1], "theora", 6) == 0) {
				// Hack for Theora video: Parse the "identification" hdr to get the
				// "KFGSHIFT" parameter:
				fIsTheora = True;
	...
	// Call the parent class to complete the normal file write with the (previous)
	// input frame:
