...
class AC3AudioStreamParser : public StreamParser {
    ...
    private:
	static void afterGettingSavedFrame(void *clientData, unsigned frameSize, unsigned numTruncatedBytes,
...
AC3AudioStreamFramer *AC3AudioStreamFramer::createNew(UsageEnvironment &env, FramedSource *inputSource,
...
struct timeval AC3AudioStreamFramer::currentFramePlayTime() const
{
	...
	unsigned const uSeconds =
	        (freq == 0) ? 0 : ((numSamples * 2 * MILLION) / freq + 1) / 2; // rounds to nearest integer
...
void AC3AudioStreamFramer::handleNewData(void *clientData, unsigned char *ptr, unsigned size,
...
AC3AudioStreamParser::AC3AudioStreamParser(AC3AudioStreamFramer *usingSource, FramedSource *inputSource)
        : StreamParser(inputSource, FramedSource::handleClosure, usingSource, &AC3AudioStreamFramer::handleNewData,
                       usingSource)
...
{
	if (ourStreamCode == 0)
		return; // we assume that there's no stream code at the beginning of the
		        // data
...
void AC3AudioStreamParser::readAndSaveAFrame()
{
	...
	fSavedFrameFlag = 0;
	fUsingSource->getNextFrame(fSavedFrame, maxAC3FrameSize, afterGettingSavedFrame, this, onSavedFrameClosure,
	                           this);
	fUsingSource->envir().taskScheduler().doEventLoop(&fSavedFrameFlag);
}

void AC3AudioStreamParser::afterGettingSavedFrame(void *clientData, unsigned frameSize, unsigned /*numTruncatedBytes*/,
                                                  struct timeval /*presentationTime*/,
