...
MultiFramedRTPSink::MultiFramedRTPSink(UsageEnvironment &env, Groupsock *rtpGS, unsigned char rtpPayloadType,
...
{
	setPacketSizes(1000, 1448); /*align ethernet package size*/
...
void MultiFramedRTPSink::doSpecialFrameHandling(unsigned /*fragmentationOffset*/, unsigned char * /*frameStart*/,
...
void MultiFramedRTPSink::setFrameSpecificHeaderBytes(unsigned char const *bytes, unsigned numBytes,
...
void MultiFramedRTPSink::buildAndSendPacket(Boolean isFirstPacket)
{
	...
	unsigned rtpHdr = 0x80000000; // RTP version 2; marker ('M') bit not set (by
	        // default; it can be set later)
...
void MultiFramedRTPSink::afterGettingFrame(void *clientData, unsigned numBytesRead, unsigned numTruncatedBytes,
                                            ...
                                            struct timeval presentationTime, unsigned durationInMicroseconds)
{
	if (fIsFirstPacket) {
		...
		gettimeofdayMonotonic(&fNextSendTime, NULL);
	}
	...
	if (numTruncatedBytes > 0) {
		...
		envir() << "MultiFramedRTPSink::afterGettingFrame1(): The input frame data "
		        ...
		        << ", *before* creating this 'RTPSink'.  (Current value is " << OutPacketBuffer::maxSize
		        << ".)\n";
	}
	...
	if (fNumFramesUsedSoFar > 0) {
		if ((fPreviousFrameEndedFragmentation && !allowOtherFramesAfterLastFragment()) ||
			...
			fOutBuf->setOverflowData(fOutBuf->curPacketSize(), frameSize, presentationTime,
	...
	if (numFrameBytesToUse > 0) {
		...
		if (fOutBuf->wouldOverflow(frameSize)) {
			...
			fOutBuf->setOverflowData(fOutBuf->curPacketSize() + numFrameBytesToUse, overflowBytes,
	...
	if (numFrameBytesToUse == 0 && frameSize > 0) {
		...
		// do this now, in case "doSpecialFrameHandling()" calls "setFramePadding()"
		// to append padding bytes

		// Here's where any payload format specific processing gets done:
		doSpecialFrameHandling(curFragmentationOffset, frameStart, numFrameBytesToUse, presentationTime,
		                       overflowBytes);
...
// Send the packet:
	...
	if (fNoFramesLeft) {
		...
		if (fOutBuf->haveOverflowData() /*only MJPEG use here*/) {
			/*source size > RTP size, but have only 1 Nalu*/
			sendNext(this);
		}
