...
MPEG4LATMAudioRTPSource::MPEG4LATMAudioRTPSource(UsageEnvironment &env, Groupsock *RTPgs,
                                                 unsigned char rtpPayloadFormat, unsigned rtpTimestampFrequency)
        : MultiFramedRTPSource(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency, new LATMBufferedPacketFactory)
                                       ...
                                       unsigned &audioSpecificConfigSize)
{
	...
	if (!parseStreamMuxConfigStr(configStr, audioMuxVersion, allStreamsSameTimeFraming, numSubFrames, numProgram,
	                             numLayer, audioSpecificConfig, audioSpecificConfigSize)) {
		audioSpecificConfigSize = 0;
		return NULL;
	}
