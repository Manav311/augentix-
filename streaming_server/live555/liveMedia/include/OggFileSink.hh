...
class OggFileSink : public FileSink {
	...
	static OggFileSink *createNew(UsageEnvironment &env, char const *fileName,
	                              ...
	                              // representing 'packed configuration headers' ("identification",
	                              // "comment", "setup")
	...
	int64_t fGranulePositionAdjustment; // used to ensure that "fGranulePosition"
	        // stays monotonic
	...
	// Because the last Ogg page before EOF needs to have a special 'eos' bit set
	// in the header,
	// we need to defer the writing of each incoming frame.  To do this, we
	// maintain a 2nd buffer:
