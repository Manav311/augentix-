...
// A subclass of "RTSPClient", used to refer to the particular
// "ProxyServerMediaSession" object being used.
// It is used only within the implementation of "ProxyServerMediaSession", but
// is defined here, in case developers wish to
// subclass it.

class ProxyRTSPClient : public RTSPClient {
    public:
	ProxyRTSPClient(class ProxyServerMediaSession &ourServerMediaSession, char const *rtspURL, char const *username,
...
typedef ProxyRTSPClient *createNewProxyRTSPClientFunc(ProxyServerMediaSession &ourServerMediaSession,
...
class ProxyServerMediaSession : public ServerMediaSession {
    public:
	static ProxyServerMediaSession *
	...
	// Hack: "tunnelOverHTTPPortNum" == 0xFFFF (i.e., all-ones) means: Stream
	// RTP/RTCP-over-TCP, but *not* using HTTP
	// "verbosityLevel" == 1 means display basic proxy setup info;
	// "verbosityLevel" == 2 means display RTSP client protocol also.
	// If "socketNumToServer" is >= 0, then it is the socket number of an
	// already-existing TCP connection to the server.
	//      (In this case, "inputStreamURL" must point to the socket's endpoint,
	...
	// This can be used - along with "describeCompletdFlag" - to check whether the
	// back-end "DESCRIBE" completed *successfully*.

    protected:
	ProxyServerMediaSession(
	        UsageEnvironment &env, RTSPServer *ourRTSPServer, char const *inputStreamURL, char const *streamName,
	...
	// If you subclass "ProxyRTSPClient", then you will also need to define your
	// own function
	// - with signature "createNewProxyRTSPClientFunc" (see above) - that creates
	// a new object
	// of this subclass.  You should also subclass "ProxyServerMediaSession" and,
	// in your
	// subclass's constructor, initialize the parent class (i.e.,
	// "ProxyServerMediaSession")
	// constructor by passing your new function as the
...
///PresentationTimeSubsessionNormalizer definitions //////////

// The following two classes are used by proxies to convert incoming streams'
// presentation times into wall-clock-aligned
// presentation times that are suitable for our "RTPSink"s (for the
// corresponding outgoing streams).
// (For multi-subsession (i.e., audio+video) sessions, the outgoing streams'
// presentation times retain the same relative
...
class PresentationTimeSessionNormalizer : public Medium {
	...
	PresentationTimeSubsessionNormalizer *createNewPresentationTimeSubsessionNormalizer(FramedSource *inputSource,
	                                                                                    RTPSource *rtpSource,
	...
	void normalizePresentationTime(PresentationTimeSubsessionNormalizer *ssNormalizer, struct timeval &toPT,
	...
	struct timeval fPTAdjustment; // Added to (RTCP-synced) subsession
	        // presentation times to 'normalize' them with
	        // wall-clock time.
};
