...
class MatroskaFileServerDemux : public Medium {
	...
	// Returns a new "ServerMediaSubsession" object that represents the next
	// preferred media track
	// (video, audio, subtitle - in that order) from the file. (Preferred media
	// tracks are based on the file's language preference.)
	...
	// As above, but creates a new "ServerMediaSubsession" object for a specific
	// track number within the Matroska file.
	// (You should not call this function more than once with the same track
	// number.)

	// The following public: member functions are called only by the
	...
	// Used by the "ServerMediaSubsession" objects to implement their
	// "createNewStreamSource()" virtual function.

    private:
	MatroskaFileServerDemux(UsageEnvironment &env, char const *fileName, onCreationFunc *onCreation,
