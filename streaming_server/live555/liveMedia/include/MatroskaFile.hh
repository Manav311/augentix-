...
class MatroskaFile : public Medium {
	...
	static void createNew(UsageEnvironment &env, char const *fileName, onCreationFunc *onCreation,
	                      void *onCreationClientData, char const *preferredLanguage = "eng");
	// Note: Unlike most "createNew()" functions, this one doesn't return a new
	// object immediately.  Instead, because this class
	// requires file reading (to parse the Matroska 'Track' headers) before a new
	// object can be initialized, the creation of a new
	// object is signalled by calling - from the event loop - an 'onCreationFunc'
	// that is passed as a parameter to "createNew()".
	...
	FramedSource *createSourceForStreaming(FramedSource *baseSource, unsigned trackNumber, unsigned &estBitrate,
	                                       unsigned &numFiltersInFrontOfTrack);
	// Takes a data source (which must be a demultiplexed track from this file)
	// and returns
	...
	// Creates a "RTPSink" object that would be appropriate for streaming the
	// specified track,
	...
	Boolean lookupCuePoint(double &cueTime, u_int64_t &resultClusterOffsetInFile,
...
// We define our own track type codes as bits (powers of 2), so we can use the
// set of track types as a bitmap, representing a set:
// (Note that MATROSKA_TRACK_TYPE_OTHER must be last, and have the largest
// value.)
...
class MatroskaTrack {
	...
	Boolean codecPrivateUsesH264FormatForH265; // a hack specifically for H.265
	        // video tracks
	unsigned headerStrippedBytesSize;
	u_int8_t *headerStrippedBytes;
	unsigned subframeSizeSize; // 0 means: frames do not have subframes (the
	        // default behavior)
...
class MatroskaDemux : public Medium {
	...
	// Returns a new stream ("FramedSource" subclass) that represents the next
	// preferred media
	// track (video, audio, subtitle - in that order) from the file. (Preferred
	// media tracks
	...
	// As above, but creates a new stream for a specific track number within the
	// Matroska file.
	// (You should not call this function more than once with the same track
	// number.)
	...
	MatroskaDemux(MatroskaFile &ourFile); // we're created only by a
	        // "MatroskaFile" (a friend)
	virtual ~MatroskaDemux();

    private:
	friend class MatroskaDemuxedTrack;
	void removeTrack(unsigned trackNumber);
	void continueReading(); // called by a demuxed track to tell us that it has a
	        // pending read ("doGetNextFrame()")
