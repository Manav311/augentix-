...
class RTSPRegisterSender : public RTSPClient {
	...
	static RTSPRegisterSender *createNew(UsageEnvironment &env, char const *remoteClientNameOrAddress,
	                                     ...
	                                     Boolean requestStreamingViaTCP = False, char const *proxyURLSuffix = NULL,
	                                     Boolean reuseConnection = False, int verbosityLevel = 0,
	                                     char const *applicationName = NULL);

	void grabConnection(int &sock,
	                    struct sockaddr_in &remoteAddress); // so that the socket
	        // doesn't get closed
	        // when we're deleted
	...
	// A subclass of "RTSPClient::RequestRecord", specific to our "REGISTER"
	// command:
	class RequestRecord_REGISTER : public RTSPClient::RequestRecord {
		...
		RequestRecord_REGISTER(unsigned cseq, RTSPClient::responseHandler *rtspResponseHandler,
		                       char const *rtspURLToRegister, Boolean reuseConnection,
