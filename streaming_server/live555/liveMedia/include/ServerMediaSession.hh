...
class ServerMediaSession : public Medium {
	...
	// a result < 0 means: subsession durations differ; the result is -(the
	// largest).
	...
	// Removes and deletes all subsessions added by "addSubsession()", returning
	// us to an 'empty' state
	// Note: If you have already added this "ServerMediaSession" to a "RTSPServer"
    ...
    protected:
	ServerMediaSession(UsageEnvironment &env, char const *streamName, char const *info, char const *description,
...
class ServerMediaSubsession : public Medium {
	...
	virtual void nullSeekStream(unsigned clientSessionId, void *streamToken, double streamEndTime,
	                            u_int64_t &numBytes);
	// Called whenever we're handling a "PLAY" command without a specified start
	// time.
	...
	// Subclasses can reimplement this iff they support seeking by 'absolute'
	// time.
