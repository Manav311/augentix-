...
class RTPSink : public MediaSink {
	...
	void deleteSocketDescriptor(int sockNum)
	{
		fRTPInterface.deleteSocketDescriptor(sockNum);
	}
    ...
    protected:
	RTPSink(UsageEnvironment &env, Groupsock *rtpGS, unsigned char rtpPayloadType, u_int32_t rtpTimestampFrequency,
	...
	Boolean fEnableRTCPReports; // whether RTCP "SR" reports should be sent for
	        // this sink (default: True)
...
class RTPTransmissionStatsDB {
	...
	void noteIncomingRR(u_int32_t SSRC, struct sockaddr_in const &lastFromAddress, unsigned lossStats,
...
class RTPTransmissionStats {
	...
	void noteIncomingRR(struct sockaddr_in const &lastFromAddress, unsigned lossStats,
	                    unsigned lastPacketNumReceived, unsigned jitter, unsigned lastSRTime,
	                    unsigned diffSR_RRTime);
