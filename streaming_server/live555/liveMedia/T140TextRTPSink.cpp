...
T140TextRTPSink::T140TextRTPSink(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat)
        : TextRTPSink(env, RTPgs, rtpPayloadFormat, 1000 /*mandatory RTP timestamp frequency for this payload format*/,
                      "T140")
        , fOurIdleFilter(NULL)
...
T140TextRTPSink::~T140TextRTPSink()
{
	...
	stopPlaying(); // call this now, because we won't have our 'idle filter' when
...
{
	// First, check whether we have an 'idle filter' set up yet. If not, create it
	// now, and insert it in front of our existing source:
...
void T140TextRTPSink::doSpecialFrameHandling(unsigned /*fragmentationOffset*/, unsigned char * /*frameStart*/,
...
void T140IdleFilter::doGetNextFrame()
{
	...
	fIdleTimerTask =
	        envir().taskScheduler().scheduleDelayedTask(IDLE_TIMEOUT_MICROSECONDS, handleIdleTimeout, this);
...
void T140IdleFilter::afterGettingFrame(void *clientData, unsigned frameSize, unsigned numTruncatedBytes,
...
void T140IdleFilter::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime,
                                       unsigned durationInMicroseconds)
{
	...
	// Then, attempt to deliver this data.  (If we can't deliver it now, we'll do
	// so the next time the reader asks for data.)
...
void T140IdleFilter::handleIdleTimeout()
{
	...
	// period' (after data was requested from downstream).
	// Send an empty 'idle' frame to our downstream "T140TextRTPSink".  (This will
	// cause an empty RTP packet to get sent.)
	deliverEmptyFrame();
}
