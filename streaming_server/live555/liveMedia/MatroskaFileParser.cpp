        ...
        , fPresentationTimeOffset(0.0)
{
	if (ourDemux == NULL) {
		...
		// In this case, parsing (of track data) doesn't start until a client starts
...
#ifdef DEBUG
		fprintf(stderr,
		        "\t=> seek time %f, file position %llu, block number "
		        "within cluster %d\n",
...
void MatroskaFileParser::continueParsing(void *clientData, unsigned char * /*ptr*/, unsigned /*size*/,
...
void MatroskaFileParser::continueParsing()
{
	if (fInputSource != NULL) {
		if (fInputSource->isCurrentlyAwaitingData())
			return; // Our input source is currently being read. Wait until that read
			        // completes

		if (!parse()) {
			// We didn't complete the parsing, because we had to read more data from
			// the source, or because we're waiting for
			// another read from downstream.  Once that happens, we'll get called
			// again.
			return;
		}
	}
...
// We've finished parsing the 'Track' information.  There are also 'Cues' in the
// file, so parse those before finishing:
// Seek to the specified position in the file.  We were already told that the
// 'Cues' begins there:
#ifdef DEBUG
					fprintf(stderr,
...
// Optimization: Seek to the specified position in the file.  We were already
// told that the 'Cluster' begins there:
#ifdef DEBUG
					fprintf(stderr,
					        "Optimization: Seeking to file position %llu (the "
					        "previously-reported location of a 'Cluster')\n",
			...
			case DELIVERING_FRAME_BYTES: {
				deliverFrameBytes();
				return False; // Halt parsing for now.  A new 'read' from downstream
				        // will cause parsing to resume.
				break;
			}
			}
		} while (!areDone);
...
#ifdef DEBUG
		fprintf(stderr, "MatroskaFileParser::parse() EXCEPTION (This is normal "
...
#ifdef DEBUG
	fprintf(stderr,
	        "MatroskaFileParser::parseStartOfFile(): Parsed id 0x%s "
	        "(%s), size: %lld\n",
...
void MatroskaFileParser::lookForNextTrack()
{
	...
	while (fCurrentParseState == LOOKING_FOR_TRACKS) {
...
#ifdef DEBUG
		fprintf(stderr,
		        "MatroskaFileParser::lookForNextTrack(): Parsed id 0x%s "
		        "(%s), size: %lld\n",
		        id.hexString(), id.stringName(), size.val());
#endif
		switch (id.val()) {
		case MATROSKA_ID_SEGMENT: { // 'Segment' header: enter this
			// Remember the position, within the file, of the start of Segment data,
			// because Seek Positions are relative to this:
...
#endif
				// The only 'Seek Position's that we care about are for 'Cluster' and
				// 'Cues':
		...
		case MATROSKA_ID_TIMECODE_SCALE: { // 'Timecode Scale' header: get this
			// value
		...
		case MATROSKA_ID_TRACKS: { // enter this, and move on to parsing 'Tracks'
			fLimitOffsetInFile =
			        fCurOffsetInFile + size.val(); // Make sure we don't read past the end of this header
	...
	while (fCurOffsetInFile < fLimitOffsetInFile) {
		...
		case MATROSKA_ID_TRACK_ENTRY: { // 'Track Entry' header: enter this
			...
			if (track != NULL && track->trackNumber == 0)
				delete track; // We had a previous "MatroskaTrack" object that was never
				        // used
		...
		case MATROSKA_ID_TRACK_TYPE: {
			...
			if (parseEBMLVal_unsigned(size, trackType) && track != NULL) {
				// We convert the Matroska 'track type' code into our own code (which we
				// can use as a bitmap):
				...
				fprintf(stderr, "\tTrack Type 0x%02x (%s)\n", trackType,
				        track->trackType == MATROSKA_TRACK_TYPE_VIDEO ?
				                "video" :
				                track->trackType == MATROSKA_TRACK_TYPE_AUDIO ?
				                "audio" :
		...
		case MATROSKA_ID_CODEC: {
			...
			if (parseEBMLVal_string(size, codecID)) {
				...
				if (track != NULL) {
					...
					// Also set the track's "mimeType" field, if we can deduce it from the
					// "codecID":
				...
				if (track != NULL) {
					...
					if (track->codecID != NULL) {
						if (strcmp(track->codecID, "V_MPEG4/ISO/AVC") == 0) { // H.264
							...
							// H.265 'codec private' data is *supposed* to use the format
							// that's described in
							// http://lists.matroska.org/pipermail/matroska-devel/2013-September/004567.html
							// However, some Matroska files use the same format that was used
							// for H.264.
							// We check for this here, by checking various fields that are
							// supposed to be
							...
							if (codecPrivateSize < 23 ||
								...
								// The 'correct' format isn't being used, so assume the H.264
								// format instead:
								...
								// 'lengthSizeMinusOne':
								if (codecPrivateSize >= 5)
									track->subframeSizeSize = (codecPrivate[4]) &
									                          (0x3 + 1);
							} else {
								// This looks like the 'correct' format:
								track->codecPrivateUsesH264FormatForH265 = False;

								// Byte 21 of the 'codec private' data contains
								// 'lengthSizeMinusOne':
								track->subframeSizeSize = (codecPrivate[21]) &
								                          (0x3 + 1);
							}
						}
					}
		...
		case MATROSKA_ID_CONTENT_ENCODING: { // 'Content Encodings' or 'Content
			// Encoding' header: enter this
			break;
		}
		case MATROSKA_ID_CONTENT_COMPRESSION: { // 'Content Compression' header:
			// enter this
			// Note: We currently support only 'Header Stripping' compression, not
			// 'zlib' compression (the default algorithm).
			// Therefore, we disable this track, unless/until we later see that
			// 'Header Stripping' is supported:
			if (track != NULL)
			...
			if (parseEBMLVal_unsigned(size, contentCompAlgo)) {
				...
				fprintf(stderr, "\tContent Compression Algorithm %d (%s)\n", contentCompAlgo,
				        contentCompAlgo == 0 ? "zlib" :
				                               contentCompAlgo == 3 ? "Header Stripping" : "<unknown>");
#endif
				// The only compression algorithm that we support is #3: Header
				// Stripping; disable the track otherwise
				if (track != NULL)
		...
		case MATROSKA_ID_CONTENT_ENCRYPTION: { // 'Content Encrpytion' header: skip
			// this
			// Note: We don't currently support encryption at all.  Therefore, we
			// disable this track:
			if (track != NULL)
				track->isEnabled = False;
			// Fall through to...
...
void MatroskaFileParser::lookForNextBlock()
{
	...
	while (fCurrentParseState == LOOKING_FOR_BLOCK) {
...
#ifdef DEBUG
		fprintf(stderr,
		        "MatroskaFileParser::lookForNextBlock(): Parsed id 0x%s "
		        "(%s), size: %lld\n",
		        id.hexString(), id.stringName(), size.val());
#endif
		switch (id.val()) {
		...
		case MATROSKA_ID_BLOCK: { // 'SimpleBlock' or 'Block' header: enter this
			// (and we're done)
			fBlockSize = (unsigned)size.val();
			fCurrentParseState = PARSING_BLOCK;
			break;
		}
		case MATROSKA_ID_BLOCK_DURATION: { // 'Block Duration' header: get this
			// value (but we currently don't do
			// anything with it)
...
// Attachments are parsed only if we're in DEBUG mode (otherwise we just skip
// over them):
#ifdef DEBUG
...
Boolean MatroskaFileParser::parseCues()
{
	...
	while (fCurOffsetInFile < fLimitOffsetInFile) {
		...
		case MATROSKA_ID_CUE_TRACK_POSITIONS: { // 'Cue Track Positions' header:
			// enter this
			break;
		}
		case MATROSKA_ID_CUE_TRACK: { // 'Cue Track' header: get this value (but
			// only for debugging; we don't do anything
			// with it)
			unsigned cueTrack;
		...
		case MATROSKA_ID_CUE_CLUSTER_POSITION: { // 'Cue Cluster Position' header:
			// get this value
		...
		case MATROSKA_ID_CUE_BLOCK_NUMBER: { // 'Cue Block Number' header: get this
			// value
...
#endif
				// Record this cue point (overwriting any existing entry for this cue
				// time):
...
void MatroskaFileParser::parseBlock()
{
	...
	do {
		...
		// If this track is not being read, then skip the rest of this block, and
		...
		fprintf(stderr, "\ttrack number %d, timecode %d (=> %f seconds), %s lacing\n", fBlockTrackNumber,
		        ...
		        (lacingType == NoLacing) ?
		                ...
		                (lacingType == XiphLacing) ? "Xiph" :
		                                             (lacingType == FixedSizeLacing) ? "fixed-size" : "EBML");
#endif
		...
		if (lacingType == NoLacing) {
			...
			for (i = 0; i < fNumFramesInBlock - 1; ++i) {
				if (lacingType == EBMLLacing) {
					...
					if (i == 0) {
						...
						// The value we read is a signed value, that's added to the previous
						// frame size, to get the current frame size:
						unsigned toSubtract = (fsv > 0xFFFFFF) ? 0x07FFFFFF :
						                                         (fsv > 0xFFFF) ?
						                                         0x0FFFFF :
						                                         (fsv > 0xFF) ? 0x1FFF : 0x3F;
			...
			// Compute the size of the final frame within the block (from the block's
			// size, and the frame sizes already computed):)
			unsigned headerBytesSeen = curOffset() - blockStartPos;
		...
		// know the sizes of all frames within the block.
		// If we have 'stripped bytes' that are common to (the front of) all frames,
		// then count them now:
...
// The next time he asks for a frame, he'll get it.
		...
		if (track->haveSubframes()) {
			// The next "track->subframeSizeSize" bytes contain the length of a
			// 'subframe':
		...
		// Compute the presentation time of this frame (from the cluster timecode,
		// the block timecode, and the default duration):
		...
		if (fPresentationTimeOffset == 0.0) {
			// This is the first time we've computed a presentation time.  Compute an
			// offset to make the presentation times aligned
		...
		if (track->haveSubframes()) {
			// If this is a 'subframe', use a duration of 0 instead (unless it's the
			// last 'subframe'):
			if (fCurOffsetWithinFrame + frameSize + track->subframeSizeSize <
			    fFrameSizesWithinBlock[fNextFrameNumberToDeliver]) {
				// There's room for at least one more subframe after this, so give this
				// subframe a duration of 0
				durationInMicroseconds = 0;
			}
		}

		if (track->defaultDuration == 0) {
			// Adjust the frame duration to keep the sum of frame durations aligned
			...
			if (demuxedTrack->prevPresentationTime().tv_sec != 0) { // not the first time for this track
				demuxedTrack->durationImbalance() +=
				        (presentationTime.tv_sec - demuxedTrack->prevPresentationTime().tv_sec) *
				                1000000 +
			...
			if (demuxedTrack->durationImbalance() > 0) {
				...
				int const adjustmentThreshold = 100000; // don't increase the duration
				        // by more than this amount (in
				        // case there's a mistake)
		...
		getCommonFrameBytes(track, demuxedTrack->to(), demuxedTrack->frameSize(),
...
void MatroskaFileParser::deliverFrameBytes()
{
	do {
		...
		while (fCurFrameNumBytesToGet > 0) {
			...
			unsigned numBytesToGet = fCurFrameNumBytesToGet > BANK_SIZE ? BANK_SIZE :
		...
		while (fCurFrameNumBytesToSkip > 0) {
			...
			unsigned numBytesToSkip = fCurFrameNumBytesToSkip > BANK_SIZE ? BANK_SIZE :
...
#ifdef DEBUG
		fprintf(stderr, "\tdelivered frame #%d: %d bytes", fNextFrameNumberToDeliver,
		...
		fprintf(stderr, " @%u.%06u (%.06f from start); duration %u us\n",
		        ...
		        demuxedTrack->presentationTime().tv_sec + demuxedTrack->presentationTime().tv_usec / 1000000.0 -
		                fPresentationTimeOffset,
		        demuxedTrack->durationInMicroseconds());
#endif

		if (!track->haveSubframes() || fCurOffsetWithinFrame + track->subframeSizeSize >=
		                                       fFrameSizesWithinBlock[fNextFrameNumberToDeliver]) {
			// Either we don't have subframes, or there's no more room for another
			// subframe => We're completely done with this frame now:
			++fNextFrameNumberToDeliver;
			fCurOffsetWithinFrame = 0;
		}
		if (fNextFrameNumberToDeliver == fNumFramesInBlock) {
			// We've delivered all of the frames from this block.  Look for another
			// block next:
			fCurrentParseState = LOOKING_FOR_BLOCK;
		} else {
			fCurrentParseState = DELIVERING_FRAME_WITHIN_BLOCK;
...
void MatroskaFileParser::getCommonFrameBytes(MatroskaTrack *track, u_int8_t *to, unsigned numBytesToGet,
                                             unsigned numBytesToSkip)
{
	if (track->headerStrippedBytesSize > fCurOffsetWithinFrame) {
		// We have some common 'header stripped' bytes that remain to be prepended
		// to the frame.  Use these first:
...
Boolean MatroskaFileParser::parseEBMLNumber(EBMLNumber &num)
{
	...
	for (i = 0; i < EBML_NUMBER_MAX_LEN; ++i) {
		while (1) {
			...
			// If we're looking for an id, skip any leading bytes that don't contain a
			// '1' in the first 4 bits:
			if (i == 0 /*we're a leading byte*/ && !num.stripLeading1 /*we're looking for an id*/ &&
			    (num.data[i] & 0xF0) == 0) {
				setParseState(); // ensures that we make forward progress if the parsing
				        // gets interrupted
				continue;
			}
			break;
...
{
	if (size.val() == 4) {
		// Normal case.  Read the value as if it were a 4-byte integer, then copy it
		// to the 'float' result:
...
void MatroskaFileParser::skipRemainingHeaderBytes(Boolean isContinuation)
{
	...
	// Hack: To avoid tripping into a parser 'internal error' if we try to skip an
	// excessively large
	// distance, break up the skipping into manageable chunks, to ensure forward
	// progress:
	unsigned const maxBytesToSkip = bankSize();
	while (fNumHeaderBytesToSkip > 0) {
		unsigned numBytesToSkipNow = fNumHeaderBytesToSkip < maxBytesToSkip ? (unsigned)fNumHeaderBytesToSkip :
		                                                                      maxBytesToSkip;
		setParseState();
...
void MatroskaFileParser::seekToFilePosition(u_int64_t offsetInFile)
{
	ByteStreamFileSource *fileSource =
	        (ByteStreamFileSource *)fInputSource; // we know it's a "ByteStreamFileSource"
	if (fileSource != NULL) {
...
{
	// Because we're resuming parsing after seeking to a new position in the file,
