...
void OggFileServerDemux::createNew(UsageEnvironment &env, char const *fileName, onCreationFunc *onCreation,
...
#ifdef DEBUG
		fprintf(stderr,
		        "Created 'ServerMediaSubsession' object for track #%d: no "
		        "codecID (%s)\n",
...
FramedSource *OggFileServerDemux::newDemuxedTrack(unsigned clientSessionId, u_int32_t trackNumber)
{
	...
	if (clientSessionId != 0 && clientSessionId == fLastClientSessionId) {
		...
		// different
		// client sessions do not overlap - so all demuxed tracks are created for
		// one "OggDemux" at a time.
		// Also, the "clientSessionId != 0" test is a hack, because 'session 0' is
		// special; its audio and video streams
...
OggFileServerDemux::OggFileServerDemux(UsageEnvironment &env, char const *fileName, onCreationFunc *onCreation,
                                       void *onCreationClientData)
        : Medium(env)
        , fFileName(fileName)
