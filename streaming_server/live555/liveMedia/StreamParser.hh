...
class StreamParser {
	...
	typedef void(clientContinueFunc)(void *clientData, unsigned char *ptr, unsigned size,
	...
	u_int32_t test4Bytes1(unsigned char **data)
	{ // as above, but doesn't advance ptr
		ensureValidBytes(4);
	...
	static void afterGettingBytes(void *clientData, unsigned numBytesRead, unsigned numTruncatedBytes,
...
class StreamParserByPtr {
	...
	typedef void(clientContinueFunc)(void *clientData, unsigned char *ptr, unsigned size,
	                                 struct timeval presentationTime);
	StreamParserByPtr(FramedSource *inputSource, FramedSource::onCloseFunc *onInputCloseFunc,
	                  void *onInputCloseClientData, clientContinueFunc *clientContinueFunc,
	                  void *clientContinueClientData);
	virtual ~StreamParserByPtr();
	...
	void keepFrameBytes(unsigned int frameSize)
	{
		fCurParserIndex += frameSize;
	}
