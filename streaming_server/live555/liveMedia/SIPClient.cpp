...
SIPClient *SIPClient::createNew(UsageEnvironment &env, unsigned char desiredAudioRTPPayloadFormat,
                                char const *mimeSubtype, int verbosityLevel, char const *applicationName)
{
...
SIPClient::SIPClient(UsageEnvironment &env, unsigned char desiredAudioRTPPayloadFormat, char const *mimeSubtype,
        ...
        , fInviteCmdSize(0)
{
	...
	if (srcPort.num() != 0) {
		...
		if (fOurSocket == NULL) {
			env << "ERROR: Failed to create socket for addr " << fOurAddressStr << ", port " << fOurPortNum
			    << ": " << env.getResultMsg() << "\n";
		}
	}
	...
	unsigned userAgentNameSize = fApplicationNameSize + strlen(libPrefix) + strlen(libName) +
...
void SIPClient::setProxyServer(unsigned proxyServerAddress, portNumBits proxyServerPortNum)
{
...
char *SIPClient::invite1(Authenticator *authenticator)
{
	do {
		...
		char const *const inviteSDPFmt = "v=0\r\n"
		                                 ...
		                                 "m=audio %u RTP/AVP %u\r\n"
		                                 "%s";
		unsigned inviteSDPFmtSize = strlen(inviteSDPFmt) + 20 /* max int len */ + 20 + fOurAddressStrSize +
		                            fApplicationNameSize + fOurAddressStrSize + 5 /* max short len */ +
		                            3 /* max char len */
		                            + rtpmapLineSize;
		delete[] fInviteSDPDescription;
		fInviteSDPDescription = new char[inviteSDPFmtSize];
		sprintf(fInviteSDPDescription, inviteSDPFmt, fCallId, fCSeq, fOurAddressStr, fApplicationName,
		        fOurAddressStr, fClientStartPortNum, fDesiredAudioRTPPayloadFormat, rtpmapLine);
		...
		char const *const cmdFmt = "INVITE %s SIP/2.0\r\n"
		                           ...
		                           "Content-Length: %d\r\n\r\n"
		                           "%s";
		unsigned inviteCmdSize = strlen(cmdFmt) + fURLSize + 2 * fUserNameSize + fOurAddressStrSize +
		                         20 /* max int len */
		                         + fOurAddressStrSize + 5 /* max port len */
		                         + fURLSize + fUserNameSize + fOurAddressStrSize + 5 + 20 + fOurAddressStrSize +
		                         20 + strlen(authenticatorStr) + fUserAgentHeaderStrLen + 20 + inviteSDPSize;
		delete[] fInviteCmd;
		fInviteCmd = new char[inviteCmdSize];
		sprintf(fInviteCmd, cmdFmt, fURL, fUserName, fUserName, fOurAddressStr, fFromTag, fOurAddressStr,
		        fOurPortNum, fURL, fUserName, fOurAddressStr, fOurPortNum, fCallId, fOurAddressStr, ++fCSeq,
		        authenticatorStr, fUserAgentHeaderStr, inviteSDPSize, fInviteSDPDescription);
...
void SIPClient::timerAHandler(void *clientData)
{
	...
	if (client->fVerbosityLevel >= 1) {
		client->envir() << "RETRANSMISSION " << ++client->fTimerACount << ", after "
		                << client->fTimerALen / 1000000.0 << " additional seconds\n";
	}
...
Boolean SIPClient::sendACK()
{
	...
	do {
		char const *const cmdFmt = "ACK %s SIP/2.0\r\n"
		                           ...
		                           "CSeq: %d ACK\r\n"
		                           "Content-Length: 0\r\n\r\n";
		unsigned cmdSize = strlen(cmdFmt) + fURLSize + 2 * fUserNameSize + fOurAddressStrSize +
		                   20 /* max int len */
		                   + fOurAddressStrSize + 5 /* max port len */
		                   + fURLSize + fToTagStrSize + 20 + fOurAddressStrSize + 20;
		cmd = new char[cmdSize];
		sprintf(cmd, cmdFmt, fURL, fUserName, fUserName, fOurAddressStr, fFromTag, fOurAddressStr, fOurPortNum,
		        fURL, fToTagStr, fCallId, fOurAddressStr,
...
Boolean SIPClient::sendBYE()
{
	...
	do {
		...
		cmd = new char[cmdSize];
		sprintf(cmd, cmdFmt, fURL, fUserName, fUserName, fOurAddressStr, fFromTag, fOurAddressStr, fOurPortNum,
		        fURL, fToTagStr, fCallId, fOurAddressStr, ++fCSeq);
...
{
	if (authenticator != NULL && authenticator->realm() != NULL && authenticator->nonce() != NULL &&
	    authenticator->username() != NULL && authenticator->password() != NULL) {
		// We've been provided a filled-in authenticator, so use it:
		char const *const authFmt = "Proxy-Authorization: Digest username=\"%s\", "
		                            "realm=\"%s\", nonce=\"%s\", response=\"%s\", "
		                            "uri=\"%s\"\r\n";
		...
		unsigned authBufSize = strlen(authFmt) + strlen(authenticator->username()) +
		                       strlen(authenticator->realm()) + strlen(authenticator->nonce()) + strlen(url) +
		                       strlen(response);
...
Boolean SIPClient::sendRequest(char const *requestString, unsigned requestLength)
{
	...
	return fOurSocket->output(envir(), 255, (unsigned char *)requestString, requestLength);
}
...
unsigned SIPClient::getResponse(char *&responseBuffer, unsigned responseBufferSize)
{
	...
	while (bytesRead < (int)responseBufferSize) {
		...
		Boolean readSuccess =
		        fOurSocket->handleRead(toPosn, responseBufferSize - bytesRead, bytesReadNow, fromAddr);
