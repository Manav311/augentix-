...
Boolean MediaSession::initializeWithSDP(char const *sdpDescription)
{
	...
	while (sdpLine != NULL) {
		...
		if ((sscanf(sdpLine, "m=%s %hu RTP/AVP %u", mediumName, &subsession->fClientPortNum, &payloadFormat) ==
		             3 ||
			...
			protocolName = "RTP";
		            ...
		            sscanf(sdpLine, "m=%s %hu udp %u", mediumName, &subsession->fClientPortNum,
		                   &payloadFormat) == 3 ||
		            sscanf(sdpLine, "m=%s %hu RAW/RAW/UDP %u", mediumName, &subsession->fClientPortNum,
		                   &payloadFormat) == 3) &&
		           payloadFormat <= 127) {
...
static Boolean parseSourceFilterAttribute(char const *sdpLine, struct in_addr &sourceAddr)
{
	...
	do {
		...
		netAddressBits sourceAddrBits = *(netAddressBits *)(addresses.firstAddress()->data());
		if (sourceAddrBits == 0)
			break;
        ...
        , fSessionId(NULL)
{
	...
	// This won't work for MPEG-4 (unless the value is <10), because for MPEG-4,
	// the value
	...
	do {
		...
		if (fClientPortNum != 0 && (honorSDPPortChoice || IsMulticastAddress(tempAddr.s_addr))) {
			...
			if (protocolIsRTP) {
				...
				// use an even-numbered port for RTP, and the next (odd-numbered) port
				// for RTCP
			}
			if (isSSM()) {
			...
			// Port numbers were not specified in advance, so we use ephemeral port
			// numbers.
			// Create sockets until we get a port-number pair (even: RTP; even+1:
			// RTCP).
			// We need to make sure that we don't keep trying to use the same bad port
			// numbers over
			// and over again, so we store bad sockets in a table, and delete them all
			// when we're done.
			...
			NoReuse dummy(env());
			// ensures that our new ephemeral port number won't be one that's already
			// in use
			...
			while (1) {
				...
				if (fRTPSocket == NULL) {
					env().setResultMsg("MediaSession::initiate(): unable to create RTP "
					                   "and RTCP sockets");
					break;
				}
		...
		if (isSSM() && fRTCPSocket != NULL) {
			// Special case for RTCP SSM: Send RTCP packets back to the source via
			// unicast:
		...
		if (fRTPSource != NULL && fRTCPSocket != NULL) {
			...
			fRTCPInstance = RTCPInstance::createNew(env(), fRTCPSocket, totSessionBandwidth,
...
double MediaSubsession::getNormalPlayTime(struct timeval const &presentationTime)
{
	...
	// using RTCP.
	// If it hasn't, then - as a special case - we need to use the RTP timestamp
	// to compute the NPT.
	if (!rtpSource()->hasBeenSynchronizedUsingRTCP()) {
		...
		// Common case: We have been synchronized using RTCP.  This means that the
		// "presentationTime" parameter
		...
		if (rtpInfo.infoIsNew) {
			...
			// structure was last filled in.  Use this "presentationTime" to compute
			// "fNPT_PTS_Offset":
			...
			if (fNPT_PTS_Offset == 0.0)
				return 0.0; // error: The "rtpInfo" structure was apparently never
				        // filled in
...
Boolean MediaSubsession::parseSDPAttribute_rtpmap(char const *sdpLine)
{
	...
	if (sscanf(sdpLine, "a=rtpmap: %u %[^/]/%u/%u", &rtpmapPayloadFormat, codecName, &rtpTimestampFrequency,
	           &numChannels) == 4 ||
		...
		if (strcmp(fProtocolName, "UDP") == 0) {
			...
			if (strcmp(fCodecName, "QCELP") == 0) { // QCELP audio
				...
				fReadSource = AMRAudioRTPSource::createNew(
				        env(), fRTPSocket, fRTPSource, fRTPPayloadFormat, False /*isWideband*/,
				        fNumChannels, attrVal_bool("octet-align"), attrVal_unsigned("interleaving"),
				        attrVal_bool("robust-sorting"), attrVal_bool("crc"));
				// Note that fReadSource will differ from fRTPSource in this case
			} else if (strcmp(fCodecName, "AMR-WB") == 0) { // AMR audio (wideband)
				fReadSource = AMRAudioRTPSource::createNew(
				        env(), fRTPSocket, fRTPSource, fRTPPayloadFormat, True /*isWideband*/,
				        fNumChannels, attrVal_bool("octet-align"), attrVal_unsigned("interleaving"),
				        attrVal_bool("robust-sorting"), attrVal_bool("crc"));
				// Note that fReadSource will differ from fRTPSource in this case
				...
				fReadSource = fRTPSource = MPEG4GenericRTPSource::createNew(
				        ...
				        attrVal_str("mode"), attrVal_unsigned("sizelength"),
				...
				if (fReceiveRawJPEGFrames) {
					// Special case (used when proxying JPEG/RTP streams): Receive each
					// JPEG/RTP packet, including the special RTP headers:
					fReadSource = fRTPSource = SimpleRTPSource::createNew(
					        env(), fRTPSocket, fRTPPayloadFormat, fRTPTimestampFrequency,
					        "video/JPEG", 0 /*special offset*/,
					        False /*doNormalMBitRule => ignore the 'M' bit*/);
				} else {
					// Normal case: Receive each JPEG frame as a complete, displayable
					// JPEG image:
				...
				if (strcmp(fCodecName, "VND.ONVIF.METADATA") == 0) {
					// This RTP payload format uses the RTP "M" bit to indicate the end of
					// the content (a XML document):
...
{
	if (strValue == NULL) {
		// No value was given for this attribute, so consider it to be a Boolean,
		// with value True:
		fIntValue = 1;
	} else {
		// Try to parse "strValue" as an integer.  If we can't, assume an integer
		// value of 0:
