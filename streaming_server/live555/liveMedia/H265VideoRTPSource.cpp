...
H265VideoRTPSource::H265VideoRTPSource(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
...
Boolean H265VideoRTPSource::processSpecialHeader(BufferedPacket *packet, unsigned &resultSpecialHeaderSize)
{
	...
	case 49: { // Fragmentation Unit (FU)
		// This NALU begins with the 2-byte Payload Header, the 1-byte FU header,
		// and (optionally)
		// the 2-byte DONL header.
		// If the start bit is set, we reconstruct the original NAL header at the
		// end of these
...
{
	if (!fExpectDONFields) {
		// Without DON fields in the input stream, we just increment our "AbsDon"
		// count each time:
