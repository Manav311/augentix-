...
class InputESSourceRecord {
    public:
	InputESSourceRecord(MPEG2TransportStreamFromESSource &parent, FramedSource *inputSource, u_int8_t streamId,
	                    int mpegVersion, InputESSourceRecord *next);
...
void MPEG2TransportStreamFromESSource::addNewVideoSource(FramedSource *inputSource, int mpegVersion)
{
...
void MPEG2TransportStreamFromESSource::addNewAudioSource(FramedSource *inputSource, int mpegVersion)
{
...
void MPEG2TransportStreamFromESSource::addNewInputSource(FramedSource *inputSource, u_int8_t streamId, int mpegVersion)
{
	if (inputSource == NULL)
		return;
	fInputSources = new InputESSourceRecord(*this, inputSource, streamId, mpegVersion, fInputSources);
}

////////// InputESSourceRecord implementation //////////

InputESSourceRecord::InputESSourceRecord(MPEG2TransportStreamFromESSource &parent, FramedSource *inputSource,
                                         u_int8_t streamId, int mpegVersion, InputESSourceRecord *next)
        : fNext(next)
        , fParent(parent)
...
void InputESSourceRecord::askForNewData()
{
	...
	if (fInputBufferBytesAvailable < LOW_WATER_MARK && !fInputSource->isCurrentlyAwaitingData()) {
		...
		fInputSource->getNextFrame(&fInputBuffer[fInputBufferBytesAvailable],
		                           INPUT_BUFFER_SIZE - fInputBufferBytesAvailable, afterGettingFrame, this,
...
Boolean InputESSourceRecord::deliverBufferToClient()
{
	...
	if (PES_packet_length > 0xFFFF) {
		// Set the PES_packet_length field to 0.  This indicates an unbounded length
		// (see ISO 13818-1, 2.4.3.7)
		PES_packet_length = 0;
	}
	...
	// Do the delivery:
	fParent.handleNewBuffer(fInputBuffer, fInputBufferBytesAvailable, fMPEGVersion, fSCR);

	return True;
}

void InputESSourceRecord::afterGettingFrame(void *clientData, unsigned frameSize, unsigned numTruncatedBytes,
                                            struct timeval presentationTime, unsigned /*durationInMicroseconds*/)
{
...
void InputESSourceRecord::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
                                             struct timeval presentationTime)
{
	if (numTruncatedBytes > 0) {
		fParent.envir() << "MPEG2TransportStreamFromESSource: input buffer too "
		                   "small; increase \"MAX_INPUT_ES_FRAME_SIZE\" in "
		                   "\"MPEG2TransportStreamFromESSource\" by at least "
	...
	if (fInputBufferBytesAvailable == SIMPLE_PES_HEADER_SIZE) {
		...
		fSCR.highBit =
		        ((presentationTime.tv_sec * 45000 + (presentationTime.tv_usec * 9) / 200) & 0x80000000) != 0;
