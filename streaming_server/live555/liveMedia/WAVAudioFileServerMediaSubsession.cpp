...
WAVAudioFileServerMediaSubsession *WAVAudioFileServerMediaSubsession::createNew(UsageEnvironment &env,
                                                                                char const *fileName,
...
WAVAudioFileServerMediaSubsession::WAVAudioFileServerMediaSubsession(UsageEnvironment &env, char const *fileName,
...
void WAVAudioFileServerMediaSubsession::seekStreamSource(FramedSource *inputSource, double &seekNPT,
...
void WAVAudioFileServerMediaSubsession::setStreamSourceDuration(FramedSource *inputSource, double streamDuration,
                                                                u_int64_t &numBytes)
{
	WAVAudioFileSource *wavSource;
	if (fBitsPerSample > 8) {
...
FramedSource *WAVAudioFileServerMediaSubsession::createNewStreamSource(unsigned /*clientSessionId*/,
                                                                       unsigned &estBitrate)
{
	...
	do {
		...
		if (fAudioFormat == WA_PCM) {
			if (fBitsPerSample == 16) {
				...
				if (fConvertToULaw) {
					// Add a filter that converts from raw 16-bit PCM audio to 8-bit u-law
					// audio:
...
RTPSink *WAVAudioFileServerMediaSubsession::createNewRTPSink(Groupsock *rtpGroupsock,
	...
	do {
		...
		if (fAudioFormat == WA_PCM) {
			if (fBitsPerSample == 16) {
				...
				mimeType = "L24";
			} else { // fBitsPerSample == 8 (we assume that fBitsPerSample == 4 is
		...
		return SimpleRTPSink::createNew(envir(), rtpGroupsock, payloadFormatCode, fSamplingFrequency, "audio",
