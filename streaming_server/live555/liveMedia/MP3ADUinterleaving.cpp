...
MP3ADUinterleaverBase::MP3ADUinterleaverBase(UsageEnvironment &env, FramedSource *inputSource)
...
void MP3ADUinterleaverBase::afterGettingFrame(void *clientData, unsigned numBytesRead, unsigned /*numTruncatedBytes*/,
...
class InterleavingFrames {
	...
	void getReleasingFrameParams(unsigned char index, unsigned char *&dataPtr, unsigned &bytesInUse,
	                             struct timeval &presentationTime, unsigned &durationInMicroseconds);
	void setFrameParams(unsigned char index, unsigned char icc, unsigned char ii, unsigned frameSize,
	                    struct timeval presentationTime, unsigned durationInMicroseconds);
...
MP3ADUinterleaver *MP3ADUinterleaver::createNew(UsageEnvironment &env, Interleaving const &interleaving,
                                                FramedSource *inputSource)
{
...
void MP3ADUinterleaver::doGetNextFrame()
{
	...
	if (fFrames->haveReleaseableFrame()) {
		...
		fInputSource->getNextFrame(dataPtr, bytesAvailable, &MP3ADUinterleaverBase::afterGettingFrame, this,
		                           handleClosure, this);
	}
}
...
void MP3ADUinterleaver::releaseOutgoingFrame()
{
	...
	fFrames->getReleasingFrameParams(fFrames->nextIndexToRelease(), fromPtr, fFrameSize, fPresentationTime,
...
void MP3ADUinterleaver::afterGettingFrame(unsigned numBytesRead, struct timeval presentationTime,
                                          unsigned durationInMicroseconds)
{
	// Set the (icc,ii) and frame size of the newly-read frame:
	fFrames->setFrameParams(fPositionOfNextIncomingFrame, fICC, fII, numBytesRead, presentationTime,
...
class DeinterleavingFrames {
	...
	void getIncomingFrameParamsAfter(unsigned frameSize, struct timeval presentationTime,
	                                 unsigned durationInMicroseconds, unsigned char &icc, unsigned char &ii);
	void getReleasingFrameParams(unsigned char *&dataPtr, unsigned &bytesInUse, struct timeval &presentationTime,
	                             unsigned &durationInMicroseconds);
...
void MP3ADUdeinterleaver::doGetNextFrame()
{
	...
	NOTE:
		This code no longer works, because it uses synchronous reads,
		        which are no longer supported.static unsigned const framesPerPacket = 3;
		...
		while (1) {
			...
			if (packetIsLost) {
				...
				fInputSource->syncGetNextFrame(dummyBuf, sizeof dummyBuf, numBytesRead,
				                               presentationTime);
			} else {
				break; // from while (1)
			}
...
void MP3ADUdeinterleaver::afterGettingFrame(unsigned numBytesRead, struct timeval presentationTime,
                                            unsigned durationInMicroseconds)
{
...
void InterleavingFrames::getReleasingFrameParams(unsigned char index, unsigned char *&dataPtr, unsigned &bytesInUse,
                                                 struct timeval &presentationTime, unsigned &durationInMicroseconds)
{
	InterleavingFrameDescriptor &desc = fDescriptors[index];
	dataPtr = &desc.frameData[0];
...
void InterleavingFrames::setFrameParams(unsigned char index, unsigned char icc, unsigned char ii, unsigned frameSize,
                                        struct timeval presentationTime, unsigned durationInMicroseconds)
{
	InterleavingFrameDescriptor &desc = fDescriptors[index];
...
void DeinterleavingFrames::getIncomingFrameParamsAfter(unsigned frameSize, struct timeval presentationTime,
                                                       unsigned durationInMicroseconds, unsigned char &icc,
                                                       unsigned char &ii)
{
	DeinterleavingFrameDescriptor &desc = fDescriptors[MAX_CYCLE_SIZE];
	desc.frameDataSize = frameSize;
...
void DeinterleavingFrames::getReleasingFrameParams(unsigned char *&dataPtr, unsigned &bytesInUse,
                                                   struct timeval &presentationTime, unsigned &durationInMicroseconds)
{
