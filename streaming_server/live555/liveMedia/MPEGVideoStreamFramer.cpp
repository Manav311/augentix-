...
{
	return pictures == arg2.pictures && seconds == arg2.seconds && minutes == arg2.minutes && hours == arg2.hours &&
	       days == arg2.days;
}
...
void MPEGVideoStreamFramer::reset()
{
	...
	gettimeofdayMonotonic(&fPresentationTimeBase, NULL);
}
...
void MPEGVideoStreamFramer::computePresentationTime(unsigned numAdditionalPictures)
{
	...
	double pictureTime =
	        fFrameRate == 0.0 ? 0.0 : (tc.pictures + fPicturesAdjustment + numAdditionalPictures) / fFrameRate;
	...
	fprintf(stderr,
	        "MPEGVideoStreamFramer::computePresentationTime(%d) -> "
	        "%lu.%06ld [%lu.%06ld]\n",
	        numAdditionalPictures, fPresentationTime.tv_sec, fPresentationTime.tv_usec, diffPT.tv_sec,
	        diffPT.tv_usec);
#endif
}

void MPEGVideoStreamFramer::setTimeCode(unsigned hours, unsigned minutes, unsigned seconds, unsigned pictures,
...
void MPEGVideoStreamFramer::continueReadProcessing(void *clientData, unsigned char * /*ptr*/, unsigned /*size*/,
...
void MPEGVideoStreamFramer::continueReadProcessing()
{
	...
	if (acquiredFrameSize > 0) {
		...
		fDurationInMicroseconds = (fFrameRate == 0.0 || ((int)fPictureCount) < 0) ?
		                                  0 :
