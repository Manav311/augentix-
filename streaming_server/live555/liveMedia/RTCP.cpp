...
static double dTimeNow()
{
	...
	gettimeofdayMonotonic(&timeNow, NULL);
...
RTCPInstance::RTCPInstance(UsageEnvironment &env, Groupsock *RTCPgs, unsigned totSessionBW, unsigned char const *cname,
                           RTPSink *sink, RTPSource const *source, Boolean isSSMSource)
        : Medium(env)
...
RTCPInstance *RTCPInstance::createNew(UsageEnvironment &env, Groupsock *RTCPgs, unsigned totSessionBW,
                                      unsigned char const *cname, RTPSink *sink, RTPSource const *source,
                                      Boolean isSSMSource)
{
	return new RTCPInstance(env, RTCPgs, totSessionBW, cname, sink, source, isSSMSource);
}
...
void RTCPInstance::setSpecificRRHandler(netAddressBits fromAddress, Port fromPort, TaskFunc *handlerTask,
                                        void *clientData)
{
	...
	RRHandlerRecord *existingRecord =
	        (RRHandlerRecord *)fSpecificRRHandlerTable->Add(fromAddress, (~0), fromPort, rrHandler);
...
void RTCPInstance::unsetSpecificRRHandler(netAddressBits fromAddress, Port fromPort)
{
	...
	RRHandlerRecord *rrHandler = (RRHandlerRecord *)(fSpecificRRHandlerTable->Lookup(fromAddress, (~0), fromPort));
	if (rrHandler != NULL) {
...
void RTCPInstance::incomingReportHandler1()
{
	do {
		...
		if (fNumBytesAlreadyRead >= maxRTCPPacketSize) {
			envir() << "RTCPInstance error: Hit limit when reading incoming packet "
			           "over TCP. Increase \"maxRTCPPacketSize\"\n";
			break;
		}
		Boolean readResult = fRTCPInterface.handleRead(&fInBuf[fNumBytesAlreadyRead],
		                                               ...
		                                               fromAddress, packetReadWasIncomplete);
		...
		if (fIsSSMSource && !packetWasFromOurHost) {
			...
			// a SSM source, and SSM receivers send back RTCP "RR"
			// packets via unicast).  'Reflect' the packet by resending it to the
			// multicast group, so that any other receivers can also
			// get to see it.
			...
			// NOTE: The test for "!packetWasFromOurHost" means that we won't reflect
			// RTCP packets that come from other processes on
			// the same host as us.  The reason for this is that the 'packet size'
			// test above is not 100% reliable; some packets
			// that were truly looped back from us might not be detected as such, and
			// this might lead to infinite forwarding/receiving
			// of some packets.  To avoid this possibility, we only reflect RTCP
			// packets that we know for sure originated elsewhere.
			// (Note, though, that if we ever re-enable the code in
			// "Groupsock::multicastSendOnly()", then we could remove the test for
		...
		if (tcpReadStreamSocketNum < 0) {
			// Note that "fromAddress" is valid only if we're receiving over UDP (not
			// over TCP):
			fprintf(stderr, " (from address %s, port %d)", AddressString(fromAddress).val(),
		...
		while (1) {
			...
			case RTCP_PT_RR: {
				...
				if (fSink != NULL) {
					...
					for (unsigned i = 0; i < rc; ++i) {
						...
						ADVANCE(4);
						// We care only about reports about our own transmission, not
						// others'
			...
			case RTCP_PT_BYE: {
				...
				fprintf(stderr, "BYE\n");
#endif
				// If a 'BYE handler' was set, arrange for it to be called at the end of
				// this routine.
				// (Note: We don't call it immediately, in case it happens to cause
				// "this" to be deleted.)
				if (fByeHandlerTask != NULL &&
				    (!fByeHandleActiveParticipantsOnly ||
				     (fSource != NULL && fSource->receptionStatsDB().lookup(reportSenderSSRC) != NULL) ||
		...
		// Finally, if we need to call a "BYE" handler, do so now (in case it causes
		// "this" to get deleted):
...
Boolean RTCPInstance::addReport(Boolean alwaysAdd)
{
	...
	if (fSink != NULL) {
		if (!alwaysAdd) {
			...
			// Hack: Don't send a SR during those (brief) times when the timestamp of
			// the
			// next outgoing RTP packet has been preset, to ensure that that timestamp
			// gets
...
void RTCPInstance::addSR()
{
	...
	struct timeval timeNow;
	struct timeval rtpTimeStampNow; // for RTPTimeStamp
	gettimeofdayMonotonic(&timeNow, NULL);
	gettimeofdayMonotonic(&rtpTimeStampNow, NULL);
	...
	unsigned rtpTimestamp = fSink->convertToRTPTimestamp(rtpTimeStampNow);
...
void RTCPInstance::schedule(double nextTime)
{
	...
	if ((int)nextTask() !=0) {
		...
		envir().taskScheduler().rescheduleDelayedTask(nextTask(), usToGo);
#endif
	} else {
...
void RTCPInstance::reschedule(double nextTime)
{
	...
	schedule(nextTime);
#else
	envir().taskScheduler().rescheduleDelayedTask(nextTask(), nextTime);
#endif
}
