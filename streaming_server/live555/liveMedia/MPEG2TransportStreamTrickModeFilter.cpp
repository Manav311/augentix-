...
MPEG2TransportStreamTrickModeFilter::createNew(UsageEnvironment &env, FramedSource *inputSource,
...
#define isIFrameStart(type)                                                                     \
	((type) == 0x81 /*actually, a VSH*/ || (type) == 0x85 /*actually, a SPS, for H.264*/ || \
...
void MPEG2TransportStreamTrickModeFilter::doGetNextFrame()
{
	...
	while (1) {
		...
		// packet
		// (from the input source) the data comes from, and where in the Transport
		// Stream
		// packet it comes from:
		u_int8_t recordType;
		float recordPCR;
		Boolean endOfIndexFile = False;
		if (!fIndexFile->readIndexRecordValues(fNextIndexRecordNum, fDesiredTSPacketNum, fDesiredDataOffset,
		                                       fDesiredDataSize, recordPCR, recordType)) {
		...
		//    'I' : isNonIFrameStart(recordType) ? 'j' : 'x', recordPCR);
		fNextIndexRecordNum += (fState == DELIVERING_SAVED_FRAME) ? 1 : fDirection;

		// Handle this index record, depending on the record type and our current
		// state:
		switch (fState) {
		case SKIPPING_FRAME:
...
void MPEG2TransportStreamTrickModeFilter::afterGettingFrame(void *clientData, unsigned frameSize,
