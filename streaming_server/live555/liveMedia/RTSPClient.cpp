...
RTSPClient *RTSPClient::createNew(UsageEnvironment &env, char const *rtspURL, int verbosityLevel,
...
unsigned RTSPClient::sendPlayCommand(MediaSession &session, responseHandler *responseHandler, double start, double end,
...
unsigned RTSPClient::sendPlayCommand(MediaSubsession &subsession, responseHandler *responseHandler, double start,
                                     double end, float scale, Authenticator *authenticator)
{
	if (authenticator != NULL)
		fCurrentAuthenticator = *authenticator;
	sendDummyUDPPackets(subsession); // hack to improve NAT traversal
	return sendRequest(
	        new RequestRecord(++fCSeq, "PLAY", responseHandler, NULL, &subsession, 0, start, end, scale));
}

unsigned RTSPClient::sendPlayCommand(MediaSession &session, responseHandler *responseHandler, char const *absStartTime,
                                     char const *absEndTime, float scale, Authenticator *authenticator)
{
	if (authenticator != NULL)
		fCurrentAuthenticator = *authenticator;
	sendDummyUDPPackets(session); // hack to improve NAT traversal
	return sendRequest(
	        new RequestRecord(++fCSeq, responseHandler, absStartTime, absEndTime, scale, &session, NULL));
}
                                     ...
                                     Authenticator *authenticator)
{
	...
	sendDummyUDPPackets(subsession); // hack to improve NAT traversal
	return sendRequest(
	        new RequestRecord(++fCSeq, responseHandler, absStartTime, absEndTime, scale, NULL, &subsession));
}
                                             ...
                                             Authenticator *authenticator)
{
	...
	unsigned result = sendRequest(new RequestRecord(++fCSeq, "SET_PARAMETER", responseHandler, &session, NULL,
	                                                False, 0.0, 0.0, 0.0, paramString));
	delete[] paramString;
	return result;
}

unsigned RTSPClient::sendGetParameterCommand(MediaSession &session, responseHandler *responseHandler,
                                             char const *parameterName, Authenticator *authenticator)
{
	if (authenticator != NULL)
		fCurrentAuthenticator = *authenticator;
	...
	unsigned result = sendRequest(new RequestRecord(++fCSeq, "GET_PARAMETER", responseHandler, &session, NULL,
	                                                False, 0.0, 0.0, 0.0, paramString));
	delete[] paramString;
	return result;
}
...
Boolean RTSPClient::parseRTSPURL(UsageEnvironment &env, char const *url, char *&username, char *&password,
                                 NetAddress &address, portNumBits &portNum, char const **urlSuffix)
{
	do {
		// Parse the URL as
...
unsigned RTSPClient::responseBufferSize = 20000; // default value; you can
        // reassign this in your
        // application if you need to

RTSPClient::RTSPClient(UsageEnvironment &env, char const *rtspURL, int verbosityLevel, char const *applicationName,
        ...
        , fHTTPTunnelingConnectionIsPending(False)
{
	...
	unsigned userAgentNameSize = strlen(applicationName) + strlen(libPrefix) + strlen(libName) +
...
RTSPClient::~RTSPClient()
{
	RTPInterface::clearServerRequestAlternativeByteHandler(
	        envir(), fInputSocketNum); // in case we were receiving RTP-over-TCP
	reset();
...
unsigned RTSPClient::sendRequest(RequestRecord *request)
{
	...
	do {
		...
		if (!fRequestsAwaitingConnection.isEmpty()) {
			// A connection is currently pending (with at least one enqueued request).
			// Enqueue this request also:
		...
		// If requested (and we're not already doing it, or have done it), set up
		// the special protocol for tunneling RTSP-over-HTTP:
		...
		if (!setRequestFields(request, cmdURL, cmdURLWasAllocated, protocolStr, extraHeaders,
		...
		char const *const cmdFmt = "%s %s %s\r\n"
		                           ...
		                           "\r\n"
		                           "%s";
		unsigned cmdSize = strlen(cmdFmt) + strlen(request->commandName()) + strlen(cmdURL) +
		                   strlen(protocolStr) + 20 /* max int len */
		                   + strlen(authenticatorStr) + fUserAgentHeaderStrLen + strlen(extraHeaders) +
		                   strlen(contentLengthHeader) + contentStrLen;
		cmd = new char[cmdSize];
		sprintf(cmd, cmdFmt, request->commandName(), cmdURL, protocolStr, request->cseq(), authenticatorStr,
		        fUserAgentHeaderStr, extraHeaders, contentLengthHeader, contentStr);
		...
		if (fTunnelOverHTTPPortNum != 0 && strcmp(request->commandName(), "GET") != 0 &&
			...
			// When we're tunneling RTSP-over-HTTP, we Base-64-encode the request
			// before we send it.
			// (However, we don't do this for the HTTP "GET" and "POST" commands that
			// we use to set up the tunnel.)
			char *origCmd = cmd;
	...
	// An error occurred, so call the response handler immediately (indicating the
	// error):
	delete[] cmd;
...
static char *createRangeString(double start, double end, char const *absStartTime, char const *absEndTime)
{
	...
	if (absStartTime != NULL) {
		...
		if (absEndTime == NULL) {
			...
			// There's both a start and an end time; include them both in the "Range:"
			// hdr
...
{
	// Set various fields that will appear in our outgoing request, depending upon
	// the particular command that we are sending.
	...
	if (strcmp(request->commandName(), "DESCRIBE") == 0) {
		...
		// If we're currently part of a session, create a "Session:" header (in case
		// the server wants this to indicate
		...
		unsigned transportSize = strlen(transportFmt) + strlen(transportTypeStr) + strlen(modeStr) +
		...
		// When sending more than one "SETUP" request, include a "Session:" header
		// in the 2nd and later commands:
		char *sessionStr = createSessionString(fLastSessionId);

		// The "Transport:" and "Session:" (if present) headers make up the 'extra
		// headers':
		...
		// Begin by re-parsing our RTSP URL, to get the stream name (which we'll use
		// as our 'cmdURL'
		// in the subsequent request), and the server address (which we'll use in a
		// "Host:" header):
		char *username;
		char *password;
		...
		if (strcmp(request->commandName(), "GET") == 0) {
			...
			unsigned extraHeadersSize =
			        strlen(extraHeadersFmt) + strlen(serverAddressString.val()) + strlen(fSessionCookie);
		...
		if (strcmp(request->commandName(), "PLAY") == 0) {
			// Create "Session:", "Scale:", and "Range:" headers; these make up the
			// 'extra headers':
			char *sessionStr = createSessionString(sessionId);
			char *scaleStr = createScaleString(request->scale(), originalScale);
			char *rangeStr = createRangeString(request->start(), request->end(), request->absStartTime(),
			                                   request->absEndTime());
...
int RTSPClient::openConnection()
{
	do {
		...
		if (fInputSocketNum < 0)
			break;
		ignoreSigPipeOnSocket(fInputSocketNum); // so that servers on the same host
		        // that get killed don't also kill
		        // us
		...
		if (connectResult < 0)
			...
			// The connection succeeded.  Arrange to handle responses to requests sent
			// on it:
			envir().taskScheduler().setBackgroundHandling(
			        fInputSocketNum, SOCKET_READABLE | SOCKET_EXCEPTION,
			        (TaskScheduler::BackgroundHandlerProc *)&incomingDataHandler, this);
...
int RTSPClient::connectToServer(int socketNum, portNumBits remotePortNum)
{
	...
	if (connect(socketNum, (struct sockaddr *)&remoteName, sizeof remoteName) != 0) {
		...
		if (err == EINPROGRESS || err == EWOULDBLOCK) {
			// The connection is pending; we'll need to handle it later.  Wait for our
			// socket to be 'writable', or have an exception.
			envir().taskScheduler().setBackgroundHandling(
...
char *RTSPClient::createAuthenticatorString(char const *cmd, char const *url)
{
	...
	if (auth.realm() != NULL && auth.username() != NULL && auth.password() != NULL) {
		...
		if (auth.nonce() != NULL) { // Digest authentication
			...
			unsigned authBufSize = strlen(authFmt) + strlen(auth.username()) + strlen(auth.realm()) +
...
Boolean RTSPClient::parseResponseCode(char const *line, unsigned &responseCode, char const *&responseString)
{
	if (sscanf(line, "RTSP/%*s%u", &responseCode) != 1 && sscanf(line, "HTTP/%*s%u", &responseCode) != 1)
		return False;
	// Note: We check for HTTP responses as well as RTSP responses, both in order
	// to setup RTSP-over-HTTP tunneling,
	// and so that we get back a meaningful error if the client tried to
	// mistakenly send a RTSP command to a HTTP-only server.
...
void RTSPClient::handleIncomingRequest()
{
	...
	if (!parseRTSPRequestString(fResponseBuffer, fResponseBytesAlreadySeen, cmdName, sizeof cmdName, urlPreSuffix,
	                            sizeof urlPreSuffix, urlSuffix, sizeof urlSuffix, cseq, sizeof cseq, sessionId,
...
Boolean RTSPClient::parseTransportParams(char const *paramsStr, char *&serverAddressStr, portNumBits &serverPortNum,
                                         unsigned char &rtpChannelId, unsigned char &rtcpChannelId)
{
	...
	// If we're multicast, and have a "destination=" (multicast) address, then use
	// this
	// as the 'server' address (because some weird servers don't specify the
	// multicast
	...
	//   - We saw a "interleaved=" field, indicating RTP/RTCP-over-TCP streaming,
	//   or
...
Boolean RTSPClient::parseRTPInfoParams(char const *&paramsStr, u_int16_t &seqNum, u_int32_t &timestamp)
{
	...
	// "paramsStr" now consists of a ';'-separated list of parameters, ending with
	// ',' or '\0'.
...
Boolean RTSPClient::handleSETUPResponse(MediaSubsession &subsession, char const *sessionParamsStr,
                                        char const *transportParamsStr, Boolean streamUsingTCP)
{
...
Boolean RTSPClient::handlePLAYResponse(MediaSession &session, MediaSubsession &subsession, char const *scaleParamsStr,
	...
	do {
		if (&session != NULL) {
			...
			if (rangeParamsStr != NULL &&
			    !parseRangeParam(rangeParamsStr, session.playStartTime(), session.playEndTime(),
			                     session._absStartTime(), session._absEndTime(), startTimeIsNow))
				break;
			rangeOK = True;
			...
			Boolean startTimeIsNow;
			if (rangeParamsStr != NULL &&
			    !parseRangeParam(rangeParamsStr, subsession._playStartTime(), subsession._playEndTime(),
...
{
	do {
		// If "parameterName" is non-empty, it may be (possibly followed by ':' and
		// whitespace) at the start of the result string:
		...
		// The rest of "resultValueStr" should be our desired result, but first trim
...
Boolean RTSPClient::handleAuthenticationFailure(char const *paramsStr)
{
	...
	if (alreadyHadRealm || fCurrentAuthenticator.username() == NULL || fCurrentAuthenticator.password() == NULL) {
		...
		// so the new "WWW-Authenticate:" header information won't help us.  We
...
void RTSPClient::constructSubsessionURL(MediaSubsession const &subsession, char const *&prefix, char const *&separator,
                                        char const *&suffix)
{
...
Boolean RTSPClient::setupHTTPTunneling1()
{
	...
	//     http://developer.apple.com/quicktime/icefloe/dispatch028.html and
	//     http://images.apple.com/br/quicktime/pdf/QTSS_Modules.pdf
	if (fVerbosityLevel >= 1) {
		envir() << "Requesting RTSP-over-HTTP tunneling (on port " << fTunnelOverHTTPPortNum << ")\n\n";
	}

	// Begin by sending a HTTP "GET", to set up the server->client link.  Continue
	// when we handle the response:
...
void RTSPClient::responseHandlerForHTTP_GET1(int responseCode, char *responseString)
{
	...
	do {
		...
		// server->client link, set up a second TCP connection
		// (to the same server & port as before) for the client->server link.  All
		// future output will be to this new socket.
		fOutputSocketNum = setupStreamSocket(envir(), 0);
		if (fOutputSocketNum < 0)
			break;
		ignoreSigPipeOnSocket(fOutputSocketNum); // so that servers on the same host
		        // that killed don't also kill us
		...
		if (connectResult < 0)
			...
			// A connection is pending.  Continue setting up RTSP-over-HTTP when the
	...
	// An error occurred.  Dequeue the pending request(s), and tell them about the
	// error:
	fHTTPTunnelingConnectionIsPending = False;
...
Boolean RTSPClient::setupHTTPTunneling2()
{
	...
	// Send a HTTP "POST", to set up the client->server link.  (Note that we won't
	// see a reply to the "POST".)
...
void RTSPClient::connectionHandler1()
{
	...
	// Move all requests awaiting connection into a new, temporary queue, to clear
	// "fRequestsAwaitingConnection"
	// (so that "sendRequest()" doesn't get confused by
	// "fRequestsAwaitingConnection" being nonempty, and enqueue them all over
	// again).
...
void RTSPClient::handleResponseBytes(int newBytesRead)
{
	do {
		...
		if (newBytesRead >= (int)fResponseBufferBytesLeft) {
			// We filled up our response buffer.  Treat this as an error (for the
			// first response handler):
			envir().setResultMsg("RTSP response was truncated. Increase "
			                     "\"RTSPClient::responseBufferSize\"");
		}

		// An error occurred while reading our TCP socket.  Call all pending
		// response handlers, indicating this error.
		// (However, the "RTSP response was truncated" error is applied to the first
	...
	do {
		// Data was read OK.  Look through the data that we've read so far, to see
		// if it contains <CR><LF><CR><LF>.
		...
		// <CR><LF><CR><LF>), parse them to get the response code, CSeq,
		// and various other header parameters.  To do this, we first make a copy of
		// the received header data, because we'll be
		...
		do {
				...
				if (checkForHeader(lineStart, "CSeq:", 5, headerParamsStr)) {
					...
					while ((request = fRequestsAwaitingResponse.dequeue()) != NULL) {
						if (request->cseq() <
							...
							// We never received (and will never receive) a response for this
							// handler, so delete it:
							if (fVerbosityLevel >= 1 &&
							    strcmp(request->commandName(), "POST") != 0) {
								envir() << "WARNING: The server did not respond to our \""
								        << request->commandName()
								        << "\" request (CSeq: " << request->cseq()
								        << ").  The server appears to be buggy (perhaps not "
								           "handling pipelined requests properly).\n";
							}
							delete request;
						} else if (request->cseq() == cseq) {
							// This is the handler that we want. Remove its record, but
							// remember it, so that we can later call its handler:
					...
					// If we've already seen a "WWW-Authenticate:" header, then we replace
					// it with this new one only if
					...
					// Note: we accept "Allow:" instead of "Public:", so that "OPTIONS"
					// requests made to HTTP servers will work.
			...
			if (foundRequest == NULL) {
				// Hack: The response didn't have a "CSeq:" header; assume it's for our
				// most recent request:
			...
			if (contentLength > numBodyBytes) {
				// We need to read more data.  First, make sure we have enough space for
				// it:
				...
				if (numExtraBytesNeeded > remainingBufferSize) {
					...
					sprintf(tmpBuf,
					        ...
					        "\"Content-Length:\" %d (need a buffer size of >= %d "
					        "bytes\n",
			...
			// We now have a complete response (including all bytes specified by the
			...
			if (foundRequest != NULL) {
				...
				if (responseCode == 200) {
					...
					if (strcmp(foundRequest->commandName(), "SETUP") == 0) {
						...
						if (!handlePLAYResponse(*foundRequest->session(),
						                        *foundRequest->subsession(), scaleParamsStr,
					...
					if (strcmp(foundRequest->commandName(), "GET") == 0) {
						// Note: If a HTTP "GET" command (for RTSP-over-HTTP tunneling)
						// returns "401 Unauthorized", then we resend it
						// (with an "Authorization:" header), just as we would for a RTSP
						// command.  However, we do so using a new TCP connection,
						// because some servers close the original connection after
						// returning the "401 Unauthorized".
						resetTCPSockets(); // forces the opening of a new connection for the
						        // resent command
					}
				...
				if (needToResendCommand) {
					...
					if (!resendCommand(foundRequest))
						break;
					delete[] headerDataCopy;
					return; // without calling our response handler; the response to the
					        // resent command will do that
				}
			}
		...
		if (numExtraBytesAfterResponse > 0) {
			// An unusual case; usually due to having received pipelined responses.
			// Move the extra bytes to the front of the buffer:
			char *responseEnd = &fResponseBuffer[fResponseBytesAlreadySeen - numExtraBytesAfterResponse];

			// But first: A hack to save a copy of the response 'body', in case it's
			// needed below for "resultString":
		...
		if (foundRequest != NULL && foundRequest->handler() != NULL) {
			...
			if (responseSuccess) {
				if (responseCode == 200) {
					...
					// Note: The "strDup(bodyStart)" call assumes that the body is encoded
					// without interior '\0' bytes
				} else {
...
RTSPClient::RequestRecord::RequestRecord(unsigned cseq, responseHandler *handler, char const *absStartTime,
                                         char const *absEndTime, float scale, MediaSession *session,
                                         MediaSubsession *subsession)
        : fNext(NULL)
        , fCSeq(cseq)
        , fCommandName("PLAY")
...
HandlerServerForREGISTERCommand::createNew(UsageEnvironment &env, onRTSPClientCreationFunc *creationFunc, Port ourPort,
...
HandlerServerForREGISTERCommand::HandlerServerForREGISTERCommand(UsageEnvironment &env,
                                                                 onRTSPClientCreationFunc *creationFunc, int ourSocket,
        ...
        : RTSPServer(env, ourSocket, ourPort, authDatabase, 30 /*small reclamationTestSeconds*/,
                     10 /* Agtx now hasn't use this class. Just prevent warning */)
...
RTSPClient *HandlerServerForREGISTERCommand::createNewRTSPClient(char const *rtspURL, int verbosityLevel,
                                                                 char const *applicationName, int socketNumToServer)
{
