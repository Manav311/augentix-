        ...
        , fNumIndexRecords(0)
{
	...
	if (indexFileSize % INDEX_RECORD_SIZE != 0) {
		env << "Warning: Size of the index file \"" << indexFileName << "\" (" << (unsigned)indexFileSize
                                                             ...
                                                             unsigned long &indexRecordNumber)
{
	...
	do {
		...
		while (ixRight - ixLeft > 1 && pcrLeft < npt && npt <= pcrRight) {
			unsigned long ixNew =
			        ixLeft + (unsigned long)(((npt - pcrLeft) / (pcrRight - pcrLeft)) * (ixRight - ixLeft));
...
void MPEG2TransportStreamIndexFile::lookupPCRFromTSPacketNum(unsigned long &tsPacketNumber,
                                                             Boolean reverseToPreviousCleanPoint, float &pcr,
                                                             unsigned long &indexRecordNumber)
{
...
Boolean MPEG2TransportStreamIndexFile::rewindToCleanPoint(unsigned long &ixFound)
{
	...
	while (ixFound > 0) {
		...
		// cleanly resume
		// handling the stream.  For H.264, this is a SPS.  For H.265, this is a
		// VPS.
