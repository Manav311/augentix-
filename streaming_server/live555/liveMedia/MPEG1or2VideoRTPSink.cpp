                                                             ...
                                                             unsigned numBytesInFrame) const
{
	...
	return numBytesInFrame >= 4 && frameStart[0] == 0 && frameStart[1] == 0 && frameStart[2] == 1 &&
...
void MPEG1or2VideoRTPSink::doSpecialFrameHandling(unsigned fragmentationOffset, unsigned char *frameStart,
                                                  ...
                                                  unsigned numRemainingBytes)
{
	...
	if (fragmentationOffset == 0) {
		...
		unsigned startCode = (frameStart[0] << 24) | (frameStart[1] << 16) | (frameStart[2] << 8) |
		                     frameStart[3];
		...
		if (startCode == VIDEO_SEQUENCE_HEADER_START_CODE) {
			...
			if (numBytesInFrame < 8)
				return; // shouldn't happen
			unsigned next4Bytes = (frameStart[4] << 24) | (frameStart[5] << 16) | (frameStart[6] << 8) |
			                      frameStart[7];
	...
	unsigned videoSpecificHeader =
	        ...
	        // AN == N == 0
	        (fSequenceHeaderPresent << 13) | (fPacketBeginsSlice << 12) | (fPacketEndsSlice << 11) |
	        (fPictureState.picture_coding_type << 8) | fPictureState.vector_code_bits;
