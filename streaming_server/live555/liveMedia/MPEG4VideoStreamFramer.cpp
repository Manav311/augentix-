...
#ifdef DEBUG
		fprintf(stderr, "MPEG4VideoStreamParser::parse() EXCEPTION (This is normal "
...
unsigned MPEG4VideoStreamParser::parseVisualObject()
{
	...
	if (visual_object_type != 1) {
		usingSource()->envir() << "MPEG4VideoStreamParser::parseVisualObject(): "
		                          "Warning: We don't handle visual_object_type "
		                       << visual_object_type << "\n";
	}
...
void MPEG4VideoStreamParser::analyzeVOLHeader()
{
	...
	do {
		...
		if (marker_bit != 1) { // sanity check
			usingSource()->envir() << "MPEG4VideoStreamParser::analyzeVOLHeader(): "
			                          "marker_bit 1 not set!\n";
			break;
		}
...
unsigned MPEG4VideoStreamParser::parseVideoObjectLayer()
{
	...
	if (!isVideoObjectLayerStartCode(next4Bytes)) {
		usingSource()->envir() << "MPEG4VideoStreamParser::parseVideoObjectLayer():"
		                          " This appears to be a 'short video header', "
		                          "which we current don't support\n";
	}
	...
	setParseState((next4Bytes == GROUP_VOP_START_CODE) ? PARSING_GROUP_OF_VIDEO_OBJECT_PLANE :
	                                                     PARSING_VIDEO_OBJECT_PLANE);
...
unsigned MPEG4VideoStreamParser::parseGroupOfVideoObjectPlane()
{
	...
	fprintf(stderr, "time_code: 0x%05x, hours %d, minutes %d, marker_bit %d, seconds %d\n", time_code,
	        time_code_hours, time_code_minutes, marker_bit, time_code_seconds);
#endif
	...
	// Note: Because the GOV header can appear anywhere (not just at a 1s point),
	// we
	// don't pass "fTotalTicksSinceLastTimeCode" as the "picturesSinceLastGOP"
	// parameter.
	fSecondsSinceLastTimeCode = 0;
	if (fixed_vop_rate)
		fTotalTicksSinceLastTimeCode = 0;
...
unsigned MPEG4VideoStreamParser::parseVideoObjectPlane()
{
	...
	if ((timeInfo & mask) == 0) {
		usingSource()->envir() << "MPEG4VideoStreamParser::parseVideoObjectPlane():"
		                          " marker bit not set!\n";
	}
	mask >>= 1;
	...
	if (fixed_vop_time_increment > 0) {
		...
		if (vop_time_increment > 0 || modulo_time_base > 0) {
			...
			// Note: "fSecondsSinceLastTimeCode" and "fPrevNewTotalTicks" are not
			// used.
		}
	} else {
...
#ifdef DEBUG
			fprintf(stderr, "Buggy MPEG-4 video stream: \"vop_time_increment\" did "
			                "not change!\n");
#endif
			// The following assumes that we don't have 'B' frames.  If we do, then
			// TARFU!
...
#ifdef DEBUG
				fprintf(stderr, "Buggy MPEG-4 video stream: \"vop_time_increment\" "
