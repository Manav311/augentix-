...
{
	if (&rightSide != this) {
		if (publicKey() == NULL || strcmp(publicKey(), rightSide.publicKey()) != 0) {
			clean();
...
GroupEId::GroupEId(struct in_addr const &groupAddr, portNumBits portNum, Scope const &scope,
...
void GroupEId::init(struct in_addr const &groupAddr, struct in_addr const &sourceFilterAddr, portNumBits portNum,
                    Scope const &scope, unsigned numSuccessiveGroupAddrs)
{
