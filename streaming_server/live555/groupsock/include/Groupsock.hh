...
class Groupsock : public OutputSocket {
	...
	Groupsock(UsageEnvironment &env, struct in_addr const &groupAddr, struct in_addr const &sourceFilterAddr,
	          Port port);
	...
	Boolean output(UsageEnvironment &env, u_int8_t ttl, unsigned char *buffer, unsigned bufferSize,
    ...
    private:
	int outputToAllMembersExcept(DirectedNetInterface *exceptInterface, u_int8_t ttlToFwd, unsigned char *data,
...
class GroupsockLookupTable {
	...
	Groupsock *Fetch(UsageEnvironment &env, netAddressBits groupAddress, netAddressBits sourceFilterAddr, Port port,
	                 Boolean &isNew);
	// Creates a new Groupsock if none already exists
    ...
    private:
	Groupsock *AddNew(UsageEnvironment &env, netAddressBits groupAddress, netAddressBits sourceFilterAddress,
