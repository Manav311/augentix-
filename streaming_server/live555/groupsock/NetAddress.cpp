...
// "hostname" is not an IP address string; try resolving it as a real host name
// instead:
	...
	while (p != NULL) {
		if (p->ai_addrlen < 4)
			continue;
		fAddressArray[i++] =
		        new NetAddress((u_int8_t const *)&(((struct sockaddr_in *)p->ai_addr)->sin_addr.s_addr), 4);
		p = p->ai_next;
	}
...
void AddressString::init(netAddressBits addr)
{
	...
	sprintf(fVal, "%u.%u.%u.%u", (addrNBO >> 24) & 0xFF, (addrNBO >> 16) & 0xFF, (addrNBO >> 8) & 0xFF,
	        addrNBO & 0xFF);
}
