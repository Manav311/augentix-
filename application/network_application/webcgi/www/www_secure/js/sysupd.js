app.controller('HC_Ctrl', [
  ...
  function ($scope, $interval, $location, $http, fileUpload) {
    ...
    $scope.msg_t = {
      ...
      UpgradeFailed_msg: [
        'Upgrade failed. Rebooting. Please manually connect again after rebooting!',
        '更新失敗。重新開機中，請在完成開機後手動重新連接。',
        '更新失败.重新开机中,请在完成开机后手动重新连接.',
      ],
    };
      ...
      $http({
        ...
        function (resp) {
          ...
          if (resp.data.rval == 0) {
            ...
            $scope.cmdStatus = $scope.msg_t.UpgradeFailed_msg[$scope.lang];
          }
          console.log(resp);
        },
        function (resp) {
