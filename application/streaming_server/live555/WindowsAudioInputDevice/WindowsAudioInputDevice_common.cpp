...
WindowsAudioInputDevice_common::WindowsAudioInputDevice_common(UsageEnvironment &env, int inputPortNumber,
...
void WindowsAudioInputDevice_common::audioReadyPoller1()
{
	if (readHead != NULL) {
		...
		nextTask() = envir().taskScheduler().scheduleDelayedTask(uSecondsToDelay, (TaskFunc *)audioReadyPoller,
		                                                         this);
	}
}
...
static void CALLBACK waveInCallback(HWAVEIN /*hwi*/, UINT uMsg, DWORD /*dwInstance*/, DWORD dwParam1,
                                    DWORD /*dwParam2*/)
{
	switch (uMsg) {
	case WIM_DATA:
...
Boolean WindowsAudioInputDevice_common::openWavInPort(int index, unsigned numChannels, unsigned samplingFrequency,
                                                      unsigned granularityInMS)
{
	...
	// Use a 10-second input buffer, to allow for CPU competition from video,
	// etc.,
	...
	// Set this process's priority high. I'm not sure how much this is really
	// needed,
...
unsigned WindowsAudioInputDevice_common::readFromBuffers(unsigned char *to, unsigned numBytesWanted,
                                                         struct timeval &creationTime)
{
	// Begin by computing the creation time of (the first bytes of) this returned
	// audio data:
	if (readHead != NULL) {
		int hdrIndex = readHead - readHdrs;
		creationTime = readTimes[hdrIndex];

		// Adjust this time to allow for any data that's already been read from this
		// buffer:
