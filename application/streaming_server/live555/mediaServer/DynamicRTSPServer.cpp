...
ServerMediaSession *DynamicRTSPServer::lookupServerMediaSession(char const *streamName)
{
	...
	if (!fileExists) {
		...
		if (!smsExists) {
			// Create a new "ServerMediaSession" object for streaming from the named
			// file.
...
static ServerMediaSession *createNewSMS(UsageEnvironment &env, char const *fileName, FILE * /*fid*/)
{
	...
	if (strcmp(extension, ".aac") == 0) {
		...
		// To stream using 'ADUs' rather than raw MP3 frames, uncomment the
		// following:
...
#endif
#endif
		sms->addSubsession(MP3AudioFileServerMediaSubsession::createNew(env, fileName, reuseSource, useADUs,
		                                                                interleaving));
		...
		// Use an index file name that's the same as the TS file name, except with
		// ".tsx":
		...
		sms->addSubsession(
		        MPEG2TransportFileServerMediaSubsession::createNew(env, fileName, indexFileName, reuseSource));
		...
		sms->addSubsession(
		        WAVAudioFileServerMediaSubsession::createNew(env, fileName, reuseSource, convertToULaw));
	} else if (strcmp(extension, ".dv") == 0) {
		// Assumed to be a DV Video file
		// First, make sure that the RTPSinks' buffers will be large enough to
		// handle the huge size of DV frames (as big as 288000).
		OutPacketBuffer::maxSize = 600000;

		NEW_SMS("DV Video");
		sms->addSubsession(DVVideoFileServerMediaSubsession::createNew(env, fileName, reuseSource));
	} else if (strcmp(extension, ".mkv") == 0 || strcmp(extension, ".webm") == 0) {
		// Assumed to be a Matroska file (note that WebM ('.webm') files are also
		// Matroska files)
	...
	} else if (strcmp(extension, ".ogg") == 0 || strcmp(extension, ".ogv") == 0 ||
