...
// Implementation
//
// NOTE: If you want to develop your own RTSP client application (or embed RTSP
// client functionality into your own application),
// then we don't recommend using this code as a model, because it is too complex
// (with many options).
...
int main(int argc, char **argv)
{
	...
	while (argc > 1) {
		...
		case 'P': { // specify an interval (in seconds) between writing successive
			// output files
		...
		case 'k': { // specify a username and password to be used to authentication
			// an incoming "REGISTER" command (for use with -R)
			if (argc < 4)
				usage(); // there's no argv[3] (for the "password")
		...
		case 'h': { // specify a height (pixels) for an output QuickTime or AVI
			// movie
		...
		case 'f': { // specify a frame rate (per second) for an output QT or AVI
			// movie
		...
		case 'U': {
			// specify initial absolute seek time (trick play), using a string of the
			// form "YYYYMMDDTHHMMSSZ" or "YYYYMMDDTHHMMSS.<frac>Z"
	...
	// There must be exactly one "rtsp://" URL at the end (unless '-R' was used,
	// in which case there's no URL)
	...
	if (outputCompositeFile && !movieWidthOptionSet) {
		*env << "Warning: The -q, -4 or -i option was used, but not -w.  Assuming "
		        "a video width of "
		     << movieWidth << " pixels\n";
	}
	if (outputCompositeFile && !movieHeightOptionSet) {
		*env << "Warning: The -q, -4 or -i option was used, but not -h.  Assuming "
		        "a video height of "
		     << movieHeight << " pixels\n";
	}
	if (outputCompositeFile && !movieFPSOptionSet) {
		*env << "Warning: The -q, -4 or -i option was used, but not -f.  Assuming "
		        "a video frame rate of "
	...
	if (authDBForREGISTER != NULL && !createHandlerServerForREGISTERCommand) {
		*env << "If \"-k <username> <password>\" is used, then -R (or \"-R "
		        "<port-num>\") must also be used!\n";
		usage();
	}
	...
	if (!createReceivers && notifyOnPacketArrival) {
		*env << "Warning: Because we're not receiving stream data, the -n flag has "
		        "no effect\n";
	}
	if (durationSlop < 0) {
	...
	if (createHandlerServerForREGISTERCommand) {
		handlerServerForREGISTERCommand = HandlerServerForREGISTERCommand::createNew(
		        *env, continueAfterClientCreation0, handlerServerForREGISTERCommandPortNum, authDBForREGISTER,
		        verbosityLevel, progName);
		if (handlerServerForREGISTERCommand == NULL) {
			*env << "Failed to create a server for handling incoming \"REGISTER\" "
			        "commands: "
			     << env->getResultMsg() << "\n";
		} else {
...
void continueAfterClientCreation0(RTSPClient *newRTSPClient, Boolean requestStreamingOverTCP)
{
	...
	// Having handled one "REGISTER" command (giving us a "rtsp://" URL to stream
	// from), we don't handle any more:
			...
			if (!subsession->initiate(simpleRTPoffsetArg)) {
				...
				*env << "Created receiver for \"" << subsession->mediumName() << "/"
				...
				if (subsession->rtpSource() != NULL) {
					...
					// Set the RTP source's OS socket buffer size as appropriate - either
					// if we were explicitly asked (using -B),
					// or if the desired FileSink buffer size happens to be larger than
					// the current OS socket buffer size.
					// (The latter case is a heuristic, on the assumption that if the user
					// asked for a large FileSink buffer size,
					// then the input data rate may be large enough to justify increasing
					// the OS socket buffer size also.)
					...
					if (socketInputBufferSize > 0 || fileSinkBufferSize > curBufferSize) {
						...
						if (socketInputBufferSize > 0) { // The user explicitly asked for
							// the new socket buffer size;
							// announce it:
...
void continueAfterSETUP(RTSPClient *, int resultCode, char *resultString)
{
	if (resultCode == 0) {
		...
		*env << "Failed to setup \"" << subsession->mediumName() << "/" << subsession->codecName()
...
void createOutputFiles(char const *periodicFilenameSuffix)
{
	...
	if (outputQuickTimeFile || outputAVIFile) {
		if (periodicFilenameSuffix[0] == '\0') {
			// Normally (unless the '-P <interval-in-seconds>' option was given) we
			// output to 'stdout':
			sprintf(outFileName, "stdout");
		} else {
			// Otherwise output to a type-specific file name, containing
		...
		if (outputQuickTimeFile) {
			...
			if (qtOut == NULL) {
				*env << "Failed to create a \"QuickTimeFileSink\" for outputting to \"" << outFileName
				     << "\": " << env->getResultMsg() << "\n";
				shutdown();
			} else {
			...
			aviOut = AVIFileSink::createNew(*env, *session, outFileName, fileSinkBufferSize, movieWidth,
			                                movieHeight, movieFPS, packetLossCompensate);
			if (aviOut == NULL) {
				*env << "Failed to create an \"AVIFileSink\" for outputting to \"" << outFileName
				     << "\": " << env->getResultMsg() << "\n";
				shutdown();
			} else {
				*env << "Outputting to the file: \"" << outFileName << "\"\n";
		...
		while ((subsession = iter.next()) != NULL) {
			...
			if (singleMedium == NULL || periodicFilenameSuffix[0] != '\0') {
				...
				snprintf(outFileName, sizeof outFileName, "%s%s-%s-%d%s", fileNamePrefix,
				         subsession->mediumName(), subsession->codecName(), ++streamCounter,
				         periodicFilenameSuffix);
			} else {
			...
			if (strcmp(subsession->mediumName(), "video") == 0) {
				if (strcmp(subsession->codecName(), "H264") == 0) {
					...
					// For H.265 video stream, we use a special sink that adds 'start
					// codes',
				...
				if (strcmp(subsession->codecName(), "AMR") == 0 ||
					...
					// For AMR audio streams, we use a special sink that inserts AMR frame
					// hdrs:
					fileSink = AMRAudioFileSink::createNew(*env, outFileName, fileSinkBufferSize,
					                                       oneFilePerFrame);
				...
				*env << "Failed to create FileSink for \"" << outFileName
				...
				if (singleMedium == NULL) {
					...
					*env << "Outputting data from the \"" << subsession->mediumName() << "/"
					     << subsession->codecName() << "\" subsession to \"" << outFileName
					     << "\"\n";
				}
...
void createPeriodicOutputFiles()
{
	// Create a filename suffix that notes the time interval that's being
	// recorded:
	char periodicFileNameSuffix[100];
	snprintf(periodicFileNameSuffix, sizeof periodicFileNameSuffix, "-%05d-%05d", fileOutputSecondsSoFar,
...
void continueAfterPLAY(RTSPClient *, int resultCode, char *resultString)
{
	...
	if (duration > 0) {
		// First, adjust "duration" based on any change to the play range (that was
		// specified in the "PLAY" response):
		double rangeAdjustment =
		        (session->playEndTime() - session->playStartTime()) - (endTime - initialSeekTime);
...
#ifdef USE_SIGNALS
		pid_t ourPid = getpid();
		*env << actionString << " (signal with \"kill -HUP " << (int)ourPid << "\" or \"kill -USR1 "
		     << (int)ourPid << "\" to terminate)...\n";
#else
		*env << actionString << "...\n";
#endif
	}
...
void subsessionByeHandler(void *clientData)
{
	...
	MediaSubsession *subsession = (MediaSubsession *)clientData;
	*env << "Received RTCP \"BYE\" on \"" << subsession->mediumName() << "/" << subsession->codecName()
	     << "\" subsession (after " << secsDiff << " seconds)\n";
...
void shutdown(int exitCode)
{
	if (areAlreadyShuttingDown)
		return; // in case we're called after receiving a RTCP "BYE" while in the
...
void signalHandlerShutdown(int /*sig*/)
{
	*env << "Got shutdown signal\n";
	waitForResponseToTEARDOWN = False; // to ensure that we end, even if the
	        // server does not respond to our TEARDOWN
	shutdown(0);
}
...
void checkForPacketArrival(void * /*clientData*/)
{
	...
	if (notifyTheUser) {
		...
		*env << (syncStreams ? "Synchronized d" : "D") << "ata packets have begun arriving [" << timestampStr
		     << "]\007\n";
		return;
	}
	...
	arrivalCheckTimerTask =
	        env->taskScheduler().scheduleDelayedTask(uSecsToDelay, (TaskFunc *)checkForPacketArrival, NULL);
}
