...
void BasicTaskScheduler0::deleteEventTrigger(EventTriggerId eventTriggerId)
{
	...
	if (eventTriggerId == fLastUsedTriggerMask) { // common-case optimization:
		...
		// However, we do the reasonable thing if the user happened to 'or' together
		// two or more "EventTriggerId"s:
...
{
	// First, record the "clientData".  (Note that we allow "eventTriggerId" to be
	// a combination of bits for multiple events.)
	EventTriggerId mask = 0x80000000;
	for (unsigned i = 0; i < MAX_NUM_EVENT_TRIGGERS; ++i) {
		if ((eventTriggerId & mask) != 0) {
...
void HandlerSet::assignHandler(int socketNum, int conditionSet, TaskScheduler::BackgroundHandlerProc *handlerProc,
                               void *clientData)
{
