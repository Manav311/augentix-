               ...
               struct sockaddr_in &fromAddress)
{
	...
	if (bytesRead < 0) {
		...
		// "recvfrom()" on a stream socket can return 0 if the remote end has closed
		// the connection.  Treat this as an error:
		return -1;
	}
...
Boolean writeSocket(UsageEnvironment &env, int socket, struct in_addr address, Port port, u_int8_t ttlArg,
...
Boolean writeSocket(UsageEnvironment &env, int socket, struct in_addr address, Port port, unsigned char *buffer,
                    unsigned bufferSize)
{
	do {
...
static unsigned increaseBufferTo(UsageEnvironment &env, int bufOptName, int socket, unsigned requestedSize)
{
	...
	printf("[%s::%d]tcp buffer size: %d\n", __func__, __LINE__, getBufferSize(env, bufOptName, socket));
	return getBufferSize(env, bufOptName, socket);
}
...
static void clearMulticastAllSocketOption(int socket)
{
	...
	// Linux kernel's default behavior.
...
// Ignore the call's result.  Should it fail, we'll still receive packets (just
...
Boolean socketJoinGroupSSM(UsageEnvironment &env, int socket, netAddressBits groupAddress,
...
Boolean socketLeaveGroupSSM(UsageEnvironment & /*env*/, int socket, netAddressBits groupAddress,
                            netAddressBits sourceFilterAddr)
{
	if (!IsMulticastAddress(groupAddress))
		return True; // ignore this case
...
netAddressBits ourIPAddress(UsageEnvironment &env)
{
	...
	if (ourAddress == 0) {
		...
		do {
			...
			if (!writeSocket(env, sock, testAddr, testPort, 0, testString, testStringLength))
				break;
...
char const *timestampString()
{
	...
	gettimeofdayMonotonic(&tvNow, NULL);
	...
	if (!isInitialized) {
      ...
      */
			...
			epochOffset.QuadPart = tp->tv_sec * tickFrequency.QuadPart +
			                       (tp->tv_usec * tickFrequency.QuadPart) / 1000000L - tickNow.QuadPart;
