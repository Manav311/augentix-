 ...
 * inet_addr() as taking no arguments.
 * This confuses C++.  To overcome this, we use our own routine, implemented in
 * C.
 */
...
static long randtbl[DEG_3 + 1] = {
	TYPE_3,     0x9a319039, 0x32d9c024, 0x9b663182, 0x5da1f342, 0xde3b81e0, 0xdf0a6fb5, 0xf103bc02,
...
long our_random()
{
	...
	if (rand_type == TYPE_0) {
		...
		/* Make copies of "rptr" and "fptr" before working with them, in case we're
     * being called concurrently by multiple threads: */
		long *rp = rptr;
		long *fp = fptr;
     ...
     * allowing for concurrent access): */
		if (!(fp == rp + SEP_3 || fp + DEG_3 == rp + SEP_3)) {
			/* A rare case that should occur only if we're being called concurrently
       * by multiple threads. */
...
u_int32_t our_random32()
{
	/* Return a 32-bit random number.
     ...
     (Actually, to increase the likelhood of randomness, we take the middle 16
     bits of two successive calls to "our_random()")
  */
