...
ServerMediaSession *ServerMediaSession::createNew(UsageEnvironment &env, char const *streamName, char const *info,
...
ServerMediaSession::ServerMediaSession(UsageEnvironment &env, char const *streamName, char const *info,
                                       char const *description, Boolean isSSM, char const *miscSDPLines)
        : Medium(env)
        , fIsSSM(isSSM)
        ...
        , fDeleteWhenUnreferenced(False)
{
	...
	gettimeofdayMonotonic(&fCreationTime, NULL);
}
...
void ServerMediaSession::testScaleFactor(float &scale)
{
	...
	// value.  If the subsessions' actual scales still differ, re-set them all to
	// 1.
...
float ServerMediaSession::duration() const
{
	...
	for (ServerMediaSubsession *subsession = fSubsessionsHead; subsession != NULL; subsession = subsession->fNext) {
		// Hack: If any subsession supports seeking by 'absolute' time, then return
		// a negative value, to indicate that only subsessions
...
char *ServerMediaSession::generateSDPDescription()
{
	...
	printf("%s: IP address length = %d\n", __func__, ipAddressStrSize);

	// For a SSM sessions, we need a "a=source-filter: incl ..." line also:
	char *sourceFilterLine;
	if (fIsSSM) {
		printf("%s: Is SSM session\n", __func__);
	...
	do {
		...
		for (subsession = fSubsessionsHead; subsession != NULL; subsession = subsession->fNext) {
			...
			if (sdpLines == NULL) {
				fprintf(stderr, "failed to generate SDP line, %s\n", streamName());
				/*if one of Audio or Video subsession failed to create, stream can't start*/
				sdpLength = 0;
				return NULL;
			}
			sdpLength += strlen(sdpLines);
		}
		printf("%s: Total SDP length = %d\n", __func__, sdpLength);
		if (sdpLength == 0)
			break; // the session has no usable subsessions

		// Unless subsessions have differing durations, we also have a "a=range:"
		// line:
		float dur = duration();
		if (dur == 0.0) {
		...
		} else { // subsessions have differing durations, so "a=range:" lines go
			// there
			rangeLine = strDup("");
		}
		...
		char const *const sdpPrefixFmt = "v=0\r\n"
		                                 ...
		                                 "a=x-qt-text-inf:%s\r\n"
		                                 "%s";
		sdpLength += strlen(sdpPrefixFmt) + 20 + 6 + 20 + ipAddressStrSize + strlen(fDescriptionSDPString) +
		             strlen(fInfoSDPString) + strlen(libNameStr) + strlen(libVersionStr) +
		             strlen(sourceFilterLine) + strlen(rangeLine) + strlen(fDescriptionSDPString) +
		             strlen(fInfoSDPString) + strlen(fMiscSDPLines);
		sdpLength += 1000; // in case the length of the "subsession->sdpLines()"
		        // calls below change
		sdp = new char[sdpLength];
		if (sdp == NULL)
			break;

		printf("%s: Printing SDP prefix\n", __func__);
...
void ServerMediaSubsession::seekStream(unsigned /*clientSessionId*/, void * /*streamToken*/, double & /*seekNPT*/,
                                       double /*streamDuration*/, u_int64_t &numBytes)
{
	// default implementation: do nothing
	numBytes = 0;
}
void ServerMediaSubsession::seekStream(unsigned /*clientSessionId*/, void * /*streamToken*/, char *&absStart,
                                       char *&absEnd)
{
...
void ServerMediaSubsession::nullSeekStream(unsigned /*clientSessionId*/, void * /*streamToken*/, double streamEndTime,
                                           u_int64_t &numBytes)
{
	// default implementation: do nothing
	numBytes = 0;
...
void ServerMediaSubsession::setServerAddressAndPortForSDP(netAddressBits addressBits, portNumBits portBits)
{
...
char const *ServerMediaSubsession::rangeSDPLine() const
{
	...
	// (as indicated by "fParentSession->duration() >= 0"), there's no "a=range:"
	// line:
