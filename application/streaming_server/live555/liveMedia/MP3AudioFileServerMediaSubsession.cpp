...
MP3AudioFileServerMediaSubsession::createNew(UsageEnvironment &env, char const *fileName, Boolean reuseFirstSource,
...
MP3AudioFileServerMediaSubsession::MP3AudioFileServerMediaSubsession(UsageEnvironment &env, char const *fileName,
                                                                     Boolean reuseFirstSource, Boolean generateADUs,
...
FramedSource *MP3AudioFileServerMediaSubsession::createNewStreamSourceCommon(FramedSource *baseMP3Source,
                                                                             unsigned mp3NumBytes, unsigned &estBitrate)
{
	...
	do {
		...
		if (fGenerateADUs) {
			...
			// ADUs back to MP3.  This allows us to seek within the input stream
			// without
...
void MP3AudioFileServerMediaSubsession::getBaseStreams(FramedSource *frontStream, FramedSource *&sourceMP3Stream,
                                                       ADUFromMP3Source *&aduStream /*if any*/)
{
	if (fGenerateADUs) {
		// There's an ADU stream.
		if (fInterleaving != NULL) {
			// There's an interleaving filter in front of the ADU stream.  So go back
			// one, to reach the ADU stream:
		...
		// There's no filter in front of the source MP3 stream (and there's no ADU
		// stream):
		aduStream = NULL;
...
void MP3AudioFileServerMediaSubsession::seekStreamSource(FramedSource *inputSource, double &seekNPT,
