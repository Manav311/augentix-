...
DVVideoRTPSource *DVVideoRTPSource::createNew(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
                                              unsigned rtpTimestampFrequency)
{
	return new DVVideoRTPSource(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency);
}

DVVideoRTPSource::DVVideoRTPSource(UsageEnvironment &env, Groupsock *rtpGS, unsigned char rtpPayloadFormat,
                                   unsigned rtpTimestampFrequency)
...
Boolean DVVideoRTPSource::processSpecialHeader(BufferedPacket *packet, unsigned &resultSpecialHeaderSize)
{
	...
	fCurrentPacketBeginsFrame = data[0] == DV_SECTION_HEADER && (data[1] & 0xf8) == 0 &&
