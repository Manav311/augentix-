...
MPEG4VideoStreamDiscreteFramer::MPEG4VideoStreamDiscreteFramer(UsageEnvironment &env, FramedSource *inputSource,
                                                               Boolean leavePresentationTimesUnmodified)
...
void MPEG4VideoStreamDiscreteFramer::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
	...
	if (frameSize >= 4 && fTo[0] == 0 && fTo[1] == 0 && fTo[2] == 1) {
		...
		if (fTo[i] == 0xB0) { // VISUAL_OBJECT_SEQUENCE_START_CODE
			...
			for (i = 7; i < frameSize; ++i) {
				if ((fTo[i] == 0xB3 /*GROUP_VOP_START_CODE*/ || fTo[i] == 0xB6 /*VOP_START_CODE*/) &&
		...
		if (i < frameSize) {
			...
			if (nextCode == 0xB6 /*VOP_START_CODE*/ && i + 5 < frameSize) {
				...
				u_int32_t next4Bytes = (fTo[i] << 24) | (fTo[i + 1] << 16) | (fTo[i + 2] << 8) |
				                       fTo[i + 3];
				i += 4;
				...
				if (!fLeavePresentationTimesUnmodified && vop_coding_type == 2 /*B*/
					...
					double usIncrement = vop_time_increment_resolution == 0 ?
					                             0.0 :
...
void MPEG4VideoStreamDiscreteFramer::analyzeVOLHeader()
{
	...
	for (i = 3; i < fNumConfigBytes; ++i) {
		if (fConfigBytes[i] >= 0x20 && fConfigBytes[i] <= 0x2F && fConfigBytes[i - 1] == 1 &&
