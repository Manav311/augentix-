...
class CuePoint {
	...
	static void addCuePoint(CuePoint *&root, double cueTime, u_int64_t clusterOffsetInFile,
	                        unsigned blockNumWithinCluster /* 1-based */, Boolean &needToReviseBalanceOfParent);
	// If "cueTime" == "root.fCueTime", replace the existing data, otherwise add
	// to the left or right subtree.
	// (Note that this is a static member function because - as a result of tree
	// rotation - "root" might change.)

	Boolean lookup(double &cueTime, u_int64_t &resultClusterOffsetInFile, unsigned &resultBlockNumWithinCluster);

	static void fprintf(FILE *fid, CuePoint *cuePoint); // used for debugging;
	        // it's static to allow
	        // for "cuePoint == NULL"

    private:
	// The "CuePoint" tree is implemented as an AVL Tree, to keep it balanced (for
	// efficient lookup).
...
void MatroskaFile::createNew(UsageEnvironment &env, char const *fileName, onCreationFunc *onCreation,
                             void *onCreationClientData, char const *preferredLanguage)
{
        ...
        , fChosenSubtitleTrackNumber(0)
{
	...
	if (inputSource == NULL) {
		...
		handleEndOfTrackHeaderParsing(); // we have no file, and thus no tracks, but
		        // we still need to signal this
	} else {
...
void MatroskaFile::handleEndOfTrackHeaderParsing()
{
	...
	// figure out which ones should be played.
	// The Matroska 'specification' is rather imprecise about this (as usual).
	...
	//     - If more than one is 'default', choose the first one that matches our
	//     preferred language, or the first if none matches.
	//     - If none is 'default', choose the first one that matches our preferred
	//     language, or the first if none matches.
	...
	if (numTracks > 0) {
		...
		while ((track = iter.next()) != NULL) {
			...
			// Assign flags for this track so that, when sorted, the largest value
			// becomes our choice:
...
{
	if (fCuePoints == NULL)
		return 0.0; // Hack, because the RTSP server code assumes that duration > 0
		        // => seekable. (fix this) #####

	return segmentDuration() * (timecodeScale() / 1000000000.0f);
}

FramedSource *MatroskaFile::createSourceForStreaming(FramedSource *baseSource, unsigned trackNumber,
                                                     unsigned &estBitrate, unsigned &numFiltersInFrontOfTrack)
{
	...
	if (track != NULL) { // should always be true
		if (strcmp(track->mimeType, "audio/MPEG") == 0) {
			...
			estBitrate = 500;
			// Allow for the possibility of very large NAL units being fed to the sink
			// object:
                                                   ...
                                                   unsigned char rtpPayloadTypeIfDynamic)
{
	...
	do {
		...
		if (strcmp(track->mimeType, "audio/MPEG") == 0) {
			...
			// configuration
			// information.  Use this to generate a hexadecimal 'config' string for
			// the new RTP sink:
			...
			result = MPEG4GenericRTPSink::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic,
			                                        track->samplingFrequency, "audio", "AAC-hbr", configStr,
			                                        track->numChannels);
			delete[] configStr;
		} else if (strcmp(track->mimeType, "audio/AC3") == 0) {
			result = AC3AudioRTPSink::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic,
			...
			// configuration
			// information, containing the "Identification", "Comment", and "Setup"
			// headers.
			...
			do {
				...
				for (i = 0; i < numHeaders; ++i) {
					...
					if (headerType == 1 ||
						...
						delete[] newHeader; // because it was a header type that we don't
						        // understand
					}
				}
				if (!success)
				...
				if (isTheora) {
					result = TheoraVideoRTPSink::createNew(
					        envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, identificationHeader,
					        identificationHeaderSize, commentHeader, commentHeaderSize, setupHeader,
					        setupHeaderSize);
				} else { // Vorbis
					result = VorbisAudioRTPSink::createNew(
					        envir(), rtpGroupsock, rtpPayloadTypeIfDynamic,
					        track->samplingFrequency, track->numChannels, identificationHeader,
					        identificationHeaderSize, commentHeader, commentHeaderSize, setupHeader,
					        setupHeaderSize);
				}
			} while (0);
			...
			do {
				...
				for (i = 0; i < numSPSs; ++i) {
					...
					if (SPS == NULL && nal_unit_type == 7 /*sanity check*/) { // save the first one
						...
						// Save the first 3 bytes of the SPS (after the nal_unit_header) as
						// 'profile_level_id':
						if (SPSSize >= 1 /*'profile_level_id' offset within SPS*/ +
						                       3 /*num bytes needed*/) {
			...
			result = H264VideoRTPSink::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, SPS,
			                                     SPSSize, PPS, PPSSize, profile_level_id);
			...
			for (i = 0; i < 12; ++i)
				profileTierLevelHeaderBytes[i] = 0;
			...
			do {
				...
				if (track->codecPrivateUsesH264FormatForH265) {
					...
					for (unsigned j = 0; j < numOfArrays; ++j) {
						++ptr;
						CHECK_PTR; // skip the
						        // 'array_completeness'|'reserved'|'NAL_unit_type' byte
				...
				if (VPSSize > 0) {
					...
					if (vpsSize >=
					    6 /*'profile_tier_level' offset*/ + 12 /*num 'profile_tier_level' bytes*/) {
				...
				if (SPSSize > 0) {
					...
					if (spsSize >=
					    3 /*'profile_tier_level' offset*/ + 12 /*num 'profile_tier_level' bytes*/) {
			...
			// Parse the 12 'profile tier level' bytes to get parameters needed for
			// "H265VideoRTPSink":
			...
			sprintf(interopConstraintsStr, "%02X%02X%02X%02X%02X%02X", interop_constraints[0],
			        interop_constraints[1], interop_constraints[2], interop_constraints[3],
			        interop_constraints[4], interop_constraints[5]);

			result = H265VideoRTPSink::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, VPS,
			                                     VPSSize, SPS, SPSSize, PPS, PPSSize, profileSpace,
			                                     profileId, tierFlag, levelId, interopConstraintsStr);
...
MatroskaDemux::~MatroskaDemux()
{
	...
	// - but not the "MatroskaDemuxedTrack"s themselves; that should have already
	// happened:
...
void MatroskaDemux::handleEndOfFile()
{
	...
	// we don't call the handlers until after we've
...
void CuePoint::addCuePoint(CuePoint *&root, double cueTime, u_int64_t clusterOffsetInFile,
                           unsigned blockNumWithinCluster, Boolean &needToReviseBalanceOfParent)
{
	...
	if (root == NULL) {
		...
		if (needToReviseOurBalance) {
			// We need to change our 'balance' number, perhaps while also performing a
			// rotation to bring ourself back into balance:
			if (root->fBalance == 0) {
				// We were balanced before, but now we're unbalanced (by 1) on the
				// "direction" side:
				root->fBalance = -1 + 2 * direction; // -1 for "direction" 0; 1 for "direction" 1
				needToReviseBalanceOfParent = True;
			} else if (root->fBalance == 1 - 2 * direction) { // 1 for "direction" 0;
				// -1 for "direction" 1
				// We were unbalanced (by 1) on the side opposite to where we added an
				// entry, so now we're balanced:
				root->fBalance = 0;
			} else {
				// We were unbalanced (by 1) on the side where we added an entry, so now
				// we're unbalanced by 2, and have to rebalance:
				if (root->fSubTree[direction]->fBalance ==
				    -1 + 2 * direction) { // -1 for "direction" 0; 1 for "direction" 1
					// We're 'doubly-unbalanced' on this side, so perform a single
					// rotation in the opposite direction:
					root->fBalance = root->fSubTree[direction]->fBalance = 0;
					rotate(1 - direction, root);
				} else {
					// This is the Left-Right case (for "direction" 0) or the Right-Left
					// case (for "direction" 1); perform two rotations:
					char newParentCurBalance =
					        root->fSubTree[direction]->fSubTree[1 - direction]->fBalance;
