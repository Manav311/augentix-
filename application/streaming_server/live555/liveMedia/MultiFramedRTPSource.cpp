...
MultiFramedRTPSource::MultiFramedRTPSource(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
...
void MultiFramedRTPSource::doGetNextFrame1()
{
	while (fNeedDelivery) {
		...
		nextPacket->use(fTo, fMaxSize, frameSize, fNumTruncatedBytes, fCurPacketRTPSeqNum,
		                fCurPacketRTPTimestamp, fPresentationTime, fCurPacketHasBeenSynchronizedUsingRTCP,
		                fCurPacketMarkerBit);
		...
		if (fCurrentPacketCompletesFrame) {
			...
			if (fReorderingBuffer->isEmpty()) {
				...
				// so this code will not get
				// executed again without having first returned to the event loop.  Call
				// our 'after getting' function
	...
	do {
		...
		if (!bPacket->fillInData(fRTPInterface, packetReadWasIncomplete)) {
			if (bPacket->bytesAvailable() == 0) {
				envir() << "MultiFramedRTPSource error: Hit limit when reading "
		...
		if (rtpSSRC != fLastReceivedSSRC) {
			...
			// handle streams that contain multiple SSRCs,
			// but we can handle a single-SSRC stream where the SSRC changes
			// occasionally:
...
void BufferedPacket::getNextEnclosedFrameParameters(unsigned char *&framePtr, unsigned dataSize, unsigned &frameSize,
...
void BufferedPacket::assignMiscParams(unsigned short rtpSeqNo, unsigned rtpTimestamp, struct timeval presentationTime,
                                      Boolean hasBeenSyncedUsingRTCP, Boolean rtpMarkerBit, struct timeval timeReceived)
{
	fRTPSeqNo = rtpSeqNo;
...
void BufferedPacket::use(unsigned char *to, unsigned toSize, unsigned &bytesUsed, unsigned &bytesTruncated,
...
BufferedPacket *ReorderingPacketBuffer::getNextCompletedPacket(Boolean &packetLossPreceded)
{
	...
	if (fThresholdTime == 0) {
		...
		struct timeval timeNow;
		gettimeofday(&timeNow, NULL);
		unsigned uSecondsSinceReceived = (timeNow.tv_sec - fHeadPacket->timeReceived().tv_sec) * 1000000 +
