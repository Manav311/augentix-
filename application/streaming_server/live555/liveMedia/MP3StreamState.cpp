...
{
	if (fHasXingTOC) {
		// The file is VBR, with a Xing TOC; use it to determine which byte to seek
		// to:
...
Boolean MP3StreamState::readFrame(unsigned char *outBuf, unsigned outBufSize, unsigned &resultFrameSize,
...
{
	char const *formatStr = "bandwidth %d MPEGnumber %d MPEGlayer %d "
	                        "samplingFrequency %d isStereo %d playTime %d isVBR "
	                        "%d";
	...
	sprintf(buffer, formatStr, fr().bitrate, fr().isMPEG2 ? 2 : 1, fr().layer, fr().samplingFreq, fr().isStereo,
	        fpt, fIsVBR);
#else
	snprintf(buffer, bufferSize, formatStr, fr().bitrate, fr().isMPEG2 ? 2 : 1, fr().layer, fr().samplingFreq,
	         fr().isStereo, fpt, fIsVBR);
#endif
}
...
read_again:
	...
	fr().hdr = ((unsigned long)hbuf[0] << 24) | ((unsigned long)hbuf[1] << 16) | ((unsigned long)hbuf[2] << 8) |
