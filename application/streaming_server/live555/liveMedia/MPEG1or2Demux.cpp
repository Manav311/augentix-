...
void MPEG1or2Demux::registerReadInterest(u_int8_t streamIdTag, unsigned char *to, unsigned maxSize,
                                         FramedSource::afterGettingFunc *afterGettingFunc, void *afterGettingClientData,
                                         FramedSource::onCloseFunc *onCloseFunc, void *onCloseClientData)
{
	...
	if (out.isCurrentlyAwaitingData) {
		envir() << "MPEG1or2Demux::registerReadInterest(): attempt to read stream id " << (void *)streamIdTag
		        << " more than once!\n";
		envir().internalError();
	}
...
Boolean MPEG1or2Demux::useSavedData(u_int8_t streamIdTag, unsigned char *to, unsigned maxSize,
                                    FramedSource::afterGettingFunc *afterGettingFunc, void *afterGettingClientData)
{
	struct OutputDescriptor &out = fOutput[streamIdTag];
	...
	if (afterGettingFunc != NULL) {
		...
		(*afterGettingFunc)(afterGettingClientData, totNumBytesCopied, 0 /* numTruncatedBytes */,
		                    presentationTime, 0 /* durationInMicroseconds ?????#####*/);
	}
	return True;
}

void MPEG1or2Demux::continueReadProcessing(void *clientData, unsigned char * /*ptr*/, unsigned /*size*/,
                                           struct timeval /*presentationTime*/)
{
	...
	while (fNumPendingReads > 0) {
		...
		if (acquiredStreamIdTag != 0) {
			...
			if (newOut.fAfterGettingFunc != NULL) {
				(*newOut.fAfterGettingFunc)(newOut.afterGettingClientData, newOut.frameSize,
...
void MPEG1or2Demux::getNextFrame(u_int8_t streamIdTag, unsigned char *to, unsigned maxSize,
                                 FramedSource::afterGettingFunc *afterGettingFunc, void *afterGettingClientData,
                                 FramedSource::onCloseFunc *onCloseFunc, void *onCloseClientData)
{
	...
	registerReadInterest(streamIdTag, to, maxSize, afterGettingFunc, afterGettingClientData, onCloseFunc,
	                     onCloseClientData);
...
MPEGProgramStreamParser::MPEGProgramStreamParser(MPEG1or2Demux *usingDemux, FramedSource *inputSource)
        : StreamParser(inputSource, MPEG1or2Demux::handleClosure, usingDemux, &MPEG1or2Demux::continueReadProcessing,
                       usingDemux)
...
#ifdef DEBUG
		fprintf(stderr, "MPEGProgramStreamParser::parse() EXCEPTION (This is "
...
void MPEGProgramStreamParser::parsePackHeader()
{
	...
	if ((nextByte & 0xF0) == 0x20) { // MPEG-1
		...
		fUsingDemux->envir() << "StreamParser::parsePack() saw strange byte " << (void *)nextByte
...
void MPEGProgramStreamParser::parseSystemHeader()
{
	...
	// According to the MPEG-1 and MPEG-2 specs, "remaining_header_length" should
	// be
	// at least 6 bytes.  Check this now:
	if (remaining_header_length < 6) {
		fUsingDemux->envir()
		        << "StreamParser::parseSystemHeader(): saw strange header_length: " << remaining_header_length
		        << " < 6\n";
	}
...
unsigned char MPEGProgramStreamParser::parsePESPacket()
{
	...
	if (stream_id == RAW_PES) {
		restoreSavedParserState(); // so we deliver from the beginning of the PES
		        // packet
	...
	if (PES_packet_length < bytesSkipped) {
		...
		if (out.isCurrentlyAwaitingData) {
			...
			if (PES_packet_length > out.maxSize) {
				fUsingDemux->envir() << "MPEGProgramStreamParser::parsePESPacket() "
				                        "error: PES_packet_length ("
				                     << PES_packet_length << ") exceeds max frame size asked for ("
				                     << out.maxSize << ")\n";
...
#ifdef DEBUG
			fprintf(stderr,
			        "%d, %d bytes of PES_packet_data (out.maxSize: %d); "
			        "first 4 bytes: 0x%08x\n",
...
// The next time he asks for a frame, he'll get it.
			...
			// Save this data, so that the reader will get it when he later asks for
			// it.
