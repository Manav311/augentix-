...
MPEG4GenericRTPSource *MPEG4GenericRTPSource::createNew(UsageEnvironment &env, Groupsock *RTPgs,
                                                        ...
                                                        char const *mediumName, char const *mode, unsigned sizeLength,
                                                        unsigned indexLength, unsigned indexDeltaLength)
{
	return new MPEG4GenericRTPSource(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency, mediumName, mode,
...
MPEG4GenericRTPSource::MPEG4GenericRTPSource(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
                                             unsigned rtpTimestampFrequency, char const *mediumName, char const *mode,
                                             unsigned sizeLength, unsigned indexLength, unsigned indexDeltaLength)
...
unsigned MPEG4GenericBufferedPacket::nextEnclosedFrameSize(unsigned char *& /*framePtr*/, unsigned dataSize)
{
	...
	if (fOurSource->fNextAUHeader >= numAUHeaders) {
		fOurSource->envir()
		        << "MPEG4GenericBufferedPacket::nextEnclosedFrameSize(" << dataSize << "): data error ("
		        << auHeader << "," << fOurSource->fNextAUHeader << "," << numAUHeaders << ")!\n";
		return dataSize;
	}
...
static unsigned const samplingFrequencyFromIndex[16] = { 96000, 88200, 64000, 48000, 44100, 32000, 24000, 22050,
