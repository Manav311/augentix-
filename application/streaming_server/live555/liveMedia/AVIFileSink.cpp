...
{
	AVIFileSink *newSink = new AVIFileSink(env, inputSession, outputFileName, bufferSize, movieWidth, movieHeight,
...
void AVIFileSink::afterGettingFrame(void *clientData, unsigned packetDataSize, unsigned numTruncatedBytes,
...
void AVISubsessionIOState::useFrame(SubsessionBuffer &buffer)
{
	...
	if (fPrevPresentationTime.tv_usec != 0 || fPrevPresentationTime.tv_sec != 0) {
		int uSecondsDiff = (presentationTime.tv_sec - fPrevPresentationTime.tv_sec) * 1000000 +
