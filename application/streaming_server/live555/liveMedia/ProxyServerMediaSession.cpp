...
// RTSP servers that acts as a 'proxy' for
...
{
	return new ProxyRTSPClient(ourServerMediaSession, rtspURL, username, password, tunnelOverHTTPPortNum,
	                           verbosityLevel, socketNumToServer);
}
...
void ProxyServerMediaSession::continueAfterDESCRIBE(char const *sdpDescription)
{
	...
	// ("resultString"), then iterate through its
	// "MediaSubsession" objects, to set up corresponding "ServerMediaSubsession"
	// objects that we'll use to serve the stream's tracks.
	do {
		...
		for (MediaSubsession *mss = iter.next(); mss != NULL; mss = iter.next()) {
			...
			if (fVerbosityLevel > 0) {
				envir() << *this << " added new \"ProxyServerMediaSubsession\" for "
				        << mss->protocolName() << "/" << mss->mediumName() << "/" << mss->codecName()
				        << " track\n";
			}
		}
	} while (0);
...
void ProxyServerMediaSession::resetDESCRIBEState()
{
	...
	if (fOurRTSPServer != NULL) {
		// First, close any RTSP client connections that may have already been set
		// up:
...
void ProxyRTSPClient::continueAfterDESCRIBE(char const *sdpDescription)
{
	if (sdpDescription != NULL) {
		...
		// "DESCRIBE" command (to the downstream server) and the
		// subsequent "SETUP"/"PLAY" - which doesn't occur until the first time that
		// a client requests the stream.
		// To prevent the proxied connection (between us and the downstream server)
		// from timing out, we send periodic 'liveness'
		// ("OPTIONS" or "GET_PARAMETER") commands.  (The usual RTCP liveness
		// mechanism wouldn't work here, because RTCP packets
...
void ProxyRTSPClient::continueAfterLivenessCommand(int resultCode, Boolean serverSupportsGetParameter)
{
	if (resultCode != 0) {
		...
		// We handle this by resetting our connection state with this server.  Any
		// current clients will be closed, but
		// subsequent clients will cause new RTSP "SETUP"s and "PLAY"s to get done,
		// restarting the stream.
		// Then continue by sending more "DESCRIBE" commands, to try to restore the
		// stream.

		fServerSupportsGetParameter = False; // until we learn otherwise, in
		        // response to a future "OPTIONS"
		        // command

		if (resultCode < 0) {
			// The 'liveness' command failed without getting a response from the
			// server (otherwise "resultCode" would have been > 0).
			// This suggests that the RTSP connection itself has failed.  Print this
...
#define SUBSESSION_TIMEOUT_SECONDS                                                   \
	10 /*how many seconds to wait for the last track's "SETUP" to be done (note \
	        below)*/
...
void ProxyRTSPClient::continueAfterSETUP()
{
	if (fVerbosityLevel > 0) {
		envir() << *this
		        << "::continueAfterSETUP(): head codec: " << fSetupQueueHead->fClientMediaSubsession.codecName()
	...
	if (fSetupQueueHead != NULL) {
		// There are still entries in the queue, for tracks for which we have still
		// to do a "SETUP".
		...
		if (fNumSetupsDone >= smss->fParentSession->numSubsessions()) {
			...
			// Continue by sending a "PLAY" command (an 'aggregate' "PLAY" command, on
			// the whole session):
			sendPlayCommand(smss->fClientMediaSubsession.parentSession(), NULL, -1.0f, -1.0f, 1.0f,
			                fOurAuthenticator);
			// the "-1.0f" "start" parameter causes the "PLAY" to be sent without a
			// "Range:" header, in case we'd already done
			...
			// "SETUP".  They might get "SETUP" very soon, but it's
			// also possible - if the remote client chose to play only some of the
			// session's tracks - that they might not.
			// To allow for this possibility, we set a timer.  If the timer expires
			// without the remaining subsessions getting "SETUP",
...
void ProxyRTSPClient::scheduleLivenessCommand()
{
	...
	unsigned delayMax = sessionTimeoutParameter(); // if the server specified a maximum time
...
// notification' command, even if the server previously
// indicated (in its response to our earlier "OPTIONS" command) that it
// supported "GET_PARAMETER".  This is because
// "GET_PARAMETER" crashes some camera servers (even though they claimed to
...
FramedSource *ProxyServerMediaSubsession::createNewStreamSource(unsigned clientSessionId, unsigned &estBitrate)
{
	...
	// If we haven't yet created a data source from our 'media subsession' object,
	// initiate() it to do so:
	if (fClientMediaSubsession.readSource() == NULL) {
		fClientMediaSubsession.receiveRawMP3ADUs(); // hack for MPA-ROBUST streams
		fClientMediaSubsession.receiveRawJPEGFrames(); // hack for proxying JPEG/RTP
		        // streams. (Don't do this if
		        // we're transcoding.)
		...
		if (fClientMediaSubsession.readSource() != NULL) {
			// Add to the front of all data sources a filter that will 'normalize'
			// their frames' presentation times,
			...
			FramedFilter *normalizerFilter =
			        sms->fPresentationTimeSessionNormalizer->createNewPresentationTimeSubsessionNormalizer(
			                fClientMediaSubsession.readSource(), fClientMediaSubsession.rtpSource(),
			                codecName);
			fClientMediaSubsession.addFilter(normalizerFilter);

			// Some data sources require a 'framer' object to be added, before they
			// can be fed into
			...
			if (strcmp(codecName, "H264") == 0) {
				...
				fClientMediaSubsession.addFilter(MPEG1or2VideoStreamDiscreteFramer::createNew(
				        envir(), fClientMediaSubsession.readSource(), False, 5.0,
				        True /* leave PTs unmodified*/));
			} else if (strcmp(codecName, "DV") == 0) {
				fClientMediaSubsession.addFilter(
				        DVVideoStreamFramer::createNew(envir(), fClientMediaSubsession.readSource(),
	...
	if (clientSessionId != 0) {
		...
		if (!fHaveSetupStream) {
			// This is our first "SETUP".  Send RTSP "SETUP" and later "PLAY" commands
			// to the proxied server, to start streaming:
			// (Before sending "SETUP", enqueue ourselves on the "RTSPClient"s 'SETUP
			...
			// Hack: If there's already a pending "SETUP" request (for another track),
			// don't send this track's "SETUP" right away, because
			// the server might not properly handle 'pipelined' requests.  Instead,
			// wait until after previous "SETUP" responses come back.
			if (queueWasEmpty) {
				proxyRTSPClient->sendSetupCommand(fClientMediaSubsession, ::continueAfterSETUP, False,
			...
			// This is a "SETUP" from a new client.  We know that there are no other
			// currently active clients (otherwise we wouldn't
			// have been called here), so we know that the substream was previously
			// "PAUSE"d.  Send "PLAY" downstream once again,
			// to resume the stream:
			if (!proxyRTSPClient->fLastCommandWasPLAY) { // so that we send only one
				// "PLAY"; not one for each
				// subsession
				proxyRTSPClient->sendPlayCommand(fClientMediaSubsession.parentSession(), NULL,
...
void ProxyServerMediaSubsession::closeStreamSource(FramedSource *inputSource)
{
	...
	// Because there's only one input source for this 'subsession' (regardless of
	// how many downstream clients are proxying it),
...
RTPSink *ProxyServerMediaSubsession::createNewRTPSink(Groupsock *rtpGroupsock, unsigned char rtpPayloadTypeIfDynamic,
                                                      FramedSource *inputSource)
{
	if (verbosityLevel() > 0) {
    ...
    newSink = AMRAudioRTPSink::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic,
		...
		newSink = SimpleRTPSink::createNew(envir(), rtpGroupsock, 26, 90000, "video", "JPEG", 1 /*numChannels*/,
		                                   False /*allowMultipleFramesPerPacket*/, False /*doNormalMBitRule*/);
		...
		newSink = MPEG4GenericRTPSink::createNew(
		        envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, fClientMediaSubsession.rtpTimestampFrequency(),
		...
		newSink = SimpleRTPSink::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, 48000, "audio",
		...
		// Proxying of these codecs is currently *not* supported, because the data
		// received by the "RTPSource" object is not in a
		...
		return NULL;
	} else if (strcmp(codecName, "QCELP") == 0 || strcmp(codecName, "H261") == 0 ||
	           strcmp(codecName, "H263-1998") == 0 || strcmp(codecName, "H263-2000") == 0 ||
	           strcmp(codecName, "X-QT") == 0 || strcmp(codecName, "X-QUICKTIME") == 0) {
		// This codec requires a specialized RTP payload format; however, we don't
		// yet have an appropriate "RTPSink" subclass for it:
		if (verbosityLevel() > 0) {
			envir() << "\treturns NULL (because we don't have a \"RTPSink\" subclass "
	...
	// input frames have been RTCP-synchronized,
	...
	if (strcmp(codecName, "H264") == 0 || strcmp(codecName, "H265") == 0 || strcmp(codecName, "MP4V-ES") == 0 ||
	    strcmp(codecName, "MPV") == 0 || strcmp(codecName, "DV") == 0) {
		// There was a separate 'framer' object in front of the
		// "PresentationTimeSubsessionNormalizer", so go back one object to get it:
...
void ProxyServerMediaSubsession::subsessionByeHandler()
{
	...
	// This "BYE" signals that our input source has (effectively) closed, so pass
	// this onto the front-end clients:
...
{
	fSubsessionNormalizers = new PresentationTimeSubsessionNormalizer(*this, inputSource, rtpSource, codecName,
                                                                  ...
                                                                  struct timeval &toPT, struct timeval const &fromPT)
{
	...
	if (!hasBeenSynced) {
		// If "fromPT" has not yet been RTCP-synchronized, then it was generated by
		// our own receiving code, and thus
		// is already aligned with 'wall-clock' time.  Just copy it 'as is' to
		// "toPT":
		toPT = fromPT;
	} else {
		if (fMasterSSNormalizer == NULL) {
			// Make "ssNormalizer" the 'master' subsession - meaning that its
			// presentation time is adjusted to align with 'wall clock'
			// time, and the presentation times of other subsessions (if any) are
			// adjusted to retain their relative separation with
			// those of the master:
			...
			// Note: It's OK if one or both of these fields underflows; the result
			// still works out OK later.
		}
		...
		// Because "ssNormalizer"s relayed presentation times are accurate from now
		// on, enable RTCP "SR" reports for its "RTPSink":
...
PresentationTimeSubsessionNormalizer::PresentationTimeSubsessionNormalizer(PresentationTimeSessionNormalizer &parent,
                                                                           FramedSource *inputSource,
...
void PresentationTimeSubsessionNormalizer::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
                                                             struct timeval presentationTime,
                                                             unsigned durationInMicroseconds)
{
	// This filter is implemented by passing all frames through unchanged, except
	// that "fPresentationTime" is changed:
	...
	// the raw JPEG/RTP payloads, without interpreting them,
	// we need to also 'copy' the RTP 'M' (marker) bit from the "RTPSource" to the
	// "RTPSink":
