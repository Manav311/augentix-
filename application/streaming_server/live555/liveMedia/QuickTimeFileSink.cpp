...
class ChunkDescriptor {
    public:
	ChunkDescriptor(int64_t offsetInFile, unsigned size, unsigned frameSize, unsigned frameDuration,
	                struct timeval presentationTime);

	ChunkDescriptor *extendChunk(int64_t newOffsetInFile, unsigned newSize, unsigned newFrameSize,
	                             unsigned newFrameDuration, struct timeval newPresentationTime);
...
class SubsessionIOState {
	...
	unsigned useFrame1(unsigned sourceDataSize, struct timeval presentationTime, unsigned frameDuration,
...
QuickTimeFileSink::QuickTimeFileSink(UsageEnvironment &env, MediaSession &inputSession, char const *outputFileName,
                                     unsigned bufferSize, unsigned short movieWidth, unsigned short movieHeight,
                                     unsigned movieFPS, Boolean packetLossCompensate, Boolean syncStreams,
                                     Boolean generateHintTracks, Boolean generateMP4Format)
        : Medium(env)
...
QuickTimeFileSink *QuickTimeFileSink::createNew(UsageEnvironment &env, MediaSession &inputSession,
                                                char const *outputFileName, unsigned bufferSize,
                                                unsigned short movieWidth, unsigned short movieHeight,
                                                unsigned movieFPS, Boolean packetLossCompensate, Boolean syncStreams,
                                                Boolean generateHintTracks, Boolean generateMP4Format)
{
	QuickTimeFileSink *newSink = new QuickTimeFileSink(env, inputSession, outputFileName, bufferSize, movieWidth,
...
void QuickTimeFileSink::afterGettingFrame(void *clientData, unsigned packetDataSize, unsigned numTruncatedBytes,
...
void QuickTimeFileSink::onRTCPBye(void *clientData)
{
	...
	ioState->envir() << "Received RTCP \"BYE\" on \"" << subsession.mediumName() << "/" << subsession.codecName()
...
static Boolean timevalGE(struct timeval const &tv1, struct timeval const &tv2)
{
	return (unsigned)tv1.tv_sec > (unsigned)tv2.tv_sec ||
	       (tv1.tv_sec == tv2.tv_sec && (unsigned)tv1.tv_usec >= (unsigned)tv2.tv_usec);
}
...
Boolean SubsessionIOState::setQTstate()
{
	...
	char const *noCodecWarning3 = "\" track, so we'll insert a dummy \"????\" "
	                              "Media Data Atom instead.  A separate, "
	                              "codec-specific editing pass will be needed "
	...
	do {
		...
		if (isHintTrack()) {
			...
			if (strcmp(fOurSubsession.codecName(), "X-QT") == 0 ||
				...
				// It might be different from the RTP timestamp frequency (e.g.,
				// aacPlus).
				...
				envir() << noCodecWarning1 << "Audio" << noCodecWarning2 << fOurSubsession.codecName()
				        << noCodecWarning3;
			...
			if (strcmp(fOurSubsession.codecName(), "X-QT") == 0 ||
				...
				envir() << noCodecWarning1 << "Video" << noCodecWarning2 << fOurSubsession.codecName()
				        << noCodecWarning3;
				fQTMediaDataAtomCreator = &QuickTimeFileSink::addAtom_dummy;
				fQTEnableTrack = False; // disable this track in the movie
			}
		} else {
			envir() << "Warning: We don't implement a QuickTime Media Handler for "
			           "media type \""
		...
		envir() << "Warning: We don't have sufficient codec-specific information "
		           "(e.g., sample sizes) to fully generate the \""
		        << fOurSubsession.mediumName() << "/" << fOurSubsession.codecName()
		        << "\" track, so we'll disable this track in the movie.  A "
		           "separate, codec-specific editing pass will be needed before "
...
void SubsessionIOState::useFrameForHinting(unsigned frameSize, struct timeval presentationTime,
                                           unsigned startSampleNumber)
{
	...
	if (ppt.tv_sec != 0 || ppt.tv_usec != 0) {
		double duration =
		        (presentationTime.tv_sec - ppt.tv_sec) + (presentationTime.tv_usec - ppt.tv_usec) / 1000000.0;
		if (duration < 0.0)
			duration = 0.0;
		...
		if (hackm4a) {
			...
			// frequency,
			// (as can happen with aacPlus), then we need to scale
			// "hintSampleDuration"
			// accordingly:
	...
	if (hack263) {
		...
		if (sizeLength + indexLength != 16) {
			envir() << "Warning: unexpected 'sizeLength' " << sizeLength << " and 'indexLength' "
			        << indexLength << "seen when creating hint track\n";
		}
...
unsigned SubsessionIOState::useFrame1(unsigned sourceDataSize, struct timeval presentationTime, unsigned frameDuration,
                                      int64_t destFileOffset)
{
	...
	if (fTailChunk == NULL) {
		newTailChunk = fHeadChunk =
		        new ChunkDescriptor(destFileOffset, sourceDataSize, frameSize, frameDuration, presentationTime);
	} else {
		newTailChunk = fTailChunk->extendChunk(destFileOffset, sourceDataSize, frameSize, frameDuration,
		                                       presentationTime);
	}
...
ChunkDescriptor::ChunkDescriptor(int64_t offsetInFile, unsigned size, unsigned frameSize, unsigned frameDuration,
                                 struct timeval presentationTime)
        : fNextChunk(NULL)
...
ChunkDescriptor *ChunkDescriptor::extendChunk(int64_t newOffsetInFile, unsigned newSize, unsigned newFrameSize,
                                              unsigned newFrameDuration, struct timeval newPresentationTime)
{
	...
	ChunkDescriptor *newDescriptor =
	        new ChunkDescriptor(newOffsetInFile, newSize, newFrameSize, newFrameDuration, newPresentationTime);
	...
	if (oneByteLength) {
		...
		if (strLength >= 256) {
			envir() << "QuickTimeFileSink::addArbitraryString(\"" << str
			        << "\") saw string longer than we know how to handle (" << strLength << "\n";
		}
...
while (chunk != NULL) {
	...
	double movieDurationOfEdit = (chunkStartTime.tv_sec - editStartTime.tv_sec) +
...
unsigned QuickTimeFileSink::addAtom_hdlr2()
{
	...
	unsigned QuickTimeFileSink::addAtom_genericMedia()
	{
		...
		QuickTimeGenericRTPSource *rtpSource =
		        (QuickTimeGenericRTPSource *)fCurrentIOState->fOurSubsession.rtpSource();
		QuickTimeGenericRTPSource::QTState &qtState = rtpSource->qtState;
		...
		unsigned QuickTimeFileSink::addAtom_soundMediaGeneral()
		{
			...
			{
				// The beginning of this atom looks just like a general Sound Media
				// atom,
				...
				unsigned QuickTimeFileSink::addAtom_mp4a()
				{
					...
					unsigned QuickTimeFileSink::addAtom_rtp()
					{
						...
						addAtom(tims);
						size += addWord(
						        fCurrentIOState->fOurSubsession.rtpTimestampFrequency());
						addAtomEnd;
						...
						while (chunk != NULL) {
							...
							if (sampleDuration != prevSampleDuration) {
								...
								if (chunk != fCurrentIOState->fHeadChunk) {
									...
									size += addWord(
									        prevSampleDuration); // Sample duration
									numSamplesSoFar = 0;
								}
							}
						...
						if (fCurrentIOState->fHeadSyncFrame != NULL) {
							...
							// Then, run through the chunk descriptors, counting up the total
							// nuber of samples:
							...
							// Then, write out the sample numbers that we deem correspond to
							// 'sync samples':
							unsigned i;
							for (i = 0; i < numSamplesSoFar; i += 12) {
								...
								// (Perhaps we should really try to keep track of which
								// 'samples' ('frames' for video) really are 'key frames'?)
								size += addWord(i + 1);
								++numEntries;
							}
						...
						if (haveSingleEntryTable) {
							if (fCurrentIOState->isHintTrack() &&
								...
								// The following doesn't seem right, but seems to do the right
								// thing:
								sampleSize =
						...
						unsigned QuickTimeFileSink::addAtom_sdp()
						{
							...
							sprintf(rtpmapString, "%s/%d", ourSubsession.codecName(),
