...
JPEGVideoStreamFramer *JPEGVideoStreamFramer::createNew(UsageEnvironment &env, FramedSource *inputSource)
{
...
{
	/*MPI layer use JPEG type 1 Y:U:V*/
	return 1;
}
u_int8_t JPEGVideoStreamFramer::qFactor()
{
	/*Will be overwrite by Parser*/
	return 70;
}
...
u_int8_t const *JPEGVideoStreamFramer::quantizationTables(u_int8_t &precision, u_int16_t &length)
{
	/*Will be prase from camera source by Parser*/
	return NULL;
}

JPEGVideoStreamFramer::JPEGVideoStreamFramer(UsageEnvironment &env, FramedSource *inputSource, Boolean createParser,
                                             double frameRate)
        : JPEGVideoSource((env))
{
...
void JPEGVideoStreamFramer::continueReadProcessing(void *clientData, unsigned char * /*ptr*/, unsigned /*size*/,
...
void JPEGVideoStreamFramer::continueReadProcessing()
{
	...
	//	avgBitrate = (totalBytes * 8 * TIMESCALE) / totalDuration;
