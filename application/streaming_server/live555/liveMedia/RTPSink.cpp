...
RTPSink::RTPSink(UsageEnvironment &env, Groupsock *rtpGS, unsigned char rtpPayloadType, unsigned rtpTimestampFrequency,
...
u_int32_t RTPSink::convertToRTPTimestamp(struct timeval tv)
{
	...
	fprintf(stderr, "fTimestampBase: 0x%08x, tv: %lu.%06ld\n\t=> RTP timestamp: 0x%08x\n", fTimestampBase,
	        tv.tv_sec, tv.tv_usec, rtpTimestamp);
	fflush(stderr);
#endif
...
void RTPTransmissionStatsDB::noteIncomingRR(u_int32_t SSRC, struct sockaddr_in const &lastFromAddress,
...
{
	gettimeofdayMonotonic(&fTimeCreated, NULL);
...
void RTPTransmissionStats::noteIncomingRR(struct sockaddr_in const &lastFromAddress, unsigned lossStats,
                                          ...
                                          unsigned diffSR_RRTime)
{
	...
	gettimeofdayMonotonic(&fTimeReceived, NULL);
	...
	fprintf(stderr,
	        ...
	        "lastPacketNumReceived 0x%08x, jitter 0x%08x, lastSRTime "
	        "0x%08x, diffSR_RRTime 0x%08x\n",
...
unsigned RTPTransmissionStats::roundTripDelay() const
{
	...
	// recently-received
	// RTCP RR packet.  Use the method noted in the RTP/RTCP specification (RFC
	// 3350).
