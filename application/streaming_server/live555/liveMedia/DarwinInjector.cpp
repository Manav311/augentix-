...
DarwinInjector *DarwinInjector::createNew(UsageEnvironment &env, char const *applicationName, int verbosityLevel)
{
...
DarwinInjector::DarwinInjector(UsageEnvironment &env, char const *applicationName, int verbosityLevel)
        : Medium(env)
...
{
	if (rtpSink == NULL)
		return; // "rtpSink" should be non-NULL
...
// Define a special subclass of "RTSPClient" that has a pointer field to a
// "DarwinInjector".  We'll use this to implement RTSP ops:
class RTSPClientForDarwinInjector : public RTSPClient {
	...
	RTSPClientForDarwinInjector(UsageEnvironment &env, char const *rtspURL, int verbosityLevel,
	                            char const *applicationName, DarwinInjector *ourDarwinInjector)
...
Boolean DarwinInjector::setDestination(char const *remoteRTSPServerNameOrAddress, char const *remoteFileName,
                                       char const *sessionName, char const *sessionInfo,
                                       portNumBits remoteRTSPServerPortNumber, char const *remoteUserName,
                                       char const *remotePassword, char const *sessionAuthor,
                                       char const *sessionCopyright, int timeout)
{
	char *sdp = NULL;
	char *url = NULL;
	...
	do {
		...
		unsigned urlLen = strlen(urlFmt) + strlen(remoteRTSPServerNameOrAddress) + 5 /* max short len */ +
		...
		unsigned sdpLen = strlen(sdpFmt) + 20 /* max int len */ + 20 /* max int len */
		                  + strlen(sessionName) + strlen(sessionInfo) +
		                  strlen(remoteRTSPServerAddressStr.val()) + strlen(sessionName) + strlen(sessionInfo) +
		                  strlen(fApplicationName) + strlen(sessionAuthor) + strlen(sessionCopyright) +
		                  fSubstreamSDPSizes;
		unsigned const sdpSessionId = our_random32();
		unsigned const sdpVersion = sdpSessionId;
		sdp = new char[sdpLen];
		sprintf(sdp, sdpFmt, sdpSessionId, sdpVersion, // o= line
		        sessionName, // s= line
		        sessionInfo, // i= line
		...
		while ((subsession = iter.next()) != NULL) {
			...
			(void)fRTSPClient->sendSetupCommand(*subsession, genericResponseHandler,
			                                    True /*streamOutgoing*/, True /*streamUsingTCP*/);
			// Now block (but handling events) until we get a response:
			envir().taskScheduler().doEventLoop(&fWatchVariable);
			...
			if (ss->rtcpInstance() != NULL) {
				ss->rtcpInstance()->setStreamSocket(fRTSPClient->socketNum(), streamChannelId++);
			}
			ss = ss->next();
		}
		if (subsession != NULL)
			break; // an error occurred above
        ...
        , fRTCPInstance(rtcpInstance)
{
	...
	if (rtpPayloadType >= 96) {
		...
		sprintf(rtpmapLine, rtpmapFmt, rtpPayloadType, rtpPayloadFormatName, rtpTimestampFrequency,
		        encodingParamsPart);
	...
	char const *const sdpFmt = "m=%s 0 RTP/AVP %u\r\n"
	                           "%s" // "a=rtpmap:" line (if present)
	                           "%s" // auxilliary (e.g., "a=fmtp:") line (if present)
	                           "a=control:trackID=%u\r\n";
	unsigned sdpFmtSize = strlen(sdpFmt) + strlen(mediaType) + 3 /* max char len */
	                      + rtpmapLineSize + auxSDPLineSize + 20 /* max int len */;
