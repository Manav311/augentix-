...
JPEGVideoStreamParser::JPEGVideoStreamParser(JPEGVideoStreamFramer *usingSource, FramedSource *inputSource)
        : StreamParser(inputSource, FramedSource::handleClosure, usingSource,
                       &JPEGVideoStreamFramer::continueReadProcessing, usingSource)
{
}
...
void JPEGVideoStreamParser::registerReadInterest(unsigned char *to, unsigned maxSize)
{
...
{
	u_int32_t frameSize = 0; // The current frame size
...
#ifdef DEBUG
		fprintf(stderr,
		        "JPEGVideoStreamParser::parse() EXCEPTION (This is normal behavior - *not* an error)\n");
#endif
		//    frameSize=0;
	}
...
int JPEGVideoStreamParser::parseJPEGFrame()
{
	...
	if (fLimit < fTo + JPEG_REQUIRE_HEADER_SIZE_BYTES) {
		fprintf(stderr, "JPEG parser buf overflow\r\n");
	}
	...
	if (sz < 0) {
		fprintf(stderr, "JPEG parser buf loss sz\r\n");
	}
	 /* first frame */
	if (sz == 5) {
