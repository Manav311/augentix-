...
OggFileParser::OggFileParser(OggFile &ourFile, FramedSource *inputSource, FramedSource::onCloseFunc *onEndFunc,
                             void *onEndClientData, OggDemux *ourDemux)
        ...
        , fSavedPacket(NULL)
{
	if (ourDemux == NULL) {
		...
		// In this case, parsing (of page data) doesn't start until a client starts
...
#ifdef DEBUG
		fprintf(stderr, "OggFileParser::parse() EXCEPTION (This is normal behavior "
		                "- *not* an error)\n");
#endif
...
Boolean OggFileParser::parseStartOfFile()
{
	...
	// we have
	// collected all required headers for Vorbis, Theora, or Opus track(s) (if
	// any).
...
u_int8_t OggFileParser::parseInitialPage()
{
	...
	// If this is a BOS page, examine the first 8 bytes of the first 'packet', to
	// see whether
	...
	if ((header_type_flag & 0x02) != 0) { // BOS
		...
		// Because this is not a BOS page, the specified track should already have
		// been seen:
	...
	if (track != NULL) { // sanity check
		...
		if (track->mimeType != NULL &&
			...
			// Make a copy of each packet, until we get the three special headers that
			// we need:
			...
			for (unsigned j = 0; j < fPacketSizeTable->numCompletedPackets && track->weNeedHeaders(); ++j) {
				...
				// The start of the packet tells us whether its a header that we know
				// about:
				...
				if (headerIsKnown) {
					...
					// Save this header (deleting any old header of the same type that
					// we'd saved before)
					...
					if (!track->weNeedHeaders()) {
						// We now have all of the needed Vorbis, Theora, or Opus headers for
						// this track:
						--fNumUnfulfilledTracks;
					}
					// Note: The above code won't work if a required header is fragmented
					// over
...
static Boolean parseVorbisSetup_codebook(LEBitVector &bv)
{
	...
	if (!ordered) {
		...
		for (unsigned i = 0; i < codebook_entries; ++i) {
			...
			do {
				(void)codewordLength;
			} while (0); /* Remove compilation warning */
#endif
		}
	} else { // ordered
#ifdef DEBUG_SETUP_HEADER
		...
		do {
			...
			fprintf(stderr, "\t\t\t\tcodeword length[%d..%d]:\t%d\n", current_entry,
			        current_entry + number - 1, current_length);
#endif
	...
	for (unsigned i = 0; i < vorbis_mapping_count; ++i) {
		...
		if (bv.getBits(1)) { // "square polar channel mapping is in use"
			...
			for (unsigned j = 0; j < vorbis_mapping_coupling_steps; ++j) {
				...
				bv.skipBits(2 * ilog_audio_channels_minus_1); // "vorbis_mapping_magnitude",
		...
		if (vorbis_mapping_submaps > 1) {
			for (unsigned j = 0; j < audio_channels; ++j) {
				...
				if (vorbis_mapping_mux >= vorbis_mapping_submaps) {
					fprintf(stderr,
					        "Vorbis Mappings, read bad \"vorbis_mapping_mux\" %d "
					        "(>= \"vorbis_mapping_submaps\" %d)\n",
...
#endif
		bv.skipBits(16 + 16 + 8); // "vorbis_mode_windowtype",
...
Boolean OggFileParser::validateHeader(OggTrack *track, u_int8_t const *p, unsigned headerSize)
{
	...
	if (strcmp(track->mimeType, "audio/VORBIS") == 0) {
		...
		if (firstByte == 1) { // "identification" header
			...
			// Note the two 'block sizes' (samples per packet), and their durations in
			// microseconds:
			...
			fprintf(stderr, "\t%u Hz, %u-channel, %u kbps (est), block sizes: %u,%u (%u,%u us)\n",
			        track->samplingFrequency, track->numChannels, track->estBitrate, blocksize_0,
			        blocksize_1, track->vtoHdrs.uSecsPerPacket[0], track->vtoHdrs.uSecsPerPacket[1]);
#endif
			// To be valid, "blocksize_0" must be <= "blocksize_1", and both must be
			// in [64,8192]:
			...
			// Unfortunately these come
			// near the end of the header, so we have to parse lots of other crap
			// first.
			p += 7;
		...
		if (firstByte == 0x80) { // "identification" header
			...
			// We don't care about the contents of the Theora "setup" header; just
			// assume it's valid
		}
	} else { // Opus audio
...
// Deliver the next packet:
#ifdef DEBUG
	fprintf(stderr, "\t[track: %s] Delivering packet %d (%d bytes%s)\n", demuxedTrack->MIMEtype(), packetNum,
	        packetSize, packetNum == fPacketSizeTable->numCompletedPackets ? " (incomplete)" : "");
#endif
	...
	if (strcmp(track->mimeType, "audio/VORBIS") == 0) {
		if ((firstByte & 0x01) != 0) { // This is a header packet
			...
			// Extract the next "track->vtoHdrs.ilog_vorbis_mode_count_minus_1" bits
			// of the first byte:
		...
		if (firstByte == 0x4F /*'O'*/ && secondByte == 0x70 /*'p*/) { // This is a header packet
			...
			// Parse the first byte to figure out the duration of each frame, and then
			// (if necessary)
...
#ifdef DEBUG
	fprintf(stderr,
	        "\tgranule_position 0x%08x%08x, bitstream_serial_number "
	        "0x%08x, page_sequence_number 0x%08x, CRC_checksum 0x%08x, "
