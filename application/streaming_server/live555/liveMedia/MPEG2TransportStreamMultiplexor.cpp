...
MPEG2TransportStreamMultiplexor::MPEG2TransportStreamMultiplexor(UsageEnvironment &env)
        ...
        , fProgramMapVersion(0)
        , fPreviousInputProgramMapVersion(0xFF)
        , fCurrentInputProgramMapVersion(0xFF)
        , fPCR_PID(0)
        , fCurrentPID(0)
        , fInputBuffer(NULL)
        , fInputBufferSize(0)
...
void MPEG2TransportStreamMultiplexor::doGetNextFrame()
{
	...
	if ((fOutgoingPacketCounter % 10) == 0) {
		// To avoid excessive recursion (and stack overflow) caused by excessively
		// large input frames,
...
void MPEG2TransportStreamMultiplexor::handleNewBuffer(unsigned char *buffer, unsigned bufferSize, int mpegVersion,
	...
	if (stream_id == 0xBE) { // padding_stream; ignore
		...
		if (fPCR_PID == 0) { // set it to this stream, if it's appropriate:
			if ((!fHaveVideoStreams &&
			     (streamType == 3 || streamType == 4 || streamType == 0xF)) /* audio stream */
			    || (streamType == 1 || streamType == 2 || streamType == 0x10 || streamType == 0x1B ||
			        streamType == 0x24) /* video stream */) {
...
void MPEG2TransportStreamMultiplexor::deliverDataToClient(u_int8_t pid, unsigned char *buffer, unsigned bufferSize,
	...
	if (fMaxSize < TRANSPORT_PACKET_SIZE) {
		...
		*header++ = adaptation_field_control | (continuity_counter & 0x0F);
		...
		if (adaptation_field_control == 0x30) {
			...
			u_int8_t adaptation_field_length = (numHeaderBytes == 5) ? 0 :
