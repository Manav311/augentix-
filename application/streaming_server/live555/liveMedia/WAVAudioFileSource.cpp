        ...
        , fAudioFormat(WA_UNKNOWN)
{
	...
	do {
		...
		if (fAudioFormat != WA_PCM && fAudioFormat != WA_PCMA && fAudioFormat != WA_PCMU &&
			...
			env.setResultMsg("Audio format is not one that we handle (PCM/PCMU/PCMA "
			                 "or IMA ADPCM)");
			break;
		}
	...
	// Now that we've finished reading the WAV header, all future reads (of audio
	// samples) from the file will be asynchronous:
