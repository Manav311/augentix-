...
VorbisAudioRTPSink *VorbisAudioRTPSink::createNew(UsageEnvironment &env, Groupsock *RTPgs, u_int8_t rtpPayloadFormat,
                                                  ...
                                                  u_int8_t *setupHeader, unsigned setupHeaderSize, u_int32_t identField)
{
	return new VorbisAudioRTPSink(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency, numChannels,
	                              identificationHeader, identificationHeaderSize, commentHeader, commentHeaderSize,
                                                  ...
                                                  char const *configStr)
{
	...
	parseVorbisOrTheoraConfigStr(configStr, identificationHeader, identificationHeaderSize, commentHeader,
	                             commentHeaderSize, setupHeader, setupHeaderSize, identField);

	VorbisAudioRTPSink *resultSink = new VorbisAudioRTPSink(
	        env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency, numChannels, identificationHeader,
...
VorbisAudioRTPSink::VorbisAudioRTPSink(UsageEnvironment &env, Groupsock *RTPgs, u_int8_t rtpPayloadFormat,
                                       u_int32_t rtpTimestampFrequency, unsigned numChannels,
                                       u_int8_t *identificationHeader, unsigned identificationHeaderSize,
                                       u_int8_t *commentHeader, unsigned commentHeaderSize, u_int8_t *setupHeader,
        ...
        , fFmtpSDPLine(NULL)
{
	if (identificationHeaderSize >= 28) {
		...
		int bitrate = bitrate_nominal > 0 ?
		                      bitrate_nominal :
		                      bitrate_maximum > 0 ? bitrate_maximum : bitrate_minimum > 0 ? bitrate_minimum : 0;
		if (bitrate > 0)
			estimatedBitrate() = ((unsigned)bitrate) / 1000;
	}

	// Generate a 'config' string from the supplied configuration headers:
	char *base64PackedHeaders = generateVorbisOrTheoraConfigStr(identificationHeader, identificationHeaderSize,
...
void VorbisAudioRTPSink::doSpecialFrameHandling(unsigned fragmentationOffset, unsigned char *frameStart,
...
{
	// We allow more than one frame to be packed into an outgoing RTP packet, but
	// no more than 15:
                                      ...
                                      unsigned setupHeaderSize, u_int32_t identField)
{
	...
	unsigned sizeSize[2]; // The number of bytes used to encode the lengths of the
	        // first two headers (but not the length of the 3rd)
	...
	if (setupHeaderSize > 0) {
		...
		sizeSize[1] = 0; // We have at most two headers, so the second one's length
		        // isn't encoded
	}
	if (numHeaders == 0)
	...
	// Then figure out the size of the packed configuration headers, and allocate
	// space for this:
	...
	*p++ = numHeaders - 1; // "n. of headers"
	...
	// Having set up the 'packed configuration headers', Base-64-encode this, for
	// our result:
