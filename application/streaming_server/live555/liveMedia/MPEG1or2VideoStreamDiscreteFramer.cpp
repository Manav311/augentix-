...
MPEG1or2VideoStreamDiscreteFramer::createNew(UsageEnvironment &env, FramedSource *inputSource, Boolean iFramesOnly,
                                             double vshPeriod, Boolean leavePresentationTimesUnmodified)
{
	// Need to add source type checking here???  #####
	return new MPEG1or2VideoStreamDiscreteFramer(env, inputSource, iFramesOnly, vshPeriod,
	                                             leavePresentationTimesUnmodified);
}

MPEG1or2VideoStreamDiscreteFramer::MPEG1or2VideoStreamDiscreteFramer(UsageEnvironment &env, FramedSource *inputSource,
...
void MPEG1or2VideoStreamDiscreteFramer::afterGettingFrame(void *clientData, unsigned frameSize,
...
void MPEG1or2VideoStreamDiscreteFramer::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
                                                           ...
                                                           unsigned durationInMicroseconds)
{
	...
	if (frameSize >= 4 && fTo[0] == 0 && fTo[1] == 0 && fTo[2] == 1) {
		...
		if (nextCode == 0x00 /*PICTURE_START_CODE*/ && i + 2 < frameSize) {
			...
			// If this is not an "I" frame, but we were asked for "I" frames only,
			// then try again:
			...
			if (!fLeavePresentationTimesUnmodified && picture_coding_type == 3 /*B*/
				...
				unsigned usIncrement =
				        fFrameRate == 0.0 ? 0 : (unsigned)((trIncrement * MILLION) / fFrameRate);
