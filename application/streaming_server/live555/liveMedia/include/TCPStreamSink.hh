...
class TCPStreamSink : public MediaSink {
	...
	// "socketNum" is the socket number of an existing, writable TCP socket (which
	// should be non-blocking).
	// The caller is responsible for closing this socket later (when this object
	// no longer exists).

    protected:
	TCPStreamSink(UsageEnvironment &env,
	              int socketNum); // called only by "createNew()"
    ...
    private:
	void processBuffer(); // common routine, called from both the 'socket
	        // writable' and 'incoming data' handlers below
