...
class AC3AudioStreamFramer : public FramedFilter {
	...
	// If "streamCode" != 0, then we assume that there's a 1-byte code at the
	// beginning of each chunk of data that we read from
	// our source.  If that code is not the value we want, we discard the chunk of
	// data.
	// However, if "streamCode" == 0 (the default), then we don't expect this
	// 1-byte code.
