...
class H264or5VideoStreamFramer : public MPEGVideoStreamFramer {
    public:
	void getVPSandSPSandPPS(u_int8_t *&vps, unsigned &vpsSize, u_int8_t *&sps, unsigned &spsSize, u_int8_t *&pps,
	                        unsigned &ppsSize) const
	{
		// Returns pointers to copies of the most recently seen VPS (video parameter
		// set)
	...
	void setVPSandSPSandPPS(u_int8_t *vps, unsigned vpsSize, u_int8_t *sps, unsigned spsSize, u_int8_t *pps,
	                        unsigned ppsSize)
	{
		// Assigns copies of the VPS, SPS and PPS NAL units.  If this function is
		// not called,
		// then these NAL units are assigned only if/when they appear in the input
		// stream.
	...
	{
		return fProfileTierLevelHeaderBytes;
	}
	// used for H.265 only
	...
	struct timeval fNextPresentationTime; // the presentation time to be used for
	        // the next NAL unit to be
	        // parsed/delivered after this
