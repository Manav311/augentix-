...
/* enough data to ensure that it contains an intact 6-block header (which occurs
 * at the start of a 150-block sequence) */
...
class DVVideoStreamFramer : public FramedFilter {
	...
	// Set "sourceIsSeekable" to True if the input source is a seekable object
	// (e.g. a file), and the server that uses us
	// does a seek-to-zero on the source before reading from it.  (Our RTSP server
