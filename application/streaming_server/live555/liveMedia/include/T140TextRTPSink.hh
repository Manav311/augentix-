...
// Because the T.140 text RTP payload format specification recommends that
// (empty) RTP packets be sent during 'idle periods'
// when no new text is available, we implement "T140TextRTPSink" using a
// separate "T140IdleFilter" class - sitting in front
// -  that delivers, to the "T140TextRTPSink", a continuous sequence of
...
class T140IdleFilter : public FramedFilter {
	...
	char *fBuffer;
	unsigned fBufferedNumTruncatedBytes; // a count of truncated bytes from the
	        // upstream
