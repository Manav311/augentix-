...
class RTPSource : public FramedSource {
	...
	// Note: This is the SSRC in the most recently received RTP packet; not *our*
	// SSRC
	...
	// Note that RTP receivers will usually not need to call either of the
	// following two functions, because
	// RTP sequence numbers and timestamps are usually not useful to receivers.
	// (Our implementation of RTP reception already does all needed handling of
    ...
    private:
	friend class MediaSubsession; // "MediaSubsession" is the only outside class
	        // that ever needs to see RTP timestamps!
	...
	u_int32_t fSSRC;
	Boolean fEnableRTCPReports; // whether RTCP "RR" reports should be sent for
	        // this source (default: True)
...
class RTPReceptionStatsDB {
	...
	void noteIncomingPacket(u_int32_t SSRC, u_int16_t seqNum, u_int32_t rtpTimestamp, unsigned timestampFrequency,
	                        Boolean useForJitterCalculation, struct timeval &resultPresentationTime,
	                        Boolean &resultHasBeenSyncedUsingRTCP, unsigned packetSize /* payload only */);

	// The following is called whenever a RTCP SR packet is received:
	void noteIncomingSR(u_int32_t SSRC, u_int32_t ntpTimestampMSW, u_int32_t ntpTimestampLSW,
...
class RTPReceptionStats {
    ...
    private:
	void noteIncomingPacket(u_int16_t seqNum, u_int32_t rtpTimestamp, unsigned timestampFrequency,
	                        Boolean useForJitterCalculation, struct timeval &resultPresentationTime,
	                        Boolean &resultHasBeenSyncedUsingRTCP, unsigned packetSize /* payload only */);
