...
class OggFile : public Medium {
	...
	// object
	// immediately.  Instead, because this class requires file reading (to parse
	// the
	// Ogg track headers) before a new object can be initialized, the creation of
	// a new object
	// is signalled by calling - from the event loop - an 'onCreationFunc' that is
	// passed as
	...
	FramedSource *createSourceForStreaming(FramedSource *baseSource, u_int32_t trackNumber, unsigned &estBitrate,
...
class OggTrack {
	...
	struct _vtoHdrs {
		...
		unsigned uSecsPerPacket[2]; // computed as
		        // (blocksize[i]*1000000)/samplingFrequency
		unsigned vorbis_mode_count;
		unsigned ilog_vorbis_mode_count_minus_1;
		u_int8_t *vorbis_mode_blockflag;
		// an array (of size "vorbis_mode_count") of indexes into the (2-entry)
		// "blocksize" array
	...
	{
		return vtoHdrs.header[0] == NULL || vtoHdrs.header[1] == NULL ||
