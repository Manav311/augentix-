...
class UserAuthenticationDatabase {
	...
	// If "passwordsAreMD5" is True, then each password stored into, or removed
	// from,
...
#ifndef RTSP_DEFAULT_MAX_CLIENT
#define RTSP_DEFAULT_MAX_CLIENT 4 // for incoming requests, and outgoing responses
#endif
...
class RTSPServer : public Medium {
	...
	static RTSPServer *createNew(UsageEnvironment &env, Port ourPort = 554,
	                             ...
	                             unsigned maxClientNumber = RTSP_DEFAULT_MAX_CLIENT);
	...
	// Removes the "ServerMediaSession" object from our lookup table, so it will
	// no longer be accessible by new RTSP clients.
	// (However, any *existing* RTSP client sessions that use this
	// "ServerMediaSession" object will continue streaming.
	//  The "ServerMediaSession" object will not get deleted until all of these
	//  RTSP client sessions have closed.)
	// (To both delete the "ServerMediaSession" object *and* close all RTSP client
	...
	// Closes (from the server) all RTSP client sessions that are currently using
	// this "ServerMediaSession" object.
	// Note, however, that the "ServerMediaSession" object remains accessible by
	// new RTSP clients.
	...
	// Equivalent to:
	//     "closeAllClientSessionsForServerMediaSession(streamName);
	//     removeServerMediaSession(streamName);

	typedef void(responseHandlerForREGISTER)(RTSPServer *rtspServer, unsigned requestId, int resultCode,
	                                         char *resultString);
	unsigned registerStream(ServerMediaSession *serverMediaSession, char const *remoteClientNameOrAddress,
	                        ...
	                        Boolean receiveOurStreamViaTCP = False, char const *proxyURLSuffix = NULL);
	// 'Register' the stream represented by "serverMediaSession" with the given
	// remote client (specifed by name and port number).
	// This is done using our custom "REGISTER" RTSP command.
	// The function returns a unique number that can be used to identify the
	// request; this number is also passed to "responseHandler".
	// When a response is received from the remote client (or the "REGISTER"
	// request fails), the specified response handler
	//   (if non-NULL) is called.  (Note that the "resultString" passed to the
	//   handler was dynamically allocated,
	//   and should be delete[]d by the handler after use.)
	// If "receiveOurStreamViaTCP" is True, then we're requesting that the remote
	// client access our stream using RTP/RTCP-over-TCP.
	//   (Otherwise, the remote client may choose regular RTP/RTCP-over-UDP
	//   streaming.)
	// "proxyURLSuffix" (optional) is used only when the remote client is also a
	// proxy server.
	//   It tells the proxy server the suffix that it should use in its "rtsp://"
	//   URL (when front-end clients access the stream)
	...
	// (If "clientSocket" is non-negative, then it is used (by calling
	// "getsockname()") to determine
	...
	// Changes the server's authentication database to "newDB", returning a
	// pointer to the old database (if there was one).
	// "newDB" may be NULL (you can use this to disable authentication at runtime,
	// if desired).

	Boolean setUpTunnelingOverHTTP(Port httpPort);
	// (Attempts to) enable RTSP-over-HTTP tunneling on the specified port.
	// Returns True iff the specified port can be used in this way (i.e., it's not
	// already being used for a separate HTTP server).
    ...
    protected:
	RTSPServer(UsageEnvironment &env, int ourSocket, Port ourPort, UserAuthenticationDatabase *authDatabase,
	...
	virtual char const *allowedCommandNames(); // used to implement
	...
	// Note: "responseStr" is dynamically allocated (or NULL), and should be
	// delete[]d after the call
	...
	// on each client (e.g., based on client IP address), without using digest
	// authentication.
	...
	// another hook that allows subclassed servers to do server-specific access
	// checking
	// - this time after normal digest authentication has already taken place (and
	...
	class RTSPClientConnection {
		...
		// Make the handler functions for each command virtual, to allow subclasses
		// to reimplement them, if necessary:
		virtual void handleCmd_OPTIONS();
		// You probably won't need to subclass/reimplement this function;
		// reimplement "RTSPServer::allowedCommandNames()" instead.
		...
		// You probably won't need to subclass/reimplement this function;
		//     reimplement "RTSPServer::weImplementREGISTER()" and
		//     "RTSPServer::implementCmd_REGISTER()" instead.
		...
		virtual Boolean parseHTTPRequestString(char *resultCmdName, unsigned resultCmdNameMaxSize,
		                                       char *urlSuffix, unsigned urlSuffixMaxSize, char *sessionCookie,
	...
	// The state of an individual client session (using one or more sequential TCP
	// connections) handled by a RTSP server:
	class RTSPClientSession {
		...
		// Make the handler functions for each command virtual, to allow subclasses
		// to redefine them:
		virtual void handleCmd_SETUP(RTSPClientConnection *ourClientConnection, char const *urlPreSuffix,
		                             char const *urlSuffix, char const *fullRequestStr);
		virtual void handleCmd_withinSession(RTSPClientConnection *ourClientConnection, char const *cmdName,
    ...
    protected:
	// If you subclass "RTSPClientConnection", then you must also redefine this
	// virtual function in order
	// to create new objects of your subclass:
	virtual RTSPClientConnection *createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr);

	// If you subclass "RTSPClientSession", then you must also redefine this
	// virtual function in order
	// to create new objects of your subclass:
	...
	HashTable *fServerMediaSessions; // maps 'stream name' strings to
	        // "ServerMediaSession" objects
	HashTable *fClientConnections; // the "ClientConnection" objects that we're using
	HashTable *fClientConnectionsForHTTPTunneling; // maps client-supplied
	        // 'session cookie' strings to
	        // "RTSPClientConnection"s
	// (used only for optional RTSP-over-HTTP tunneling)
	HashTable *fClientSessions; // maps 'session id' strings to
	...
	unsigned fMaxAliveClientNumber;
};

////////// A subclass of "RTSPServer" that implements the "REGISTER" command to
///set up proxying on the specified URL //////////

class RTSPServerWithREGISTERProxying : public RTSPServer {
    public:
	static RTSPServerWithREGISTERProxying *
	createNew(UsageEnvironment &env, Port ourPort = 554, UserAuthenticationDatabase *authDatabase = NULL,
	          UserAuthenticationDatabase *authDatabaseForREGISTER = NULL, unsigned reclamationTestSeconds = 65,
	          Boolean streamRTPOverTCP = False, int verbosityLevelForProxying = 0);
