...
class H265VideoRTPSource : public MultiFramedRTPSource {
    public:
	static H265VideoRTPSource *createNew(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
	                                     Boolean expectDONFields = False, unsigned rtpTimestampFrequency = 90000);
	// "expectDONFields" is True iff we expect incoming H.265/RTP packets to
	// contain
	// DONL and DOND fields.  I.e., if "tx-mode == "MST" or sprop-depack-buf-nalus
	// > 0".
	...
	// the 'absolute decoding order number (AbsDon)' for the most-recently
	// delivered NAL unit

    protected:
	H265VideoRTPSource(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
