...
unsigned int JPEGFrameParser::scanJpegMarker(const unsigned char *data, unsigned int size, unsigned int *offset)
{
...
int JPEGFrameParser::readSOF(const unsigned char *data, unsigned int size, unsigned int *offset)
{
	int i, j;
	CompInfo elem;
	CompInfo info[3] = {
	...
	*offset += sof_size;
	...
	/* ERRORS */
wrong_size:
	fprintf(stderr, "Wrong SOF size\n");
	return -1;

wrong_length:
	fprintf(stderr, "Wrong SOF length\n");
	return -1;
...
unsigned int JPEGFrameParser::readDQT(const unsigned char *data, unsigned int size, unsigned int offset)
{
	...
	while (quant_size > 0) {
		...
		//printf( "Copy quantization table: %u\n", id);
	...
	/* ERRORS */
too_small:
	fprintf(stderr, "DQT is too small\n");
	return size;

small_quant_size:
	fprintf(stderr, "Quantization table is too small\n");
	return size;
...
int JPEGFrameParser::readDRI(const unsigned char *data, unsigned int size, unsigned int *offset)
{
