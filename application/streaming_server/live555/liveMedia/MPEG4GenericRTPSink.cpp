        ...
        , fConfigString(strDup(configString))
{
	...
	sprintf(fmtp, fmtpFmt, rtpPayloadType(), strcmp(fSDPMediaTypeString, "video") == 0 ? 4 : 5, fMPEG4Mode,
	        fConfigString);
...
{
	return new MPEG4GenericRTPSink(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency, sdpMediaTypeString,
	                               mpeg4Mode, configString, numChannels);
}

Boolean MPEG4GenericRTPSink::frameCanAppearAfterPacketStart(unsigned char const * /*frameStart*/,
...
void MPEG4GenericRTPSink::doSpecialFrameHandling(unsigned fragmentationOffset, unsigned char *frameStart,
