...
RTSPServerSupportingHTTPStreaming::RTSPServerSupportingHTTPStreaming(UsageEnvironment &env, int ourSocket,
                                                                     Port rtspPort,
...
RTSPServerSupportingHTTPStreaming::RTSPClientConnectionSupportingHTTPStreaming::
        RTSPClientConnectionSupportingHTTPStreaming(RTSPServer &ourServer, int clientSocket,
...
void RTSPServerSupportingHTTPStreaming::RTSPClientConnectionSupportingHTTPStreaming::handleHTTPCmd_StreamingGET(
        char const *urlSuffix, char const * /*fullRequestStr*/)
{
	// If "urlSuffix" ends with
	// "?segment=<offset-in-seconds>,<duration-in-seconds>", then strip this off,
	// and send the
	// specified segment.  Otherwise, construct and send a playlist that consists
	// of segments from the specified file.
	do {
		...
		do {
			...
			// We can't send multi-subsession streams over HTTP (because there's no
			// defined way to multiplex more than one subsession).
			...
			if (subsession == NULL) {
				// Treat an 'empty' ServerMediaSession the same as one that doesn't
				// exist at all:
				handleHTTPCmd_notFound();
				break;
			}

			// Call "getStreamParameters()" to create the stream's source.  (Because
			// we're not actually streaming via RTP/RTCP, most
			...
			// Seek the stream source to the desired place, with the desired duration,
			// and (as a side effect) get the number of bytes:
			double dOffsetInSeconds = (double)offsetInSeconds;
			u_int64_t numBytes;
			subsession->seekStream(fClientSessionId, streamToken, dOffsetInSeconds,
			                       (double)durationInSeconds, numBytes);
			unsigned numTSBytesToStream = (unsigned)numBytes;

			if (numTSBytesToStream == 0) {
				// For some reason, we do not know the size of the requested range.  We
			...
			snprintf((char *)fResponseBuffer, sizeof fResponseBuffer,
			         ...
			         "\r\n",
			         dateHeader(), LIVEMEDIA_LIBRARY_VERSION_STRING, lastModifiedHeader(streamName),
			         numTSBytesToStream);
			// Send the response now, because we're about to add more data (from the
			// source):
	...
	// "?segment=<offset-in-seconds>,<duration-in-seconds>".
	// Construct and send a playlist that describes segments from the specified
	// file.
	...
	// To be able to construct a playlist for the requested file, we need to know
	// its duration:
	...
	char const *const playlistPrefixFmt = "#EXTM3U\r\n"
	                                      "#EXT-X-ALLOW-CACHE:YES\r\n"
	                                      "#EXT-X-MEDIA-SEQUENCE:0\r\n"
	                                      "#EXT-X-TARGETDURATION:%d\r\n";
	unsigned const playlistPrefixFmt_maxLen = strlen(playlistPrefixFmt) + maxIntLen;

	char const *const playlistMediaFileSpecFmt = "#EXTINF:%d,\r\n"
	                                             "%s?segment=%d,%d\r\n";
	unsigned const playlistMediaFileSpecFmt_maxLen =
	        strlen(playlistMediaFileSpecFmt) + maxIntLen + strlen(urlSuffix) + 2 * maxIntLen;

	char const *const playlistSuffixFmt = "#EXT-X-ENDLIST\r\n";
	unsigned const playlistSuffixFmt_maxLen = strlen(playlistSuffixFmt);

	// Figure out the 'target duration' that will produce a playlist that will fit
	// in our response buffer.  (But make it at least 10s.)
	...
	// playlist):
	send(fClientOutputSocket, (char const *)fResponseBuffer, strlen((char *)fResponseBuffer), 0);
	fResponseBuffer[0] = '\0'; // We've already sent the response.  This tells the
...
void RTSPServerSupportingHTTPStreaming::RTSPClientConnectionSupportingHTTPStreaming::afterStreaming(void *clientData)
{
	...
	if (clientConnection->fRecursionCount > 0) {
		...
		clientConnection->fIsActive = False; // will cause the object to get deleted
