...
FramedSource *OggFileServerMediaSubsession::createNewStreamSource(unsigned clientSessionId, unsigned &estBitrate)
{
	...
	return fOurDemux.ourOggFile()->createSourceForStreaming(baseSource, fTrack->trackNumber, estBitrate,
	                                                        fNumFiltersInFrontOfTrack);
}

RTPSink *OggFileServerMediaSubsession::createNewRTPSink(Groupsock *rtpGroupsock, unsigned char rtpPayloadTypeIfDynamic,
                                                        FramedSource * /*inputSource*/)
{
	return fOurDemux.ourOggFile()->createRTPSinkForTrackNumber(fTrack->trackNumber, rtpGroupsock,
