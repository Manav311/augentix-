...
// Reset the fid to the beginning of the file:
		...
		fprintf(stderr,
		        ...
		        profile, sampling_frequency_index, samplingFrequencyTable[sampling_frequency_index],
...
ADTSAudioFileSource::ADTSAudioFileSource(UsageEnvironment &env, FILE *fid, u_int8_t profile,
...
void ADTSAudioFileSource::doGetNextFrame()
{
	...
	fprintf(stderr, "Read frame: syncword 0x%x, protection_absent %d, frame_length %d\n", syncword,
	        protection_absent, frame_length);
	if (syncword != 0xFFF)
