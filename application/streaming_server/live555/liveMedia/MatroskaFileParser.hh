...
class MatroskaFileParser : public StreamParser {
    public:
	MatroskaFileParser(MatroskaFile &ourFile, FramedSource *inputSource, FramedSource::onCloseFunc *onEndFunc,
	                   void *onEndClientData, MatroskaDemux *ourDemux = NULL);
	virtual ~MatroskaFileParser();

	void seekToTime(double &seekNPT);

	// StreamParser 'client continue' function:
	static void continueParsing(void *clientData, unsigned char *ptr, unsigned size,
	...
	// Parsing functions:
	Boolean parse();
	// returns True iff we have finished parsing to the end of all 'Track' headers
	// (on initialization)
	...
	unsigned fCurOffsetWithinFrame,
	        fSavedCurOffsetWithinFrame; // used if track->haveSubframes()
