...
H264or5VideoStreamDiscreteFramer::H264or5VideoStreamDiscreteFramer(int hNumber, UsageEnvironment &env,
...
void H264or5VideoStreamDiscreteFramer::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
                                                          ...
                                                          unsigned durationInMicroseconds)
{
	...
	// Begin by checking for a (likely) common error: NAL units that (erroneously)
	// begin with a
	// 0x00000001 or 0x000001 'start code'.  (Those start codes should only be in
	// byte-stream data;
	...
	if (frameSize >= 4 && fTo[0] == 0 && fTo[1] == 0 && ((fTo[2] == 0 && fTo[3] == 1) || fTo[2] == 1)) {
		envir() << "H264or5VideoStreamDiscreteFramer error: MPEG 'start code' seen "
		           "in the input\n";
	...
	// Next, check whether this NAL unit ends the current 'access unit'
	// (basically, a video frame).
