...
{
	return env << "[" << ((r.recordType() & 0x80) != 0 ? "1" : "") << recordTypeStr[r.recordType() & 0x7F] << ":"
	           << (unsigned)r.transportPacketNumber() << ":" << r.startOffset() << "(" << r.size() << ")@"
	           << r.pcr() << "]";
}
#endif
...
void MPEG2IFrameIndexFromTransportStream::doGetNextFrame()
{
	...
	// We need to read some more Transport Stream packets.  Check whether we have
	// room:
	...
	fInputSource->getNextFrame(fInputBuffer, sizeof fInputBuffer, afterGettingFrame, this, handleInputClosure,
	                           this);
}

void MPEG2IFrameIndexFromTransportStream::afterGettingFrame(void *clientData, unsigned frameSize,
...
void MPEG2IFrameIndexFromTransportStream::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
                                                             ...
                                                             unsigned durationInMicroseconds)
{
	...
	if (totalHeaderSize > 5 && (fInputBuffer[5] & 0x10) != 0) {
		// There's a PCR:
		u_int32_t pcrBaseHigh = (fInputBuffer[6] << 24) | (fInputBuffer[7] << 16) | (fInputBuffer[8] << 8) |
		                        fInputBuffer[9];
		...
		if (!fHaveSeenFirstPCR) {
			...
			// (because it indicates buggy Transport Stream data), and compensate for
			// it.
			envir() << "\nWarning: At about " << fLastPCR - fFirstPCR
			        << " seconds into the file, the PCR timestamp decreased - from " << fLastPCR << " to "
			        << pcr << "\n";
	...
	if ((PID != fVideo_PID) || !(adaptation_field_control == 1 || adaptation_field_control == 3) ||
	...
	if (fIsH264) {
		...
		case VISUAL_OBJECT_SEQUENCE_START_CODE:
			...
			while (1) {
				if (!parseToNextCode(nextCode))
					return False;
				if (nextCode == GROUP_START_CODE || nextCode == PICTURE_START_CODE ||
		...
		default: // picture
			...
			while (1) {
				...
				if (nextCode == VIDEO_SEQUENCE_START_CODE ||
				    nextCode == VISUAL_OBJECT_SEQUENCE_START_CODE || nextCode == GROUP_START_CODE ||
				    nextCode == GROUP_VOP_START_CODE || nextCode == PICTURE_START_CODE ||
				    nextCode == VOP_START_CODE)
					break;
				fParseBufferParseEnd += 4; // skip over the code that we just saw
			}
...
{
#ifdef DEBUG
	envir() << "Compacting parse buffer: [" << fParseBufferFrameStart << "," << fParseBufferParseEnd << ","
	        << fParseBufferDataEnd << "]";
#endif
