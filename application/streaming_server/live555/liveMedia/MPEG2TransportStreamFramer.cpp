...
void MPEG2TransportStreamFramer::afterGettingFrame(void *clientData, unsigned frameSize, unsigned /*numTruncatedBytes*/,
...
void MPEG2TransportStreamFramer::afterGettingFrame1(unsigned frameSize, struct timeval presentationTime)
{
	...
	if (syncBytePosition == fFrameSize) {
		...
		fInputSource->getNextFrame(&fTo[fFrameSize], syncBytePosition, afterGettingFrame, this,
	...
	for (unsigned i = 0; i < numTSPackets; ++i) {
		if (!updateTSPacketDurationEstimate(&fTo[i * TRANSPORT_PACKET_SIZE], timeNow)) {
			// We hit a preset limit (based on PCR) within the stream.  Handle this as
			// if the input source has closed:
			handleClosure();
			return;
		}
	}
...
Boolean MPEG2TransportStreamFramer::updateTSPacketDurationEstimate(unsigned char *pkt, double timeNow)
{
	...
	if (pidStatus == NULL) {
		...
		fprintf(stderr, "PID 0x%x, FIRST PCR 0x%08x+%d:%03x == %f @ %f, pkt #%lu\n", pid, pcrBaseHigh,
		        pkt[10] >> 7, pcrExt, clock, timeNow, fTSPacketCount);
#endif
	} else {
		...
		if (fTSPacketDurationEstimate == 0.0) { // we've just started
			...
			fTSPacketDurationEstimate = durationPerPacket * NEW_DURATION_WEIGHT +
...
#ifdef DEBUG_PCR
		fprintf(stderr,
		        "PID 0x%x, PCR 0x%08x+%d:%03x == %f @ %f (diffs %f @ %f), "
		        "pkt #%lu, discon %d => this duration %f, new estimate %f, "
		        "mean PCR period=%f\n",
		        pid, pcrBaseHigh, pkt[10] >> 7, pcrExt, clock, timeNow, clock - pidStatus->firstClock,
		        timeNow - pidStatus->firstRealTime, fTSPacketCount, discontinuity_indicator != 0,
		        durationPerPacket, fTSPacketDurationEstimate, meanPCRPeriod);
#endif
	}
