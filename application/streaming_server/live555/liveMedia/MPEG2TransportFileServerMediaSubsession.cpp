                                                                                            ...
                                                                                            Boolean reuseFirstSource)
{
	...
	if (indexFileName != NULL && reuseFirstSource) {
		...
		// source.  Fix this:
		env << "MPEG2TransportFileServerMediaSubsession::createNew(): ignoring the "
		       "index file name, because \"reuseFirstSource\" is set\n";
		indexFile = NULL;
	} else {
...
MPEG2TransportFileServerMediaSubsession::MPEG2TransportFileServerMediaSubsession(
        UsageEnvironment &env, char const *fileName, MPEG2TransportStreamIndexFile *indexFile, Boolean reuseFirstSource)
...
void MPEG2TransportFileServerMediaSubsession::seekStream(unsigned clientSessionId, void *streamToken, double &seekNPT,
...
{
	return SimpleRTPSink::createNew(envir(), rtpGroupsock, 33, 90000, "video", "MP2T", 1, True,
	                                False /*no 'M' bit*/);
}
...
unsigned long ClientTrickPlayState::updateStateFromNPT(double npt, double streamDuration)
{
	...
	if (streamDuration > 0.0) {
		// fNPT might have changed when we looked it up in the index file.  Adjust
		// "streamDuration" accordingly:
		...
		if (streamDuration > 0.0) {
			...
			if (fNextScale == 1.0f) {
				...
				// Use the index file to figure out how many Transport Packets we get to
				// stream:
				...
				// It'd be difficult to figure out how many Transport Packets we need to
				// stream, so instead set a PCR
				// limit in the trick play stream.  (We rely upon the fact that PCRs in
				// the trick play stream start at 0.0)
...
void ClientTrickPlayState::updateStateOnScaleChange()
{
	...
	if (fNextScale != 1.0f) {
		...
		fTrickModeFilter = MPEG2TransportStreamTrickModeFilter::createNew(env, fOriginalTransportStreamSource,
		                                                                  fIndexFile, int(fNextScale));
...
void ClientTrickPlayState::updateStateOnPlayChange(Boolean reverseToPreviousVSH)
{
	...
	if (fTrickPlaySource == NULL) {
		...
		if (fIndexFile->readIndexRecordValues(fIxRecordNum, transportRecordNum, offset, size, pcr,
		                                      recordType)) {
