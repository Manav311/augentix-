...
FramedSource *OggFile::createSourceForStreaming(FramedSource *baseSource, u_int32_t trackNumber, unsigned &estBitrate,
...
RTPSink *OggFile::createRTPSinkForTrackNumber(u_int32_t trackNumber, Groupsock *rtpGroupsock,
                                              unsigned char rtpPayloadTypeIfDynamic)
{
	...
	if (strcmp(track->mimeType, "audio/VORBIS") == 0) {
		...
		result = SimpleRTPSink::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, 48000, "audio",
...
void OggDemux::handleEndOfFile()
{
	...
	// we don't call the handlers until after we've first iterated through all of
	// the tracks.
