...
#define VIDEO_FRAME_BUFFER_SIZE_BYTES      (512 * 1024UL)
#define AUDIO_FRAME_BUFFER_SIZE_BYTES      (16 * 1024UL)
...
INT32 main(INT32 argc, CHAR* argv[])
{
    ...
    if (!audioCapturerHandle) {
        ...
        if (audioCapturerSetFormat(audioCapturerHandle, AUD_FMT_G711A, AUD_CHN_MONO, AUD_SAM_8K, AUD_BIT_8)) {
    ...
    if (!audioPlayerHandle) {
        ...
        if (audioPlayerSetFormat(audioPlayerHandle, AUD_FMT_AAC, AUD_CHN_MONO, AUD_SAM_44_1K, AUD_BIT_16)) {
    ...
    VideoFormat selectedFormat = VID_FMT_H264;
    if (videoCapability.formats & (1 << (VID_FMT_H265 - 1))) {
        selectedFormat = VID_FMT_H265;
    }

    VideoResolution selectedResolution = VID_RES_720P;
    if (videoCapability.resolutions & (1 << (VID_RES_2K - 1))) {
        selectedResolution = VID_RES_2K;
    }
    else if (videoCapability.resolutions & (1 << (VID_RES_1080P - 1))) {
        selectedResolution = VID_RES_1080P;
    }
    CHK_STATUS_ERR(videoCapturerSetFormat(videoCapturerHandle, selectedFormat, selectedResolution), STATUS_INVALID_OPERATION, "Unable to set video format");
