...
int AAC_encoderInit(TyMediaAACHandle *phdl, int channels, int sample_rate, int bitrate)
{
    ...
    if ((err = aacEncInfo(phdl->enc, &info)) != AACENC_OK) {
        log_err("Unable to get the encoder info:%d\n", err);
        return err;
    }

    phdl->pcmBuf = malloc(info.frameLength * 4);
    if (phdl->pcmBuf == NULL) {
        log_err("OOM unable allocate buffer for AAC encoder!\n");
        return -ENOMEM;
    }
    phdl->frameSize = info.frameLength * 2;
    phdl->pcmLen = 0;

    log_notice("AAC frame_size = %d\n", phdl->frameSize);

    return 0;
}

int AAC_encoderGetData(TyMediaAACHandle *phdl, char *pcm, int nb_pcm, int nb_samples, char *aac, int *pnb_aac)
{
    (void) nb_samples;
    ...
    memcpy(phdl->pcmBuf + phdl->pcmLen, pcm, nb_pcm);
    phdl->pcmLen += nb_pcm;

    if (phdl->pcmLen < phdl->frameSize) {
        return -1;
    }
    ...
    iargs.numInSamples = phdl->frameSize / 2;
    ibuf.numBufs = 1;
    ibuf.bufs = (void **)&phdl->pcmBuf;
    ibuf.bufferIdentifiers = &iidentify;
    ibuf.bufSizes = &phdl->frameSize;
    ...
    *pnb_aac = oargs.numOutBytes;
    int left_len = phdl->pcmLen - phdl->frameSize;

    if (left_len > 0) {
        memmove(phdl->pcmBuf, phdl->pcmBuf + phdl->frameSize, left_len);
