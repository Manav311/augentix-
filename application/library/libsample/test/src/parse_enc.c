...
static void print_mcvc_rc_param(MPI_RC_MODE_E mode, MPI_MCVC_RC_ATTR_S *rc)
{
	...
	case MPI_RC_MODE_VBR:
		printf("VBR: bit_rate = %u, fluc_level = %u, regression_speed = %u, scene_smooth = %u, i_continue_weight = %u, "
		       ...
		       rc->vbr.max_bit_rate, rc->vbr.fluc_level, rc->vbr.regression_speed, rc->vbr.scene_smooth,
		       rc->vbr.i_continue_weight, rc->vbr.quality_level_index, rc->vbr.max_qp, rc->vbr.i_qp_offset,
		       rc->vbr.motion_tolerance_level, rc->vbr.motion_tolerance_qp);
		break;
	case MPI_RC_MODE_CBR:
	...
	case MPI_RC_MODE_SBR:
		printf("SBR: bit_rate = %u, fluc_level = %u, regression_speed = %u, scene_smooth = %u, i_continue_weight = %u, "
		       ...
		       rc->sbr.bit_rate, rc->sbr.fluc_level, rc->sbr.regression_speed, rc->sbr.scene_smooth,
		       rc->sbr.i_continue_weight, rc->sbr.min_qp, rc->sbr.max_qp, rc->sbr.i_qp_offset,
		       rc->sbr.adjust_br_thres_pc, rc->sbr.adjust_step_times, rc->sbr.converge_frame,
		       rc->sbr.motion_tolerance_level, rc->sbr.motion_tolerance_qp);
		break;
	case MPI_RC_MODE_CQP:
...
static void print_vc_rc_param(MPI_VENC_TYPE_E mode, MPI_VC_RC_ATTR_S *rc)
{
	...
	case MPI_RC_MODE_SBR:
		printf("SBR: bit_rate = %u, fluc_level = %u, max_q_factor = %u, min_q_factor = %u, adjust_br_thres_pc = %u, "
		       ...
		       rc->bit_rate, rc->fluc_level, rc->max_q_factor, rc->min_q_factor, rc->adjust_br_thres_pc,
		       rc->adjust_step_times, rc->converge_frame);
		break;
	case MPI_RC_MODE_CQP:
...
void print_venc_attr(MPI_VENC_ATTR_S *p)
{
	...
	case MPI_VENC_TYPE_H264:
		...
		printf("venc_profile = %d, rc_mode = %d, gop_size = %u, frm_rate_o = %d, max_frame_size = %u\n",
		       p->h264.profile, p->h264.rc.mode, p->h264.rc.gop, p->h264.rc.frm_rate_o,
		       p->h264.rc.max_frame_size);
		print_mcvc_rc_param(p->h264.rc.mode, &p->h264.rc);
		break;
	case MPI_VENC_TYPE_H265:
		printf("H265:\n");
		printf("venc_profile = %d, rc_mode = %d, gop_size = %u, frm_rate_o = %d, max_frame_size = %u\n",
		       p->h265.profile, p->h265.rc.mode, p->h265.rc.gop, p->h265.rc.frm_rate_o,
		       p->h265.rc.max_frame_size);
