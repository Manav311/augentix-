...
extern int calcNewLine(int startx, int starty, int endx, int endy, int thickness, OsdRegion *targetReg,
                       OsdLine *output);
extern uint32_t CEILINGALIGN16(uint32_t x);
extern uint32_t ALIGN16(uint32_t x);
...
INT32 moveOsdInstance(MPI_OSD_BIND_ATTR_S *bind, OSD_HANDLE *handle)
{
	...
	if (ret != MPI_SUCCESS) {
		libosd_log_err("Failed to get MPI_getOsdBindAttr, %d", ret);
	}
	bind_old.point.x = bind->point.x;
	bind_old.point.y = bind->point.y;
	ret = MPI_setOsdBindAttr(*handle, &bind_old);
	if (ret != MPI_SUCCESS) {
		libosd_log_err("Set OSD %d to encoder channel %d failed. err: %d", *handle, bind->idx.chn, ret);
	}
...
int main(int argc, char **argv)
{
	...
	libosd_log_info("stop ENC %dn", channel);
	if (channel == 1) {
		stopEnc(0);
	}
	stopEnc(channel);
	...
	OsdRegion region[4] = {
		{ .startX = 32, .startY = 32, .width = 48, .height = 48 },
		{ .startX = chn_width / 2 + 32, .startY = 32, .width = 480, .height = 480 },
		{ .startX = chn_width / 2 + 32, .startY = chn_height / 2 + 32, .width = 720, .height = 480 },
		{ .startX = 32, .startY = chn_height / 2 + 32, .width = 480, .height = 480 }
	};
#if 0
	/*for move region[3]*/
	OsdRegion region2Move;
	memcpy(&region2move, &region[3], sizeof(OsdRegion));
#endif
	...
	OsdText txt = {
		.size = 35, .background = WHITE, .color = { 0x07, 0x02, 0x00 }, .outline_color = { 0x07, 0x0c, 0x00 }
	};
	...
	for (int i = 0; i < OSD_NUM; i++) {
		if (MPI_updateOsdCanvas(g_osd_chn_handle[0][i]) != MPI_SUCCESS) {
