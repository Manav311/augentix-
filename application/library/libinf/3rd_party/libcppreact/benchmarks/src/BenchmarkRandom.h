...
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D, typename TValue> class RandomGraphGenerator {
    public:
	...
	void Generate()
	{
		...
		Func1T f1Slow = [this](TValue a1) {
			SlowDelayFunc();
			return Function1(a1);
		};
		Func2T f2Slow = [this](TValue a1, TValue a2) {
			SlowDelayFunc();
			return Function2(a1, a2);
		};
		Func3T f3Slow = [this](TValue a1, TValue a2, TValue a3) {
			SlowDelayFunc();
			return Function3(a1, a2, a3);
		};
		Func4T f4Slow = [this](TValue a1, TValue a2, TValue a3, TValue a4) {
			SlowDelayFunc();
			return Function4(a1, a2, a3, a4);
		};

		Func1T f1Fast = [this](TValue a1) {
			FastDelayFunc();
			return Function1(a1);
		};
		Func2T f2Fast = [this](TValue a1, TValue a2) {
			FastDelayFunc();
			return Function2(a1, a2);
		};
		Func3T f3Fast = [this](TValue a1, TValue a2, TValue a3) {
			FastDelayFunc();
			return Function3(a1, a2, a3);
		};
		Func4T f4Fast = [this](TValue a1, TValue a2, TValue a3, TValue a4) {
			FastDelayFunc();
			return Function4(a1, a2, a3, a4);
		};
			...
			for (int w = 0; w < Width; w++) {
				...
				if (edgeNodeIt != edgeNodes.end() && cur == *edgeNodeIt) {
					...
					if (edgeCount == 2) {
						...
						nodes[cur] =
						        (nodes[kNode0], nodes[rNode1], nodes[rNode2], nodes[rNode3])
						                ->*f4;
					}
				} else {
...
struct BenchmarkParams_Random {
	BenchmarkParams_Random(int w, int h, int k, int fastDelay, int slowDelay, int edgeCount, int slowCount,
	                       bool randInput, int edgeSeed = 2014, int slowSeed = 46831)
	        : W(w)
	        , H(h)
	        , K(k)
	        , FastDelay(fastDelay)
	...
	{
		out << "W = " << W << ", H = " << H << ", K = " << K << ", FastDelay = " << FastDelay
		    << ", SlowDelay = " << SlowDelay << ", EdgeCount = " << EdgeCount << ", SlowCount = " << SlowCount
		    << ", WithRandomInput = " << WithRandomInput << ", EdgeSeed = " << EdgeSeed
		    << ", SlowSeed = " << SlowSeed;
	}
...
template <typename D> struct Benchmark_Random : public BenchmarkBase<D> {
			...
			generator.FastDelayFunc = [&initializing, &params] {
				if (!initializing) {
					...
					while (std::chrono::high_resolution_clock::now() - t0 <
					       std::chrono::milliseconds(params.FastDelay))
						;
				}
			};
		} else {
		...
		if (params.SlowDelay > 0) {
			generator.SlowDelayFunc = [&initializing, &params] {
				if (!initializing) {
					auto t0 = std::chrono::high_resolution_clock::now();
					while (std::chrono::high_resolution_clock::now() - t0 <
					       std::chrono::milliseconds(params.SlowDelay))
						;
				}
			};
		} else {
