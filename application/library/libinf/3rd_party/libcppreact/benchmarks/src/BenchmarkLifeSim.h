...
/// Time
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D> class Time {
    public:
	...
	SignalT<int> Season = MakeSignal(DayOfYear, [](int day) -> int { return day < 180 ? winter : summer; });
};

///////////////////////////////////////////////////////////////////////////////////////////////////
/// Region
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D> class Region {
	Time<D> &theTime;
	...
	SignalT<int> AnimalCount =
	        Iterate(EnterOrLeave, 0, [](int m, int count) { return m == enter ? count + 1 : count - 1; });

	SignalT<int> FoodPerDay = MakeSignal(theTime.Season, calculateFoodPerDay);

	SignalT<int> FoodOutputPerDay = MakeSignal(With(FoodPerDay, AnimalCount), calculateFoodOutputPerDay);
	...
	bool IsInRegion(PositionT pos) const
	{
		...
		return get<0>(Bounds) <= pos.first && pos.first <= get<1>(Bounds) && get<2>(Bounds) <= pos.second &&
		       pos.second <= get<3>(Bounds);
	}
...
/// World
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D> class World {
    public:
	USING_REACTIVE_DOMAIN(D)
...
template <typename D> class Animal {
	...
	Animal(Time<D> &time, World<D> &world, Region<D> *initRegion, unsigned seed)
	        ...
	        , Position(Iterate(Moving, initRegion->Center(), With(CurrentRegion),
	        ...
	        , NewRegion(MakeSignal(Position, [this](PositionT pos) { return theWorld.GetRegion(pos); }))
	        , regionChangeContinuation_(MakeContinuation(RegionChanged, With(CurrentRegion),
...
template <typename D> struct Benchmark_LifeSim : public BenchmarkBase<D> {
