...
template <typename T> struct Incrementer {
...
template <typename T> struct Decrementer {
	T operator()(Token, T v) const
	{
		return v - 1;
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename TParams> class OperationsTest : public testing::Test {
    public:
	template <EPropagationMode mode> class MyEngine : public TParams::template EngineT<mode> {
	};
...
TYPED_TEST_P(OperationsTest, Monitor1)
{
	...
	auto obs = Observe(Monitor(target).Filter(filterFunc), [&](int v) { results.push_back(v); });
...
TYPED_TEST_P(OperationsTest, IterateByRef1)
{
	...
	auto src = MakeEventSource<D, int>();
	auto f = Iterate(src, std::vector<int>(), [](int d, std::vector<int> &v) { v.push_back(d); });
...
TYPED_TEST_P(OperationsTest, IterateByRef2)
{
	...
	auto x = Iterate(src, std::vector<int>(), [](Token, std::vector<int> &v) { v.push_back(123); });
...
TYPED_TEST_P(OperationsTest, SyncedTransform1)
{
	...
	auto out1 = Transform(src1, With(sum, prod, diff),
	                      [](Token, int sum, int prod, int diff) { return make_tuple(sum, prod, diff); });

	auto out2 = Transform(src2, With(sum, prod, diff),
	                      [](int e, int sum, int prod, int diff) { return make_tuple(e, sum, prod, diff); });
...
TYPED_TEST_P(OperationsTest, SyncedIterate1)
{
	...
	auto out1 =
	        Iterate(src1, make_tuple(0, 0), With(op1, op2), [](Token, const tuple<int, int> &t, int op1, int op2) {
		        return make_tuple(get<0>(t) + op1, get<1>(t) + op2);
	        });

	auto out2 = Iterate(src2, make_tuple(0, 0, 0), With(op1, op2),
...
TYPED_TEST_P(OperationsTest, SyncedIterate2)
{
	...
	auto out1 = Iterate(src1, vector<int>{}, With(op1, op2), [](Token, vector<int> &v, int op1, int op2) -> void {
		v.push_back(op1);
		v.push_back(op2);
	});

	auto out2 = Iterate(src2, vector<int>{}, With(op1, op2), [](int e, vector<int> &v, int op1, int op2) -> void {
		v.push_back(e);
		v.push_back(op1);
		v.push_back(op2);
	});
...
TYPED_TEST_P(OperationsTest, SyncedIterate3)
{
	...
	auto out1 =
	        Iterate(src1, make_tuple(0, 0), With(op1, op2),
	                [](EventRange<Token> range, const tuple<int, int> &t, int op1, int op2) {
		                return make_tuple(get<0>(t) + (op1 * range.Size()), get<1>(t) + (op2 * range.Size()));
	                });
	...
	auto out2 = Iterate(src2, make_tuple(0, 0, 0), With(op1, op2),
	                    [](EventRange<int> range, const tuple<int, int, int> &t, int op1, int op2) {
		                    ...
		                    return make_tuple(get<0>(t) + sum, get<1>(t) + (op1 * range.Size()),
...
TYPED_TEST_P(OperationsTest, SyncedIterate4)
{
	...
	auto out1 = Iterate(src1, vector<int>{}, With(op1, op2),
	...
	auto out2 = Iterate(src2, vector<int>{}, With(op1, op2),
...
TYPED_TEST_P(OperationsTest, SyncedEventFilter1)
{
	...
	auto filtered = Filter(in, With(sig1, sig2),
	                       [](const string &s, int sig1, int sig2) { return s == "Hello World" && sig1 > sig2; });
...
TYPED_TEST_P(OperationsTest, SyncedEventTransform1)
{
	...
	auto transformed =
	        Transform(merged, With(first, last), [](string s, const string &first, const string &last) -> string {
...
TYPED_TEST_P(OperationsTest, SyncedEventProcess1)
{
	...
	auto processed =
	        Process<float>(merged, With(mult), [&](EventRange<int> range, EventEmitter<float> out, int mult) {
		        for (const auto &e : range) {
			        *out = 0.1f * e * mult;
			        *out = 1.5f * e * mult;
		        }
...
///////////////////////////////////////////////////////////////////////////////////////////////////
REGISTER_TYPED_TEST_CASE_P(OperationsTest, Iterate1, Iterate2, Iterate3, Monitor1, Hold1, Pulse1, Snapshot1,
                           IterateByRef1, IterateByRef2, SyncedTransform1, SyncedIterate1, SyncedIterate2,
                           SyncedIterate3, SyncedIterate4, SyncedEventFilter1, SyncedEventTransform1,
                           SyncedEventProcess1);
