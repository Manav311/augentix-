...
/// SignalTest fixture
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename TParams> class TransactionTest : public testing::Test {
    public:
...
TYPED_TEST_P(TransactionTest, Concurrent1)
{
	...
	Observe(n17, [&](int v) { results.push_back(v); });
...
TYPED_TEST_P(TransactionTest, Concurrent2)
{
	...
	Observe(next, [&](int v) { results.push_back(v); });
...
TYPED_TEST_P(TransactionTest, Merging1)
{
	...
	Observe(n2, [&](int v) { results.push_back(v); });

	// Todo: improve this as it'll fail occasionally
	shouldSpin = true;
	std::thread t1([&] { DoTransaction<D>(allow_merging, [&] { n1 <<= 2; }); });
	std::this_thread::sleep_for(std::chrono::milliseconds(2000));
	std::thread t2([&] { DoTransaction<D>(allow_merging, [&] { n1 <<= 3; }); });
	std::this_thread::sleep_for(std::chrono::milliseconds(1000));
	std::thread t3([&] { DoTransaction<D>(allow_merging, [&] { n1 <<= 4; }); });
	std::this_thread::sleep_for(std::chrono::milliseconds(1000));
	std::thread t4([&] { DoTransaction<D>(allow_merging, [&] { n1 <<= 5; }); });
	std::this_thread::sleep_for(std::chrono::milliseconds(1000));
	shouldSpin = false;
...
TYPED_TEST_P(TransactionTest, Async1)
{
	...
	Observe(s1, [&](int v) { results.push_back(v); });

	TransactionStatus st;

	AsyncTransaction<D>(st, [&] { in <<= 10; });

	AsyncTransaction<D>(st, [&] { in <<= 20; });

	AsyncTransaction<D>(st, [&] { in <<= 30; });
...
TYPED_TEST_P(TransactionTest, AsyncMerging1)
{
	...
	AsyncTransaction<D>(allow_merging, st, [&] { in <<= 20; });

	AsyncTransaction<D>(allow_merging, st, [&] { in <<= 30; });

	// Can't be merged
	AsyncTransaction<D>(st, [&] { in <<= 40; });

	// These two should be merged again
	AsyncTransaction<D>(allow_merging, st, [&] { in <<= 50; });

	AsyncTransaction<D>(allow_merging, st, [&] { in <<= 60; });
...
TYPED_TEST_P(TransactionTest, Continuation3)
{
	...
	auto contL = MakeContinuation<L, R>(Monitor(srcL), With(depL1, depL2), [&](int v, int depL1, int depL2) {
...
TYPED_TEST_P(TransactionTest, Continuation4)
{
	...
	auto cont = MakeContinuation(vect, [&](const vector<int> &v) {
		if (count == 0) {
			...
			vect.Modify([](vector<int> &v) { v.push_back(50); });
		} else if (count == 1) {
			ASSERT_EQ(v[1], 50);
	...
	vect.Modify([](vector<int> &v) { v.push_back(30); });

	ASSERT_EQ(count, 3);
}

///////////////////////////////////////////////////////////////////////////////////////////////////
REGISTER_TYPED_TEST_CASE_P(TransactionTest, Concurrent1, Concurrent2, Concurrent3, Merging1, Async1, AsyncMerging1,
                           Continuation1, Continuation2, Continuation3, Continuation4);
