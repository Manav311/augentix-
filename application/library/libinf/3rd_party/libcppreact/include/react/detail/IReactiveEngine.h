...
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D, typename TEngine> struct EngineInterface {
	...
	{
		REACT_LOG(D::Log().template Append<InputNodeAdmissionEvent>(GetObjectId(node), turn.Id()));
	...
	{
		REACT_LOG(D::Log().template Append<NodeCreateEvent>(GetObjectId(node), node.GetNodeType()));
		Instance().OnNodeCreate(node);
	}

	static void OnNodeDestroy(NodeT &node)
	{
		REACT_LOG(D::Log().template Append<NodeDestroyEvent>(GetObjectId(node)));
		Instance().OnNodeDestroy(node);
	}

	static void OnNodeAttach(NodeT &node, NodeT &parent)
	{
		REACT_LOG(D::Log().template Append<NodeAttachEvent>(GetObjectId(node), GetObjectId(parent)));
		Instance().OnNodeAttach(node, parent);
	}

	static void OnNodeDetach(NodeT &node, NodeT &parent)
	{
		REACT_LOG(D::Log().template Append<NodeDetachEvent>(GetObjectId(node), GetObjectId(parent)));
		Instance().OnNodeDetach(node, parent);
	}

	static void OnNodePulse(NodeT &node, TurnT &turn)
	{
		REACT_LOG(D::Log().template Append<NodePulseEvent>(GetObjectId(node), turn.Id()));
		Instance().OnNodePulse(node, turn);
	}

	static void OnNodeIdlePulse(NodeT &node, TurnT &turn)
	{
		REACT_LOG(D::Log().template Append<NodeIdlePulseEvent>(GetObjectId(node), turn.Id()));
		Instance().OnNodeIdlePulse(node, turn);
	}

	static void OnDynamicNodeAttach(NodeT &node, NodeT &parent, TurnT &turn)
	{
		REACT_LOG(D::Log().template Append<DynamicNodeAttachEvent>(GetObjectId(node), GetObjectId(parent),
		                                                           turn.Id()));
	...
	{
		REACT_LOG(D::Log().template Append<DynamicNodeDetachEvent>(GetObjectId(node), GetObjectId(parent),
		                                                           turn.Id()));
...
/// EPropagationMode
///////////////////////////////////////////////////////////////////////////////////////////////////
enum EPropagationMode { sequential_propagation, parallel_propagation };
