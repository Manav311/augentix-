...
template <typename D> class InputManager;
...
enum ETransactionFlags { allow_merging = 1 << 0 };
...
/// EInputMode
///////////////////////////////////////////////////////////////////////////////////////////////////
enum EInputMode { consecutive_input, concurrent_input };

///////////////////////////////////////////////////////////////////////////////////////////////////
/// IContinuationTarget
///////////////////////////////////////////////////////////////////////////////////////////////////
struct IContinuationTarget {
	virtual void AsyncContinuation(TransactionFlagsT, const WaitingStatePtrT &, TransactionFuncT &&) = 0;
};

///////////////////////////////////////////////////////////////////////////////////////////////////
/// ThreadLocalInputState
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename = void> struct ThreadLocalInputState {
	static REACT_TLS bool IsTransactionActive;
};

template <typename T> REACT_TLS bool ThreadLocalInputState<T>::IsTransactionActive(false);

///////////////////////////////////////////////////////////////////////////////////////////////////
/// ContinuationManager
///////////////////////////////////////////////////////////////////////////////////////////////////
// Interface
template <EPropagationMode> class ContinuationManager {
    public:
	void StoreContinuation(IContinuationTarget &target, TransactionFlagsT flags, TransactionFuncT &&cont);
	...
	template <typename D> void DetachQueuedObservers();
};

// Non thread-safe implementation
template <> class ContinuationManager<sequential_propagation> {
	struct Data_ {
...
template <> class ContinuationManager<parallel_propagation> {
	struct Data_ {
		Data_(IContinuationTarget *target, TransactionFlagsT flags, TransactionFuncT &&func)
		        : Target(target)
		        , Flags(flags)
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// Interface
template <EInputMode> class TransactionQueue {
    public:
	class QueueEntry {
	    public:
	...
	template <typename F> bool TryMergeSync(F &&inputFunc);

	template <typename F> bool TryMergeAsync(F &&inputFunc, WaitingStatePtrT &&waitingStatePtr);
...
// Non thread-safe implementation
template <> class TransactionQueue<consecutive_input> {
    public:
	class QueueEntry {
	    public:
		explicit QueueEntry(TransactionFlagsT flags)
...
// Thread-safe implementation
template <> class TransactionQueue<concurrent_input> {
    public:
	class QueueEntry {
	    public:
		explicit QueueEntry(TransactionFlagsT flags)
		...
		template <typename F, typename P> bool TryMerge(F &&inputFunc, P &&waitingStatePtr)
		{
			if (!isMergeable_)
				return false;
...
// Interface
template <typename D, EInputMode> class AsyncWorker {
    public:
...
// Disabled
template <typename D> struct AsyncWorker<D, consecutive_input> {
    public:
	AsyncWorker(InputManager<D> &mgr)
	{
	}
...
// Enabled
template <typename D> struct AsyncWorker<D, concurrent_input> {
...
/// InputManager
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D> class InputManager : public IContinuationTarget {
    private:
	...
	template <typename, EInputMode> friend class AsyncWorker;
	...
	template <typename F> void DoTransaction(TransactionFlagsT flags, F &&func)
	{
	...
	template <typename R, typename V> void AddInput(R &r, V &&v)
	{
	...
	template <typename R, typename F> void ModifyInput(R &r, const F &func)
	{
		if (ThreadLocalInputState<>::IsTransactionActive) {
	...
	//IContinuationTarget
	virtual void AsyncContinuation(TransactionFlagsT flags, const WaitingStatePtrT &waitingStatePtr,
	...
	template <typename R, typename V> void addSimpleInput(R &r, V &&v)
	{
		QueueEntryT tr(0);
	...
	template <typename R, typename F> void modifySimpleInput(R &r, const F &func)
	{
		QueueEntryT tr(0);
	...
	template <typename R, typename V> void addTransactionInput(R &r, V &&v)
	{
		r.AddInput(std::forward<V>(v));
		changedInputs_.push_back(&r);
	}

	template <typename R, typename F> void modifyTransactionInput(R &r, const F &func)
	{
		r.ModifyInput(func);
		changedInputs_.push_back(&r);
	}
	...
	void processAsyncQueue()
	{
		...
		while (true) {
			...
			if (canMerge &&
			    transactionQueue_.TryMergeAsync(std::move(item.Func), std::move(item.WaitingStatePtr)))
				continue;
...
template <typename D> class DomainSpecificInputManager {
    public:
