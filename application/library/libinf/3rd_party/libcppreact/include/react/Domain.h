...
template <typename D, typename E> class Events;
...
template <typename D> class Observer;

template <typename D> class ScopedObserver;

template <typename D> class Reactor;
...
class TransactionStatus {
	...
	template <typename D, typename F> friend void AsyncTransaction(TransactionStatus &status, F &&func);
...
/// Continuation
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename D, typename D2 = D> class Continuation : public REACT_IMPL::ContinuationBase<D, D2> {
    private:
...
auto MakeContinuation(TransactionFlagsT flags, const Signal<D, S> &trigger, FIn &&func) -> Continuation<D, DOut>
{
	static_assert(DOut::is_concurrent, "MakeContinuation: Target domain does not support concurrent input.");
...
auto MakeContinuation(TransactionFlagsT flags, const Events<D, E> &trigger, FIn &&func) -> Continuation<D, DOut>
{
	static_assert(DOut::is_concurrent, "MakeContinuation: Target domain does not support concurrent input.");
	...
	using WrapperT =
	        typename std::conditional<IsCallableWith<F, void, EventRange<E> >::value, F,
	                                  typename std::conditional<IsCallableWith<F, void, E>::value,
	                                                            AddContinuationRangeWrapper<E, F>, void>::type>::type;
...
auto MakeContinuation(TransactionFlagsT flags, const Events<D, E> &trigger, const SignalPack<D, TDepValues...> &depPack,
                      FIn &&func) -> Continuation<D, DOut>
{
	...
	using WrapperT = typename std::conditional<
	        IsCallableWith<F, void, EventRange<E>, TDepValues...>::value, F,
	        typename std::conditional<IsCallableWith<F, void, E, TDepValues...>::value,
	                                  AddContinuationRangeWrapper<E, F, TDepValues...>, void>::type>::type;
		...
		auto operator()(const Signal<D, TDepValues> &... deps) -> Continuation<D, DOut>
		{
			return Continuation<D, DOut>(
			        std::make_shared<SyncedContinuationNode<D, DOut, E, WrapperT, TDepValues...> >(
			                MyFlags, GetNodePtr(MyTrigger), std::forward<FIn>(MyFunc),
			                GetNodePtr(deps)...));
		}
	...
	return REACT_IMPL::apply(NodeBuilder_(flags, trigger, std::forward<FIn>(func)), depPack.Data);
}

template <typename D, typename DOut = D, typename E, typename FIn, typename... TDepValues>
auto MakeContinuation(const Events<D, E> &trigger, const SignalPack<D, TDepValues...> &depPack, FIn &&func)
        -> Continuation<D, DOut>
{
...
/// DoTransaction
///////////////////////////////////////////////////////////////////////////////////////////////
template <typename D, typename F> void DoTransaction(F &&func)
{
	using REACT_IMPL::DomainSpecificInputManager;
	DomainSpecificInputManager<D>::Instance().DoTransaction(0, std::forward<F>(func));
}

template <typename D, typename F> void DoTransaction(TransactionFlagsT flags, F &&func)
{
	using REACT_IMPL::DomainSpecificInputManager;
...
/// AsyncTransaction
///////////////////////////////////////////////////////////////////////////////////////////////
template <typename D, typename F> void AsyncTransaction(F &&func)
{
	static_assert(D::is_concurrent, "AsyncTransaction: Domain does not support concurrent input.");

	using REACT_IMPL::DomainSpecificInputManager;
	DomainSpecificInputManager<D>::Instance().AsyncTransaction(0, nullptr, std::forward<F>(func));
}

template <typename D, typename F> void AsyncTransaction(TransactionFlagsT flags, F &&func)
{
	static_assert(D::is_concurrent, "AsyncTransaction: Domain does not support concurrent input.");

	using REACT_IMPL::DomainSpecificInputManager;
	DomainSpecificInputManager<D>::Instance().AsyncTransaction(flags, nullptr, std::forward<F>(func));
}

template <typename D, typename F> void AsyncTransaction(TransactionStatus &status, F &&func)
{
	static_assert(D::is_concurrent, "AsyncTransaction: Domain does not support concurrent input.");

	using REACT_IMPL::DomainSpecificInputManager;

	DomainSpecificInputManager<D>::Instance().AsyncTransaction(0, status.statePtr_, std::forward<F>(func));
}

template <typename D, typename F> void AsyncTransaction(TransactionFlagsT flags, TransactionStatus &status, F &&func)
{
	static_assert(D::is_concurrent, "AsyncTransaction: Domain does not support concurrent input.");

	using REACT_IMPL::DomainSpecificInputManager;
	DomainSpecificInputManager<D>::Instance().AsyncTransaction(flags, status.statePtr_, std::forward<F>(func));
}
...
#define USING_REACTIVE_DOMAIN(name)                                              \
	template <typename S> using SignalT = Signal<name, S>;                   \
                                                                                 \
	template <typename S> using VarSignalT = VarSignal<name, S>;             \
                                                                                 \
	template <typename E = Token> using EventsT = Events<name, E>;           \
                                                                                 \
	template <typename E = Token> using EventSourceT = EventSource<name, E>; \
                                                                                 \
