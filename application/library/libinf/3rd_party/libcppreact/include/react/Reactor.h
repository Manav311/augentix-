...
/*****************************************/ REACT_BEGIN /*****************************************/
	...
	class Context {
		...
		template <typename E> E &Await(const Events<D, E> &evn)
		{
			return node_.Await(GetNodePtr(evn));
		}

		template <typename E, typename F> void RepeatUntil(const Events<D, E> &evn, F &&func)
		{
			node_.RepeatUntil(GetNodePtr(evn), std::forward<F>(func));
		}

		template <typename S> const S &Get(const Signal<D, S> &sig)
		{
