...
template <> class gate<1> : public composite_node<tuple<signal_t>, tuple<signal_t> > {
    protected:
...
template <> class gate<2> : public composite_node<tuple<signal_t, signal_t>, tuple<signal_t> > {
    protected:
...
template <> class gate<3> : public composite_node<tuple<signal_t, signal_t, signal_t>, tuple<signal_t> > {
    protected:
...
template <> class gate<4> : public composite_node<tuple<signal_t, signal_t, signal_t, signal_t>, tuple<signal_t> > {
    protected:
	        ...
	        , gate_fn(g, 1, b)
	{
		make_edge(in_ports, gate_fn);
		base_type::input_ports_type input_tuple(input_port<0>(in_ports), input_port<1>(in_ports),
		                                        input_port<2>(in_ports), input_port<3>(in_ports));
		base_type::output_ports_type output_tuple(output_port<0>(gate_fn));
		base_type::set_external_ports(input_tuple, output_tuple);
		base_type::add_visible_nodes(in_ports, gate_fn);
	}
...
class pulse {
	class clock_body {
	    ...
	    public:
		clock_body(size_t &_ms, int &_reps)
		        : ms(_ms)
		        , reps(_reps)
		        , val(low)
		{
...
class buffer : public gate<1> {
    ...
    public:
	buffer(graph &g)
	        : gate<1>(g, buffer_body())
	{
	}
	buffer(const buffer &src)
	        : gate<1>(src.my_graph, buffer_body())
	{
	}
	~buffer()
	{
...
class not_gate : public gate<1> {
	...
	not_gate(const not_gate &src)
	        : gate<1>(src.my_graph, not_body())
	{
	}
	~not_gate()
	{
...
template <int N> class and_gate : public gate<N> {
	...
	and_gate(const and_gate<N> &src)
	        : gate<N>(src.my_graph, and_body())
	{
	}
	~and_gate()
	{
...
template <int N> class or_gate : public gate<N> {
	using gate<N>::my_graph;
	typedef typename gate<N>::ports_type ports_type;
	typedef typename gate<N>::input_port_t::output_type from_input;
	class or_body {
	...
	or_gate(const or_gate &src)
	        : gate<N>(src.my_graph, or_body())
	{
	}
	~or_gate()
	{
...
template <int N> class xor_gate : public gate<N> {
	using gate<N>::my_graph;
	typedef typename gate<N>::ports_type ports_type;
	...
	xor_gate(const xor_gate &src)
	        : gate<N>(src.my_graph, xor_body())
	{
	}
	~xor_gate()
	{
...
template <int N> class nor_gate : public gate<N> {
	using gate<N>::my_graph;
	typedef typename gate<N>::ports_type ports_type;
	typedef typename gate<N>::input_port_t input_port_t;
	class nor_body {
	...
	nor_gate(const nor_gate &src)
	        : gate<N>(src.my_graph, nor_body())
	{
	}
	~nor_gate()
	{
...
class led {
    ...
    public:
	led(graph &g, string l, bool rc = false)
	        : my_graph(g)
	        , label(l)
	        , state(undefined)
	        , report_changes(rc)
...
class digit : public gate<4> {
	...
	class digit_body {
	    ...
	    public:
		digit_body(unsigned int &s, string &l, bool &r)
		        : state(s)
		        , label(l)
		        , report_changes(r)
		{
