...
void ParseInputArgs(int argc, char *argv[])
{
	utility::parse_cli_arguments(argc, argv,
	                             utility::cli_argument_pack()
	                                     ...
	                                     .positional_arg(cfg::threads, "n-of-threads",
	                                                     utility::thread_number_range_desc)
...
struct rng {
	...
	unsigned short operator()(size_t &seed)
	{
...
template <typename Index> struct edge {
	Index start;
	Index end;
	edge(Index _p1, Index _p2)
	        : start(_p1)
	        , end(_p2){};
};

template <typename T> ostream &operator<<(ostream &_ostr, point<T> _p)
{
	return _ostr << '(' << _p.x << ',' << _p.y << ')';
}

template <typename T> istream &operator>>(istream &_istr, point<T> _p)
{
	return _istr >> _p.x >> _p.y;
}

template <typename T> bool operator==(point<T> p1, point<T> p2)
{
...
template <typename T> double cross_product(const point<T> &start, const point<T> &end1, const point<T> &end2)
{
...
void WriteResults(int nthreads, double initTime, double calcTime)
{
	...
	if (!silent) {
		cout << "  Number of nodes:" << cfg::numberOfPoints << "  Number of threads:" << nthreads
		     << "  Initialization time:" << setw(10) << setprecision(3) << initTime
		     << "  Calculation time:" << setw(10) << setprecision(3) << calcTime << endl;
	}
}
};
