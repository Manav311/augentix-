...
namespace internal
{
#if ((__GNUC__ * 100 + __GNUC_MINOR__ >= 404 && __GXX_EXPERIMENTAL_CXX0X__) || _MSC_VER >= 1600) && \
...
template <typename T> void swap(std::auto_ptr<T> &ptr1, std::auto_ptr<T> &ptr2)
{
	std::auto_ptr<T> tmp;
	tmp = ptr2;
	ptr2 = ptr1;
...
//TODO: add tcs
template <class dest_type> dest_type &string_to(std::string const &s, dest_type &result)
{
...
template <typename type> class type_impl : public type_base {
	...
	type_impl(std::string a_name, std::string a_description, type &a_target,
	          validating_function_type a_validating_function = NULL)
	...
	template <typename t> static bool is_null_c_str(t &)
	{
		return false;
	}
...
class argument {
	...
	argument(std::string a_name, std::string a_description, type &dest,
	         bool (*a_validating_function)(const type &) = NULL)
...
class cli_argument_pack {
	...
	cli_argument_pack &arg(type &dest, std::string const &name, std::string const &description,
	                       bool (*validate)(const type &) = NULL)
	{
	...
	template <typename type>
	cli_argument_pack &positional_arg(type &dest, std::string const &name, std::string const &description,
	                                  bool (*validate)(const type &) = NULL)
	{
		internal::argument a(name, description, dest, validate);
		add_arg(a);
			...
			for (std::size_t j = 1; j < argc; j++) {
				...
				//then see is it a named flag
					...
					//then try it as positional argument without name specified
						...
						assert(found_positional_arg !=
						       args_map.end() /*&&"positional_arg_names and args_map are out of sync"*/);
	...
	std::string usage_string(const std::string &binary_name) const
	{
		...
		str << " Program usage is:" << endl
		    << " " << binary_name << command_line_params << endl
		    << endl
		    << " where:" << endl
		    << summary_description;
		return str.str();
...
namespace internal
{
template <typename T> bool is_power_of_2(T val)
{
...
step_function_descriptor step_function_descriptors[] = { step_function_descriptor('*', step_function_multiply),
...
struct thread_number_range {
	...
	thread_number_range(int (*auto_number_of_threads_)(), int low_ = 1, int high_ = -1,
	                    internal::thread_range_step step_ =
	                            internal::thread_range_step(internal::step_function_power2_ladder, 4))
	...
	friend std::istream &operator>>(std::istream &i, thread_number_range &range)
	{
		try {
			...
			struct string_to_number_of_threads {
				int auto_value;
				string_to_number_of_threads(int auto_value_)
				        : auto_value(auto_value_)
				{
				}
...
inline void parse_cli_arguments(int argc, const char *argv[], utility::cli_argument_pack cli_pack)
{
	...
	try {
		...
		std::cerr << "error occurred while parsing command line." << std::endl
