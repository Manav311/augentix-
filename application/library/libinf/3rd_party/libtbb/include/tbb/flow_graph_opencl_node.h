...
template <typename Factory> class opencl_buffer_impl;
...
class opencl_graph : public graph {
	...
	opencl_graph()
	        : my_opencl_foundation(NULL)
	{
	}
	//! Constructs a graph with an user context
	explicit opencl_graph(task_group_context &context)
	        : graph(context)
	        , my_opencl_foundation(NULL)
	{
	}
	...
	template <typename... Args> friend class opencl_node;
	template <typename DeviceFilter> friend class opencl_factory;
};

template <typename T, typename Factory> class dependency_msg;

template <typename T, typename Factory> class proxy_dependency_receiver;

template <typename T, typename Factory> class receiver<dependency_msg<T, Factory> > {
    public:
	...
	class ordinary_receiver : public receiver<T>, tbb::internal::no_assign {
	    ...
	    public:
		ordinary_receiver(receiver<dependency_msg<T, Factory> > &owner)
		        : my_owner(owner)
		{
		}
		...
		/* override */ bool register_predecessor(predecessor_type &p)
		{
			...
			typename predecessor_map_type::iterator it = my_predecessor_map.emplace(
			        std::piecewise_construct_t(), std::make_tuple(&p), std::tie(p));
		...
		typedef std::multimap<predecessor_type *, typename dependency_predecessor_type::proxy>
...
class proxy_dependency_receiver : public receiver<dependency_msg<T, Factory> >, tbb::internal::no_assign {
	...
	/* override */ task *try_put_task(const dependency_msg<T, Factory> &d)
	{
		...
		receiver<T> *r = &my_r;
		d.register_callback([r](const T &t) { r->try_put(t); });
		d.clear_event();
...
template <typename T, typename Factory> class sender<dependency_msg<T, Factory> > {
    public:
	sender()
	        : my_ordinary_sender(*this)
	{
	}
	...
	class ordinary_sender : public sender<T>, tbb::internal::no_assign {
	    ...
	    public:
		ordinary_sender(sender<dependency_msg<T, Factory> > &owner)
		        : my_owner(owner)
		{
		}
		...
		/* override */ bool register_successor(successor_type &r)
		{
			...
			typename successor_map_type::iterator it = my_successor_map.emplace(
			        std::piecewise_construct_t(), std::make_tuple(&r), std::tie(r));
...
template <typename T, typename Factory> inline void make_edge(sender<T> &s, receiver<dependency_msg<T, Factory> > &r)
{
	make_edge(s, r.ordinary_receiver());
}

template <typename T, typename Factory> inline void make_edge(sender<dependency_msg<T, Factory> > &s, receiver<T> &r)
{
	make_edge(s.ordinary_sender(), r);
}

template <typename T, typename Factory> inline void remove_edge(sender<T> &s, receiver<dependency_msg<T, Factory> > &r)
{
	remove_edge(s, r.ordinary_receiver());
}

template <typename T, typename Factory> inline void remove_edge(sender<dependency_msg<T, Factory> > &s, receiver<T> &r)
{
...
template <> std::string device_info<std::string>(cl_device_id d, cl_device_info i)
{
	size_t required;
...
template <> std::string platform_info<std::string>(cl_platform_id p, cl_platform_info i)
{
	size_t required;
...
class opencl_device {
	...
	template <typename T> void info(cl_device_info i, T &t) const
	{
...
#if CL_VERSION_2_0
		if (major_version() >= 2)
			return (device_info<cl_command_queue_properties>(my_cl_device_id,
			                                                 CL_DEVICE_QUEUE_ON_HOST_PROPERTIES) &
			        CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE) != 0;
		else
#endif /* CL_VERSION_2_0 */
			return (device_info<cl_command_queue_properties>(my_cl_device_id, CL_DEVICE_QUEUE_PROPERTIES) &
			        CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE) != 0;
	}
...
template <typename T, typename Factory = default_opencl_factory> class dependency_msg {
    public:
	typedef T value_type;

	dependency_msg() = default;
	explicit dependency_msg(const T &data)
	        : my_data(data)
	{
	}
	...
	{
		if (my_is_event) {
			enforce_cl_retcode(clFlush(event_info<cl_command_queue>(my_event, CL_EVENT_COMMAND_QUEUE)),
	...
	template <typename Callback> void register_callback(Callback c) const
	{
		...
		enforce_cl_retcode(clSetEventCallback(my_event, CL_COMPLETE, register_callback_func,
		                                      new callback<Callback, T>(*my_graph, c, my_data)),
...
template <typename K, typename T, typename Factory> K key_from_message(const dependency_msg<T, Factory> &dmsg)
{
...
template <typename T, typename Factory = default_opencl_factory> class opencl_subbuffer;

template <typename T, typename Factory = default_opencl_factory> class opencl_buffer {
    public:
	...
	template <access_type a = read_write> iterator<a> begin() const
	{
		return access<a>();
	}

	template <access_type a = read_write> iterator<a> end() const
	{
...
template <typename DeviceFilter> class opencl_factory {
	...
	~opencl_factory()
	{
		if (my_devices.size()) {
			for (opencl_device d : my_devices) {
				enforce_cl_retcode(clReleaseCommandQueue(d.my_cl_command_queue),
	...
	void enque_map_buffer(opencl_device device, opencl_buffer_impl<Factory> &buffer,
	                      dependency_msg<void *, Factory> &dmsg)
	{
		cl_event const *e1 = dmsg.get_event();
		cl_event e2;
		cl_int err;
		void *ptr = clEnqueueMapBuffer(device.my_cl_command_queue, buffer.get_cl_mem(), false,
		                               CL_MAP_READ | CL_MAP_WRITE, 0, buffer.size(), e1 == NULL ? 0 : 1, e1,
		                               &e2, &err);
	...
	template <typename Factory>
	void enque_unmap_buffer(opencl_device device, opencl_memory<Factory> &memory,
	                        dependency_msg<void *, Factory> &dmsg)
	{
		cl_event const *e1 = dmsg.get_event();
		cl_event e2;
		enforce_cl_retcode(clEnqueueUnmapMemObject(device.my_cl_command_queue, memory.get_cl_mem(),
		                                           memory.get_host_ptr(), e1 == NULL ? 0 : 1, e1, &e2),
	...
	cl_event enqueue_kernel(opencl_device device, cl_kernel kernel, GlbNDRange &&global_work_size,
	                        LclNDRange &&local_work_size, cl_uint num_events, cl_event *event_list)
	{
		...
		cl_event event;
		enforce_cl_retcode(clEnqueueNDRangeKernel(device.my_cl_command_queue, kernel, s, g_offset.data(),
...
template <typename Factory> dependency_msg<void *, Factory> opencl_memory<Factory>::receive(const cl_event *e)
{
	dependency_msg<void *, Factory> d = e ? dependency_msg<void *, Factory>(my_host_ptr, *e) :
	                                        dependency_msg<void *, Factory>(my_host_ptr);
...
class opencl_foundation : tbb::internal::no_assign {
	...
	opencl_foundation(opencl_graph &g)
	        : my_default_opencl_factory(g)
	        , my_default_device_selector()
	{
		...
		for (cl_platform_id p : platforms) {
			cl_uint num_devices;
			enforce_cl_retcode(clGetDeviceIDs(p, CL_DEVICE_TYPE_ALL, 0, NULL, &num_devices),
		...
		for (cl_platform_id p : platforms) {
			...
			enforce_cl_retcode(clGetDeviceIDs(p, CL_DEVICE_TYPE_ALL,
			                                  (cl_uint)std::distance(it, devices.end()), &*it,
			                                  &num_devices),
			                   "clGetDeviceIDs failed");
			it += num_devices;
		}
	...
	{
		return my_default_device_selector;
	}
...
template <typename DeviceFilter> void opencl_factory<DeviceFilter>::init_once()
{
	{
		tbb::spin_mutex::scoped_lock lock(my_devices_mutex);
		if (!my_devices.size())
	...
	for (opencl_device_list::iterator it = ++my_devices.begin(); it != my_devices.end(); ++it)
		enforce_cl_retcode(it->platform() == platform_id ? CL_SUCCESS : CL_INVALID_PLATFORM,
	...
	cl_context_properties context_properties[3] = { CL_CONTEXT_PLATFORM, (cl_context_properties)platform_id,
		                                        (cl_context_properties)NULL };
	cl_int err;
	cl_context ctx = clCreateContext(context_properties, (cl_uint)cl_device_ids.size(), cl_device_ids.data(), NULL,
	                                 NULL, &err);
	...
	for (opencl_device &d : my_devices) {
...
#endif
		{
			cl_command_queue_properties props =
			        d.out_of_order_exec_mode_on_host_present() ? CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE : 0;
...
template <typename T, typename Factory>
opencl_buffer<T, Factory>::opencl_buffer(opencl_graph &g, size_t size)
        : my_impl(std::make_shared<impl_type>(size * sizeof(T), g.get_opencl_foundation().get_default_opencl_factory()))
{
}

enum class opencl_program_type { SOURCE, PRECOMPILED, SPIR };

template <typename Factory = default_opencl_factory> class opencl_program : tbb::internal::no_assign {
    public:
	...
	opencl_program(const char *program_name)
	        : opencl_program(std::string(program_name))
	{
	}
	opencl_program(const std::string &program_name)
	        : opencl_program(opencl_program_type::SOURCE, program_name)
	{
	}
	...
	class opencl_program_builder {
		...
		opencl_program_builder(Factory &f, const std::string &name, cl_program program, cl_uint num_devices,
		                       cl_device_id *device_list, const char *options, cl_callback_type callback,
		                       void *user_data)
		{
			...
			if (err == CL_BUILD_PROGRAM_FAILURE) {
				...
				for (opencl_device d : devices) {
					...
					cl_int query_err = clGetProgramBuildInfo(
					        program, d.my_cl_device_id, CL_PROGRAM_BUILD_LOG, 0, NULL, &log_size);
					enforce_cl_retcode(query_err, "Failed to get build log size");
					if (log_size) {
						std::vector<char> output;
						output.resize(log_size);
						query_err = clGetProgramBuildInfo(program, d.my_cl_device_id,
						                                  CL_PROGRAM_BUILD_LOG, output.size(),
						                                  output.data(), NULL);
	...
	class opencl_device_filter {
		...
		opencl_device_filter(cl_uint &num_devices, cl_device_id *device_list, Filter filter,
		                     const char *message)
		{
	...
	{
		cl_uint num_devices;
		enforce_cl_retcode(clGetContextInfo(f.context(), CL_CONTEXT_NUM_DEVICES, sizeof(num_devices),
		                                    &num_devices, NULL),
		...
		enforce_cl_retcode(clGetContextInfo(f.context(), CL_CONTEXT_DEVICES, num_devices * sizeof(cl_device_id),
		                                    device_list, NULL),
		                   "Failed to get OpenCL context info");
		...
		case opencl_program_type::SOURCE: {
			...
			opencl_program_builder(f, my_arg_str, my_cl_program, num_devices, device_list, options,
		...
		case opencl_program_type::PRECOMPILED: {
			file_reader fr(my_arg_str);
			std::vector<const unsigned char *> s(num_devices,
			...
			cl_int err;
			my_cl_program = clCreateProgramWithBinary(f.context(), num_devices, device_list, l.data(),
			                                          s.data(), bin_statuses.data(), &err);
			if (err != CL_SUCCESS) {
				std::string statuses_str;
				for (cl_int st : bin_statuses)
					statuses_str += std::to_string(st);
				enforce_cl_retcode(err, std::string("Failed to create program, error " +
				                                    std::to_string(err) + " : ") +
				                                my_arg_str + std::string(", binary_statuses = ") +
				                                statuses_str);
			}
			opencl_program_builder(f, my_arg_str, my_cl_program, num_devices, device_list, options,
			                       /*callback*/ NULL, /*user data*/ NULL);
...
template <int N1, int N2 = N1> port_ref_impl<N1, N2> port_ref()
{
	return port_ref_impl<N1, N2>();
};

template <typename T> struct num_arguments {
	static const int value = 1;
};

template <int N1, int N2> struct num_arguments<port_ref_impl<N1, N2> (*)()> {
...
template <typename T> T or_return_values(T &&t)
{
	return t;
}
template <typename T, typename... Rest> T or_return_values(T &&t, Rest &&... rest)
{
...
template <typename T>
typename std::enable_if<is_native_object_type<T>::value, typename T::native_object_type>::type
get_native_object(const T &t)
{
	return t.native_object();
}

template <typename T> typename std::enable_if<!is_native_object_type<T>::value, T>::type get_native_object(T t)
{
	return t;
}
...
template <typename T, typename Factory>
typename std::enable_if<is_memory_object_type<T>::value>::type send_if_memory_object(opencl_device device,
                                                                                     dependency_msg<T, Factory> &dmsg)
{
	const T &t = dmsg.data(false);
...
receive_if_memory_object(const dependency_msg<T, Factory> &dmsg)
{
	const T &t = dmsg.data(false);
	typedef typename T::memory_object_type mem_obj_t;
	mem_obj_t mem_obj = t.memory_object();
...
template <typename Key> class opencl_device_with_key {
...
class opencl_node<tuple<Ports...>, JP, Factory>
        : public composite_node<tuple<dependency_msg<Ports, Factory>...>, tuple<dependency_msg<Ports, Factory>...> > {
	...
	template <int... S> typename base_type::input_ports_type get_input_ports(internal::sequence<S...>)
	{
		return std::tie(internal::input_port<S>(my_indexer_node)...);
	}

	template <int... S> typename base_type::output_ports_type get_output_ports(internal::sequence<S...>)
	{
	...
	class device_selector_base {
	    public:
		virtual void operator()(const indexer_node_output_type &v,
	...
	template <typename UserFunctor> class device_selector : public device_selector_base, tbb::internal::no_assign {
	    public:
		device_selector(UserFunctor uf, opencl_node &n, Factory &f)
		        : my_user_functor(uf)
		        , my_node(n)
		        , my_factory(f)
		{
		...
		/* override */ void operator()(const indexer_node_output_type &v,
		                               typename device_selector_node::output_ports_type &op)
		{
			send_and_put(my_port_epoches[v.tag()], v, op, input_sequence());
			__TBB_ASSERT(
			        (std::is_same<typename key_from_policy<JP>::is_key_matching, std::false_type>::value) ||
	    ...
	    private:
		template <int... S>
		void send_and_put(size_t &epoch, const indexer_node_output_type &v,
		                  typename device_selector_node::output_ports_type &op, internal::sequence<S...>)
		{
			typedef void (device_selector<UserFunctor>::*send_and_put_fn)(
			        size_t &, const indexer_node_output_type &,
			        typename device_selector_node::output_ports_type &);
			static std::array<send_and_put_fn, NUM_INPUTS> dispatch = {
				{ &device_selector<UserFunctor>::send_and_put_impl<S>... }
			};
		...
		template <int N>
		void send_and_put_impl(size_t &epoch, const indexer_node_output_type &v,
		                       typename device_selector_node::output_ports_type &op)
		{
			typedef typename tuple_element<
			        N + 1, typename device_selector_node::output_ports_type>::type::output_type elem_type;
			elem_type e = internal::cast_to<elem_type>(v);
			opencl_device device = get_device(
			        get_key(typename key_from_policy<JP>::is_key_matching(), e, epoch), get<0>(op));
			send_if_memory_object(device, e);
			get<N + 1>(op).try_put(e);
		}

		template <typename DevicePort> opencl_device get_device(key_type key, DevicePort &dp)
		{
			typename std::unordered_map<typename std::decay<key_type>::type, epoch_desc>::iterator it =
			        my_devices.find(key);
	...
	class opencl_kernel_base : tbb::internal::no_copy {
		cl_kernel clone_kernel() const
		{
			...
			for (size_t curr_size = 32;; curr_size <<= 1) {
				...
				enforce_cl_retcode(clGetKernelInfo(my_kernel, CL_KERNEL_FUNCTION_NAME, curr_size,
				                                   kernel_name.data(), &ret_size),
			...
			cl_program program;
			enforce_cl_retcode(clGetKernelInfo(my_kernel, CL_KERNEL_PROGRAM, sizeof(program), &program,
			                                   &ret_size),
			                   "Failed to get kernel info");
		...
		// ------------- NDRange getters ------------- //
		template <typename NDRange> NDRange ndrange_value(NDRange &&r, const kernel_input_tuple &) const
		{
			return r;
		}
		template <int N>
		typename tuple_element<N + 1, kernel_input_tuple>::type::value_type
		ndrange_value(port_ref_impl<N, N>, const kernel_input_tuple &ip) const
		{
			// "+1" since get<0>(ip) is opencl_device.
			return get<N + 1>(ip).data(false);
		}
		template <int N1, int N2> void ndrange_value(port_ref_impl<N1, N2>, const kernel_input_tuple &ip) const
		{
			__TBB_STATIC_ASSERT(
			        N1 == N2,
			        "Do not use a port_ref range (e.g. port_ref<0,2>) as an argument for the set_ndranges routine");
		}
		template <int N>
		typename tuple_element<N + 1, kernel_input_tuple>::type::value_type
		ndrange_value(port_ref_impl<N, N> (*)(), const kernel_input_tuple &ip) const
		...
		virtual void enqueue(const ndranges_mapper_base *ndranges_mapper, const kernel_input_tuple &ip,
		                     output_ports_type &op, graph &g) = 0;
		...
		template <typename GlbNDRange, typename LclNDRange>
		cl_event enqueue(GlbNDRange &&glb_range, LclNDRange &&lcl_range, int num_events,
		                 std::array<cl_event, NUM_INPUTS> events, const kernel_input_tuple &ip)
		{
			return my_factory.enqueue_kernel(get<0>(ip).device(), my_kernel, ndrange_value(glb_range, ip),
			                                 ndrange_value(lcl_range, ip), num_events, events.data());
		}

	    protected:
		opencl_kernel_base(const opencl_program<Factory> &p, const std::string &kernel_name, Factory &f)
		        : my_kernel(p.get_kernel(kernel_name, f))
		        , my_factory(f)
		{
		}

		opencl_kernel_base(const opencl_kernel_base &k)
		        : my_kernel(k.clone_kernel())
	...
	class ndranges_mapper_base {
	    public:
		virtual cl_event enqueue_kernel(opencl_kernel_base *k, const kernel_input_tuple &ip, int num_events,
		                                const std::array<cl_event, NUM_INPUTS> &events) const = 0;
	...
	template <typename... Args> class opencl_kernel : public opencl_kernel_base {
		typedef typename opencl_kernel_base::output_ports_type output_ports_type;
		// --------- Kernel argument helpers --------- //
		template <int Place, typename T> void set_one_kernel_arg(const T &t)
		{
			auto p = get_native_object(t);
			enforce_cl_retcode(clSetKernelArg(this->my_kernel, Place, sizeof(p), &p),
			                   "Failed to set a kernel argument");
		}

		template <int Place, int N> int set_one_arg_from_range(const kernel_input_tuple &ip)
		{
			// "+1" since get<0>(ip) is opencl_device
		...
		{
			set_args_range<Place, N1>(
			        ip, typename internal::make_sequence<port_ref_impl<N1, N2>::size>::type());
		}
		...
		template <int Place, typename T> void set_arg_impl(const kernel_input_tuple &, const T &t)
		{
			set_one_kernel_arg<Place>(t);
		}

		template <int> void set_args(const kernel_input_tuple &)
		{
		}
		...
		{
			__TBB_ASSERT((static_cast<typename std::array<cl_event, NUM_INPUTS>::size_type>(num_events) <
			              events.size()),
			             NULL);
			if (e)
				events[num_events++] = *e;
		...
		template <int... S>
		int generate_event_list(std::array<cl_event, NUM_INPUTS> &events, const kernel_input_tuple &ip,
		                        internal::sequence<S...>)
		{
			int num_events = 0;
		...
		template <int... S>
		bool update_events_and_try_put(graph &g, cl_event e, const kernel_input_tuple &ip,
		                               output_ports_type &op, internal::sequence<S...>)
		{
		...
		class set_args_func : tbb::internal::no_assign {
		    public:
			set_args_func(opencl_kernel &k, const kernel_input_tuple &ip)
			        : my_opencl_kernel(k)
			        , my_ip(ip)
			{
			}
		...
		static void CL_CALLBACK decrement_wait_count_callback(cl_event, cl_int event_command_exec_status,
		                                                      void *data)
		{
			tbb::internal::suppress_unused_warning(event_command_exec_status);
			__TBB_ASSERT(event_command_exec_status == CL_COMPLETE, NULL);
	    ...
	    public:
		opencl_kernel(const opencl_program<Factory> &p, const std::string &kernel_name, Factory &f,
		              Args &&... args)
		...
		opencl_kernel(const opencl_kernel_base &k)
		        : opencl_kernel_base(k)
		        , my_args_pack(k.my_args_pack)
		{
		}

		opencl_kernel(const opencl_kernel_base &k, Args &&... args)
		        : opencl_kernel_base(k)
		        , my_args_pack(std::forward<Args>(args)...)
		{
		}

		/* override */ void enqueue(const ndranges_mapper_base *ndrange_mapper, const kernel_input_tuple &ip,
		                            output_ports_type &op, graph &g)
		{
			...
			if (!update_events_and_try_put(g, e, ip, op, input_sequence())) {
				...
				enforce_cl_retcode(clSetEventCallback(e, CL_COMPLETE, decrement_wait_count_callback,
				                                      &g),
	...
	class ndranges_mapper : public ndranges_mapper_base, tbb::internal::no_assign {
		...
		ndranges_mapper(GRange &&glb, LRange &&lcl)
		        : my_global_work_size(glb)
		        , my_local_work_size(lcl)
		{
		}

		/*override*/ cl_event enqueue_kernel(opencl_kernel_base *k, const kernel_input_tuple &ip,
		                                     int num_events,
		                                     const std::array<cl_event, NUM_INPUTS> &events) const
		{
	...
	class kernel_body : tbb::internal::no_assign {
	    public:
		kernel_body(const opencl_node &node)
		        : my_opencl_node(node)
		{
		}
	...
	opencl_kernel_base *make_opencl_kernel(const opencl_program<Factory> &p, const std::string &kernel_name,
	                                       Factory &f, Args &&... args) const
	{
	...
	void set_ndranges_impl(GlobalNDRange &&global_work_size,
	                       LocalNDRange &&local_work_size = std::array<size_t, 3>({ { 0, 0, 0 } }))
	{
		if (my_ndranges_mapper)
			delete my_ndranges_mapper;
		my_ndranges_mapper = new ndranges_mapper<typename std::decay<GlobalNDRange>::type,
		                                         typename std::decay<LocalNDRange>::type>(
	...
	opencl_node(opencl_graph &g, const opencl_program<Factory> &p, const std::string &kernel_name, DeviceSelector d,
	            Factory &f)
	        : base_type(g)
	        , my_indexer_node(g)
	...
	template <typename T> void set_ndranges(std::initializer_list<T> global_work_size)
	{
		set_ndranges_impl(internal::initializer_list_wrapper<T>(global_work_size));
	}

	template <typename GlobalNDRange> void set_ndranges(GlobalNDRange &&global_work_size)
	{
	...
	{
		set_ndranges_impl(internal::initializer_list_wrapper<T>(global_work_size),
		                  std::forward<LocalNDRange>(local_work_size));
	}

	template <typename T1, typename T2 = T1>
	void set_ndranges(std::initializer_list<T1> global_work_size, std::initializer_list<T2> local_work_size)
	{
		set_ndranges_impl(internal::initializer_list_wrapper<T1>(global_work_size),
		                  internal::initializer_list_wrapper<T2>(local_work_size));
	}
	...
	{
		set_ndranges_impl(std::forward<GlobalNDRange>(global_work_size),
		                  internal::initializer_list_wrapper<T>(local_work_size));
	}

	template <typename... Args> void set_args(Args &&... args)
	{
		// Copy the base class of opencl_kernal and create new storage for "Args...".
		opencl_kernel_base *new_opencl_kernel =
		        new opencl_kernel<Args...>(*my_opencl_kernel, std::forward<Args>(args)...);
		delete my_opencl_kernel;
		my_opencl_kernel = new_opencl_kernel;
	}

    protected:
	/* override */ void reset_node(reset_flags = rf_reset_protocol)
	{
...
class opencl_node<tuple<Ports...>, JP> : public opencl_node<tuple<Ports...>, JP, default_opencl_factory> {
	...
	opencl_node(opencl_graph &g, const std::string &kernel)
	        : base_type(g, kernel, g.get_opencl_foundation().get_default_device_selector(),
	                    g.get_opencl_foundation().get_default_opencl_factory())
	{
	}
	opencl_node(opencl_graph &g, const opencl_program<default_opencl_factory> &p, const std::string &kernel)
	        : base_type(g, p, kernel, g.get_opencl_foundation().get_default_device_selector(),
	                    g.get_opencl_foundation().get_default_opencl_factory())
	{
	}
	template <typename DeviceSelector>
