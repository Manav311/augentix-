...
template <typename T> void handle_unconstructed_elements(T *array, size_t n_of_elements)
{
...
class concurrent_vector_base_v3 {
	...
	class segment_value_t {
		...
		template <typename argument_type> friend bool operator!=(segment_value_t const &lhs, argument_type arg)
		{
			return !(lhs == arg);
		}
	...
	friend void enforce_segment_allocated(segment_value_t const &s,
	...
	class segment_t {
		...
		template <memory_semantics M> segment_value_t load() const
		{
			return segment_value_t(array.load<M>());
		}

		template <memory_semantics M> void store(segment_not_used)
		{
			array.store<M>(0);
		}
		...
		{
			__TBB_ASSERT(
			        segment_value_t(allocated_segment_pointer) == segment_allocated(),
	...
	void internal_grow(size_type start, size_type finish, size_type element_size, internal_array_op2 init,
	                   const void *src);
	...
	void *__TBB_EXPORTED_METHOD internal_compact(size_type element_size, void *table, internal_array_op1 destroy,
	...
	void __TBB_EXPORTED_METHOD internal_assign(const concurrent_vector_base_v3 &src, size_type element_size,
	                                           internal_array_op1 destroy, internal_array_op2 assign,
	                                           internal_array_op2 copy);
	//! Obsolete
	...
	void __TBB_EXPORTED_METHOD internal_resize(size_type n, size_type element_size, size_type max_size,
	                                           const void *src, internal_array_op1 destroy,
	                                           internal_array_op2 init);
	size_type __TBB_EXPORTED_METHOD internal_grow_to_at_least_with_result(size_type new_size,
...
inline void swap(concurrent_vector_base_v3::segment_t &lhs, concurrent_vector_base_v3::segment_t &rhs)
        __TBB_NOEXCEPT(true)
{
	lhs.swap(rhs);
}
...
class concurrent_vector : protected internal::allocator_base<T, A>, private internal::concurrent_vector_base {
    private:
	        ...
	        : internal::allocator_base<T, A>(a)
	{
		...
		__TBB_TRY
		{
			internal_resize(n, sizeof(T), max_size(), static_cast<const void *>(&t), &destroy_array,
			                &initialize_array_by);
		}
		__TBB_CATCH(...)
		{
	        ...
	        : internal::allocator_base<T, A>(a)
	{
		...
		__TBB_TRY
		{
			internal_assign_range(first, last,
			                      static_cast<is_integer_tag<std::numeric_limits<I>::is_integer> *>(0));
		}
		__TBB_CATCH(...)
		{
			segment_t *table = my_segment.load<relaxed>();
	...
	template <class M> concurrent_vector &operator=(const concurrent_vector<T, M> &vector)
	{
		if (static_cast<void *>(this) != static_cast<const void *>(&vector))
			internal_assign(vector.internal_vector_base(), sizeof(T), &destroy_array, &assign_array,
			                &copy_array);
		return *this;
	}
	...
	{
		return iterator(*this, delta ? internal_grow_by(delta, sizeof(T), &initialize_array, NULL) :
		                               my_early_size.load());
	}
	...
	{
		internal_resize(n, sizeof(T), max_size(), static_cast<const void *>(p), &destroy_array,
	...
	template <class I> void internal_assign_range(I first, I last, is_integer_tag<true> *)
	{
		internal_assign_n(static_cast<size_type>(first), &static_cast<T &>(last));
	}
	//! inline proxy assign by iterators
	template <class I> void internal_assign_range(I first, I last, is_integer_tag<false> *)
	{
...
template <typename T, class A> void concurrent_vector<T, A>::shrink_to_fit()
{
	...
	__TBB_TRY
	{
		...
		if (internal_compact(sizeof(T), &old, &destroy_array, copy_or_move_array))
			internal_free_segments(old.table, pointers_per_long_table,
...
template <typename T, class A> T &concurrent_vector<T, A>::internal_subscript(size_type index) const
{
	...
	__TBB_ASSERT(my_segment.load<acquire>() != my_storage || k < pointers_per_short_table,
	...
	__TBB_ASSERT(segment_value != segment_allocation_failed(),
...
template <typename T, class A> inline void swap(concurrent_vector<T, A> &a, concurrent_vector<T, A> &b)
{
	a.swap(b);
}
