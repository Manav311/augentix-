...
template <typename Item> class parallel_do_feeder : internal::no_copy {
	parallel_do_feeder()
	{
	}
...
template <class Body, typename Item> class parallel_do_operator_selector {
	...
	template <typename A1, typename A2, typename CvItem>
	static void internal_call(const Body &obj, A1 &arg1, A2 &arg2,
    ...
    public:
	template <typename A1, typename A2> static void call(const Body &obj, A1 &arg1, A2 &arg2)
	{
...
void select_parallel_do(Iterator first, Iterator last, const Body &body,
...
template <typename Range, typename Body> void parallel_do(Range &rng, const Body &body)
{
	parallel_do(tbb::internal::first(rng), tbb::internal::last(rng), body);
}

template <typename Range, typename Body> void parallel_do(const Range &rng, const Body &body)
{
	parallel_do(tbb::internal::first(rng), tbb::internal::last(rng), body);
}
...
template <typename Range, typename Body> void parallel_do(Range &rng, const Body &body, task_group_context &context)
{
