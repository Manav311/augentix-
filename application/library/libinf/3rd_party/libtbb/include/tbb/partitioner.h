...
template <typename Partition> struct partition_type_base {
	...
	template <typename Range> split_type get_split()
	{
		return split();
	}
	Partition &self()
...
template <typename Partition> struct adaptive_mode : partition_type_base<Partition> {
	...
	adaptive_mode(adaptive_mode &src, const proportional_split &split_obj)
	        : my_divisor(do_split(src, split_obj))
	{
	}
...
template <typename Partition> struct linear_affinity_mode : adaptive_mode<Partition> {
...
template <class Mode> struct unbalancing_partition_type : Mode {
	using Mode::self;
	unbalancing_partition_type()
	        : Mode()
	{
	}
	unbalancing_partition_type(unbalancing_partition_type &p, split)
	        : Mode(p, split())
	{
	}
	unbalancing_partition_type(unbalancing_partition_type &p, const proportional_split &split_obj)
	        : Mode(p, split_obj)
	{
	}
...
template <class Mode> struct balancing_partition_type : unbalancing_partition_type<Mode> {
	using Mode::self;
	...
	{ // part of old should_execute_range()
		if (!(self().my_divisor / Mode::my_partition::factor)) { // if not from the top P tasks of binary tree
			...
			if (t.is_stolen_task() &&
			    t.parent()->ref_count() >= 2) { // runs concurrently with the left task
