...
template <typename Container, typename Value>
hash_map_iterator<Container, Value>::hash_map_iterator(const Container &map, size_t index, const bucket *b,
                                                       node_base *n)
        : my_map(&map)
        , my_index(index)
        , my_bucket(b)
...
class concurrent_hash_map : protected internal::hash_map_base {
	...
	concurrent_hash_map &operator=(concurrent_hash_map &&table)
	{
		if (this != &table) {
			typedef typename tbb::internal::allocator_traits<
			        allocator_type>::propagate_on_container_move_assignment pocma_t;
	...
	{
		return const_cast<concurrent_hash_map *>(this)->lookup(/*insert*/ false, key, NULL, NULL,
	...
	{
		result.release();
		return const_cast<concurrent_hash_map *>(this)->lookup(/*insert*/ false, key, NULL, &result,
		                                                       /*write=*/false, &do_not_allocate_node);
	}
	...
	template <typename Accessor, typename... Args> bool generic_emplace(Accessor &&result, Args &&... args)
	{
		...
		return lookup(/*insert*/ true, node_ptr->item.first, NULL, accessor_location(result),
	...
	template <typename I> void internal_copy(I first, I last);
...
bool concurrent_hash_map<Key, T, HashCompare, A>::lookup(
        bool op_insert, const Key &key, const T *t, const_accessor *result, bool write,
        node *(*allocate_node)(node_allocator_type &, const Key &, const T *), node *tmp_n)
{
...
void concurrent_hash_map<Key, T, HashCompare, A>::rehash(size_type sz)
{
	...
	for (; b <= mask; b++, bp++) {
		...
		__TBB_ASSERT(is_valid(n) || n == internal::empty_rehashed || n == internal::rehash_req,
		...
		if (n ==
		    internal::rehash_req) { // rehash bucket, conditional because rehashing of a previous bucket may affect this one
			hashcode_t h = b;
			bucket *b_old = bp;
			do {
				__TBB_ASSERT(h > 1, "The lowermost buckets can't be rehashed");
	...
	int current_size = int(my_size), buckets = int(mask) + 1, empty_buckets = 0,
	...
	if (!reported && buckets >= 512 &&
	    (2 * empty_buckets > current_size || 2 * overpopulated_buckets > current_size)) {
...
void concurrent_hash_map<Key, T, HashCompare, A>::clear()
{
	...
	for (segment_index_t b = 0; b <= m; b++) {
		...
		for (; is_valid(n); n = n->next) {
			...
			h &= m;
			__TBB_ASSERT(h == b || get_bucket(h)->node_list == internal::rehash_req,
			             "hash() function changed for key in table or internal error");
		}
#endif
	}
	...
	printf("items=%d buckets: capacity=%d rehashed=%d empty=%d overpopulated=%d"
	       ...
	       current_size, int(m + 1), buckets, empty_buckets, overpopulated_buckets, unsigned(my_info_resizes),
