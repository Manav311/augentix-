...
class untyped_sender {
	...
	template <typename X> bool try_get(X &t)
	{
	...
	template <typename X> bool try_reserve(X &t)
	{
		return try_reserve_wrapper(internal::async_helpers<X>::to_void_ptr(t),
		                           internal::async_helpers<X>::is_async_type);
	}
...
#endif
    protected:
	template <typename X> task *try_put_task(const X &t)
	{
		return try_put_task_wrapper(internal::async_helpers<X>::to_void_ptr(t),
		                            internal::async_helpers<X>::is_async_type);
	}
...
template <typename K, typename T> K key_from_message(const T &t)
{
	return t.key();
}
...
#if __TBB_PREVIEW_ASYNC_NODE
	template <typename Input, typename Output, typename Policy, typename Allocator> friend class async_node;
...
template <typename C, typename N> typename graph_iterator<C, N>::pointer graph_iterator<C, N>::operator->() const
{
	return current_node;
}
...
template <typename T> struct allocate_buffer {
	static const bool value = false;
};

template <> struct allocate_buffer<queueing> {
...
class multifunction_node
	...
	typedef typename internal::wrap_tuple_elements<N, internal::multifunction_output, Output>::type
	        output_ports_type;
...
class split_node : public multifunction_node<TupleType, TupleType, rejecting, Allocator> {
	...
	split_node(graph &g)
	        : base_type(g, unlimited, splitting_body())
	{
...
template <typename T> class broadcast_node : public graph_node, public receiver<T>, public sender<T> {
	...
	/*override sender*/ built_successors_type &built_successors()
	{
		return my_successors.built_successors();
	}
	...
	/*override receiver*/ built_predecessors_type &built_predecessors()
	{
		return my_built_predecessors;
	}
...
class buffer_node : public graph_node,
	...
	template <typename derived_type> void handle_operations_impl(buffer_operation *op_list, derived_type *derived)
	{
		...
		while (op_list) {
			...
			case put_item:
				internal_push(tmp);
				try_forwarding = (tmp->status == SUCCEEDED);
				break;
			case try_fwd_task:
		...
		if (try_forwarding && !forwarder_busy) {
			if (this->my_graph.is_active()) {
				forwarder_busy = true;
				task *new_task = new (task::allocate_additional_child_of(*(this->my_graph.root_task())))
				        internal::forward_task_bypass<buffer_node<input_type, A> >(*this);
...
class priority_queue_node : public buffer_node<T, A> {
	...
	void reheap()
	{
		...
		while (child < mark) {
			...
			if (child + 1 < mark && compare(this->get_my_item(child), this->get_my_item(child + 1)))
				++target;
...
class join_node<OutputTuple, reserving> : public internal::unfolded_join_node<tbb::flow::tuple_size<OutputTuple>::value,
                                                                              reserving_port, OutputTuple, reserving> {
    private:
...
template <typename OutputTuple>
class join_node<OutputTuple, queueing> : public internal::unfolded_join_node<tbb::flow::tuple_size<OutputTuple>::value,
                                                                             queueing_port, OutputTuple, queueing> {
    private:
	static const int N = tbb::flow::tuple_size<OutputTuple>::value;
...
class join_node<OutputTuple, key_matching<K, KHash> >
        : public internal::unfolded_join_node<tbb::flow::tuple_size<OutputTuple>::value, key_matching_port, OutputTuple,
                                              key_matching<K, KHash> > {
    private:
	static const int N = tbb::flow::tuple_size<OutputTuple>::value;
	typedef typename internal::unfolded_join_node<N, key_matching_port, OutputTuple, key_matching<K, KHash> >
	        unfolded_type;
...
template <typename T0, typename T1 = null_type, typename T2 = null_type, typename T3 = null_type,
          ...
          typename T8 = null_type, typename T9 = null_type>
class indexer_node;
...
template <typename T> inline void make_edge(sender<T> &p, receiver<T> &s)
{
...
template <typename T> inline void make_edge(sender<T> &p, receiver<typename T::async_msg_data_type> &s)
{
	internal_make_edge(p, s);
}

template <typename T> inline void make_edge(sender<typename T::async_msg_data_type> &p, receiver<T> &s)
{
	internal_make_edge(p, s);
}
...
template <typename T> inline void remove_edge(sender<T> &p, receiver<T> &s)
{
...
template <typename T> inline void remove_edge(sender<T> &p, receiver<typename T::async_msg_data_type> &s)
{
	internal_remove_edge(p, s);
}

template <typename T> inline void remove_edge(sender<typename T::async_msg_data_type> &p, receiver<T> &s)
{
	internal_remove_edge(p, s);
}
#endif // __TBB_PREVIEW_ASYNC_MSG
...
class composite_node<tbb::flow::tuple<InputTypes...>, tbb::flow::tuple<OutputTypes...> > : public graph_node {
	...
	{
		tbb::internal::itt_make_task_group(tbb::internal::ITT_DOMAIN_FLOW, this, tbb::internal::FLOW_NODE, &g,
		                                   tbb::internal::FLOW_GRAPH, tbb::internal::FLOW_COMPOSITE_NODE);
	...
	{
		__TBB_STATIC_ASSERT(NUM_INPUTS == tbb::flow::tuple_size<input_ports_type>::value,
		                    "number of arguments does not match number of input ports");
		__TBB_STATIC_ASSERT(NUM_OUTPUTS == tbb::flow::tuple_size<output_ports_type>::value,
...
#if TBB_PREVIEW_FLOW_GRAPH_TRACE
	template <typename... NodeTypes> void add_visible_nodes(const NodeTypes &... n)
	{
		internal::add_nodes_impl(this, true, n...);
	}

	template <typename... NodeTypes> void add_nodes(const NodeTypes &... n)
	{
...
#if TBB_PREVIEW_FLOW_GRAPH_TRACE
		tbb::internal::fgt_internal_input_helper<T, NUM_INPUTS>::register_port(
...
#if TBB_PREVIEW_FLOW_GRAPH_TRACE
		tbb::internal::fgt_internal_output_helper<T, NUM_OUTPUTS>::register_port(
...
template <typename Output> class async_gateway {
    public:
	typedef Output output_type;
...
template <typename Input, typename Ports, typename AsyncGateway, typename Body> class async_body {
    public:
	...
	async_body(const async_body &other)
	        : my_body(other.my_body)
	        , my_async_gateway(other.my_async_gateway)
	{
	}
...
template <typename Input, typename Output, typename Policy = queueing,
          typename Allocator = cache_aligned_allocator<Input> >
class async_node : public multifunction_node<Input, tuple<Output>, Policy, Allocator>,
    ...
    protected:
	typedef typename internal::multifunction_input<Input, typename base_type::output_ports_type, Allocator>
	        mfn_input_type;
	...
	async_node(graph &g, size_t concurrency, Body body)
	        : base_type(
	                  g, concurrency,
	                  internal::async_body<Input, typename base_type::output_ports_type, async_gateway_type, Body>(
	                          body, this))
	{
		tbb::internal::fgt_multioutput_node<1>(tbb::internal::FLOW_ASYNC_NODE, &this->graph_node::my_graph,
		                                       static_cast<receiver<input_type> *>(this), this->output_ports());
	}
	...
	/*override*/ void async_reserve()
	{
		...
		tbb::internal::fgt_async_reserve(static_cast<receiver<input_type> *>(this),
		                                 &this->graph_node::my_graph);
	}
	...
	{
		typedef internal::multifunction_body<input_type, typename base_type::output_ports_type> mfn_body_type;
		typedef internal::async_body<Input, typename base_type::output_ports_type, async_gateway_type, Body>
		        async_body_type;
		mfn_body_type &body_ref = *this->my_body;
		async_body_type ab = dynamic_cast<internal::multifunction_body_leaf<
		        input_type, typename base_type::output_ports_type, async_body_type> &>(body_ref)
		                             .get_body();
		return ab.get_body();
	}
