...
struct async_helpers<
        T, typename std::enable_if<std::is_base_of<async_msg<typename T::async_msg_data_type>, T>::value>::type> {
...
template <typename T> class async_storage {
	        ...
	        : my_data(std::forward<C>(data))
	{
		...
		__TBB_STATIC_ASSERT((is_same_type<typename strip<C>::type, typename strip<T>::type>::value),
