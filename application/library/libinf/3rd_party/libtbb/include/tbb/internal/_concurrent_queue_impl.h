		...
		__TBB_TRY
		{
			...
			if (srcp != src.tail_page) {
				...
				size_t last_index = modulo_power_of_two(
				        tail_counter / concurrent_queue_rep_base::n_queue, base.my_rep->items_per_page);
				if (last_index == 0)
...
micro_queue<T>::make_copy(concurrent_queue_base_v3<T> &base, const concurrent_queue_rep_base::page *src_page,
                          size_t begin_in_page, size_t end_in_page, ticket &g_index, item_constructor_t construct_item)
{
	concurrent_queue_page_allocator &pa = base;
...
template <typename T> concurrent_queue_base_v3<T>::concurrent_queue_base_v3()
{
	...
	my_rep->items_per_page = item_size <= 8 ? 32 :
	                                          item_size <= 16 ?
	                                          16 :
	                                          item_size <= 32 ? 8 : item_size <= 64 ? 4 : item_size <= 128 ? 2 : 1;
}
...
template <typename T>
void concurrent_queue_base_v3<T>::assign(const concurrent_queue_base_v3 &src, item_constructor_t construct_item)
{
	concurrent_queue_rep<T> &r = *my_rep;
...
class concurrent_queue_iterator : public concurrent_queue_iterator_base,
	...
	concurrent_queue_iterator(const concurrent_queue_base_v3 &queue)
	        : concurrent_queue_iterator_base_v3(queue,
	                                            __TBB_offsetof(concurrent_queue_base_v3::padded_page<Value>, last))
	{
	}
