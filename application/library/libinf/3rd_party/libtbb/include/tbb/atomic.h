...
#define __TBB_DECL_ATOMIC_LOAD_STORE_PRIMITIVES(M)                                            \
	template <> struct atomic_load_store_traits<M> {                                      \
		template <typename T> inline static T load(const volatile T &location)        \
		{                                                                             \
			return __TBB_load_##M(location);                                      \
		}                                                                             \
		template <typename T> inline static void store(volatile T &location, T value) \
		{                                                                             \
...
template <typename T> struct atomic_impl {
	...
	template <typename value_t> static typename converter<value_t>::bits_type to_bits(value_t value)
	{
...
//on suncc for 64 bit types for 32 bit target
	...
	{
		return to_value<value_type>(internal::atomic_traits<sizeof(value_type), M>::fetch_and_store(
	...
	{
		return to_value<value_type>(internal::atomic_traits<sizeof(value_type), M>::compare_and_swap(
		        &my_storage.my_value, to_bits(value), to_bits(comparand)));
	}
	...
	template <memory_semantics M> void store(value_type value)
	{
...
template <typename I, typename D, typename StepType> struct atomic_impl_with_arithmetic : atomic_impl<I> {
...
#endif
	template <memory_semantics M> value_type fetch_and_add(D addend)
	{
		return value_type(internal::atomic_traits<sizeof(value_type), M>::fetch_and_add(
	...
	template <memory_semantics M> value_type fetch_and_increment()
	{
	...
	template <memory_semantics M> value_type fetch_and_decrement()
	{
...
#define __TBB_DECL_ATOMIC_ALT(T, U)                                                        \
	template <> struct atomic<T> : internal::atomic_impl_with_arithmetic<T, T, char> { \
		...
		T operator=(U rhs)                                                         \
		{                                                                          \
			return store_with_release(T(rhs));                                 \
		}                                                                          \
		atomic<T> &operator=(const atomic<T> &rhs)                                 \
		{                                                                          \
			store_with_release(rhs);                                           \
...
template <memory_semantics M, typename T> T load(const atomic<T> &a)
{
	return a.template load<M>();
}

template <memory_semantics M, typename T> void store(atomic<T> &a, T value)
{
...
template <typename T> inline atomic<T> &as_atomic(T &t)
{
	return (atomic<T> &)t;
}
