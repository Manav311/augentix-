...
template <size_t N, typename function1, typename function2, typename function3> class spawner : public task {
	...
	task *execute()
	{
		if (is_recycled) {
			...
			internal::function_invoker<function2> *invoker2 = new (allocate_child())
			...
			if (n > 2) {
				internal::function_invoker<function3> *invoker3 = new (allocate_child())
...
class parallel_invoke_helper : public empty_task {
	...
	{
		internal::function_invoker<function> *invoker = new (allocate_child())
		        internal::function_invoker<function>(std::forward<function>(_func));
		__TBB_ASSERT(invoker, "Child task allocation failed");
		spawn(*invoker);
	}

	template <typename function> void add_children(function &&_func, tbb::task_group_context &)
	{
	...
	void add_children(function1 &&_func1, function2 &&_func2, function &&... _func)
	{
		...
		spawner_type &sub_root = *new (allocate_child()) spawner_type(std::forward<function1>(_func1),
	...
	void add_children(const function1 &_func1, const function2 &_func2)
	{
		// The third argument is dummy, it is ignored actually.
		parallel_invoke_noop noop;
		internal::spawner<2, function1, function2, parallel_invoke_noop> &sub_root =
		        *new (allocate_child())
		                internal::spawner<2, function1, function2, parallel_invoke_noop>(_func1, _func2, noop);
		spawn(sub_root);
	}
	// three arguments
	...
	{
		internal::spawner<3, function1, function2, function3> &sub_root = *new (
...
template <typename T1, typename... T> task_group_context &get_context(T1 && /*ignored*/, T &&... t)
{
...
template <typename F0, typename F1, typename... F> void parallel_invoke(F0 &&f0, F1 &&f1, F &&... f)
{
	...
	internal::parallel_invoke_impl(selector_type(), std::forward<F0>(f0), std::forward<F1>(f1),
	                               std::forward<F>(f)...);
}
...
void parallel_invoke(const F0 &f0, const F1 &f1, const F2 &f2, const F3 &f3, const F4 &f4, const F5 &f5, const F6 &f6,
                     const F7 &f7, tbb::task_group_context &context)
{
	internal::parallel_invoke_cleaner cleaner(4, context);
	internal::parallel_invoke_helper &root = cleaner.root;
...
template <typename F0, typename F1, typename F2, typename F3, typename F4, typename F5, typename F6, typename F7,
          typename F8>
void parallel_invoke(const F0 &f0, const F1 &f1, const F2 &f2, const F3 &f3, const F4 &f4, const F5 &f5, const F6 &f6,
                     const F7 &f7, const F8 &f8, tbb::task_group_context &context)
{
	internal::parallel_invoke_cleaner cleaner(4, context);
	internal::parallel_invoke_helper &root = cleaner.root;
...
void parallel_invoke(const F0 &f0, const F1 &f1, const F2 &f2, const F3 &f3, const F4 &f4, const F5 &f5, const F6 &f6,
                     const F7 &f7, const F8 &f8, const F9 &f9, tbb::task_group_context &context)
{
	internal::parallel_invoke_cleaner cleaner(4, context);
	internal::parallel_invoke_helper &root = cleaner.root;
