...
#define __TBB_ETS_USE_CPP11                                                                                          \
	(__TBB_CPP11_RVALUE_REF_PRESENT && __TBB_CPP11_VARIADIC_TEMPLATES_PRESENT && __TBB_CPP11_DECLTYPE_PRESENT && \
	...
	void table_elementwise_copy(const ets_base &other, void *(*add_element)(ets_base &, void *))
	{
		...
		for (array *r = other.my_root; r; r = r->next) {
			for (size_t i = 0; i < r->size(); ++i) {
				slot &s1 = r->at(i);
				if (!s1.empty()) {
					for (size_t j = root->start(tbb::tbb_hash<key_type>()(s1.key));;
					     j = (j + 1) & mask) {
						slot &s2 = root->at(j);
						if (s2.empty()) {
...
bool operator==(const segmented_iterator<SegmentedContainer, T> &i, const segmented_iterator<SegmentedContainer, U> &j)
{
...
class enumerable_thread_specific : internal::ets_base<ETS_key_type> {
	...
	typedef typename internal::enumerable_thread_specific_iterator<internal_collection_type, const value_type>
	        const_iterator;
	...
	template <typename Finit
	          ...
	          typename = typename internal::enable_if<
	                  internal::is_callable_no_args<typename internal::strip<Finit>::type>::value>::type
#endif
	          >
	enumerable_thread_specific(Finit finit)
	        : my_construct_callback(internal::callback_leaf<T, internal::construct_by_finit<T, Finit> >::make(
	...
	template <typename P1, typename... P,
	          typename = typename internal::enable_if<
	                  !internal::is_callable_no_args<typename internal::strip<P1>::type>::value &&
	                  !internal::is_compatible_ets<T, typename internal::strip<P1>::type>::value &&
	                  !internal::is_same_type<T, typename internal::strip<P1>::type>::value>::type>
	enumerable_thread_specific(P1 &&arg1, P &&... args)
	        : my_construct_callback(internal::callback_leaf<T, internal::construct_by_args<T, P1, P...> >::make(
	...
	void internal_copy(const enumerable_thread_specific<T, A2, C2> &other)
	{
		...
		__TBB_STATIC_ASSERT(
		        (internal::is_compatible_ets<T, typename internal::strip<decltype(other)>::type>::value),
...
template <typename Container> flattened2d<Container> flatten2d(const Container &c)
{
