...
template <typename Range, typename Body> class sum_node : public task {
	...
	task *create_child(const Range &range_, final_sum_type &f, sum_node *n, final_sum_type *incoming_,
	                   Body *stuff_last_)
	{
		if (!n) {
	...
	/*override*/ task *execute()
	{
		if (my_body) {
			...
			sum_node &c = *this;
			task *b = c.create_child(Range(my_range, split()), *my_left_sum, my_right, my_left_sum,
			                         my_stuff_last);
			task *a = my_left_is_final ? NULL :
			                             c.create_child(my_range, *my_body, my_left, my_incoming, NULL);
...
template <typename Range, typename Body, typename Partitioner = simple_partitioner> class start_scan : public task {
	...
	start_scan(sum_node_type *&return_slot_, const Range &range_, final_sum_type &body_,
...
template <typename Range, typename Body, typename Partitioner> task *start_scan<Range, Body, Partitioner>::execute()
{
	...
	task *next_task = NULL;
	if ((my_is_right_child && !treat_as_stolen) || !my_range.is_divisible() ||
		...
		finish_pass1_type &c = *new (allocate_continuation())
		                               finish_pass1_type(*my_return_slot, my_sum, *result);
		// Split off right child
		start_scan &b = *new (c.allocate_child())
		                        start_scan(/*my_return_slot=*/result->my_right, *this, result);
...
/** @ingroup algorithms **/
template <typename Range, typename Body> void parallel_scan(const Range &range, Body &body)
{
