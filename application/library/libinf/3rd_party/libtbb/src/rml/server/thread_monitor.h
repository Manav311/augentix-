...
#define __RML_DECL_THREAD_ROUTINE unsigned WINAPI
	...
	static handle_type launch(thread_routine_type thread_routine, void *arg, size_t stack_size,
	                          const size_t *worker_index = NULL);
...
#if __TBB_WIN8UI_SUPPORT
inline thread_monitor::handle_type thread_monitor::launch(thread_routine_type thread_function, void *arg, size_t,
                                                          const size_t *)
{
...
inline thread_monitor::handle_type thread_monitor::launch(thread_routine_type thread_routine, void *arg,
                                                          size_t stack_size, const size_t *worker_index)
{
	unsigned thread_id;
	...
	HANDLE h = (HANDLE)_beginthreadex(NULL, unsigned(stack_size), thread_routine, arg,
	                                  STACK_SIZE_PARAM_IS_A_RESERVATION | create_flags, &thread_id);
	if (!h) {
		fprintf(stderr, "thread_monitor::launch: _beginthreadex failed\n");
		exit(1);
	}
	if (number_of_processor_groups > 1) {
		tbb::internal::MoveThreadIntoProcessorGroup(
		        h, tbb::internal::FindProcessorGroupIndex(static_cast<int>(*worker_index)));
		ResumeThread(h);
	}
	return h;
}
