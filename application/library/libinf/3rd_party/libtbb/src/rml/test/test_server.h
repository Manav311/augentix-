...
const size_t ClientStackSize[MaxClient] = { 1000000
#if !TRIVIAL
	                                    ,
	                                    2000000, 1000000, 4000000
#endif /* TRIVIAL */
};
...
template <typename Client> class ClientBase : public Client {
    protected:
	...
	void set_server(rml::server *s)
	{
		my_server = s;
	}

	unsigned default_concurrency() const
	{
		ASSERT(my_server, NULL);
		return my_server->default_concurrency();
	}
...
template <typename Client> typename Client::job *ClientBase<Client>::create_one_job()
{
...
struct warning_tracker {
	...
	bool all_set()
	{
		return n_more_than_available > 0 && n_too_many_threads > 0 && n_system_overload > 0;
	}
} tracker;
...
class Checker {
	...
	Checker(rml::server &server)
	        : default_concurrency(int(server.default_concurrency()))
	{
	}
};
...
template <typename Factory, typename Client> class DoOneConnection : NoAssign {
...
template <typename Factory, typename Client> void DoOneConnection<Factory, Client>::operator()(size_t i) const
{
	...
	if (TestSingleConnection)
		__TBB_ASSERT_EX(uintptr_t(factory.scratch_ptr) == uintptr_t(default_concurrency),
		                "under/over subscription?");
#endif
}

//! Test with n_threads threads and n_client clients.
template <typename Factory, typename Client> void SimpleTest()
{
	...
	for (int n_thread = MinThread; n_thread <= MaxThread; ++n_thread) {
		...
		for (int n_client = 1; n_client <= int(MaxClient); ++n_client) {
			...
			NativeParallelFor(n_client,
			                  DoOneConnection<Factory, Client>(n_thread, Nesting(0, 0), 0, false));
		}
	...
	ASSERT(Harness::ConcurrencyTracker::PeakParallelism() > 1 || server_concurrency == 0,
	       "No multiple connections exercised?");
#endif /* !TRIVIAL */
	// Let RML catch up.
...
template <typename Factory, typename Client> void VerifyInitialization(int n_thread)
{
	Client *client = new Client;
