...
template <typename SchedulerTraits> class custom_scheduler : private generic_scheduler {
	...
	{
		static_cast<custom_scheduler *>(governor::local_scheduler())
		        ->scheduler_type::local_wait_for_all(parent, child);
	}
...
task *custom_scheduler<SchedulerTraits>::receive_or_steal_task(__TBB_atomic reference_count &completion_ref_count)
{
	...
	if (outermost_dispatch_level) {
		if (intptr_t skipped_priority = my_arena->my_skipped_fifo_priority) {
			...
			if (my_arena->my_skipped_fifo_priority.compare_and_swap(0, skipped_priority) ==
			            skipped_priority &&
...
#if __TBB_TASK_PRIORITY
				if (outermost_worker_level ||
				    my_arena->my_top_priority > my_arena->my_bottom_priority) {
...
void custom_scheduler<SchedulerTraits>::local_wait_for_all(task &parent, task *child)
{
		...
		try {
				...
				for (;;) {
					...
					while (t) {
						...
						if (!t->prefix().context->my_cancellation_requested)
							...
							__TBB_ASSERT(1L << t->state() & (1L << task::allocated |
							                                 1L << task::ready |
							                                 1L << task::reexecute),
							             NULL);
						assert_task_pool_valid();
						...
						if (p != *my_ref_top_priority &&
							...
							if (p < effective_reference_priority()) {
								if (!my_offloaded_tasks) {
									my_offloaded_task_list_tail_link =
									        &t->prefix().next_offloaded;
...
#endif
						{
							...
							GATHER_STATISTIC(my_counters.avg_arena_concurrency +=
							                 my_arena->num_workers_active());
							GATHER_STATISTIC(my_counters.avg_assigned_workers +=
							                 my_arena->my_num_workers_allotted);
#if __TBB_TASK_PRIORITY
							GATHER_STATISTIC(my_counters.avg_arena_prio += p);
							GATHER_STATISTIC(my_counters.avg_market_prio +=
							                 my_market->my_global_top_priority);
#endif /* __TBB_TASK_PRIORITY */
							ITT_STACK(SchedulerTraits::itt_possible, callee_enter,
							          t->prefix().context->itt_caller);
							t_next = t->execute();
							ITT_STACK(SchedulerTraits::itt_possible, callee_leave,
							          t->prefix().context->itt_caller);
							if (t_next) {
								__TBB_ASSERT(
								        t_next->state() == task::allocated,
								...
								if (next_affinity != 0 &&
								    next_affinity != my_affinity_id)
									GATHER_STATISTIC(
									        ++my_counters.affinity_ignored);
#endif
							}
						}
						assert_task_pool_valid();
			...
			stealing_ground:
				...
				if (quit_point == all_local_work_done) {
					__TBB_ASSERT(!is_task_pool_published() && is_quiescent_local_task_pool_reset(),
					             NULL);
	...
	if (!ConcurrentWaitsEnabled(parent)) {
		if (parent.prefix().ref_count != parents_work_done) {
			...
			__TBB_ASSERT(worker_outermost_level(), "Worker thread exits nested dispatch loop prematurely");
			return;
		}
	...
	__TBB_ASSERT(!is_worker() || !CancellationInfoPresent(*my_dummy_task), "Worker's dummy task context modified");
