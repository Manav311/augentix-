...
class FastRandom {
	...
	template <typename T> void init(T seed)
	{
...
template <typename T1, typename T2, class Pred> T1 atomic_update(tbb::atomic<T1> &dst, T2 newValue, Pred compare)
{
	T1 oldValue = dst;
...
template <typename F> void atomic_do_once(const F &initializer, atomic<do_once_state> &state)
{
