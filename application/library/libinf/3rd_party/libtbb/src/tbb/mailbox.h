...
struct task_proxy : public task {
	...
	template <intptr_t from_bit> inline task *extract_task()
	{
...
class mail_outbox : padded<unpadded_mail_outbox> {
	task_proxy *internal_pop()
	{
		...
		if (task_proxy *second = first->next_in_mailbox) {
			...
			my_first = NULL;
			if (as_atomic(my_last).compare_and_swap(&my_first, &first->next_in_mailbox) ==
				...
				while (!(second = first->next_in_mailbox))
					backoff.pause();
				my_first = second;
			}
		}
