...
template <typename T> class basic_tls {
#if USE_PTHREAD
	...
	int destroy()
	{
		return pthread_key_delete(my_key);
	}
	void set(T value)
	{
		pthread_setspecific(my_key, (void *)value);
	}
	T get()
	{
		return (T)pthread_getspecific(my_key);
	}
#else /* USE_WINTHREAD */
...
template <typename T> class tls : public basic_tls<T> {
...
template <typename T> class tls<T *> : basic_tls<T *> {
	typedef basic_tls<T *> base;
