...
class circular_doubly_linked_list_with_sentinel : no_copy {
	...
	struct node_t {
		...
		node_t *prev;
		explicit node_t()
		        : next((node_t *)(uintptr_t)0xcdcdcdcd)
		        , prev((node_t *)(uintptr_t)0xcdcdcdcd)
		{
		}
	};
	...
	inline node_t *front() const
	{
		return head.next;
	}
	inline node_t *last() const
	{
		return head.prev;
	}
	inline node_t *begin() const
	{
		return front();
	}
	inline const node_t *end() const
	{
		return &head;
	}
...
class concurrent_monitor : no_copy {
	...
	template <typename WaitUntil, typename Context> void wait(WaitUntil until, Context on);
	...
	{
		return static_cast<thread_context *>(n);
	}
};

template <typename WaitUntil, typename Context> void concurrent_monitor::wait(WaitUntil until, Context on)
{
	bool slept = false;
...
template <typename P> void concurrent_monitor::notify_relaxed(const P &predicate)
{
	if (waitset_ec.empty())
		return;
	waitset_t temp;
