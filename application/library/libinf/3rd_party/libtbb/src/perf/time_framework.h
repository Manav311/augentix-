...
template <typename Tester, int scale = 1> class TimeTest : public Tester {
...
template <typename Tester> class NanosecPerValue : public Tester {
	/*override*/ value_t test(int testn, int threadn)
	{
		Timer timer;
		Tester::test(testn, threadn);
...
template <typename Tester, int scale = 1> class ValuePerSecond : public Tester {
	/*override*/ value_t test(int testn, int threadn)
	{
		Timer timer;
		Tester::test(testn, threadn);
...
template <typename Tester, int scale = 1> class NumberPerSecond : public Tester {
	/*override*/ value_t test(int testn, int threadn)
	{
		Timer timer;
		Tester::test(testn, threadn);
...
class TestRunner {
	...
	TestRunner(const char *name, Test *test)
	        : tester_name(name)
	        , tester(*static_cast<TesterBase *>(test))
	{
...
struct RunArgsBody {
	...
	RunArgsBody(const vector<TestRunner *> &a)
	        : run_list(a)
	{
	}
	...
	void operator()(const tbb::blocked_range<int> &r) const
	{
		...
		int thread = r.begin();
#endif
		for (size_t i = 0; i < run_list.size(); i++) run_list[i]->run_test(thread);
}
}
;
...
class TestProcessor {
	...
	// token of tests list
	template <typename Test> TestRunner *run(const char *name, Test *test)
	{
	...
	TestProcessor(const char *name, StatisticsCollector::Sorting sort_by = StatisticsCollector::ByAlg)
	        ...
	        , stat(NULL)
	        , end(0)
	        , report(collection_name, sort_by)
	{
	}
