...
__TBB_PERF_API extern int NumThreads, MaxConcurrency, NumCpus;
...
class __TBB_PERF_API Test {
	...
	virtual const char *HistogramName()
	{
...
template <class T> void RegisterTest()
{
	internal::RegisterTest(new T, __TBB_PERF_TEST_CLASS_NAME(T), true);
}

template <class T> void RegisterTest(T &t)
{
	internal::RegisterTest(&t, __TBB_PERF_TEST_CLASS_NAME(T), false);
}
...
__TBB_PERF_API int TestMain(int argc, char *argv[], const SessionSettings *defaultSettings = NULL);
