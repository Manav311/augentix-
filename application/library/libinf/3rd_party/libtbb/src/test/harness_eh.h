...
tbb::atomic<intptr_t> g_CurExecuted, g_ExecutedAtLastCatch, g_ExecutedAtFirstCatch, g_ExceptionsThrown,
        ...
        g_PipelinesStarted;
volatile bool g_ExceptionCaught = false, g_UnknownException = false;
...
volatile bool g_ThrowException = true,
        ...
        g_Flog = false, g_MasterExecuted = false, g_NonMasterExecuted = false;
...
class test_exception : public std::exception {
    ...
    public:
	test_exception(const char *description)
...
class solitary_test_exception : public test_exception {
    public:
	solitary_test_exception(const char *description)
	        : test_exception(description)
	{
	}
};
...
#define UPDATE_COUNTS()                                \
	{                                              \
		++g_CurExecuted;                       \
		if (g_Master == Harness::CurrentTid()) \
			g_MasterExecuted = true;       \
		else                                   \
...
#define CATCH()                                                                                                        \
	...
	{                                                                                                              \
		...
		ASSERT(__TBB_EXCEPTION_TYPE_INFO_BROKEN ||                                                             \
		               strcmp(EXCEPTION_NAME(e),                                                               \
		                      (g_SolitaryException ? typeid(solitary_test_exception) : typeid(test_exception)) \
		                              .name()) == 0,                                                           \
		       "Unexpected original exception name");                                                          \
		ASSERT(__TBB_EXCEPTION_TYPE_INFO_BROKEN || strcmp(e.what(), EXCEPTION_DESCR) == 0,                     \
		       "Unexpected original exception info");                                                          \
...
#define ASSERT_EXCEPTION()                                                                                       \
	{                                                                                                        \
		...
		ASSERT(g_ExceptionCaught || (g_ExceptionInMaster && !g_MasterExecutedThrow) ||                   \
...
class CancellatorTask : public tbb::task {
	...
	CancellatorTask(tbb::task_group_context &ctx, intptr_t threshold)
	        : m_groupToCancel(ctx)
	        , m_cancellationThreshold(threshold)
	{
		s_Ready = false;
	}
...
template <class LauncherTaskT, class CancellatorTaskT> void RunCancellationTest(intptr_t threshold = 1)
{
