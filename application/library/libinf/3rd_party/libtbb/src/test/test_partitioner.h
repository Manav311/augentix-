		...
		if (!sc.m_called) {
			...
			if (sc.m_statData) {
				...
				if (*minRangeSize > rangeSize ||
				    !sc.m_statData->m_wasMinRangeSizeWritten) { // if minimum is not an actual minimum
...
template <typename DerivedRange, typename T> class RangeBase : public RangeStatisticCollector {
    protected:
    ...
    public:
	RangeBase(size_t _begin, size_t _end, RangeStatisticData *statData, bool provide_feedback,
	          bool ensure_non_empty_size)
	...
	RangeBase(RangeBase &r, tbb::split)
	...
	DerivedRange &self()
	{
		return static_cast<DerivedRange &>(*this);
	}
	size_t round(T part)
	{
		return size_t(part);
	...
	bool is_ensure_non_emptiness()
	{
		return m_ensure_non_empty_size;
	}
};
...
class RoundedDownRange : public RangeBase<RoundedDownRange, float> {
    public:
	RoundedDownRange(size_t _begin, size_t _end, RangeStatisticData *statData, bool provide_feedback,
	                 bool ensure_non_empty_size)
	        : RangeBase<RoundedDownRange, float>(_begin, _end, statData, provide_feedback, ensure_non_empty_size)
	{
	}
...
class RoundedUpRange : public RangeBase<RoundedUpRange, float> {
    public:
	RoundedUpRange(size_t _begin, size_t _end, RangeStatisticData *statData, bool provide_feedback,
	               bool ensure_non_empty_size)
	        : RangeBase<RoundedUpRange, float>(_begin, _end, statData, provide_feedback, ensure_non_empty_size)
	{
	}
	...
	size_t round(float part)
	{
		return size_t(std::ceil(part));
	}
	static const bool is_splittable_in_proportion = true;
};
...
class Range1_2 : public RangeBase<Range1_2, float> {
    public:
	Range1_2(size_t _begin, size_t _end, RangeStatisticData *statData, bool provide_feedback,
	         bool ensure_non_empty_size)
	        : RangeBase<Range1_2, float>(_begin, _end, statData, provide_feedback, ensure_non_empty_size)
	{
	}
	Range1_2(Range1_2 &r, tbb::split)
	        : RangeBase<Range1_2, float>(r, tbb::split())
	{
	}
	Range1_2(Range1_2 &r, proportional_split &p)
	        : RangeBase<Range1_2, float>(r, p)
	{
	}
	static const bool is_splittable_in_proportion = true;
...
class Range1_999 : public RangeBase<Range1_999, float> {
    public:
	Range1_999(size_t _begin, size_t _end, RangeStatisticData *statData, bool provide_feedback,
	           bool ensure_non_empty_size)
	        : RangeBase<Range1_999, float>(_begin, _end, statData, provide_feedback, ensure_non_empty_size)
	{
	}
	Range1_999(Range1_999 &r, tbb::split)
	        : RangeBase<Range1_999, float>(r, tbb::split())
	{
	}
	Range1_999(Range1_999 &r, proportional_split &p)
	        : RangeBase<Range1_999, float>(r, p)
	{
	}
	static const bool is_splittable_in_proportion = true;
...
class Range999_1 : public RangeBase<Range999_1, float> {
    public:
	Range999_1(size_t _begin, size_t _end, RangeStatisticData *statData, bool provide_feedback,
	           bool ensure_non_empty_size)
	        : RangeBase<Range999_1, float>(_begin, _end, statData, provide_feedback, ensure_non_empty_size)
	{
	}
	Range999_1(Range999_1 &r, tbb::split)
	        : RangeBase<Range999_1, float>(r, tbb::split())
	{
	}
	Range999_1(Range999_1 &r, proportional_split &p)
	        : RangeBase<Range999_1, float>(r, p)
	{
	}
	static const bool is_splittable_in_proportion = true;
...
class InvertedProportionRange : public RangeBase<InvertedProportionRange, float> {
    public:
	InvertedProportionRange(size_t _begin, size_t _end, RangeStatisticData *statData, bool provide_feedback,
	                        bool ensure_non_empty_size)
	...
	float compute_right_part(RangeBase<InvertedProportionRange, float> &r, proportional_split &p)
	{
...
class ExactSplitRange : public RangeBase<ExactSplitRange, size_t> {
    public:
	ExactSplitRange(size_t _begin, size_t _end, RangeStatisticData *statData, bool provide_feedback,
	                bool ensure_non_empty_size)
	        : RangeBase<ExactSplitRange, size_t>(_begin, _end, statData, provide_feedback, ensure_non_empty_size)
	{
	}
...
struct TreeNode {
	...
	friend TreeNode *make_node(size_t range_begin, size_t range_end, size_t affinity, TreeNode *left,
	                           TreeNode *right);
};

TreeNode *make_node(size_t range_begin, size_t range_end, size_t affinity, TreeNode *left = NULL,
                    TreeNode *right = NULL)
{
...
class BinaryTree {
	...
	void push_node(TreeNode *node)
	{
		...
		if (m_root) {
			ASSERT(node->m_range_begin >= m_root->m_range_begin && node->m_range_end <= m_root->m_range_end,
	...
	bool has_children(TreeNode *node)
	{
	...
	bool has_left_child(TreeNode *node)
	{
		return node->m_left != NULL;
	}

	bool is_subnode(TreeNode *root_node, TreeNode *node)
	{
		return root_node->m_range_begin <= node->m_range_begin && node->m_range_end <= root_node->m_range_end;
	}

	bool are_nodes_equal(TreeNode *node1, TreeNode *node2) const
	{
		return node1->m_range_begin == node2->m_range_begin && node1->m_range_end == node2->m_range_end;
	}
...
class SimpleBody {
	...
	template <typename Range> void operator()(Range &) const
	{
	}
};
...
class SimpleReduceBody {
	...
	template <typename Range> void operator()(Range &)
	{
	}
...
class SplitConstructorAssertedRange {
	...
	{
		*this = r;
		ASSERT(!my_assert_in_nonproportional,
		       "Disproportional splitting constructor was called but should not been");
	}
...
class Range1 : public SplitConstructorAssertedRange {
	...
	Range1(Range1 &r, tbb::split)
	        : SplitConstructorAssertedRange(r, tbb::split())
	{
	}
	Range1(Range1 &r, proportional_split &proportion)
	        : SplitConstructorAssertedRange(r, proportion)
	{
	}
	static const bool is_splittable_in_proportion = true;
};
...
class Range2 : public SplitConstructorAssertedRange {
	...
	Range2(Range2 &r, tbb::split)
	        : SplitConstructorAssertedRange(r, tbb::split())
	{
	}
	Range2(Range2 &r, proportional_split &p)
	        : SplitConstructorAssertedRange(r, p)
	{
...
class Range3 : public SplitConstructorAssertedRange {
	...
	Range3(Range3 &r, tbb::split)
	        : SplitConstructorAssertedRange(r, tbb::split())
	{
	}
	Range3(Range3 &r, proportional_split &p)
	        : SplitConstructorAssertedRange(r, p)
	{
