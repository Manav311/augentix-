...
template <class T> class ForwardIterator {
	...
	ForwardIterator(const ForwardIterator &r)
	        : my_ptr(r.my_ptr)
	{
	}
...
template <class T> class RandomIterator {
	...
	difference_type operator-(const RandomIterator &r) const
	{
		return my_ptr - r.my_ptr;
	}
...
template <class T> class ConstRandomIterator {
	const T *my_ptr;
#if !HARNESS_EXTENDED_STD_COMPLIANCE
	typedef typename std::allocator<T>::difference_type difference_type;
#endif
	...
	explicit ConstRandomIterator(const T *ptr)
	        : my_ptr(ptr)
	{
	}
	ConstRandomIterator(const ConstRandomIterator &r)
	        : my_ptr(r.my_ptr)
	{
	}
	const T &operator*()
	...
	difference_type operator-(const ConstRandomIterator &r) const
	{
		return my_ptr - r.my_ptr;
	}
	...
	bool operator<(const ConstRandomIterator &r) const
	{
		return my_ptr < r.my_ptr;
	}
};
...
namespace std
{
template <typename T> struct iterator_traits<Harness::InputIterator<T> > {
	typedef std::input_iterator_tag iterator_category;
	typedef T value_type;
	typedef value_type &reference;
};

template <typename T> struct iterator_traits<Harness::ForwardIterator<T> > {
	typedef std::forward_iterator_tag iterator_category;
	typedef T value_type;
	typedef value_type &reference;
};

template <typename T> struct iterator_traits<Harness::RandomIterator<T> > {
	typedef std::random_access_iterator_tag iterator_category;
	typedef T value_type;
	typedef value_type &reference;
};

template <typename T> struct iterator_traits<Harness::ConstRandomIterator<T> > {
	typedef std::random_access_iterator_tag iterator_category;
	typedef T value_type;
