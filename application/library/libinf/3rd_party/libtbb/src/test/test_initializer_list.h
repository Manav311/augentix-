...
struct skip_test {
	template <typename container_type, typename element_type>
	static void do_test(std::initializer_list<element_type>, container_type const &)
	{ /* do nothing */
	}
};
...
#define __TBB_TEST_INIT_LIST_SUITE_SINGLE(FUNC_NAME, CONTAINER, ELEMENT_TYPE, INIT_SEQ)                                     \
	...
	{                                                                                                                   \
		...
		ASSERT(v1 == expected,                                                                                      \
		       "initialization via explicit constructor call with std::initializer_list failed");                   \
                                                                                                                            \
...
{
template <typename T> class ad_hoc_container {
	std::vector<T> vec;
	...
	ad_hoc_container(InputIterator begin, InputIterator end)
	        : vec(begin, end)
	{
	}
	ad_hoc_container(std::initializer_list<T> il)
	        : vec(il.begin(), il.end())
	{
	}
	ad_hoc_container(ad_hoc_container const &other)
	        : vec(other.vec)
	{
	}
	ad_hoc_container &operator=(ad_hoc_container const &rhs)
	{
		vec = rhs.vec;
		return *this;
	}
	ad_hoc_container &operator=(std::initializer_list<T> il)
	{
	...
	friend bool operator==(ad_hoc_container<T> const &lhs, ad_hoc_container<T> const &rhs)
	{
...
#define AD_HOC_INIT_SEQ    \
	{                  \
		1, 2, 3, 4 \
	}
__TBB_TEST_INIT_LIST_SUITE_SINGLE(TestCompilerSupportInt, initializer_list_helpers::ad_hoc_container, int,
                                  AD_HOC_INIT_SEQ)
...
__TBB_TEST_INIT_LIST_SUITE_SINGLE(TestCompilerSupportIntPair, initializer_list_helpers::ad_hoc_container,
                                  AD_HOC_INIT_SEQ_PAIR_TYPE, AD_HOC_PAIR_INIT_SEQ)
