...
#define setaffinity(mask) cpuset_setaffinity(CPU_LEVEL_WHICH, CPU_WHICH_PID, -1, sizeof(mask_t), &mask)
	...
	ASSERT_WARNING(
	        maskSize >= maxProcs,
	        "The mask size doesn't seem to be big enough to call setaffinity. The call may return an error.");

	ASSERT(max_threads <= (int)sizeof(mask_t) * CHAR_BIT,
	       "The mask size is not enough to set the requested number of threads.");
