...
template <typename counter_type = size_t> struct arena_data {
...
template <typename T, typename pocma = Harness::false_type, typename counter_type = size_t> struct arena {
	...
	arena(arena_data_t &data) __TBB_NOEXCEPT(true)
	        : my_data(&data)
	{
	}

	template <typename U1, typename U2, typename U3> friend struct arena;

	template <typename U1, typename U2>
	arena(arena<U1, U2, counter_type> const &other) __TBB_NOEXCEPT(true)
	...
	void deallocate(pointer p_arg, size_type n)
	{
		...
		__TBB_ASSERT(p >= my_data->my_buffer && p <= my_data->my_buffer + my_data->my_size,
		             "trying to deallocate pointer not from arena ?");
		__TBB_ASSERT(p + n * sizeof(T) <= my_data->my_buffer + my_data->my_size,
		             "trying to deallocate incorrect number of items?");
...
template <typename count_t = tbb::atomic<size_t> > struct allocator_counters {
	...
	{
		return lhs.items_allocated == rhs.items_allocated && lhs.items_freed == rhs.items_freed &&
		       lhs.allocations == rhs.allocations && lhs.frees == rhs.frees;
	}
};
...
template <typename base_alloc_t, typename count_t> size_t static_counting_allocator<base_alloc_t, count_t>::max_items;
...
template <typename base_alloc_t, typename count_t> count_t static_counting_allocator<base_alloc_t, count_t>::frees;
template <typename base_alloc_t, typename count_t> bool static_counting_allocator<base_alloc_t, count_t>::verbose;
template <typename base_alloc_t, typename count_t> bool static_counting_allocator<base_alloc_t, count_t>::throwing;

template <typename tag, typename count_t = tbb::atomic<size_t> > class static_shared_counting_allocator_base {
    public:
	typedef allocator_counters<count_t> counters_t;
...
template <typename tag, typename count_t> size_t static_shared_counting_allocator_base<tag, count_t>::max_items;

template <typename tag, typename count_t> count_t static_shared_counting_allocator_base<tag, count_t>::items_allocated;

template <typename tag, typename count_t> count_t static_shared_counting_allocator_base<tag, count_t>::items_freed;

template <typename tag, typename count_t> count_t static_shared_counting_allocator_base<tag, count_t>::allocations;

template <typename tag, typename count_t> count_t static_shared_counting_allocator_base<tag, count_t>::frees;

template <typename tag, typename count_t> bool static_shared_counting_allocator_base<tag, count_t>::verbose;

template <typename tag, typename count_t> bool static_shared_counting_allocator_base<tag, count_t>::throwing;

template <typename tag, typename base_alloc_t, typename count_t = tbb::atomic<size_t> >
class static_shared_counting_allocator : public static_shared_counting_allocator_base<tag, count_t>,
                                         public base_alloc_t {
	...
	typedef typename base_alloc_t::difference_type difference_type;
	template <typename U> struct rebind {
		typedef static_shared_counting_allocator<tag, typename base_alloc_t::template rebind<U>::other, count_t>
		        other;
	};
...
class local_counting_allocator : public base_alloc_t {
	...
	void set_counters(const count_t &a_items_allocated, const count_t &a_items_freed, const count_t &a_allocations,
	                  const count_t &a_frees, const count_t &a_max_items)
	{
	...
	template <typename allocator_t> void set_counters(const allocator_t &a)
	{
	...
	template <typename U, typename C> local_counting_allocator(const static_counting_allocator<U, C> &a) throw()
	{
		this->set_counters(a);
	}
...
template <template <typename T> class Allocator> class debug_allocator<void, Allocator> : public Allocator<void> {
    public:
...
class stateful_allocator : public Allocator<T> {
	...
	stateful_allocator() throw()
	        : unique_pointer(this)
	{
	}

	template <typename U>
	stateful_allocator(const stateful_allocator<U, pocma> &a) throw()
	        : base_allocator_type(Allocator<U>(a))
	        , unique_pointer(a.uniqe_pointer)
	{
	}
...
struct IsEqual {
	...
	template <typename T> static bool compare(const std::weak_ptr<T> &t1, const std::weak_ptr<T> &t2)
	{
		// Compare real pointers.
		return t1.lock().get() == t2.lock().get();
	}
	template <typename T> static bool compare(const std::unique_ptr<T> &t1, const std::unique_ptr<T> &t2)
	{
		// Compare real values.
		return *t1 == *t2;
	}
	...
	template <typename T1, typename T2> static bool compare(const T1 &t1, const T2 &t2)
	{
		return t1 == t2;
	}
	template <typename T1, typename T2> bool operator()(T1 &t1, T2 &t2) const
	{
