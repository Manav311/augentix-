...
{
	ASSERT((fe->x87cw & FE_RND_MODE_MASK) == ((fe->x87cw & FE_RND_MODE_MASK) >> 2 & _MCW_RC) << 2,
	       "Check float.h constants");
...
const int FE_TONEAREST = _RC_NEAR, FE_DOWNWARD = _RC_DOWN, FE_UPWARD = _RC_UP, FE_TOWARDZERO = _RC_CHOP;
...
inline void SetRoundingMode(int rnd)
{
	fesetround(rnd);
}
...
#define AssertFPMode(mode)                                                                                             \
	{                                                                                                              \
		ASSERT(GetRoundingMode() == RoundingModes[mode / NumSseModes % NumRoundingModes],                      \
		       "FPU control state has not been set correctly.");                                               \
