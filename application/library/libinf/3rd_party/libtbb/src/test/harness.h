...
namespace Harness
{
enum TestResult { Done, Skipped, Unknown };
}
...
struct InitReporter {
	...
	InitReporter()
	        : default_terminate_handler(NULL)
	{
#if TEST_USES_TBB
#if TBB_USE_ASSERT
		tbb::set_assertion_handler(ReportError);
#endif
		ASSERT_WARNING(TBB_INTERFACE_VERSION <= tbb::TBB_runtime_interface_version(),
...
template <typename T, size_t N> inline T *end(T (&array)[N])
{
...
namespace Harness
{
template <typename T, size_t N> tbb::blocked_range<T *> make_blocked_range(T (&array)[N])
{
...
template <typename Index, typename Body> class NativeParallelForTask : NoCopy {
    public:
...
template <typename Index, typename Body> void NativeParallelFor(Index n, const Body &body)
{
...
template <typename T> void zero_fill(void *array, size_t n)
{
...
template <typename T1, typename T2> T1 min(const T1 &val1, const T2 &val2)
{
...
/** Provided here to avoid including not strict safe <algorithm>.\n
    In case operands cause signed/unsigned or size mismatch warnings it is caller's
    responsibility to do the appropriate cast before calling the function. **/
template <typename T1, typename T2> T1 max(const T1 &val1, const T2 &val2)
{
	return val1 < val2 ? val2 : val1;
}
#endif /* !max */

template <typename T> static inline bool is_aligned(T arg, size_t alignment)
{
...
class NoAfterlife {
    protected:
	enum state_t { LIVE = 0x56781234, DEAD = 0xDEADBEEF } m_state;
	...
	NoAfterlife(const NoAfterlife &src)
	        : m_state(LIVE)
	{
...
static const unsigned Primes[] = { 0x9e3779b1, 0xffe6cc59, 0x2109f6dd, 0x43977ab5, 0xba5703f5, 0xb495a877, 0xe1626741,
