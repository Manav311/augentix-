...
template <bool allow_zero_initialized_state = false> struct StateTrackable : StateTrackableBase {
...
struct FooWithAssign : public Foo {
	...
	FooWithAssign(intptr_t barr)
	        : Foo(barr)
	{
	}
	FooWithAssign(FooWithAssign const &f)
	        : Foo(f)
	{
	}
	FooWithAssign &operator=(FooWithAssign const &f)
	{
		return static_cast<FooWithAssign &>(Foo::operator=(f));
	}

#if __TBB_CPP11_RVALUE_REF_PRESENT
	FooWithAssign(FooWithAssign &&f)
	        : Foo(std::move(f))
	{
	}
...
template <typename FooIteratorType> class FooIteratorBase {
    protected:
	intptr_t x_bar;

    private:
	FooIteratorType &as_derived()
	{
		return *static_cast<FooIteratorType *>(this);
	}
...
class FooIterator : public std::iterator<std::input_iterator_tag, FooWithAssign>, public FooIteratorBase<FooIterator> {
    public:
	FooIterator(intptr_t x)
	        : FooIteratorBase<FooIterator>(x)
	{
	}
...
class FooPairIterator : public std::iterator<std::input_iterator_tag, std::pair<FooWithAssign, FooWithAssign> >,
                        public FooIteratorBase<FooPairIterator> {
    public:
	FooPairIterator(intptr_t x)
	        : FooIteratorBase<FooPairIterator>(x)
	{
	}
...
template <typename Foo_type> void TestAssignOperator()
{
	Foo_type src;
	Foo_type dst;
	dst = (src);
...
#if __TBB_CPP11_RVALUE_REF_PRESENT
template <typename Foo_type> void TestMoveConstructor()
{
	Foo_type src;
...
template <typename Foo_type> void TestMoveAssignOperator()
{
	Foo_type src;
	Foo_type dst;
	dst = std::move(src);
...
#define ASSERT_THROWS_IN_TEST(expression, exception_type, message, test_name)  \
	try {                                                                  \
		...
		ASSERT_IN_TEST(false, "unexpected exception", test_name);      \
	}
...
bool is_state(Harness::StateTrackable<allow_zero_initialized_state> const &f)
{
	return f.state == desired_state;
}

template <Harness::StateTrackableBase::State desired_state> struct is_state_f {
	template <bool allow_zero_initialized_state>
	bool operator()(Harness::StateTrackable<allow_zero_initialized_state> const &f)
	{
	...
	template <typename T1, typename T2> bool operator()(std::pair<T1, T2> const &p)
	{
		return /*is_state<desired_state>(p.first) && */ is_state<desired_state>(p.second);
	}
};
...
template <typename container, typename unary_predicate> bool all_of(container const &c, unary_predicate p)
{
...
void TestAllOf()
{
	...
	ASSERT(::all_of(foos, Harness::end(foos), is_state_f<Foo::DefaultInitialized>()),
	       "all_of returned false while true expected");
	ASSERT(!::all_of(foos, Harness::end(foos), is_state_f<Foo::CopyInitialized>()),
	       "all_of returned true while false expected  ");
}

template <typename static_counter_allocator_type> struct track_allocator_memory : NoCopy {
	typedef typename static_counter_allocator_type::counters_t counters_t;

	counters_t previous_state;
	const char *const test_name;
	track_allocator_memory(const char *a_test_name)
	        : test_name(a_test_name)
	{
		static_counter_allocator_type::init_counters();
	}
	...
	{
		ASSERT_IN_TEST(static_counter_allocator_type::items_allocated ==
		                       static_counter_allocator_type::items_freed,
		               "memory leak?", test_name);
		ASSERT_IN_TEST(static_counter_allocator_type::allocations == static_counter_allocator_type::frees,
		               "memory leak?", test_name);
	}
	void save_allocator_counters()
	{
		previous_state = static_counter_allocator_type::counters();
	}
	void verify_no_more_than_x_memory_items_allocated(size_t expected_number_of_items_to_allocate)
	{
		...
		ASSERT_IN_TEST((now.items_allocated - previous_state.items_allocated) <=
		                       expected_number_of_items_to_allocate,
		               "More then excepted memory allocated ?", test_name);
	}
};

#include <vector>
template <int line_n> struct track_foo_count : NoCopy {
	bool active;
	size_t previous_state;
	const char *const test_name;
	track_foo_count(const char *a_test_name)
	        : active(true)
	        , previous_state(FooCount)
	        , test_name(a_test_name)
	{
	}
	~track_foo_count()
...
struct limit_foo_count_in_scope : NoCopy {
	size_t previous_state;
	bool active;
	limit_foo_count_in_scope(size_t new_limit, bool an_active = true)
	        : previous_state(MaxFooCount)
	        , active(an_active)
	{
		if (active) {
...
template <typename static_counter_allocator_type> struct limit_allocated_items_in_scope : NoCopy {
	size_t previous_state;
	bool active;
	limit_allocated_items_in_scope(size_t new_limit, bool an_active = true)
	        : previous_state(static_counter_allocator_type::max_items)
	        , active(an_active)
	{
		if (active) {
...
struct default_container_traits {
	...
	static container_type &construct_container(tbb::aligned_space<container_type> &storage, iterator_type begin,
	                                           iterator_type end)
	{
	...
	static container_type &construct_container(tbb::aligned_space<container_type> &storage, iterator_type begin,
	                                           iterator_type end, allocator_type const &a)
	{
...
struct memory_locations {
	...
	memory_locations(container_type const &source)
	        : locations(source.size())
	{
		for (typename container_type::const_iterator it = source.begin(); it != source.end(); ++it) {
			locations[std::distance(source.begin(), it)] = &*it;
		}
	}

	template <typename container_t> bool content_location_unchanged(container_t const &dst)
	{
		struct is_same_location {
			static bool compare(typename container_t::value_type const &v, const void *location)
			{
				return &v == location;
			}
		};
	...
	template <typename container_t> bool content_location_changed(container_t const &dst)
	{
...
template <typename container_traits, typename allocator_t> struct move_fixture : NoCopy {
	...
	move_fixture(const char *a_test_name, size_t a_container_size = default_container_size)
	        ...
	        , source(container_traits::construct_container(source_storage, init_iterator_type(0),
	                                                       init_iterator_type(container_size)))
	        , locations(source)
	        , test_name(a_test_name)
	{
	...
	move_fixture(const char *a_test_name, allocator_t const &a, size_t a_container_size = default_container_size)
	        : container_size(a_container_size)
	        , source(container_traits::construct_container(source_storage, init_iterator_type(0),
	                                                       init_iterator_type(container_size), a))
	        , locations(source)
	        , test_name(a_test_name)
	{
	...
	void init(const std::string &ctor_name)
	{
		...
		verify_content_equal_to_source(
		        source,
		        "did not properly initialized source? Or can not check container for equality with expected ?: " +
		                ctor_name);
	...
	template <typename container_type> void verify_size(container_type const &dst, const char *a_test_name)
	{
		ASSERT_IN_TEST(container_size == dst.size(),
		               "Did not construct all the elements or allocate enough memory?, while should ?",
		               a_test_name);
	}

	void verify_content_equal_to_source(container_t const &dst, const std::string &msg)
	{
		ASSERT_IN_TEST(container_traits::equal(dst, init_iterator_type(0), init_iterator_type(container_size)),
		               msg.c_str(), test_name);
	}
	...
	{
		ASSERT_IN_TEST(number_of_constructed_items <= dst.size(),
		               "incorrect test expectation/input parameters?", test_name);
		ASSERT_IN_TEST(std::equal(dst.begin(), dst.begin() + number_of_constructed_items,
		                          init_iterator_type(0)),
		               "content changed during move/copy ?", test_name);
	}
	...
	void verify_content_shallow_moved(container_t const &dst)
	{
		...
		ASSERT_IN_TEST(content_location_unchanged(dst),
		               "container move constructor actually changed element locations, while should not",
		               test_name);
		ASSERT_IN_TEST(source.empty(), "Moved from container instance should not contain any elements",
		               test_name);
	...
	void verify_content_deep_moved(container_t const &dst)
	{
		verify_size(dst, test_name);
		ASSERT_IN_TEST(
		        content_location_changed(dst),
		        "container actually did not changed element locations for unequal allocators, while should",
		        test_name);
		ASSERT_IN_TEST(all_of(dst, is_state_f<Foo::MoveInitialized>()),
		               "container did not move construct some elements?", test_name);
		ASSERT_IN_TEST(all_of(source, is_state_f<Foo::MovedFrom>()), "container did not move all the elements?",
		               test_name);
		verify_content_equal_to_source(dst);
	}
	...
	{
		ASSERT_IN_TEST(content_location_changed(dst),
		               "Vector actually did not changed element locations for unequal allocators, while should",
		               test_name);
		ASSERT_IN_TEST(::all_of(dst.begin(), dst.begin() + number_of_constructed_items,
		                        is_state_f<Foo::MoveInitialized>()),
		               "Vector did not move construct some elements?", test_name);
		if (dst.size() != number_of_constructed_items) {
			ASSERT_IN_TEST(::all_of(dst.begin() + number_of_constructed_items, dst.end(),
			                        is_state_f<Foo::ZeroInitialized>()),
		...
		ASSERT_IN_TEST(::all_of(source.begin(), source.begin() + number_of_constructed_items,
		                        is_state_f<Foo::MovedFrom>()),
		               "Vector did not move all the elements?", test_name);
		ASSERT_IN_TEST(
		        ::all_of(source.begin() + number_of_constructed_items, source.end(),
		                 std::not1(std::ptr_fun(
		                         &is_state<Foo::MovedFrom, element_type::is_zero_initialized_state_allowed>))),
		        "Vector changed elements in source after exception point?", test_name);
	}
};

template <typename T, typename pocma = Harness::false_type> struct arena_allocator_fixture : NoCopy {
...
template <typename T, typename pocma = Harness::false_type> struct two_memory_arenas_fixture : NoCopy {
	...
	two_memory_arenas_fixture(size_t size_to_allocate, const char *a_test_name)
	        ...
	        , test_name(a_test_name)
	{
		ASSERT_IN_TEST(&*source_arena_fixture.storage.begin() != &*dst_arena_fixture.storage.begin(),
		               "source and destination arena instances should use different memory regions", test_name);
		ASSERT_IN_TEST(source_allocator != dst_allocator,
		               "arenas using different memory regions should not compare equal", test_name);
		ASSERT_IN_TEST(pocma::value == tbb::internal::allocator_traits<
		                                       allocator_t>::propagate_on_container_move_assignment::value,
		               "This test require proper allocator_traits support", test_name);
		...
		ASSERT_IN_TEST(source_previous_state == source_allocator,
		               "Copy of allocator should compare equal to it's source", test_name);
		dst = std::move(source_allocator_copy);
		ASSERT_IN_TEST(
		        dst == source_previous_state,
		        "Move initialized instance of allocator should compare equal to it's source state before movement",
		        test_name);
	}
...
template <typename pocma = Harness::false_type> struct std_stateful_allocator : NoCopy {
	...
	template <typename any_allocator_t> void verify_allocator_was_moved(const any_allocator_t &)
	{
	}
};
...
struct default_stateful_fixture_make_helper {
	...
	typedef static_shared_counting_allocator<Harness::int_to_type<__LINE__>,
	                                         typename allocator_fixture_t::allocator_t, std::size_t>
	        allocator_t;
	...
	struct default_stateful_fixture : no_leaks_t,
	                                  private no_foo_leaks_in_fixture_t,
	                                  allocator_fixture_t,
	                                  move_fixture_t,
	                                  no_foo_leaks_in_test_t {
		...
		{
			no_leaks_t::verify_no_more_than_x_memory_items_allocated(
			        container_traits::expected_number_of_items_to_allocate_for_steal_move);
		}
...
template <typename container_traits> void TestMoveConstructorSingleArgument()
{
...
template <typename container_traits> void TestMoveConstructor()
{
...
template <typename container_traits> void TestMoveAssignOperatorPOCMAStateful()
{
...
template <typename container_traits> void TestMoveAssignOperatorPOCMANonStateful()
{
	...
	ASSERT(fixture.source.get_allocator() == allocator_t(),
	       "Incorrect test setup: allocator is stateful while should not?");
...
template <typename container_traits> void TestMoveAssignOperatorNotPOCMAWithUnEqualAllocator()
{
	typedef typename default_stateful_fixture_make_helper<container_traits>::type fixture_t;
	typedef typename fixture_t::container_t container_t;
...
template <typename container_traits> void TestMoveAssignOperatorNotPOCMAWithEqualAllocator()
{
	...
	ASSERT(fixture.source.get_allocator() == dst.get_allocator(),
	       "Incorrect test setup: allocators are not equal while should be?");
...
template <typename container_traits> void TestMoveAssignOperator()
{
...
template <typename container_traits> void TestConstructorWithMoveIterators()
{
	typedef typename default_stateful_fixture_make_helper<container_traits>::type fixture_t;
	typedef typename fixture_t::container_t container_t;
	...
	container_t dst(std::make_move_iterator(fixture.source.begin()), std::make_move_iterator(fixture.source.end()),
	                fixture.dst_allocator);

	fixture.verify_content_deep_moved(dst);
}

template <typename container_traits> void TestAssignWithMoveIterators()
{
	typedef typename default_stateful_fixture_make_helper<container_traits>::type fixture_t;
	typedef typename fixture_t::container_t container_t;
...
void TestExceptionSafetyGuaranteesMoveConstructorWithUnEqualAllocatorMemoryFailure()
{
	...
	limit_allocated_items_in_scope<allocator_t> allocator_limit(allocator_t::items_allocated +
	                                                            fixture.container_size / 4);
	ASSERT_THROWS_IN_TEST(container_t dst(std::move(fixture.source), fixture.dst_allocator), std::bad_alloc, "",
	                      test_name);
}
...
template <typename static_counting_allocator_type> void inline TestStaticCountingAllocatorRebound()
{
	...
	ASSERT(rebound_type::max_items == static_counting_allocator_type::max_items,
	       "rebound allocator should use the same limits");
