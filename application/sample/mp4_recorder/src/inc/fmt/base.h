...
template <typename OutputIt> inline auto get_container(OutputIt it) -> typename OutputIt::container_type &
{
...
// Bitsets of types.
enum { sint_set = set(type::int_type) | set(type::long_long_type) | set(type::int128_type),
...
template <typename T> class buffer {
	...
	constexpr buffer(grow_fun grow, T *p = nullptr, size_t sz = 0, size_t cap = 0) noexcept : ptr_(p),
	                                                                                          size_(sz),
	                                                                                          capacity_(cap),
	                                                                                          grow_(grow)
	{
...
class iterator_buffer : public Traits, public buffer<T> {
	...
	iterator_buffer(iterator_buffer &&other) noexcept : Traits(other),
	                                                    buffer<T>(grow, data_, 0, buffer_size),
	                                                    out_(other.out_)
	{
	}
...
class iterator_buffer<T *, T, fixed_buffer_traits> : public fixed_buffer_traits, public buffer<T> {
	...
	iterator_buffer(iterator_buffer &&other) noexcept : fixed_buffer_traits(other),
...
template <typename Context> class value {
	...
	template <typename T> FMT_CONSTEXPR20 FMT_ALWAYS_INLINE value(T &val)
	{
		...
		if
			constexpr(std::is_same<decltype(&val), T *>::value)
...
template <typename Context> struct arg_mapper {
	...
	template <typename T, FMT_ENABLE_IF(formattable<T>::value)> FMT_MAP_API auto do_map(T &val) -> T &
	{
		return val;
	}
	template <typename T, FMT_ENABLE_IF(!formattable<T>::value)> FMT_MAP_API auto do_map(T &) -> unformattable
	{
		return {};
	}
...
#if defined(__cpp_if_constexpr)
	if
		constexpr(!formattable) type_is_unformattable_for<T, typename Context::char_type> _;
#endif
...
// such as `fmt::vformat`.
template <typename Context, size_t NUM_ARGS, size_t NUM_NAMED_ARGS, unsigned long long DESC> struct format_arg_store {
...
template <typename Context> class basic_format_arg {
	...
	template <typename, size_t, size_t, unsigned long long> friend struct detail::format_arg_store;
   ...
   */
	...
	auto format_custom(const char_type *parse_begin, typename Context::parse_context_type &parse_ctx, Context &ctx)
	        -> bool
	{
...
FMT_DEPRECATED FMT_CONSTEXPR auto visit_format_arg(Visitor &&vis, const basic_format_arg<Context> &arg)
        -> decltype(vis(0))
{
...
template <typename Char> using buffered_context = basic_format_context<basic_appender<Char>, Char>;
...
#if FMT_USE_CONCEPTS
template <typename T, typename Char = char> concept formattable = is_formattable<remove_reference_t<T>, Char>::value;
#endif
...
struct locking : bool_constant<mapped_type_constant<T, format_context>::value == type::custom_type> {
};
...
template <typename Char, typename Handler>
FMT_CONSTEXPR FMT_INLINE auto parse_replacement_field(const Char *begin, const Char *end, Handler &&handler)
        -> const Char *
{
	++begin;
	if (begin == end) {
...
#if defined(__cpp_if_constexpr)
	if
		constexpr(std::is_default_constructible<formatter<mapped_type, char_type> >::value)
		{
...
{
	if
		constexpr(is_statically_named_arg<T>())
		{
			if (name == T::name)
				return N;
		}
	if
		constexpr(sizeof...(Args) > 0) return get_arg_index_by_name<N + 1, Args...>(name);
...
#if FMT_USE_NONTYPE_TEMPLATE_ARGS
	if
		constexpr(sizeof...(Args) > 0) return get_arg_index_by_name<0, Args...>(name);
#endif
	(void)name;
	return -1;
}
...
#if FMT_USE_CONSTEVAL
		if
			constexpr(detail::count_named_args<Args...>() == detail::count_statically_named_args<Args...>())
			{
...
template <size_t N> auto vformat_to(char (&out)[N], string_view fmt, format_args args) -> format_to_result<char *>
{
