...
class text_style {
    public:
	FMT_CONSTEXPR text_style(emphasis em = emphasis()) noexcept : set_foreground_color(),
    ...
    private:
	FMT_CONSTEXPR text_style(bool is_foreground, detail::color_type text_color) noexcept : set_foreground_color(),
	                                                                                       set_background_color(),
	                                                                                       ems()
	{
		if (is_foreground) {
...
template <typename Char> FMT_CONSTEXPR auto make_emphasis(emphasis em) noexcept -> ansi_color_escape<Char>
{
 ...
 */
template <typename... T> void print(FILE *f, const text_style &ts, format_string<T...> fmt, T &&... args)
{
 ...
 *                "Elapsed time: {0:.2f} seconds", 1.23);
 */
template <typename... T> void print(const text_style &ts, format_string<T...> fmt, T &&... args)
{
 ...
 * ```
 */
template <typename... T> inline auto format(const text_style &ts, format_string<T...> fmt, T &&... args) -> std::string
{
