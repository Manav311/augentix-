...
#pragma clang diagnostic pop
	...
	auto result =
	        f.in(mb, in_buf.begin(), in_buf.end(), from_next, std::begin(out.buf), std::end(out.buf), out.end);
...
void write_floating_seconds(memory_buffer &buf, Duration duration, int num_fractional_digits = -1)
{
	...
	fmt::format_to(
	        std::back_inserter(buf), FMT_STRING("{:.{}f}"),
...
template <typename T, FMT_ENABLE_IF(std::is_integral<T>::value)> inline auto mod(T x, int y) -> T
{
	return x % static_cast<T>(y);
}
template <typename T, FMT_ENABLE_IF(std::is_floating_point<T>::value)> inline auto mod(T x, int y) -> T
{
...
template <typename Char, typename OutputIt> auto copy_unit(string_view unit, OutputIt out, Char) -> OutputIt
{
	return std::copy(unit.begin(), unit.end(), out);
}

template <typename OutputIt> auto copy_unit(string_view unit, OutputIt out, wchar_t) -> OutputIt
{
...
template <typename Char, typename Period, typename OutputIt> auto format_duration_unit(OutputIt out) -> OutputIt
{
...
template <typename Char> struct formatter<weekday, Char> : private formatter<std::tm, Char> {
    private:
...
template <typename Char> struct formatter<day, Char> : private formatter<std::tm, Char> {
    private:
	bool use_tm_formatter_ = false;
	...
	template <typename FormatContext> auto format(day d, FormatContext &ctx) const -> decltype(ctx.out())
	{
		auto time = std::tm();
...
template <typename Char> struct formatter<month, Char> : private formatter<std::tm, Char> {
    private:
	bool localized_ = false;
	bool use_tm_formatter_ = false;
	...
	template <typename FormatContext> auto format(month m, FormatContext &ctx) const -> decltype(ctx.out())
	{
		auto time = std::tm();
...
template <typename Char> struct formatter<year, Char> : private formatter<std::tm, Char> {
    private:
	bool use_tm_formatter_ = false;
	...
	template <typename FormatContext> auto format(year y, FormatContext &ctx) const -> decltype(ctx.out())
	{
		auto time = std::tm();
...
template <typename Char> struct formatter<year_month_day, Char> : private formatter<std::tm, Char> {
    private:
	bool use_tm_formatter_ = false;
