...
static hi_void *hi_channel_host_thread(hi_void *args)
{
	hi_s32 rev_len;
	hi_s32 payload_len;
	hi_char msg[SYSTEM_CMD_SIZE + 16];
	...
	while (!g_channel_terminate) {
		...
		rev_len = recvfrom(g_channel_monitor->skfd, &msg[0], sizeof(msg), MSG_WAITALL,
...
hi_s32 hi_channel_send_to_dev(unsigned char *buf, int len)
{
	...
	memset(nlh, 0x00, sizeof(*nlh));
	nlh->nlmsg_len = NLMSG_SPACE(len);
	nlh->nlmsg_pid = NETLINK_SOCKET_PORT_ID;
	memcpy(NLMSG_DATA(nlh), buf, len);
	ret = sendto(g_channel_monitor->skfd, nlh, nlh->nlmsg_len, 0, (struct sockaddr *)&daddr,
...
hi_s32 hi_channel_init(hi_void)
{
	...
	memset(g_channel_monitor, 0, sizeof(netlink_monitor_s));
