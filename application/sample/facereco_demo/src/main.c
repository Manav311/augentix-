...
#define FR_TIC(start) clock_gettime(CLOCK_MONOTONIC_RAW, &start)

#define FR_TOC(str, start)                                                                               \
	do {                                                                                             \
		struct timespec end;                                                                     \
		uint64_t delta_us;                                                                       \
		float delta_s;                                                                           \
...
FACERECO_PARAM_S g_facereco_param;
MPI_WIN g_win_idx;
MPI_IVA_OD_PARAM_S g_od_param;
MPI_RECT_POINT_S g_chn_bdry;
MPI_SIZE_S g_chn_resoln;
int g_facereco_running;
...
static void handleSigInt(int signo)
{
	...
	g_facereco_running = 0;
	char *restartc = getenv("FR_RESTART");
	if (restartc)
...
int parse_config(char *file_name, FACERECO_PARAM_S *param)
{
	static const char *agtx_iva_eaif_data_fmt_e_map[] = { "JPEG",     "Y",     "YUV",     "RGB",
		                                              "MPI_JPEG", "MPI_Y", "MPI_YUV", "MPI_RGB" };

	struct json_object *tmp_obj, *cmd_obj, *tmp1_obj;
	int i;
	const char *str;
	...
	if (json_object_object_get_ex(cmd_obj, "identification_period", &tmp_obj)) {
		param->identification_period = json_object_get_int(tmp_obj);
	}
	if (json_object_object_get_ex(cmd_obj, "min_size", &tmp_obj)) {
		param->min_size = json_object_get_int(tmp_obj);
	}
	...
	if (!tmp_obj) {
		...
		roi_cnt = json_object_array_length(tmp_obj);
		if (roi_cnt == 4) {
			tmp1_obj = json_object_array_get_idx(tmp_obj, 0);
			param->inf_utils.roi.sx = json_object_get_int(tmp1_obj);
			tmp1_obj = json_object_array_get_idx(tmp_obj, 1);
			param->inf_utils.roi.sy = json_object_get_int(tmp1_obj);
			tmp1_obj = json_object_array_get_idx(tmp_obj, 2);
			param->inf_utils.roi.ex = json_object_get_int(tmp1_obj);
			tmp1_obj = json_object_array_get_idx(tmp_obj, 3);
			param->inf_utils.roi.ey = json_object_get_int(tmp1_obj);
		}
	}
...
#define printStr(x) printf("[INFO] %s=\"%s\"\n", #x, x)
	...
	if (roi_cnt == 4) {
		printInt(param->inf_utils.roi.sx);
		printInt(param->inf_utils.roi.sy);
		printInt(param->inf_utils.roi.ex);
		printInt(param->inf_utils.roi.ey);
	}
error:
...
void help(void)
{
	printf("USAGE:\tfacereco_demo -i <CONFIG>\t\n");
	printf("\t-i <file>\t\tface recognition config in .json file\n");
	printf("\t@obj_life_th\tfacereco module filters out object with life less than this threshold. Range[0-160]\n");
	printf("\t@pos_classify_period\tRecognize period there is a positive result for the object. Range[1-999].\n");
	printf("\t@neg_classify_period\tRecognize period there is a negative result for the object. Range[1-999].\n");
	printf("\t@pos_stop_count_th\tStop recognize for a object when it accumulates enough consecutive positive result. Range[1-99].\n");
	printf("\t@detection_period\tFace detection period. Range[1-999]\n");
	printf("\t@identification_period\tFace identification. Range[1-999].\n");
	printf("\t@inf_with_obj_list\tFlag to enable object list based face recognition application. Default 1\n");
	printf("OPTIONS:\n");
	printf("\t-c <channel>\t\tSpecify which video channel to use. (Default 0).\n");
	/* UINT8 od_qual; < Quality index of OD performance. */
	printf("\t-q <value>\t\tSpecify OD quality index.[0-63] (Default 57).\n");
	...
	printf("\tfacereco_demo -i /system/mpp/facereco_config/facereco_conf.json &\n");
#ifdef CONFIG_APP_FACERECO_SUPPORT_SEI
...
int main(int argc, char **argv)
{
	...
	*od_param = (MPI_IVA_OD_PARAM_S){
		.od_qual = 57, .od_track_refine = 42, .od_size_th = 25, .od_sen = 254, .en_stop_det = 1, .en_gmv_det = 0
	};

	FACERECO_PARAM_S *facereco_param = &g_facereco_param;

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wmissing-field-initializers"
	*facereco_param = (FACERECO_PARAM_S){
		.obj_life_th = 16,
		.target_idx = MPI_VIDEO_WIN(0, 0, 0),
		.api = EAIF_API_FACERECO,
		...
		.inf_with_obj_list = 1,
		.detection_period = 12,
		.identification_period = 12,
		.min_size = 80,
		.face_reco_model = "/system/eaif/models/facereco/inapp_facereco.ini",
		.inf_utils = { .cmd = EAIF_INF_NONE, .dir = "/usrdata/eaif/facereco", .face_db = "face.bin" }
	};
#pragma GCC diagnostic pop
	...
	do {
		printf("[INFO] starting Face Recognition\n");
		ret = runFaceRecognition(win, facereco_param);
		FR_TOC("[FR EXIT]", g_start_time);
	} while (g_restart);
