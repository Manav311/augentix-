...
int __switch_day_color_via_mpi()
{
	...
	if (ret != MPI_SUCCESS) {
		ir_control_demo_log_err("Failed to MPI_getCalAttr!");
		return ret;
	}
	...
	if (ret != MPI_SUCCESS) {
		ir_control_demo_log_err("Failed to MPI_setCalAttr!");
		return ret;
	}
	...
	if (ret != MPI_SUCCESS) {
		ir_control_demo_log_err("Failed to MPI_getDipAttr!");
		return ret;
	}
	...
	if (ret != MPI_SUCCESS) {
		ir_control_demo_log_err("Failed to MPI_setDipAttr!");
		return ret;
	}

	ret = MPI_getCsmAttr(path_idx, &csm_attr);
	if (ret != MPI_SUCCESS) {
		ir_control_demo_log_err("Failed to MPI_getCsmAttr!");
		return ret;
	}
	...
	if (ret != MPI_SUCCESS) {
		ir_control_demo_log_err("Failed to MPI_setCsmAttr!");
		return ret;
	}
...
#endif
	ir_control_demo_log_debug("finish disable PWM");
	return 0;
}

int change_light_strength_cb(int strength, void *data __attribute__((unused)))
{
	int duty_cycle = strength * g_ir_pwm.period / 1024;
	ir_control_demo_log_debug("change ir pwm strength %d", duty_cycle);
#ifdef ENABLE_PWM
	setPWMDutyCycle(&g_ir_pwm, duty_cycle);
#endif
	return 0;
}
...
int cal_scene_luma_cb(int dev, int path, int *pscene_luma, void *data __attribute__((unused)))
{
	int sceneLuma = calcMpiSceneLuma(dev, path);
	*pscene_luma = sceneLuma;
...
{
	ir_control_demo_log_debug("witch mode %d", (int)mode);
	return 0;
}
...
int main(int argc, char **argv)
{
	...
	IrControl Ir_control = { .mode = customer_ir_mode,
		                 ...
		                 .switchIRStrengthCb = change_light_strength_cb,
		                 .switchDayNightCb = switch_day_night_cb1,
		                 .extraNighttoDayCb = extra_night2day_cb,
		                 .calSceneLumaCb = cal_scene_luma_cb,
		                 .conf = config,
		                 .pData = NULL };

	ir_control_demo_log_debug("before : mpi sys init()");
	MPI_SYS_init();
	g_run_flag = 1;
	...
	g_ir_pwm.id = pwm_idx;
#endif
	ir_control_demo_log_debug("before : IR_init()");
	IR_init();
	ir_control_demo_log_debug("before : IR_setConfig()");
